...
(* functions *)
    ...
    let _Quote = Fun.SimpleQuote1 (triv (fun () -> Util.toNullable (0.03)))
    let _Tenor = Tenor
    ...
    let _Engine = Fun.DiscountingBondEngine (triv (fun () -> Util.toHandle<YieldTermStructure> (_FlatForward.Value))) (triv (fun () -> Util.toNullable (true)))
...
module FixedBondFunction =
    ...
    let FixedBond_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new FixedBond(
                                                        _Tenor.cell,
                                                        _Maturity.cell,
                                                        _FixedAmount.cell)

                                                       ) :> ICell
                let format (i : ICell) (l:string) = i :> obj
    ...
    let FixedBond_Cash
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            let format (o : System.Double) (l:string) = Helper.Range.fromModel (i :?> Cash) l

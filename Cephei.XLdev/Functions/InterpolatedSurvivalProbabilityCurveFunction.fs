    ...
    let InterpolatedSurvivalProbabilityCurve_defaultDensity
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedSurvivalProbabilityCurve.source + ".DefaultDensity1") 
                                               [| _InterpolatedSurvivalProbabilityCurve.source
                                               ;  _d.source
                                               ;  _extrapolate.source
                                               |]
    ...
    let InterpolatedSurvivalProbabilityCurve_defaultProbability
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedSurvivalProbabilityCurve.source + ".DefaultProbability1") 
                                               [| _InterpolatedSurvivalProbabilityCurve.source
                                               ;  _d1.source
                                               ;  _d2.source
                                               ;  _extrapolate.source
    ...
    let InterpolatedSurvivalProbabilityCurve_defaultProbability
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedSurvivalProbabilityCurve.source + ".DefaultProbability2") 
                                               [| _InterpolatedSurvivalProbabilityCurve.source
                                               ;  _t.source
                                               ;  _extrapolate.source
                                               |]
    ...
    let InterpolatedSurvivalProbabilityCurve_defaultProbability
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedSurvivalProbabilityCurve.source + ".DefaultProbability3") 
                                               [| _InterpolatedSurvivalProbabilityCurve.source
                                               ;  _d.source
                                               ;  _extrapolate.source
                                               |]
    ...
    let InterpolatedSurvivalProbabilityCurve_hazardRate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedSurvivalProbabilityCurve.source + ".HazardRate1") 
                                               [| _InterpolatedSurvivalProbabilityCurve.source
                                               ;  _d.source
                                               ;  _extrapolate.source
                                               |]
    ...
    let InterpolatedSurvivalProbabilityCurve_survivalProbability
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedSurvivalProbabilityCurve.source + ".SurvivalProbability1") 
                                               [| _InterpolatedSurvivalProbabilityCurve.source
                                               ;  _d.source
                                               ;  _extrapolate.source
                                               |]

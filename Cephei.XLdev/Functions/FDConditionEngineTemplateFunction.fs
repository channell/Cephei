...
module FDConditionEngineTemplateFunction =
    ...
    let FDConditionEngineTemplate_setStepCondition
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _FDConditionEngineTemplate = Helper.toCell<FDConditionEngineTemplate> fdconditionenginetemplate "FDConditionEngineTemplate" true 
    ...
    *)
    [<ExcelFunction(Name="_FDConditionEngineTemplate_calculate", Description="Create a FDConditionEngineTemplate",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FDConditionEngineTemplate_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="FDConditionEngineTemplate",Description = "Reference to FDConditionEngineTemplate")>] 
         fdconditionenginetemplate : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_FDConditionEngineTemplate.cell :?> FDConditionEngineTemplateModel).Calculate
                                                            _r.cell 
                                                       ) :> ICell
                let format (o : FDConditionEngineTemplate) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FDConditionEngineTemplate.source + ".Calculate") 
                                               [| _FDConditionEngineTemplate.source
                                               ;  _r.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let FDConditionEngineTemplate_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<FDConditionEngineTemplate> i "value" true) |>
                        Seq.toArray

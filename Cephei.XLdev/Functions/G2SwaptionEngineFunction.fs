...
module G2SwaptionEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_G2SwaptionEngine_calculate", Description="Create a G2SwaptionEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let G2SwaptionEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _G2SwaptionEngine = Helper.toCell<G2SwaptionEngine> g2swaptionengine "G2SwaptionEngine" true 
                let builder () = withMnemonic mnemonic ((_G2SwaptionEngine.cell :?> G2SwaptionEngineModel).Calculate
                                                       ) :> ICell
                let format (o : G2SwaptionEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_G2SwaptionEngine.source + ".Calculate") 
    ...
    let G2SwaptionEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _model = Helper.toCell<G2> model "model" true
    ...
    let G2SwaptionEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<G2SwaptionEngine> i "value" true) |>
                        Seq.toArray

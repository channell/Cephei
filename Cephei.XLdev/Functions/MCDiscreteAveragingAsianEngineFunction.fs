...
module MCDiscreteAveragingAsianEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_MCDiscreteAveragingAsianEngine_calculate", Description="Create a MCDiscreteAveragingAsianEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let MCDiscreteAveragingAsianEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _MCDiscreteAveragingAsianEngine = Helper.toCell<MCDiscreteAveragingAsianEngine> mcdiscreteaveragingasianengine "MCDiscreteAveragingAsianEngine" true 
                let builder () = withMnemonic mnemonic ((_MCDiscreteAveragingAsianEngine.cell :?> MCDiscreteAveragingAsianEngineModel).Calculate
                                                       ) :> ICell
                let format (o : MCDiscreteAveragingAsianEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_MCDiscreteAveragingAsianEngine.source + ".Calculate") 
    ...
    let MCDiscreteAveragingAsianEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _maxTimeStepsPerYear = Helper.toCell<int> maxTimeStepsPerYear "maxTimeStepsPerYear" true
    ...
    let MCDiscreteAveragingAsianEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<MCDiscreteAveragingAsianEngine> i "value" true) |>
                        Seq.toArray

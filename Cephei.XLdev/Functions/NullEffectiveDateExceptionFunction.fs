...
module NullEffectiveDateExceptionFunction =
    ...
    *)
    [<ExcelFunction(Name="_NullEffectiveDateException2", Description="Create a NullEffectiveDateException",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let NullEffectiveDateException_create2
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        = 
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic (Fun.NullEffectiveDateException2 
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<NullEffectiveDateException>) l

                let source = Helper.sourceFold "Fun.NullEffectiveDateException2" 
                                               [||]
                let hash = Helper.hashFold 
                                [||]
                Model.specify 
    ...
    let NullEffectiveDateException_Range 
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="Objects",Description = "Identifer for the NullEffectiveDateException")>] 
         values : obj[,])
         =
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<NullEffectiveDateException> i "value" true) |>
                        Seq.toArray

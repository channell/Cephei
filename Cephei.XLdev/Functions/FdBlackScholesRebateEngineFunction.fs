...
module FdBlackScholesRebateEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_FdBlackScholesRebateEngine_calculate", Description="Create a FdBlackScholesRebateEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FdBlackScholesRebateEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="FdBlackScholesRebateEngine",Description = "Reference to FdBlackScholesRebateEngine")>] 
         fdblackscholesrebateengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _FdBlackScholesRebateEngine = Helper.toCell<FdBlackScholesRebateEngine> fdblackscholesrebateengine "FdBlackScholesRebateEngine" true 
                let builder () = withMnemonic mnemonic ((_FdBlackScholesRebateEngine.cell :?> FdBlackScholesRebateEngineModel).Calculate
                                                       ) :> ICell
                let format (o : FdBlackScholesRebateEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FdBlackScholesRebateEngine.source + ".Calculate") 
                                               [| _FdBlackScholesRebateEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _FdBlackScholesRebateEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let FdBlackScholesRebateEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<FdBlackScholesRebateEngine> i "value" true) |>
                        Seq.toArray

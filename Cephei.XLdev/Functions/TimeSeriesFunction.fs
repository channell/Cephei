    ...
    let TimeSeries_Add
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _TimeSeries = Helper.toCell<TimeSeries> timeseries "TimeSeries" true 
                let _item = Helper.toCell<KeyValuePair<Date,T>> item "item" true
                let builder () = withMnemonic mnemonic ((_TimeSeries.cell :?> TimeSeriesModel).Add1
                                                            _item.cell 
                                                       ) :> ICell
                let format (o : TimeSeries) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_TimeSeries.source + ".Add1") 
                                               [| _TimeSeries.source
                                               ;  _item.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let TimeSeries_CopyTo
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _TimeSeries = Helper.toCell<TimeSeries> timeseries "TimeSeries" true 
                let _array = Helper.toCell<KeyValuePair<Date,T>[]> array "array" true
    ...
    let TimeSeries_GetEnumerator
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let format (o : IEnumerator<KeyValuePair<Date,T>>) (l:string) = o.ToString() :> obj
    ...
    let TimeSeries_Remove
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_TimeSeries.source + ".Remove1") 
                                               [| _TimeSeries.source
                                               ;  _key.source
                                               |]
                let hash = Helper.hashFold 

...
module AnalyticContinuousFloatingLookbackEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticContinuousFloatingLookbackEngine_calculate", Description="Create a AnalyticContinuousFloatingLookbackEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticContinuousFloatingLookbackEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="AnalyticContinuousFloatingLookbackEngine",Description = "Reference to AnalyticContinuousFloatingLookbackEngine")>] 
         analyticcontinuousfloatinglookbackengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _AnalyticContinuousFloatingLookbackEngine = Helper.toCell<AnalyticContinuousFloatingLookbackEngine> analyticcontinuousfloatinglookbackengine "AnalyticContinuousFloatingLookbackEngine" true 
                let builder () = withMnemonic mnemonic ((_AnalyticContinuousFloatingLookbackEngine.cell :?> AnalyticContinuousFloatingLookbackEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticContinuousFloatingLookbackEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticContinuousFloatingLookbackEngine.source + ".Calculate") 
                                               [| _AnalyticContinuousFloatingLookbackEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _AnalyticContinuousFloatingLookbackEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let AnalyticContinuousFloatingLookbackEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<AnalyticContinuousFloatingLookbackEngine> i "value" true) |>
                        Seq.toArray

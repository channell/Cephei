...
module BlackStyleSwaptionEngineFunction =
    ...
    let BlackStyleSwaptionEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _volatility = Helper.toHandle<Handle<SwaptionVolatilityStructure>> volatility "volatility" 
                let _displacement = Helper.toNullable<Nullable<double>> displacement "displacement"
                let _model = Helper.toCell<CashAnnuityModel> model "model" true
    ...
    *)
    [<ExcelFunction(Name="_BlackStyleSwaptionEngine_calculate", Description="Create a BlackStyleSwaptionEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let BlackStyleSwaptionEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _BlackStyleSwaptionEngine = Helper.toCell<BlackStyleSwaptionEngine> blackstyleswaptionengine "BlackStyleSwaptionEngine" true 
                let builder () = withMnemonic mnemonic ((_BlackStyleSwaptionEngine.cell :?> BlackStyleSwaptionEngineModel).Calculate
                                                       ) :> ICell
                let format (o : BlackStyleSwaptionEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_BlackStyleSwaptionEngine.source + ".Calculate") 
    ...
    let BlackStyleSwaptionEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<BlackStyleSwaptionEngine> i "value" true) |>
                        Seq.toArray

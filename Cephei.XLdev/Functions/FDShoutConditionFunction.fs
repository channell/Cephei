...
module FDShoutConditionFunction =
    ...
    let FDShoutCondition_factory
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _FDShoutCondition = Helper.toCell<FDShoutCondition> fdshoutcondition "FDShoutCondition" true 
    ...
    *)
    [<ExcelFunction(Name="_FDShoutCondition_calculate", Description="Create a FDShoutCondition",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FDShoutCondition_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="FDShoutCondition",Description = "Reference to FDShoutCondition")>] 
         fdshoutcondition : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_FDShoutCondition.cell :?> FDShoutConditionModel).Calculate
                                                            _r.cell 
                                                       ) :> ICell
                let format (o : FDShoutCondition) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FDShoutCondition.source + ".Calculate") 
                                               [| _FDShoutCondition.source
                                               ;  _r.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let FDShoutCondition_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<FDShoutCondition> i "value" true) |>
                        Seq.toArray

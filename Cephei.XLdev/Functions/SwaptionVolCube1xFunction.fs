    ...
    let SwaptionVolCube1x_marketVolCube
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".MarketVolCube1") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _i.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let SwaptionVolCube1x_recalibration
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Recalibration1") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _swapLengths.source
                                               ;  _beta.source
                                               ;  _swapTenor.source
    ...
    let SwaptionVolCube1x_recalibration
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Recalibration2") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _beta.source
                                               ;  _swapTenor.source
                                               |]
    ...
    let SwaptionVolCube1x_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _atmVolStructure = Helper.toHandle<Handle<SwaptionVolatilityStructure>> atmVolStructure "atmVolStructure" 
                ...
                let _maxErrorTolerance = Helper.toNullable<Nullable<double>> maxErrorTolerance "maxErrorTolerance"
                let _optMethod = Helper.toCell<OptimizationMethod> optMethod "optMethod" true
                let _errorAccept = Helper.toNullable<Nullable<double>> errorAccept "errorAccept"
    ...
    let SwaptionVolCube1x_atmStrike
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".AtmStrike1") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionDate.source
                                               ;  _swapTenor.source
                                               |]
    ...
    let SwaptionVolCube1x_blackVariance
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".BlackVariance1") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionDate.source
                                               ;  _swapLength.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_blackVariance
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".BlackVariance2") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTenor.source
                                               ;  _swapLength.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_blackVariance
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".BlackVariance3") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTime.source
                                               ;  _swapTenor.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_blackVariance
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".BlackVariance4") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionDate.source
                                               ;  _swapTenor.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_blackVariance
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".BlackVariance5") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTenor.source
                                               ;  _swapTenor.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_shift
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Shift1") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionDate.source
                                               ;  _swapTenor.source
                                               ;  _extrapolate.source
    ...
    let SwaptionVolCube1x_shift
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Shift2") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionDate.source
                                               ;  _swapLength.source
                                               ;  _extrapolate.source
    ...
    let SwaptionVolCube1x_shift
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Shift3") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTime.source
                                               ;  _swapLength.source
                                               ;  _extrapolate.source
    ...
    let SwaptionVolCube1x_shift
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Shift4") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTenor.source
                                               ;  _swapTenor.source
                                               ;  _extrapolate.source
    ...
    let SwaptionVolCube1x_shift
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Shift5") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTenor.source
                                               ;  _swapLength.source
                                               ;  _extrapolate.source
    ...
    let SwaptionVolCube1x_smileSection
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".SmileSection1") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTime.source
                                               ;  _swapLength.source
                                               ;  _extr.source
    ...
    let SwaptionVolCube1x_smileSection
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".SmileSection2") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionDate.source
                                               ;  _swapTenor.source
                                               ;  _extr.source
    ...
    let SwaptionVolCube1x_swapLength
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".SwapLength1") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _swapTenor.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let SwaptionVolCube1x_volatility
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Volatility1") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionDate.source
                                               ;  _swapLength.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_volatility
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Volatility2") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTenor.source
                                               ;  _swapTenor.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_volatility
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Volatility3") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTenor.source
                                               ;  _swapLength.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_volatility
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Volatility4") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTime.source
                                               ;  _swapLength.source
                                               ;  _strike.source
    ...
    let SwaptionVolCube1x_volatility
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SwaptionVolCube1x.source + ".Volatility5") 
                                               [| _SwaptionVolCube1x.source
                                               ;  _optionTime.source
                                               ;  _swapTenor.source
                                               ;  _strike.source

...
module FDDividendEuropeanEngineFunction =
    ...
    let FDDividendEuropeanEngine_factory
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _FDDividendEuropeanEngine = Helper.toCell<FDDividendEuropeanEngine> fddividendeuropeanengine "FDDividendEuropeanEngine" true 
    ...
    *)
    [<ExcelFunction(Name="_FDDividendEuropeanEngine_calculate", Description="Create a FDDividendEuropeanEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FDDividendEuropeanEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="FDDividendEuropeanEngine",Description = "Reference to FDDividendEuropeanEngine")>] 
         fddividendeuropeanengine : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _FDDividendEuropeanEngine = Helper.toCell<FDDividendEuropeanEngine> fddividendeuropeanengine "FDDividendEuropeanEngine" true 
                let builder () = withMnemonic mnemonic ((_FDDividendEuropeanEngine.cell :?> FDDividendEuropeanEngineModel).Calculate
                                                       ) :> ICell
                let format (o : FDDividendEuropeanEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FDDividendEuropeanEngine.source + ".Calculate") 
                                               [| _FDDividendEuropeanEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _FDDividendEuropeanEngine.cell
    ...
    let FDDividendEuropeanEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<FDDividendEuropeanEngine> i "value" true) |>
                        Seq.toArray

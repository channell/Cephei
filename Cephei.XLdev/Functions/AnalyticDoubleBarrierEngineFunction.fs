...
module AnalyticDoubleBarrierEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticDoubleBarrierEngine_calculate", Description="Create a AnalyticDoubleBarrierEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticDoubleBarrierEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="AnalyticDoubleBarrierEngine",Description = "Reference to AnalyticDoubleBarrierEngine")>] 
         analyticdoublebarrierengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _AnalyticDoubleBarrierEngine = Helper.toCell<AnalyticDoubleBarrierEngine> analyticdoublebarrierengine "AnalyticDoubleBarrierEngine" true 
                let builder () = withMnemonic mnemonic ((_AnalyticDoubleBarrierEngine.cell :?> AnalyticDoubleBarrierEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticDoubleBarrierEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticDoubleBarrierEngine.source + ".Calculate") 
                                               [| _AnalyticDoubleBarrierEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _AnalyticDoubleBarrierEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder

    ...
    let ExchangeRateManager_add
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _ExchangeRateManager = Helper.toCell<ExchangeRateManager> exchangeratemanager "ExchangeRateManager" true 
                let _rate = Helper.toCell<ExchangeRate> rate "rate" true
                let builder () = withMnemonic mnemonic ((_ExchangeRateManager.cell :?> ExchangeRateManagerModel).Add1
                                                            _rate.cell 
                                                       ) :> ICell
                let format (o : ExchangeRateManager) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_ExchangeRateManager.source + ".Add1") 
                                               [| _ExchangeRateManager.source
                                               ;  _rate.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let ExchangeRateManager_lookup
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_ExchangeRateManager.source + ".Lookup1") 
                                               [| _ExchangeRateManager.source
                                               ;  _source.source
                                               ;  _target.source
                                               ;  _date.source
    ...
    let ExchangeRateManager_lookup
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_ExchangeRateManager.source + ".Lookup2") 
                                               [| _ExchangeRateManager.source
                                               ;  _source.source
                                               ;  _target.source
                                               |]

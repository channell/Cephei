    ...
    let DeltaVolQuote_atmType
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _DeltaVolQuote = Helper.toCell<DeltaVolQuote> deltavolquote "DeltaVolQuote" true 
                let builder () = withMnemonic mnemonic ((_DeltaVolQuote.cell :?> DeltaVolQuoteModel).AtmType
                                                       ) :> ICell
                let format (o : AtmType) (l:string) = o.ToString() :> obj
    ...
    let DeltaVolQuote_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _deltaType = Helper.toCell<DeltaType> deltaType "deltaType" true
                let _maturity = Helper.toCell<double> maturity "maturity" true
                let _atmType = Helper.toCell<AtmType> atmType "atmType" true
    ...
    let DeltaVolQuote_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<DeltaVolQuote> i "value" true) |>
                        Seq.toArray

...
module HestonExpansionEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_HestonExpansionEngine_calculate", Description="Create a HestonExpansionEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let HestonExpansionEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_HestonExpansionEngine.cell :?> HestonExpansionEngineModel).Calculate
                                                       ) :> ICell
                let format (o : HestonExpansionEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_HestonExpansionEngine.source + ".Calculate") 
    ...
    let HestonExpansionEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _formula = Helper.toCell<HestonExpansionFormula> formula "formula" true

...
module MonteCarloCatBondEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_MonteCarloCatBondEngine_calculate", Description="Create a MonteCarloCatBondEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let MonteCarloCatBondEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_MonteCarloCatBondEngine.cell :?> MonteCarloCatBondEngineModel).Calculate
                                                       ) :> ICell
                let format (o : MonteCarloCatBondEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_MonteCarloCatBondEngine.source + ".Calculate") 

...
module FDDividendEngineFunction =
    ...
    let FDDividendEngine_factory2
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _FDDividendEngine = Helper.toCell<FDDividendEngine> fddividendengine "FDDividendEngine" true 
    ...
    *)
    [<ExcelFunction(Name="_FDDividendEngine_calculate", Description="Create a FDDividendEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FDDividendEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="FDDividendEngine",Description = "Reference to FDDividendEngine")>] 
         fddividendengine : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_FDDividendEngine.cell :?> FDDividendEngineModel).Calculate
                                                            _r.cell 
                                                       ) :> ICell
                let format (o : FDDividendEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FDDividendEngine.source + ".Calculate") 
                                               [| _FDDividendEngine.source
                                               ;  _r.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let FDDividendEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<FDDividendEngine> i "value" true) |>
                        Seq.toArray

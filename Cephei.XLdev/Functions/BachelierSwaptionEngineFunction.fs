...
module BachelierSwaptionEngineFunction =
    ...
    let BachelierSwaptionEngine_create2
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _discountCurve = Helper.toHandle<Handle<YieldTermStructure>> discountCurve "discountCurve" 
                let _vol = Helper.toHandle<Handle<SwaptionVolatilityStructure>> vol "vol" 
                let _model = Helper.toCell<CashAnnuityModel> model "model" true
    ...
    *)
    [<ExcelFunction(Name="_BachelierSwaptionEngine_calculate", Description="Create a BachelierSwaptionEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let BachelierSwaptionEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_BachelierSwaptionEngine.cell :?> BachelierSwaptionEngineModel).Calculate
                                                       ) :> ICell
                let format (o : BachelierSwaptionEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_BachelierSwaptionEngine.source + ".Calculate") 

    ...
    let BilinearInterpolation_isInRange
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _BilinearInterpolation = Helper.toCell<BilinearInterpolation> bilinearinterpolation "BilinearInterpolation" true 
    ...
    let BilinearInterpolation_value
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_BilinearInterpolation.source + ".Value1") 
                                               [| _BilinearInterpolation.source
                                               ;  _x.source
                                               ;  _y.source
    ...
    let BilinearInterpolation_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<BilinearInterpolation> i "value" true) |>
                        Seq.toArray

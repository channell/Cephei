...
module BlackSwaptionEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_BlackSwaptionEngine_calculate", Description="Create a BlackSwaptionEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let BlackSwaptionEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_BlackSwaptionEngine.cell :?> BlackSwaptionEngineModel).Calculate
                                                       ) :> ICell
                let format (o : BlackSwaptionEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_BlackSwaptionEngine.source + ".Calculate") 

    ...
    let AkimaCubicInterpolation_aCoefficients
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation" true 
    ...
    let AkimaCubicInterpolation_value
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_AkimaCubicInterpolation.source + ".Value1") 
                                               [| _AkimaCubicInterpolation.source
                                               ;  _x.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let AkimaCubicInterpolation_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<AkimaCubicInterpolation> i "value" true) |>
                        Seq.toArray

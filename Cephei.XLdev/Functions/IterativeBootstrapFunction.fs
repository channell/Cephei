...
module IterativeBootstrapFunction =
    ...
    *)
    [<ExcelFunction(Name="_IterativeBootstrap_calculate", Description="Create a IterativeBootstrap",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let IterativeBootstrap_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_IterativeBootstrap.cell :?> IterativeBootstrapModel).Calculate
                                                       ) :> ICell
                let format (o : IterativeBootstrap) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_IterativeBootstrap.source + ".Calculate") 

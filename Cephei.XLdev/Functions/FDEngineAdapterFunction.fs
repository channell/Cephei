...
module FDEngineAdapterFunction =
    ...
    *)
    [<ExcelFunction(Name="_FDEngineAdapter_calculate", Description="Create a FDEngineAdapter",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FDEngineAdapter_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_FDEngineAdapter.cell :?> FDEngineAdapterModel).Calculate
                                                       ) :> ICell
                let format (o : FDEngineAdapter) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FDEngineAdapter.source + ".Calculate") 

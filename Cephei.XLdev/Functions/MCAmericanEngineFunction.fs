...
module MCAmericanEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_MCAmericanEngine_calculate", Description="Create a MCAmericanEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let MCAmericanEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="MCAmericanEngine",Description = "Reference to MCAmericanEngine")>] 
         mcamericanengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _MCAmericanEngine = Helper.toCell<MCAmericanEngine> mcamericanengine "MCAmericanEngine" true 
                let builder () = withMnemonic mnemonic ((_MCAmericanEngine.cell :?> MCAmericanEngineModel).Calculate
                                                       ) :> ICell
                let format (o : MCAmericanEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_MCAmericanEngine.source + ".Calculate") 
                                               [| _MCAmericanEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _MCAmericanEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let MCAmericanEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _nCalibrationSamples = Helper.toCell<int> nCalibrationSamples "nCalibrationSamples" true
    ...
    let MCAmericanEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<MCAmericanEngine> i "value" true) |>
                        Seq.toArray

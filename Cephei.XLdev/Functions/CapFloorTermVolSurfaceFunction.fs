...
module CapFloorTermVolSurfaceFunction =
    ...
    *)
    [<ExcelFunction(Name="_CapFloorTermVolSurface2", Description="Create a CapFloorTermVolSurface",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let CapFloorTermVolSurface_create2
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="settlementDate",Description = "Reference to settlementDate")>] 
         settlementDate : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _dc = Helper.toCell<DayCounter> dc "dc" true
                let builder () = withMnemonic mnemonic (Fun.CapFloorTermVolSurface2 
                                                            _settlementDate.cell 
                                                            _calendar.cell 
                                                            _bdc.cell 
                                                            _optionTenors.cell 
                ...
                let source = Helper.sourceFold "Fun.CapFloorTermVolSurface2" 
                                               [| _settlementDate.source
                                               ;  _calendar.source
                                               ;  _bdc.source
                                               ;  _optionTenors.source

    ...
    let AbcdInterpolation_a
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AbcdInterpolation = Helper.toCell<AbcdInterpolation> abcdinterpolation "AbcdInterpolation" true 
    ...
    let AbcdInterpolation_value
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AbcdInterpolation = Helper.toCell<AbcdInterpolation> abcdinterpolation "AbcdInterpolation" true 
                let _x = Helper.toCell<double> x "x" true
                let builder () = withMnemonic mnemonic ((_AbcdInterpolation.cell :?> AbcdInterpolationModel).Value1
                                                            _x.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj

                let source = Helper.sourceFold (_AbcdInterpolation.source + ".Value1") 
                                               [| _AbcdInterpolation.source
                                               ;  _x.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let AbcdInterpolation_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<AbcdInterpolation> i "value" true) |>
                        Seq.toArray

...
module FdBlackScholesVanillaEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_FdBlackScholesVanillaEngine_calculate", Description="Create a FdBlackScholesVanillaEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FdBlackScholesVanillaEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="FdBlackScholesVanillaEngine",Description = "Reference to FdBlackScholesVanillaEngine")>] 
         fdblackscholesvanillaengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _FdBlackScholesVanillaEngine = Helper.toCell<FdBlackScholesVanillaEngine> fdblackscholesvanillaengine "FdBlackScholesVanillaEngine" true 
                let builder () = withMnemonic mnemonic ((_FdBlackScholesVanillaEngine.cell :?> FdBlackScholesVanillaEngineModel).Calculate
                                                       ) :> ICell
                let format (o : FdBlackScholesVanillaEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FdBlackScholesVanillaEngine.source + ".Calculate") 
                                               [| _FdBlackScholesVanillaEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _FdBlackScholesVanillaEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let FdBlackScholesVanillaEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _cashDividendModel = Helper.toCell<CashDividendModel> cashDividendModel "cashDividendModel" true

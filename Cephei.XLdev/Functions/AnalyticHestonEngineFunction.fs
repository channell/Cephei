...
module AnalyticHestonEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticHestonEngine2", Description="Create a AnalyticHestonEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticHestonEngine_create2
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="model",Description = "Reference to model")>] 
         model : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic (Fun.AnalyticHestonEngine2 
                                                            _model.cell 
                                                            _relTolerance.cell 
                ...
                let source = Helper.sourceFold "Fun.AnalyticHestonEngine2" 
                                               [| _model.source
    ...
    *)
    [<ExcelFunction(Name="_AnalyticHestonEngine_calculate", Description="Create a AnalyticHestonEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticHestonEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_AnalyticHestonEngine.cell :?> AnalyticHestonEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticHestonEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticHestonEngine.source + ".Calculate") 

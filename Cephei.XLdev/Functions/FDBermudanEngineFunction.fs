...
module FDBermudanEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_FDBermudanEngine_calculate", Description="Create a FDBermudanEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FDBermudanEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_FDBermudanEngine.cell :?> FDBermudanEngineModel).Calculate
                                                       ) :> ICell
                let format (o : FDBermudanEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FDBermudanEngine.source + ".Calculate") 

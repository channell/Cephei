...
module InterpolatingCPICapFloorEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_InterpolatingCPICapFloorEngine_calculate", Description="Create a InterpolatingCPICapFloorEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let InterpolatingCPICapFloorEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_InterpolatingCPICapFloorEngine.cell :?> InterpolatingCPICapFloorEngineModel).Calculate
                                                       ) :> ICell
                let format (o : InterpolatingCPICapFloorEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_InterpolatingCPICapFloorEngine.source + ".Calculate") 
    ...
    let InterpolatingCPICapFloorEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _priceSurf = Helper.toHandle<Handle<CPICapFloorTermPriceSurface>> priceSurf "priceSurf" 

...
module AnalyticPTDHestonEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticPTDHestonEngine_calculate", Description="Create a AnalyticPTDHestonEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticPTDHestonEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_AnalyticPTDHestonEngine.cell :?> AnalyticPTDHestonEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticPTDHestonEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticPTDHestonEngine.source + ".Calculate") 

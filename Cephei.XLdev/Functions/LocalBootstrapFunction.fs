...
module LocalBootstrapFunction =
    ...
    *)
    [<ExcelFunction(Name="_LocalBootstrap_calculate", Description="Create a LocalBootstrap",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let LocalBootstrap_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_LocalBootstrap.cell :?> LocalBootstrapModel).Calculate
                                                       ) :> ICell
                let format (o : LocalBootstrap) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_LocalBootstrap.source + ".Calculate") 

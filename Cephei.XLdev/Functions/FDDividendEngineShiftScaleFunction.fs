...
module FDDividendEngineShiftScaleFunction =
    ...
    *)
    [<ExcelFunction(Name="_FDDividendEngineShiftScale_calculate", Description="Create a FDDividendEngineShiftScale",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FDDividendEngineShiftScale_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="FDDividendEngineShiftScale",Description = "Reference to FDDividendEngineShiftScale")>] 
         fddividendengineshiftscale : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_FDDividendEngineShiftScale.cell :?> FDDividendEngineShiftScaleModel).Calculate
                                                            _r.cell 
                                                       ) :> ICell
                let format (o : FDDividendEngineShiftScale) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FDDividendEngineShiftScale.source + ".Calculate") 
                                               [| _FDDividendEngineShiftScale.source
                                               ;  _r.source
                                               |]
                let hash = Helper.hashFold 

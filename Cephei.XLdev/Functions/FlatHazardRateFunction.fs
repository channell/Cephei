...
module FlatHazardRateFunction =
    ...
    *)
    [<ExcelFunction(Name="_FlatHazardRate", Description="Create a FlatHazardRate",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    ...
    let FlatHazardRate_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _settlementDays = Helper.toCell<int> settlementDays "settlementDays" true
                let _calendar = Helper.toCell<Calendar> calendar "calendar" true
                let _hazardRate = Helper.toHandle<Handle<Quote>> hazardRate "hazardRate" 
                let _dc = Helper.toCell<DayCounter> dc "dc" true

    ...
    let SEKLibor_clone
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _SEKLibor = Helper.toCell<SEKLibor> seklibor "SEKLibor" true 
                let _h = Helper.toHandle<Handle<YieldTermStructure>> h "h" 
    ...
    let SEKLibor_forecastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_SEKLibor.source + ".ForecastFixing1") 
                                               [| _SEKLibor.source
                                               ;  _fixingDate.source
                                               |]
                let hash = Helper.hashFold 
    ...
    let SEKLibor_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<SEKLibor> i "value" true) |>
                        Seq.toArray

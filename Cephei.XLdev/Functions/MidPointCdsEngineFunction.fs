...
module MidPointCdsEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_MidPointCdsEngine_calculate", Description="Create a MidPointCdsEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let MidPointCdsEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="MidPointCdsEngine",Description = "Reference to MidPointCdsEngine")>] 
         midpointcdsengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _MidPointCdsEngine = Helper.toCell<MidPointCdsEngine> midpointcdsengine "MidPointCdsEngine" true 
                let builder () = withMnemonic mnemonic ((_MidPointCdsEngine.cell :?> MidPointCdsEngineModel).Calculate
                                                       ) :> ICell
                let format (o : MidPointCdsEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_MidPointCdsEngine.source + ".Calculate") 
                                               [| _MidPointCdsEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _MidPointCdsEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder

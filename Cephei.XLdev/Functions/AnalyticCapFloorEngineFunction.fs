...
module AnalyticCapFloorEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticCapFloorEngine_calculate", Description="Create a AnalyticCapFloorEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticCapFloorEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_AnalyticCapFloorEngine.cell :?> AnalyticCapFloorEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticCapFloorEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticCapFloorEngine.source + ".Calculate") 

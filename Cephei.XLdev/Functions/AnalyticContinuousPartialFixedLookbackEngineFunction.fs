...
module AnalyticContinuousPartialFixedLookbackEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticContinuousPartialFixedLookbackEngine_calculate", Description="Create a AnalyticContinuousPartialFixedLookbackEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticContinuousPartialFixedLookbackEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="AnalyticContinuousPartialFixedLookbackEngine",Description = "Reference to AnalyticContinuousPartialFixedLookbackEngine")>] 
         analyticcontinuouspartialfixedlookbackengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _AnalyticContinuousPartialFixedLookbackEngine = Helper.toCell<AnalyticContinuousPartialFixedLookbackEngine> analyticcontinuouspartialfixedlookbackengine "AnalyticContinuousPartialFixedLookbackEngine" true 
                let builder () = withMnemonic mnemonic ((_AnalyticContinuousPartialFixedLookbackEngine.cell :?> AnalyticContinuousPartialFixedLookbackEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticContinuousPartialFixedLookbackEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticContinuousPartialFixedLookbackEngine.source + ".Calculate") 
                                               [| _AnalyticContinuousPartialFixedLookbackEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _AnalyticContinuousPartialFixedLookbackEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder

    ...
    let InterpolatedZeroInflationCurve_zeroRate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedZeroInflationCurve.source + ".ZeroRate1") 
                                               [| _InterpolatedZeroInflationCurve.source
                                               ;  _d.source
                                               ;  _instObsLag.source
                                               ;  _forceLinearInterpolation.source
    ...
    let InterpolatedZeroInflationCurve_zeroRate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedZeroInflationCurve.source + ".ZeroRate2") 
                                               [| _InterpolatedZeroInflationCurve.source
                                               ;  _d.source
                                               ;  _instObsLag.source
                                               |]
    ...
    let InterpolatedZeroInflationCurve_zeroRate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_InterpolatedZeroInflationCurve.source + ".ZeroRate3") 
                                               [| _InterpolatedZeroInflationCurve.source
                                               ;  _d.source
                                               |]
                let hash = Helper.hashFold 

...
module BlackCallableZeroCouponBondEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_BlackCallableZeroCouponBondEngine_calculate", Description="Create a BlackCallableZeroCouponBondEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let BlackCallableZeroCouponBondEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="BlackCallableZeroCouponBondEngine",Description = "Reference to BlackCallableZeroCouponBondEngine")>] 
         blackcallablezerocouponbondengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _BlackCallableZeroCouponBondEngine = Helper.toCell<BlackCallableZeroCouponBondEngine> blackcallablezerocouponbondengine "BlackCallableZeroCouponBondEngine" true 
                let builder () = withMnemonic mnemonic ((_BlackCallableZeroCouponBondEngine.cell :?> BlackCallableZeroCouponBondEngineModel).Calculate
                                                       ) :> ICell
                let format (o : BlackCallableZeroCouponBondEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_BlackCallableZeroCouponBondEngine.source + ".Calculate") 
                                               [| _BlackCallableZeroCouponBondEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _BlackCallableZeroCouponBondEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder

    ...
    let Singapore_addedHolidays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Singapore = Helper.toCell<Singapore> singapore "Singapore" true 
    ...
    let Singapore_advance
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_Singapore.source + ".Advance1") 
                                               [| _Singapore.source
                                               ;  _d.source
                                               ;  _p.source
                                               ;  _c.source
    ...
    let Singapore_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<Singapore> i "value" true) |>
                        Seq.toArray

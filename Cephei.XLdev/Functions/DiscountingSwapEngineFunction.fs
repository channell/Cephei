...
module DiscountingSwapEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_DiscountingSwapEngine_calculate", Description="Create a DiscountingSwapEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let DiscountingSwapEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="DiscountingSwapEngine",Description = "Reference to DiscountingSwapEngine")>] 
         discountingswapengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _DiscountingSwapEngine = Helper.toCell<DiscountingSwapEngine> discountingswapengine "DiscountingSwapEngine" true 
                let builder () = withMnemonic mnemonic ((_DiscountingSwapEngine.cell :?> DiscountingSwapEngineModel).Calculate
                                                       ) :> ICell
                let format (o : DiscountingSwapEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_DiscountingSwapEngine.source + ".Calculate") 
                                               [| _DiscountingSwapEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _DiscountingSwapEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder

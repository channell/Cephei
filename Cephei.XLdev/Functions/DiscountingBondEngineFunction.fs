...
module DiscountingBondEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_DiscountingBondEngine_calculate", Description="Create a DiscountingBondEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let DiscountingBondEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_DiscountingBondEngine.cell :?> DiscountingBondEngineModel).Calculate
                                                       ) :> ICell
                let format (o : DiscountingBondEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_DiscountingBondEngine.source + ".Calculate") 

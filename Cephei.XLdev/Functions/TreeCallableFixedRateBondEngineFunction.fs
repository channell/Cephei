...
module TreeCallableFixedRateBondEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_TreeCallableFixedRateBondEngine_calculate", Description="Create a TreeCallableFixedRateBondEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let TreeCallableFixedRateBondEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _TreeCallableFixedRateBondEngine = Helper.toCell<TreeCallableFixedRateBondEngine> treecallablefixedratebondengine "TreeCallableFixedRateBondEngine" true 
                let builder () = withMnemonic mnemonic ((_TreeCallableFixedRateBondEngine.cell :?> TreeCallableFixedRateBondEngineModel).Calculate
                                                       ) :> ICell
                let format (o : TreeCallableFixedRateBondEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_TreeCallableFixedRateBondEngine.source + ".Calculate") 
    ...
    let TreeCallableFixedRateBondEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<TreeCallableFixedRateBondEngine> i "value" true) |>
                        Seq.toArray

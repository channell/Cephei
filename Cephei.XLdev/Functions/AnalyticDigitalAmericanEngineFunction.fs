...
module AnalyticDigitalAmericanEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticDigitalAmericanEngine_calculate", Description="Create a AnalyticDigitalAmericanEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticDigitalAmericanEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_AnalyticDigitalAmericanEngine.cell :?> AnalyticDigitalAmericanEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticDigitalAmericanEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticDigitalAmericanEngine.source + ".Calculate") 
                                               [| _AnalyticDigitalAmericanEngine.source
                                               |]
                let hash = Helper.hashFold 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                Model.specify 
                    ...
                    ; source = "cell Generic.List<AnalyticDigitalAmericanEngine>(" + (Helper.sourceFoldArray (s) + ")")

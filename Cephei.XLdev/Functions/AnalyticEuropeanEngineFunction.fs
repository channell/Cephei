...
module AnalyticEuropeanEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticEuropeanEngine_calculate", Description="Create a AnalyticEuropeanEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticEuropeanEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="AnalyticEuropeanEngine",Description = "Reference to AnalyticEuropeanEngine")>] 
         analyticeuropeanengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _AnalyticEuropeanEngine = Helper.toCell<AnalyticEuropeanEngine> analyticeuropeanengine "AnalyticEuropeanEngine" true 
                let builder () = withMnemonic mnemonic ((_AnalyticEuropeanEngine.cell :?> AnalyticEuropeanEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticEuropeanEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticEuropeanEngine.source + ".Calculate") 
                                               [| _AnalyticEuropeanEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _AnalyticEuropeanEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder

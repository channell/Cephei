...
module BinomialBarrierEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_BinomialBarrierEngine_calculate", Description="Create a BinomialBarrierEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let BinomialBarrierEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_BinomialBarrierEngine.cell :?> BinomialBarrierEngineModel).Calculate
                                                       ) :> ICell
                let format (o : BinomialBarrierEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_BinomialBarrierEngine.source + ".Calculate") 

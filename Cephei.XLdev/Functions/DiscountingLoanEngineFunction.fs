...
module DiscountingLoanEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_DiscountingLoanEngine_calculate", Description="Create a DiscountingLoanEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let DiscountingLoanEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_DiscountingLoanEngine.cell :?> DiscountingLoanEngineModel).Calculate
                                                       ) :> ICell
                let format (o : DiscountingLoanEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_DiscountingLoanEngine.source + ".Calculate") 

...
module IsdaCdsEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_IsdaCdsEngine_calculate", Description="Create a IsdaCdsEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let IsdaCdsEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_IsdaCdsEngine.cell :?> IsdaCdsEngineModel).Calculate
                                                       ) :> ICell
                let format (o : IsdaCdsEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_IsdaCdsEngine.source + ".Calculate") 
    ...
    let IsdaCdsEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _numericalFix = Helper.toCell<NumericalFix> numericalFix "numericalFix" true
                let _accrualBias = Helper.toCell<AccrualBias> accrualBias "accrualBias" true
                let _forwardsInCouponPeriod = Helper.toCell<ForwardsInCouponPeriod> forwardsInCouponPeriod "forwardsInCouponPeriod" true

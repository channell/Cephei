...
module JuQuadraticApproximationEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_JuQuadraticApproximationEngine_calculate", Description="Create a JuQuadraticApproximationEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let JuQuadraticApproximationEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="JuQuadraticApproximationEngine",Description = "Reference to JuQuadraticApproximationEngine")>] 
         juquadraticapproximationengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _JuQuadraticApproximationEngine = Helper.toCell<JuQuadraticApproximationEngine> juquadraticapproximationengine "JuQuadraticApproximationEngine" true 
                let builder () = withMnemonic mnemonic ((_JuQuadraticApproximationEngine.cell :?> JuQuadraticApproximationEngineModel).Calculate
                                                       ) :> ICell
                let format (o : JuQuadraticApproximationEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_JuQuadraticApproximationEngine.source + ".Calculate") 
                                               [| _JuQuadraticApproximationEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _JuQuadraticApproximationEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let JuQuadraticApproximationEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic (Fun.JuQuadraticApproximationEngine 
                                                            _Process.cell 
                                                       ) :> ICell
    ...
    let JuQuadraticApproximationEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<JuQuadraticApproximationEngine> i "value" true) |>
                        Seq.toArray
                let c = a |> Array.map (fun i -> i.cell)
                let l = new Generic.List<ICell<JuQuadraticApproximationEngine>> (c)
                ...
                Model.specify 
                    ...
                    ; source = "cell Generic.List<JuQuadraticApproximationEngine>(" + (Helper.sourceFoldArray (s) + ")")

...
module CounterpartyAdjSwapEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_CounterpartyAdjSwapEngine_calculate", Description="Create a CounterpartyAdjSwapEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let CounterpartyAdjSwapEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="CounterpartyAdjSwapEngine",Description = "Reference to CounterpartyAdjSwapEngine")>] 
         counterpartyadjswapengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _CounterpartyAdjSwapEngine = Helper.toCell<CounterpartyAdjSwapEngine> counterpartyadjswapengine "CounterpartyAdjSwapEngine" true 
                let builder () = withMnemonic mnemonic ((_CounterpartyAdjSwapEngine.cell :?> CounterpartyAdjSwapEngineModel).Calculate
                                                       ) :> ICell
                let format (o : CounterpartyAdjSwapEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_CounterpartyAdjSwapEngine.source + ".Calculate") 
                                               [| _CounterpartyAdjSwapEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _CounterpartyAdjSwapEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let CounterpartyAdjSwapEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _blackVol = Helper.toHandle<Handle<Quote>> blackVol "blackVol" 
                let _ctptyDTS = Helper.toHandle<Handle<DefaultProbabilityTermStructure>> ctptyDTS "ctptyDTS" 
                let _ctptyRecoveryRate = Helper.toCell<double> ctptyRecoveryRate "ctptyRecoveryRate" true
                let _invstDTS = Helper.toHandle<Handle<DefaultProbabilityTermStructure>> invstDTS "invstDTS" 
    ...
    let CounterpartyAdjSwapEngine_create2
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _discountCurve = Helper.toHandle<Handle<YieldTermStructure>> discountCurve "discountCurve" 
                let _swaptionEngine = Helper.toHandle<Handle<IPricingEngine>> swaptionEngine "swaptionEngine" 
                let _ctptyDTS = Helper.toHandle<Handle<DefaultProbabilityTermStructure>> ctptyDTS "ctptyDTS" 
                let _ctptyRecoveryRate = Helper.toCell<double> ctptyRecoveryRate "ctptyRecoveryRate" true
                let _invstDTS = Helper.toHandle<Handle<DefaultProbabilityTermStructure>> invstDTS "invstDTS" 
                let _invstRecoveryRate = Helper.toCell<double> invstRecoveryRate "invstRecoveryRate" true

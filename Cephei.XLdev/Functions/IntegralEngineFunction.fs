...
module IntegralEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_IntegralEngine_calculate", Description="Create a IntegralEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let IntegralEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="IntegralEngine",Description = "Reference to IntegralEngine")>] 
         integralengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _IntegralEngine = Helper.toCell<IntegralEngine> integralengine "IntegralEngine" true 
                let builder () = withMnemonic mnemonic ((_IntegralEngine.cell :?> IntegralEngineModel).Calculate
                                                       ) :> ICell
                let format (o : IntegralEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_IntegralEngine.source + ".Calculate") 
                                               [| _IntegralEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _IntegralEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let IntegralEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<IntegralEngine> i "value" true) |>
                        Seq.toArray

    ...
    let Slovakia_addedHolidays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Slovakia = Helper.toCell<Slovakia> slovakia "Slovakia" true 
    ...
    let Slovakia_advance
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_Slovakia.source + ".Advance1") 
                                               [| _Slovakia.source
                                               ;  _d.source
                                               ;  _p.source
                                               ;  _c.source
    ...
    let Slovakia_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<Slovakia> i "value" true) |>
                        Seq.toArray

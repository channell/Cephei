...
module FDAmericanEngineFunction =
    ...
    let FDAmericanEngine_factory
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _FDAmericanEngine = Helper.toCell<FDAmericanEngine> fdamericanengine "FDAmericanEngine" true 
    ...
    let FDAmericanEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold "Fun.FDAmericanEngine" 
                                               [| _Process.source
                                               ;  _timeSteps.source
                                               ;  _gridPoints.source
    ...
    *)
    [<ExcelFunction(Name="_FDAmericanEngine_calculate", Description="Create a FDAmericanEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let FDAmericanEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="FDAmericanEngine",Description = "Reference to FDAmericanEngine")>] 
         fdamericanengine : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _FDAmericanEngine = Helper.toCell<FDAmericanEngine> fdamericanengine "FDAmericanEngine" true 
                let builder () = withMnemonic mnemonic ((_FDAmericanEngine.cell :?> FDAmericanEngineModel).Calculate
                                                       ) :> ICell
                let format (o : FDAmericanEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_FDAmericanEngine.source + ".Calculate") 
                                               [| _FDAmericanEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _FDAmericanEngine.cell
    ...
    let FDAmericanEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<FDAmericanEngine> i "value" true) |>
                        Seq.toArray

...
module BinomialConvertibleEngineFunction =
    ...
    let BinomialConvertibleEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic (Fun.BinomialConvertibleEngine 
                                                            _Process.cell 
                                                            _timeSteps.cell 
                                                       ) :> ICell
    ...
    *)
    [<ExcelFunction(Name="_BinomialConvertibleEngine_calculate", Description="Create a BinomialConvertibleEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let BinomialConvertibleEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="BinomialConvertibleEngine",Description = "Reference to BinomialConvertibleEngine")>] 
         binomialconvertibleengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _BinomialConvertibleEngine = Helper.toCell<BinomialConvertibleEngine> binomialconvertibleengine "BinomialConvertibleEngine" true 
                let builder () = withMnemonic mnemonic ((_BinomialConvertibleEngine.cell :?> BinomialConvertibleEngineModel).Calculate
                                                       ) :> ICell
                let format (o : BinomialConvertibleEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_BinomialConvertibleEngine.source + ".Calculate") 
                                               [| _BinomialConvertibleEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _BinomialConvertibleEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let BinomialConvertibleEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<BinomialConvertibleEngine> i "value" true) |>
                        Seq.toArray
                ...
                Model.specify 
                    ...
                    ; source = "cell Generic.List<BinomialConvertibleEngine>(" + (Helper.sourceFoldArray (s) + ")")

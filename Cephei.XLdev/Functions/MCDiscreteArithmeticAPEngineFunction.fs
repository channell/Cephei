...
module MCDiscreteArithmeticAPEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_MCDiscreteArithmeticAPEngine_calculate", Description="Create a MCDiscreteArithmeticAPEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let MCDiscreteArithmeticAPEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_MCDiscreteArithmeticAPEngine.cell :?> MCDiscreteArithmeticAPEngineModel).Calculate
                                                       ) :> ICell
                let format (o : MCDiscreteArithmeticAPEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_MCDiscreteArithmeticAPEngine.source + ".Calculate") 

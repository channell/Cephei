...
module AnalyticDiscreteGeometricAverageStrikeAsianEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_AnalyticDiscreteGeometricAverageStrikeAsianEngine_calculate", Description="Create a AnalyticDiscreteGeometricAverageStrikeAsianEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let AnalyticDiscreteGeometricAverageStrikeAsianEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="AnalyticDiscreteGeometricAverageStrikeAsianEngine",Description = "Reference to AnalyticDiscreteGeometricAverageStrikeAsianEngine")>] 
         analyticdiscretegeometricaveragestrikeasianengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then

            try

                let _AnalyticDiscreteGeometricAverageStrikeAsianEngine = Helper.toCell<AnalyticDiscreteGeometricAverageStrikeAsianEngine> analyticdiscretegeometricaveragestrikeasianengine "AnalyticDiscreteGeometricAverageStrikeAsianEngine" true 
                let builder () = withMnemonic mnemonic ((_AnalyticDiscreteGeometricAverageStrikeAsianEngine.cell :?> AnalyticDiscreteGeometricAverageStrikeAsianEngineModel).Calculate
                                                       ) :> ICell
                let format (o : AnalyticDiscreteGeometricAverageStrikeAsianEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_AnalyticDiscreteGeometricAverageStrikeAsianEngine.source + ".Calculate") 
                                               [| _AnalyticDiscreteGeometricAverageStrikeAsianEngine.source
                                               |]
                let hash = Helper.hashFold 
                                [| _AnalyticDiscreteGeometricAverageStrikeAsianEngine.cell
                                |]
                Model.specify 
                    { mnemonic = mnemonic
                    ; creator = builder
    ...
    let AnalyticDiscreteGeometricAverageStrikeAsianEngine_Range 
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="Objects",Description = "Identifer for the AnalyticDiscreteGeometricAverageStrikeAsianEngine")>] 
         values : obj[,])
         =
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<AnalyticDiscreteGeometricAverageStrikeAsianEngine> i "value" true) |>
                        Seq.toArray

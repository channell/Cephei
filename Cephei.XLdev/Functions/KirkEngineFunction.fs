...
module KirkEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_KirkEngine_calculate", Description="Create a KirkEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let KirkEngine_calculate
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="KirkEngine",Description = "Reference to KirkEngine")>] 
         kirkengine : obj)
        = 
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _KirkEngine = Helper.toCell<KirkEngine> kirkengine "KirkEngine" true 
                let builder () = withMnemonic mnemonic ((_KirkEngine.cell :?> KirkEngineModel).Calculate
                                                       ) :> ICell
                let format (o : KirkEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_KirkEngine.source + ".Calculate") 
                                               [| _KirkEngine.source
                                               |]
    ...
    *)
    ...
    let KirkEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic (Fun.KirkEngine 
                                                            _process1.cell 
                                                            _process2.cell 
                                                            _correlation.cell 
                                                       ) :> ICell
    ...
    let KirkEngine_Range 
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="Objects",Description = "Identifer for the KirkEngine")>] 
         values : obj[,])
         =
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<KirkEngine> i "value" true) |>
                        Seq.toArray
                ...
                Model.specify 
                    ...
                    ; source = "cell Generic.List<KirkEngine>(" + (Helper.sourceFoldArray (s) + ")")

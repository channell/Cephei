    ...
    let ArmijoLineSearch_create2
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold "Fun.ArmijoLineSearch2" 
                                               [||]
                let hash = Helper.hashFold 
                                [||]
                Model.specify 
    ...
    let ArmijoLineSearch_value
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _ArmijoLineSearch = Helper.toCell<ArmijoLineSearch> armijolinesearch "ArmijoLineSearch" true 
                ...
                let builder () = withMnemonic mnemonic ((_ArmijoLineSearch.cell :?> ArmijoLineSearchModel).Value
                                                            _P.cell 
                                                            _ecType.cell 
                                                            _endCriteria.cell 
                                                            _t_ini.cell 
    ...
    let ArmijoLineSearch_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<ArmijoLineSearch> i "value" true) |>
                        Seq.toArray

...
module MCBarrierEngineFunction =
    ...
    *)
    [<ExcelFunction(Name="_MCBarrierEngine_calculate", Description="Create a MCBarrierEngine",Category="Cephei", IsThreadSafe = true, IsExceptionSafe=true)>]
    let MCBarrierEngine_calculate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_MCBarrierEngine.cell :?> MCBarrierEngineModel).Calculate
                                                       ) :> ICell
                let format (o : MCBarrierEngine) (l:string) = o.ToString() :> obj

                let source = Helper.sourceFold (_MCBarrierEngine.source + ".Calculate") 

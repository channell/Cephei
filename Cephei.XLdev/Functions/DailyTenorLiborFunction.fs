    ...
    let DailyTenorLibor_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold "Fun.DailyTenorLibor1" 
                                               [| _familyName.source
                                               ;  _settlementDays.source
                                               ;  _currency.source
                                               ;  _financialCenterCalendar.source
    ...
    let DailyTenorLibor_businessDayConvention
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _DailyTenorLibor = Helper.toCell<DailyTenorLibor> dailytenorlibor "DailyTenorLibor" true 
    ...
    let DailyTenorLibor_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<DailyTenorLibor> i "value" true) |>
                        Seq.toArray

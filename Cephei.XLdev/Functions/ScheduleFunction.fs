    ...
    let Schedule_isRegular
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let format (i : IGeneric.List<bool>) (l : string) = (Helper.Range.fromArray (i.ToArray()) l)
    ...
    let Schedule_create2
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _calendar = Helper.toCell<Calendar> calendar "calendar" true
                let _convention = Helper.toCell<BusinessDayConvention> convention "convention" true
                let _terminationDateConvention = Helper.toNullable<Nullable<BusinessDayConvention>> terminationDateConvention "terminationDateConvention"
                let _tenor = Helper.toCell<Period> tenor "tenor" true
                let _rule = Helper.toNullable<Nullable<DateGeneration.Rule>> rule "rule"
                let _endOfMonth = Helper.toNullable<Nullable<bool>> endOfMonth "endOfMonth"
                let _isRegular = Helper.toCell<IGeneric.List<bool>> isRegular "isRegular" true

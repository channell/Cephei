...
module NonCentralCumulativeChiSquareSankaranApproxFunction =
    ...
    *)
    ...
    let NonCentralCumulativeChiSquareSankaranApprox_Range 
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifer for the value")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="Objects",Description = "Identifer for the NonCentralCumulativeChiSquareSankaranApprox")>] 
         values : obj[,])
         =
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<NonCentralCumulativeChiSquareSankaranApprox> i "value" true) |>
                        Seq.toArray

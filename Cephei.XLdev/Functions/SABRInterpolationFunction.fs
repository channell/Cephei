    ...
    let SABRInterpolation_alpha
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _SABRInterpolation = Helper.toCell<SABRInterpolation> sabrinterpolation "SABRInterpolation" true 
    ...
    let SABRInterpolation_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _alpha = Helper.toNullable<Nullable<double>> alpha "alpha"
                let _beta = Helper.toNullable<Nullable<double>> beta "beta"
                let _nu = Helper.toNullable<Nullable<double>> nu "nu"
                let _rho = Helper.toNullable<Nullable<double>> rho "rho"
    ...
    let SABRInterpolation_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<SABRInterpolation> i "value" true) |>
                        Seq.toArray

    ...
    let CallableBond_callability
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CallableBond = Helper.toCell<CallableBond> callablebond "CallableBond" true 
    ...
    let CallableBond_cleanPriceOAS
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _engineTS = Helper.toHandle<Handle<YieldTermStructure>> engineTS "engineTS" 
    ...
    let CallableBond_cleanPrice
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_CallableBond.source + ".CleanPrice1") 
                                               [| _CallableBond.source
                                               ;  _Yield.source
                                               ;  _dc.source
                                               ;  _comp.source
    ...
    let CallableBond_dirtyPrice
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_CallableBond.source + ".DirtyPrice1") 
                                               [| _CallableBond.source
                                               |]
                let hash = Helper.hashFold 
                                [| _CallableBond.cell
    ...
    let CallableBond_settlementValue
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let source = Helper.sourceFold (_CallableBond.source + ".SettlementValue1") 
                                               [| _CallableBond.source
                                               |]
                let hash = Helper.hashFold 
                                [| _CallableBond.cell
    ...
    let CallableBond_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<CallableBond> i "value" true) |>
                        Seq.toArray

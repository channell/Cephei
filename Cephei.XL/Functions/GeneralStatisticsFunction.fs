    ...
    let GeneralStatistics_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.GeneralStatistics ()
                                                       ) :> ICell
    ...
    let GeneralStatistics_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<GeneralStatistics> (c)) :> ICell
                let format (i : Cephei.Cell.List<GeneralStatistics>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<GeneralStatistics>(" + (Helper.sourceFoldArray (s) + "))"))

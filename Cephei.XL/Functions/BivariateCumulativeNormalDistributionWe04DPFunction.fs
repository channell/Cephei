    ...
    let BivariateCumulativeNormalDistributionWe04DP_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.BivariateCumulativeNormalDistributionWe04DP 
                                                            _rho.cell 
                                                       ) :> ICell
    ...
    let BivariateCumulativeNormalDistributionWe04DP_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<BivariateCumulativeNormalDistributionWe04DP> (c)) :> ICell
                let format (i : Cephei.Cell.List<BivariateCumulativeNormalDistributionWe04DP>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BivariateCumulativeNormalDistributionWe04DP>(" + (Helper.sourceFoldArray (s) + "))"))

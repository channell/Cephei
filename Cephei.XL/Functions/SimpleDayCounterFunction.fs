    ...
    let SimpleDayCounter_dayCount
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _SimpleDayCounter = Helper.toModelReference<SimpleDayCounter> simpledaycounter "SimpleDayCounter"  
    ...
    let SimpleDayCounter_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<SimpleDayCounter> (c)) :> ICell
                let format (i : Cephei.Cell.List<SimpleDayCounter>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<SimpleDayCounter>(" + (Helper.sourceFoldArray (s) + "))"))

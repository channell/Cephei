    ...
    *)
    ...
    let PenaltyFunction_create
        ...
        ([<ExcelArgument(Name="initialIndex",Description = "int")>] 
         initialIndex : obj)
        ([<ExcelArgument(Name="rateHelpers",Description = "'U range")>] 
         rateHelpers : obj)
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.PenaltyFunction 
                                                            _curve.cell 
                                                            _initialIndex.cell 
                                                            _rateHelpers.cell 
                                                            _start.cell 
    ...
    let PenaltyFunction_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<PenaltyFunction> (c)) :> ICell
                let format (i : Cephei.Cell.List<PenaltyFunction>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<PenaltyFunction>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let CmsSpreadLeg_value
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CmsSpreadLeg = Helper.toModelReference<CmsSpreadLeg> cmsspreadleg "CmsSpreadLeg"  
    ...
    let CmsSpreadLeg_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<CmsSpreadLeg> (c)) :> ICell
                let format (i : Cephei.Cell.List<CmsSpreadLeg>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<CmsSpreadLeg>(" + (Helper.sourceFoldArray (s) + "))"))

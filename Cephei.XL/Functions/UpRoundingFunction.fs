    ...
    let UpRounding_Digit
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _UpRounding = Helper.toModelReference<UpRounding> uprounding "UpRounding"  
    ...
    let UpRounding_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<UpRounding> (c)) :> ICell
                let format (i : Cephei.Cell.List<UpRounding>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<UpRounding>(" + (Helper.sourceFoldArray (s) + "))"))

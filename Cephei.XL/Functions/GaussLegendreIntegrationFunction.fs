    ...
    let GaussLegendreIntegration_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.GaussLegendreIntegration 
                                                            _n.cell 
                                                       ) :> ICell
    ...
    let GaussLegendreIntegration_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<GaussLegendreIntegration> (c)) :> ICell
                let format (i : Cephei.Cell.List<GaussLegendreIntegration>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<GaussLegendreIntegration>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let BasketOption_delta
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _BasketOption = Helper.toModelReference<BasketOption> basketoption "BasketOption"  
    ...
    let BasketOption_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<BasketOption> (c)) :> ICell
                let format (i : Cephei.Cell.List<BasketOption>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BasketOption>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let ConvexMonotone2Helper_fNext
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _ConvexMonotone2Helper = Helper.toModelReference<ConvexMonotone2Helper> convexmonotone2helper "ConvexMonotone2Helper"  
    ...
    let ConvexMonotone2Helper_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<ConvexMonotone2Helper> (c)) :> ICell
                let format (i : Cephei.Cell.List<ConvexMonotone2Helper>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<ConvexMonotone2Helper>(" + (Helper.sourceFoldArray (s) + "))"))

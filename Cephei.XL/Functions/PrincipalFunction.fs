    ...
    let Principal_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.Principal ()
                                                       ) :> ICell
    ...
    let Principal_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.Principal1 
                                                            _amount.cell 
                                                            _nominal.cell 
                                                            _paymentDate.cell 
    ...
    let Principal_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Principal> (c)) :> ICell
                let format (i : Cephei.Cell.List<Principal>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Principal>(" + (Helper.sourceFoldArray (s) + "))"))

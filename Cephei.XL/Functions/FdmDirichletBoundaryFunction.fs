    ...
    let FdmDirichletBoundary_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.FdmDirichletBoundary 
                                                            _mesher.cell 
    ...
    let FdmDirichletBoundary_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<FdmDirichletBoundary> (c)) :> ICell
                let format (i : Cephei.Cell.List<FdmDirichletBoundary>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<FdmDirichletBoundary>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let SecondOrderMixedDerivativeOp_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.SecondOrderMixedDerivativeOp 
                                                            _d0.cell 
                                                            _d1.cell 
                                                            _mesher.cell 
                                                       ) :> ICell
    ...
    let SecondOrderMixedDerivativeOp_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<SecondOrderMixedDerivativeOp> (c)) :> ICell
                let format (i : Cephei.Cell.List<SecondOrderMixedDerivativeOp>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<SecondOrderMixedDerivativeOp>(" + (Helper.sourceFoldArray (s) + "))"))

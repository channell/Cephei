    ...
    let BinomialVanillaEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.BinomialVanillaEngine 
                                                            _Process.cell 
                                                            _timeSteps.cell 
    ...
    let BinomialVanillaEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<BinomialVanillaEngine> (c)) :> ICell
                let format (i : Cephei.Cell.List<BinomialVanillaEngine>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BinomialVanillaEngine>(" + (Helper.sourceFoldArray (s) + "))"))

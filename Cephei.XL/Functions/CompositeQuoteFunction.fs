    ...
    *)
    ...
    let CompositeQuote_create
        ...
        ([<ExcelArgument(Name="element1",Description = "Quote")>] 
         element1 : obj)
        ([<ExcelArgument(Name="element2",Description = "Quote")>] 
         element2 : obj)
    ...
    let CompositeQuote_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<CompositeQuote> i "value" ) |>
                        Seq.toArray
                let c = a |> Array.map (fun i -> i.cell)

                let s = a |> Array.map (fun i -> i.source)
                let builder (current : ICell) = (new Cephei.Cell.List<CompositeQuote> (c)) :> ICell
                let format (i : Cephei.Cell.List<CompositeQuote>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<CompositeQuote>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let COPCurrency_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.COPCurrency ()
                                                       ) :> ICell
    ...
    let COPCurrency_code
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).Code
                                                       ) :> ICell
    ...
    let COPCurrency_empty
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).Empty
                                                       ) :> ICell
    ...
    let COPCurrency_Equals
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).Equals
                                                            _o.cell 
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let COPCurrency_format
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).Format
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let COPCurrency_fractionsPerUnit
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).FractionsPerUnit
                                                       ) :> ICell
    ...
    let COPCurrency_fractionSymbol
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).FractionSymbol
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let COPCurrency_name
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).Name
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let COPCurrency_numericCode
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).NumericCode
                                                       ) :> ICell
                let format (o : int) (l:string) = o :> obj
    ...
    let COPCurrency_rounding
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).Rounding
                                                       ) :> ICell
    ...
    let COPCurrency_symbol
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).Symbol
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let COPCurrency_ToString
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).ToString
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let COPCurrency_triangulationCurrency
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _COPCurrency = Helper.toModelReference<COPCurrency> copcurrency "COPCurrency"  
                let builder (current : ICell) = ((COPCurrencyModel.Cast _COPCurrency.cell).TriangulationCurrency
                                                       ) :> ICell
    ...
    let COPCurrency_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<COPCurrency> (c)) :> ICell
                let format (i : Cephei.Cell.List<COPCurrency>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<COPCurrency>(" + (Helper.sourceFoldArray (s) + "))"))

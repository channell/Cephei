    ...
    let DeltaVolQuote_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.DeltaVolQuote 
                                                            _vol.cell 
                                                            _deltaType.cell 
                                                            _maturity.cell 
                                                            _atmType.cell 
    ...
    let DeltaVolQuote_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _maturity = Helper.toCell<double> maturity "maturity" 
                let _deltaType = Helper.toCell<DeltaVolQuote.DeltaType> deltaType "deltaType" 
                let builder (current : ICell) = (Fun.DeltaVolQuote1 
                                                            _delta.cell 
                                                            _vol.cell 
                                                            _maturity.cell 
                                                            _deltaType.cell 
    ...
    let DeltaVolQuote_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<DeltaVolQuote> (c)) :> ICell
                let format (i : Cephei.Cell.List<DeltaVolQuote>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<DeltaVolQuote>(" + (Helper.sourceFoldArray (s) + "))"))

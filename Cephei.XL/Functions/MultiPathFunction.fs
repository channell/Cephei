...
module MultiPathFunction =
    ...
    *)
    ...
    let MultiPath_create2
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifier for Cell")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="multiPath range",Description = "Path range")>] 
         multiPath : obj)
        = 
        if not (Model.IsInFunctionWizard()) then
    ...
    let MultiPath_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<MultiPath> (c)) :> ICell
                let format (i : Cephei.Cell.List<MultiPath>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<MultiPath>(" + (Helper.sourceFoldArray (s) + "))"))

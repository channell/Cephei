    ...
    let DiscretizedBarrierOption_checkBarrier
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _DiscretizedBarrierOption = Helper.toModelReference<DiscretizedBarrierOption> discretizedbarrieroption "DiscretizedBarrierOption"  
    ...
    let DiscretizedBarrierOption_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<DiscretizedBarrierOption> (c)) :> ICell
                let format (i : Cephei.Cell.List<DiscretizedBarrierOption>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<DiscretizedBarrierOption>(" + (Helper.sourceFoldArray (s) + "))"))

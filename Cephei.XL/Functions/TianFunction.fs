    ...
    let Tian_factory
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tian = Helper.toModelReference<Tian> tian "Tian"  
    ...
    let Tian_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Tian> (c)) :> ICell
                let format (i : Cephei.Cell.List<Tian>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Tian>(" + (Helper.sourceFoldArray (s) + "))"))

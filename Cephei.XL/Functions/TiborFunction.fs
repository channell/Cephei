    ...
    let Tibor_businessDayConvention
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).BusinessDayConvention
                                                       ) :> ICell
    ...
    let Tibor_clone
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).Clone
                                                            _forwarding.cell 
                                                       ) :> ICell
    ...
    let Tibor_endOfMonth
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).EndOfMonth
                                                       ) :> ICell
    ...
    let Tibor_forecastFixing1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).ForecastFixing1
                                                            _d1.cell 
                                                            _d2.cell 
                                                            _t.cell 
                                                       ) :> ICell
    ...
    let Tibor_forecastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).ForecastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let Tibor_forwardingTermStructure
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).ForwardingTermStructure
                                                       ) :> ICell
    ...
    let Tibor_maturityDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).MaturityDate
                                                            _valueDate.cell 
                                                       ) :> ICell
    ...
    let Tibor_currency
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).Currency
                                                       ) :> ICell
    ...
    let Tibor_dayCounter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).DayCounter
                                                       ) :> ICell
    ...
    let Tibor_familyName
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).FamilyName
                                                       ) :> ICell
    ...
    let Tibor_fixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).Fixing
                                                            _fixingDate.cell 
    ...
    let Tibor_fixingCalendar
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).FixingCalendar
                                                       ) :> ICell
    ...
    let Tibor_fixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let _valueDate = Helper.toCell<Date> valueDate "valueDate" true
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).FixingDate
                                                            _valueDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let Tibor_fixingDays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).FixingDays
                                                       ) :> ICell
    ...
    let Tibor_isValidFixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).IsValidFixingDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let Tibor_name
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).Name
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let Tibor_pastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).PastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
    ...
    let Tibor_tenor
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).Tenor
                                                       ) :> ICell
    ...
    let Tibor_update
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).Update
                                                       ) :> ICell
    ...
    let Tibor_valueDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).ValueDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let Tibor_addFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).AddFixing
                                                            _d.cell 
                                                            _v.cell 
    ...
    let Tibor_addFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" true
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).AddFixings
                                                            _d.cell 
                                                            _v.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
    ...
    let Tibor_addFixings1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" true
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).AddFixings1
                                                            _source.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
                let format (o : Tibor) (l:string) = o.ToString() :> obj
    ...
    let Tibor_allowsNativeFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).AllowsNativeFixings
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let Tibor_clearFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).ClearFixings
                                                       ) :> ICell
                let format (o : Tibor) (l:string) = o.ToString() :> obj
    ...
    let Tibor_registerWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).RegisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : Tibor) (l:string) = o.ToString() :> obj
    ...
    let Tibor_timeSeries
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).TimeSeries
                                                       ) :> ICell
    ...
    let Tibor_unregisterWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Tibor = Helper.toCell<Tibor> tibor "Tibor" true 
                let _handler = Helper.toCell<Callback> handler "handler" true
                let builder () = withMnemonic mnemonic ((_Tibor.cell :?> TiborModel).UnregisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : Tibor) (l:string) = o.ToString() :> obj
    ...
    let Tibor_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<Tibor> i "value" true) |>
                        Seq.toArray

...
module BEFCurrencyFunction =
    ...
    *)
    ...
    let BEFCurrency_code
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifier for Cell")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="BEFCurrency",Description = "BEFCurrency")>] 
         befcurrency : obj)
        = 
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _BEFCurrency = Helper.toModelReference<BEFCurrency> befcurrency "BEFCurrency"  
    ...
    let BEFCurrency_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<BEFCurrency> (c)) :> ICell
                let format (i : Cephei.Cell.List<BEFCurrency>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BEFCurrency>(" + (Helper.sourceFoldArray (s) + "))"))

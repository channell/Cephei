    ...
    let LocalVolCurve_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.LocalVolCurve 
                                                            _curve.cell 
                                                       ) :> ICell
    ...
    let LocalVolCurve_localVol
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((LocalVolCurveModel.Cast _LocalVolCurve.cell).LocalVol
                                                            _t.cell 
    ...
    let LocalVolCurve_localVol1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _underlyingLevel = Helper.toCell<double> underlyingLevel "underlyingLevel" 
                let _extrapolate = Helper.toCell<bool> extrapolate "extrapolate" 
                let builder (current : ICell) = ((LocalVolCurveModel.Cast _LocalVolCurve.cell).LocalVol1
                                                            _d.cell 
                                                            _underlyingLevel.cell 
                                                            _extrapolate.cell 
                                                       ) :> ICell

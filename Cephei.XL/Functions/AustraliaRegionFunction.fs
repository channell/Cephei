    ...
    let AustraliaRegion_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.AustraliaRegion ()
                                                       ) :> ICell
    ...
    let AustraliaRegion_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<AustraliaRegion> (c)) :> ICell
                let format (i : Cephei.Cell.List<AustraliaRegion>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<AustraliaRegion>(" + (Helper.sourceFoldArray (s) + "))"))

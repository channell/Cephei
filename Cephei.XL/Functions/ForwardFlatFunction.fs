    ...
    let ForwardFlat_global
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _ForwardFlat = Helper.toModelReference<ForwardFlat> forwardflat "ForwardFlat"  
                let builder (current : ICell) = ((ForwardFlatModel.Cast _ForwardFlat.cell).Global
                                                       ) :> ICell
    ...
    let ForwardFlat_interpolate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((ForwardFlatModel.Cast _ForwardFlat.cell).Interpolate
                                                            _xBegin.cell 
                                                            _size.cell 
                                                            _yBegin.cell 
                                                       ) :> ICell
    ...
    let ForwardFlat_requiredPoints
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _ForwardFlat = Helper.toModelReference<ForwardFlat> forwardflat "ForwardFlat"  
                let builder (current : ICell) = ((ForwardFlatModel.Cast _ForwardFlat.cell).RequiredPoints
                                                       ) :> ICell
    ...
    let ForwardFlat_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<ForwardFlat> (c)) :> ICell
                let format (i : Cephei.Cell.List<ForwardFlat>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<ForwardFlat>(" + (Helper.sourceFoldArray (s) + "))"))

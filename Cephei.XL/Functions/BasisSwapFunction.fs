    ...
    let BasisSwap_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.BasisSwap1 
                                                            _Type.cell 
                                                            _nominal.cell 
    ...
    let BasisSwap_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _pricingEngine = Helper.toCell<IPricingEngine> pricingEngine "pricingEngine"  
                let _evaluationDate = Helper.toCell<Date> evaluationDate "evaluationDate"  
                let builder (current : ICell) = (Fun.BasisSwap
                                                            _Type.cell 
                                                            _nominal.cell 
                                                            _float1Schedule.cell 
                                                            _iborIndex1.cell 
    ...
    let BasisSwap_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<BasisSwap> (c)) :> ICell
                let format (i : Cephei.Cell.List<BasisSwap>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BasisSwap>(" + (Helper.sourceFoldArray (s) + "))"))

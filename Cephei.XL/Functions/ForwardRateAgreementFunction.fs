    ...
    let ForwardRateAgreement_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.ForwardRateAgreement 
                                                            _valueDate.cell 
                                                            _maturityDate.cell 
                                                            _Type.cell 
    ...
    let ForwardRateAgreement_impliedYield
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((ForwardRateAgreementModel.Cast _ForwardRateAgreement.cell).ImpliedYield
    ...
    let ForwardRateAgreement_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<ForwardRateAgreement> (c)) :> ICell
                let format (i : Cephei.Cell.List<ForwardRateAgreement>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<ForwardRateAgreement>(" + (Helper.sourceFoldArray (s) + "))"))

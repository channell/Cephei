    ...
    let DatedOISRateHelper_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.DatedOISRateHelper 
                                                            _startDate.cell 
                                                            _endDate.cell 
                                                            _fixedRate.cell 
    ...
    let DatedOISRateHelper_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<DatedOISRateHelper> (c)) :> ICell
                let format (i : Cephei.Cell.List<DatedOISRateHelper>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<DatedOISRateHelper>(" + (Helper.sourceFoldArray (s) + "))"))

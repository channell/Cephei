    ...
    let EURLibor8M_maturityDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let _valueDate = Helper.toCell<Date> valueDate "valueDate" true
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).MaturityDate
                                                            _valueDate.cell 
                                                       ) :> ICell
    ...
    let EURLibor8M_valueDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).ValueDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let EURLibor8M_businessDayConvention
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).BusinessDayConvention
                                                       ) :> ICell
    ...
    let EURLibor8M_clone
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).Clone
                                                            _forwarding.cell 
                                                       ) :> ICell
    ...
    let EURLibor8M_endOfMonth
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).EndOfMonth
                                                       ) :> ICell
    ...
    let EURLibor8M_forecastFixing1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).ForecastFixing1
                                                            _d1.cell 
                                                            _d2.cell 
                                                            _t.cell 
                                                       ) :> ICell
    ...
    let EURLibor8M_forecastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).ForecastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let EURLibor8M_forwardingTermStructure
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).ForwardingTermStructure
                                                       ) :> ICell
    ...
    let EURLibor8M_currency
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).Currency
                                                       ) :> ICell
    ...
    let EURLibor8M_dayCounter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).DayCounter
                                                       ) :> ICell
    ...
    let EURLibor8M_familyName
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).FamilyName
                                                       ) :> ICell
    ...
    let EURLibor8M_fixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).Fixing
                                                            _fixingDate.cell 
    ...
    let EURLibor8M_fixingCalendar
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).FixingCalendar
                                                       ) :> ICell
    ...
    let EURLibor8M_fixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let _valueDate = Helper.toCell<Date> valueDate "valueDate" true
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).FixingDate
                                                            _valueDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let EURLibor8M_fixingDays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).FixingDays
                                                       ) :> ICell
    ...
    let EURLibor8M_isValidFixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).IsValidFixingDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let EURLibor8M_name
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).Name
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let EURLibor8M_pastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).PastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
    ...
    let EURLibor8M_tenor
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).Tenor
                                                       ) :> ICell
    ...
    let EURLibor8M_update
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).Update
                                                       ) :> ICell
    ...
    let EURLibor8M_addFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).AddFixing
                                                            _d.cell 
                                                            _v.cell 
    ...
    let EURLibor8M_addFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" true
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).AddFixings
                                                            _d.cell 
                                                            _v.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
    ...
    let EURLibor8M_addFixings1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" true
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).AddFixings1
                                                            _source.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
                let format (o : EURLibor8M) (l:string) = o.ToString() :> obj
    ...
    let EURLibor8M_allowsNativeFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).AllowsNativeFixings
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let EURLibor8M_clearFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).ClearFixings
                                                       ) :> ICell
                let format (o : EURLibor8M) (l:string) = o.ToString() :> obj
    ...
    let EURLibor8M_registerWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).RegisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : EURLibor8M) (l:string) = o.ToString() :> obj
    ...
    let EURLibor8M_timeSeries
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).TimeSeries
                                                       ) :> ICell
    ...
    let EURLibor8M_unregisterWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _EURLibor8M = Helper.toCell<EURLibor8M> eurlibor8m "EURLibor8M" true 
                let _handler = Helper.toCell<Callback> handler "handler" true
                let builder () = withMnemonic mnemonic ((_EURLibor8M.cell :?> EURLibor8MModel).UnregisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : EURLibor8M) (l:string) = o.ToString() :> obj
    ...
    let EURLibor8M_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<EURLibor8M> i "value" true) |>
                        Seq.toArray

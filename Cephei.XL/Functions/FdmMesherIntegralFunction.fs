    ...
    let FdmMesherIntegral_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.FdmMesherIntegral 
                                                            _mesher.cell 
                                                            _integrator1d.cell 
                                                       ) :> ICell
    ...
    let FdmMesherIntegral_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<FdmMesherIntegral> (c)) :> ICell
                let format (i : Cephei.Cell.List<FdmMesherIntegral>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<FdmMesherIntegral>(" + (Helper.sourceFoldArray (s) + "))"))

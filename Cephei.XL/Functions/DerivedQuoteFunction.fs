    ...
    let DerivedQuote_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.DerivedQuote 
                                                            _element.cell 
                                                            _f.cell 
                                                       ) :> ICell
    ...
    let DerivedQuote_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<DerivedQuote> (c)) :> ICell
                let format (i : Cephei.Cell.List<DerivedQuote>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<DerivedQuote>(" + (Helper.sourceFoldArray (s) + "))"))

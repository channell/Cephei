    ...
    let DoubleBarrierOption_impliedVolatility
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _DoubleBarrierOption = Helper.toModelReference<DoubleBarrierOption> doublebarrieroption "DoubleBarrierOption"  
    ...
    let DoubleBarrierOption_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<DoubleBarrierOption> (c)) :> ICell
                let format (i : Cephei.Cell.List<DoubleBarrierOption>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<DoubleBarrierOption>(" + (Helper.sourceFoldArray (s) + "))"))

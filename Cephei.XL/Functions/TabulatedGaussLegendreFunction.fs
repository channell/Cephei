    ...
    let TabulatedGaussLegendre_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.TabulatedGaussLegendre 
                                                            _n.cell 
                                                       ) :> ICell
    ...
    let TabulatedGaussLegendre_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<TabulatedGaussLegendre> (c)) :> ICell
                let format (i : Cephei.Cell.List<TabulatedGaussLegendre>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<TabulatedGaussLegendre>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let HUFCurrency_code
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _HUFCurrency = Helper.toModelReference<HUFCurrency> hufcurrency "HUFCurrency"  
    ...
    let HUFCurrency_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<HUFCurrency> (c)) :> ICell
                let format (i : Cephei.Cell.List<HUFCurrency>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<HUFCurrency>(" + (Helper.sourceFoldArray (s) + "))"))

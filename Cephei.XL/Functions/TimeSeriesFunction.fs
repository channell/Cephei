    ...
    let TimeSeries_Add
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _TimeSeries = Helper.toModelReference<TimeSeries> timeseries "TimeSeries"  
    ...
    let TimeSeries_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<TimeSeries> (c)) :> ICell
                let format (i : Cephei.Cell.List<TimeSeries>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<TimeSeries>(" + (Helper.sourceFoldArray (s) + "))"))

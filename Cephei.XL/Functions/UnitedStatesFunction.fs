...
module UnitedStatesFunction =
    ...
    *)
    ...
    let UnitedStates_create
        ([<ExcelArgument(Name="Mnemonic",Description = "UnitedStates")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="m",Description = "UnitedStates.Market")>] 
         m : obj)
        = 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                Model.specify 
                    ...
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "cell Generic.List<UnitedStates>(" + (Helper.sourceFoldArray (s) + ")"))

    ...
    let GaussianQuadrature_order
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _GaussianQuadrature = Helper.toModelReference<GaussianQuadrature> gaussianquadrature "GaussianQuadrature"  
    ...
    let GaussianQuadrature_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<GaussianQuadrature> (c)) :> ICell
                let format (i : Cephei.Cell.List<GaussianQuadrature>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<GaussianQuadrature>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let AnalyticDigitalAmericanEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.AnalyticDigitalAmericanEngine 
                                                            _Process.cell 
    ...
    let AnalyticDigitalAmericanEngine_knock_in
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AnalyticDigitalAmericanEngine = Helper.toModelReference<AnalyticDigitalAmericanEngine> analyticdigitalamericanengine "AnalyticDigitalAmericanEngine"  
                let builder (current : ICell) = ((AnalyticDigitalAmericanEngineModel.Cast _AnalyticDigitalAmericanEngine.cell).Knock_in
                                                       ) :> ICell
    ...
    let AnalyticDigitalAmericanEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<AnalyticDigitalAmericanEngine> (c)) :> ICell
                let format (i : Cephei.Cell.List<AnalyticDigitalAmericanEngine>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<AnalyticDigitalAmericanEngine>(" + (Helper.sourceFoldArray (s) + "))"))

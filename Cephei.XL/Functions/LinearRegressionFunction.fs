    ...
    let LinearRegression_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.LinearRegression1
                                                            _x.cell 
                                                            _y.cell 
                                                       ) :> ICell
    ...
    let LinearRegression_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _y = Helper.toCell<Generic.List<double>> y "y" 
                let builder (current : ICell) = (Fun.LinearRegression
                                                            _x.cell 
                                                            _y.cell 
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<LinearRegression>) l
    ...
    let LinearRegression_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<LinearRegression> (c)) :> ICell
                let format (i : Cephei.Cell.List<LinearRegression>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<LinearRegression>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let PositiveConstraint_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.PositiveConstraint ()
                                                       ) :> ICell
    ...
    let PositiveConstraint_update
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((PositiveConstraintModel.Cast _PositiveConstraint.cell).Update
                                                            _p.cell 
                                                            _direction.cell 
                                                            _beta.cell 
                                                       ) :> ICell
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                Model.specify 
                    ...
                    ; source =  (fun () -> "(new Cephei.Cell.List<PositiveConstraint>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let JPYCurrency_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.JPYCurrency ()
                                                       ) :> ICell
    ...
    let JPYCurrency_code
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).Code
                                                       ) :> ICell
    ...
    let JPYCurrency_empty
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).Empty
                                                       ) :> ICell
    ...
    let JPYCurrency_Equals
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).Equals
                                                            _o.cell 
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let JPYCurrency_format
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).Format
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let JPYCurrency_fractionsPerUnit
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).FractionsPerUnit
                                                       ) :> ICell
    ...
    let JPYCurrency_fractionSymbol
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).FractionSymbol
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let JPYCurrency_name
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).Name
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let JPYCurrency_numericCode
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).NumericCode
                                                       ) :> ICell
                let format (o : int) (l:string) = o :> obj
    ...
    let JPYCurrency_rounding
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).Rounding
                                                       ) :> ICell
    ...
    let JPYCurrency_symbol
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).Symbol
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let JPYCurrency_ToString
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).ToString
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let JPYCurrency_triangulationCurrency
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _JPYCurrency = Helper.toModelReference<JPYCurrency> jpycurrency "JPYCurrency"  
                let builder (current : ICell) = ((JPYCurrencyModel.Cast _JPYCurrency.cell).TriangulationCurrency
                                                       ) :> ICell
    ...
    let JPYCurrency_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let format (i : Cephei.Cell.List<JPYCurrency>) (l : string) = Helper.Range.fromModelList i l

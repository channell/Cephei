    ...
    let PdeBSM_diffusion
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _PdeBSM = Helper.toModelReference<PdeBSM> pdebsm "PdeBSM"  
    ...
    let PdeBSM_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<PdeBSM> (c)) :> ICell
                let format (i : Cephei.Cell.List<PdeBSM>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<PdeBSM>(" + (Helper.sourceFoldArray (s) + "))"))

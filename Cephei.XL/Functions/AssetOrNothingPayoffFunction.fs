    ...
    let AssetOrNothingPayoff_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.AssetOrNothingPayoff 
                                                            _Type.cell 
                                                            _strike.cell 
                                                       ) :> ICell
    ...
    let AssetOrNothingPayoff_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<AssetOrNothingPayoff> (c)) :> ICell
                let format (i : Cephei.Cell.List<AssetOrNothingPayoff>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<AssetOrNothingPayoff>(" + (Helper.sourceFoldArray (s) + "))"))

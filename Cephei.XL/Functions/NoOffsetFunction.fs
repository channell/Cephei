    ...
    let NoOffset_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<NoOffset> (c)) :> ICell
                let format (i : Cephei.Cell.List<NoOffset>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<NoOffset>(" + (Helper.sourceFoldArray (s) + "))"))

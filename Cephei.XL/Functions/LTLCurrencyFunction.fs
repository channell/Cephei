    ...
    let LTLCurrency_code
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _LTLCurrency = Helper.toModelReference<LTLCurrency> ltlcurrency "LTLCurrency"  
    ...
    let LTLCurrency_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<LTLCurrency> (c)) :> ICell
                let format (i : Cephei.Cell.List<LTLCurrency>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<LTLCurrency>(" + (Helper.sourceFoldArray (s) + "))"))

...
module Euribor4MFunction =
    ...
    *)
    ...
    let Euribor4M_businessDayConvention
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifier for Cell")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="Euribor4M",Description = "Euribor4M")>] 
         euribor4m : obj)
        = 
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Euribor4M = Helper.toModelReference<Euribor4M> euribor4m "Euribor4M"  
    ...
    let Euribor4M_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Euribor4M> (c)) :> ICell
                let format (i : Cephei.Cell.List<Euribor4M>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Euribor4M>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let PricipalLeg_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.PricipalLeg 
                                                            _schedule.cell 
    ...
    let PricipalLeg_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<PricipalLeg> (c)) :> ICell
                let format (i : Cephei.Cell.List<PricipalLeg>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<PricipalLeg>(" + (Helper.sourceFoldArray (s) + "))"))

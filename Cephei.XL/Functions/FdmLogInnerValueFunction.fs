    ...
    let FdmLogInnerValue_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.FdmLogInnerValue 
                                                            _payoff.cell 
                                                            _mesher.cell 
                                                            _direction.cell 
                                                       ) :> ICell
    ...
    let FdmLogInnerValue_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<FdmLogInnerValue> (c)) :> ICell
                let format (i : Cephei.Cell.List<FdmLogInnerValue>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<FdmLogInnerValue>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let Bkbm4M_businessDayConvention
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).BusinessDayConvention
                                                       ) :> ICell
    ...
    let Bkbm4M_clone
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).Clone
                                                            _forwarding.cell 
                                                       ) :> ICell
    ...
    let Bkbm4M_endOfMonth
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).EndOfMonth
                                                       ) :> ICell
    ...
    let Bkbm4M_forecastFixing1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).ForecastFixing1
                                                            _d1.cell 
                                                            _d2.cell 
                                                            _t.cell 
                                                       ) :> ICell
    ...
    let Bkbm4M_forecastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).ForecastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let Bkbm4M_forwardingTermStructure
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).ForwardingTermStructure
                                                       ) :> ICell
    ...
    let Bkbm4M_maturityDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).MaturityDate
                                                            _valueDate.cell 
                                                       ) :> ICell
    ...
    let Bkbm4M_currency
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).Currency
                                                       ) :> ICell
    ...
    let Bkbm4M_dayCounter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).DayCounter
                                                       ) :> ICell
    ...
    let Bkbm4M_familyName
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).FamilyName
                                                       ) :> ICell
    ...
    let Bkbm4M_fixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).Fixing
                                                            _fixingDate.cell 
    ...
    let Bkbm4M_fixingCalendar
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).FixingCalendar
                                                       ) :> ICell
    ...
    let Bkbm4M_fixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let _valueDate = Helper.toCell<Date> valueDate "valueDate" true
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).FixingDate
                                                            _valueDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let Bkbm4M_fixingDays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).FixingDays
                                                       ) :> ICell
    ...
    let Bkbm4M_isValidFixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).IsValidFixingDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let Bkbm4M_name
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).Name
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let Bkbm4M_pastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).PastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
    ...
    let Bkbm4M_tenor
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).Tenor
                                                       ) :> ICell
    ...
    let Bkbm4M_update
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).Update
                                                       ) :> ICell
    ...
    let Bkbm4M_valueDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" true
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).ValueDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let Bkbm4M_addFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).AddFixing
                                                            _d.cell 
                                                            _v.cell 
    ...
    let Bkbm4M_addFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" true
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).AddFixings
                                                            _d.cell 
                                                            _v.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
    ...
    let Bkbm4M_addFixings1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" true
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).AddFixings1
                                                            _source.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
                let format (o : Bkbm4M) (l:string) = o.ToString() :> obj
    ...
    let Bkbm4M_allowsNativeFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).AllowsNativeFixings
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let Bkbm4M_clearFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).ClearFixings
                                                       ) :> ICell
                let format (o : Bkbm4M) (l:string) = o.ToString() :> obj
    ...
    let Bkbm4M_registerWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).RegisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : Bkbm4M) (l:string) = o.ToString() :> obj
    ...
    let Bkbm4M_timeSeries
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).TimeSeries
                                                       ) :> ICell
    ...
    let Bkbm4M_unregisterWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Bkbm4M = Helper.toCell<Bkbm4M> bkbm4m "Bkbm4M" true 
                let _handler = Helper.toCell<Callback> handler "handler" true
                let builder () = withMnemonic mnemonic ((_Bkbm4M.cell :?> Bkbm4MModel).UnregisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : Bkbm4M) (l:string) = o.ToString() :> obj
    ...
    let Bkbm4M_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<Bkbm4M> i "value" true) |>
                        Seq.toArray

    ...
    let OISRateHelper_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.OISRateHelper 
    ...
    let OISRateHelper_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<OISRateHelper> (c)) :> ICell
                let format (i : Cephei.Cell.List<OISRateHelper>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<OISRateHelper>(" + (Helper.sourceFoldArray (s) + "))"))

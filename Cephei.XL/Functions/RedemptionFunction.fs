    ...
    let Redemption_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.Redemption 
                                                            _amount.cell 
                                                            _date.cell 
                                                       ) :> ICell
    ...
    let Redemption_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Redemption> (c)) :> ICell
                let format (i : Cephei.Cell.List<Redemption>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Redemption>(" + (Helper.sourceFoldArray (s) + "))"))

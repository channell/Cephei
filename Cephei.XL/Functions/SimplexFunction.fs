    ...
    let Simplex_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.Simplex 
                                                            _lambda.cell 
                                                       ) :> ICell
    ...
    let Simplex_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Simplex> (c)) :> ICell
                let format (i : Cephei.Cell.List<Simplex>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Simplex>(" + (Helper.sourceFoldArray (s) + "))"))

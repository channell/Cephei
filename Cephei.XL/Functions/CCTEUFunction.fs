    ...
    let CCTEU_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.CCTEU 
                                                            _maturityDate.cell 
                                                            _spread.cell 
                                                            _fwdCurve.cell 
                                                            _startDate.cell 
    ...
    let CCTEU_cleanPrice1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((CCTEUModel.Cast _CCTEU.cell).CleanPrice1
                                                            _Yield.cell 
                                                            _dc.cell 
                                                            _comp.cell 
                                                            _freq.cell 
    ...
    let CCTEU_dirtyPrice1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _freq = Helper.toCell<Frequency> freq "freq" 
                let _settlement = Helper.toCell<Date> settlement "settlement" 
                let builder (current : ICell) = ((CCTEUModel.Cast _CCTEU.cell).DirtyPrice1
                                                            _Yield.cell 
                                                            _dc.cell 
                                                            _comp.cell 
                                                            _freq.cell 
    ...
    let CCTEU_yield1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((CCTEUModel.Cast _CCTEU.cell).Yield1
                                                            _cleanPrice.cell 
                                                            _dc.cell 
                                                            _comp.cell 
                                                            _freq.cell 
    ...
    let CCTEU_yield
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _accuracy = Helper.toCell<double> accuracy "accuracy" 
                let _maxEvaluations = Helper.toCell<int> maxEvaluations "maxEvaluations" 
                let builder (current : ICell) = ((CCTEUModel.Cast _CCTEU.cell).Yield
                                                            _dc.cell 
                                                            _comp.cell 
                                                            _freq.cell 
                                                            _accuracy.cell 
    ...
    let CCTEU_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<CCTEU> (c)) :> ICell
                let format (i : Cephei.Cell.List<CCTEU>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<CCTEU>(" + (Helper.sourceFoldArray (s) + "))"))

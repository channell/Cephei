    ...
    let BoundaryConditionSchemeHelper_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.BoundaryConditionSchemeHelper 
                                                            _bcSet.cell 
                                                       ) :> ICell
    ...
    let BoundaryConditionSchemeHelper_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<BoundaryConditionSchemeHelper> (c)) :> ICell
                let format (i : Cephei.Cell.List<BoundaryConditionSchemeHelper>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BoundaryConditionSchemeHelper>(" + (Helper.sourceFoldArray (s) + "))"))

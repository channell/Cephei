    ...
    let NegativePowerDefaultIntensity_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.NegativePowerDefaultIntensity1 
                                                            _alpha.cell 
                                                            _p.cell 
                                                            _recovery.cell 
                                                       ) :> ICell
    ...
    let NegativePowerDefaultIntensity_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _alpha = Helper.toCell<double> alpha "alpha" 
                let _p = Helper.toCell<double> p "p" 
                let builder (current : ICell) = (Fun.NegativePowerDefaultIntensity
                                                            _alpha.cell 
                                                            _p.cell 
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<NegativePowerDefaultIntensity>) l
    ...
    let NegativePowerDefaultIntensity_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let format (i : Cephei.Cell.List<NegativePowerDefaultIntensity>) (l : string) = Helper.Range.fromModelList i l

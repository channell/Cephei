    ...
    let CADLibor_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.CADLibor1 
                                                            _tenor.cell 
                                                       ) :> ICell
    ...
    let CADLibor_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.CADLibor
                                                            _tenor.cell 
                                                            _h.cell 
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<CADLibor>) l
    ...
    let CADLibor_clone
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let _h = Helper.toHandle<YieldTermStructure> h "h" 
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).Clone
                                                            _h.cell 
                                                       ) :> ICell
    ...
    let CADLibor_maturityDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).MaturityDate
                                                            _valueDate.cell 
                                                       ) :> ICell
    ...
    let CADLibor_valueDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).ValueDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let CADLibor_businessDayConvention
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).BusinessDayConvention
                                                       ) :> ICell
    ...
    let CADLibor_endOfMonth
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).EndOfMonth
                                                       ) :> ICell
    ...
    let CADLibor_forecastFixing1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).ForecastFixing1
                                                            _d1.cell 
                                                            _d2.cell 
                                                            _t.cell 
                                                       ) :> ICell
    ...
    let CADLibor_forecastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).ForecastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let CADLibor_forwardingTermStructure
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).ForwardingTermStructure
                                                       ) :> ICell
    ...
    let CADLibor_currency
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).Currency
                                                       ) :> ICell
    ...
    let CADLibor_dayCounter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).DayCounter
                                                       ) :> ICell
    ...
    let CADLibor_familyName
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).FamilyName
                                                       ) :> ICell
    ...
    let CADLibor_fixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).Fixing
                                                            _fixingDate.cell 
    ...
    let CADLibor_fixingCalendar
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).FixingCalendar
                                                       ) :> ICell
    ...
    let CADLibor_fixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let _valueDate = Helper.toCell<Date> valueDate "valueDate" 
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).FixingDate
                                                            _valueDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let CADLibor_fixingDays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).FixingDays
                                                       ) :> ICell
    ...
    let CADLibor_isValidFixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).IsValidFixingDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let CADLibor_name
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).Name
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let CADLibor_pastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).PastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
    ...
    let CADLibor_tenor
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).Tenor
                                                       ) :> ICell
    ...
    let CADLibor_update
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).Update
                                                       ) :> ICell
    ...
    let CADLibor_addFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).AddFixing
                                                            _d.cell 
                                                            _v.cell 
    ...
    let CADLibor_addFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" 
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).AddFixings
                                                            _d.cell 
                                                            _v.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
    ...
    let CADLibor_addFixings1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" 
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).AddFixings1
                                                            _source.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
                let format (o : CADLibor) (l:string) = o.ToString() :> obj
    ...
    let CADLibor_allowsNativeFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).AllowsNativeFixings
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let CADLibor_clearFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).ClearFixings
                                                       ) :> ICell
                let format (o : CADLibor) (l:string) = o.ToString() :> obj
    ...
    let CADLibor_registerWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).RegisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : CADLibor) (l:string) = o.ToString() :> obj
    ...
    let CADLibor_timeSeries
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).TimeSeries
                                                       ) :> ICell
    ...
    let CADLibor_unregisterWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _CADLibor = Helper.toModelReference<CADLibor> cadlibor "CADLibor"  
                let _handler = Helper.toCell<Callback> handler "handler" 
                let builder (current : ICell) = ((CADLiborModel.Cast _CADLibor.cell).UnregisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : CADLibor) (l:string) = o.ToString() :> obj
    ...
    let CADLibor_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<CADLibor> (c)) :> ICell
                let format (i : Cephei.Cell.List<CADLibor>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<CADLibor>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let StrikedTypePayoff_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.StrikedTypePayoff 
                                                            _p.cell 
                                                       ) :> ICell
    ...
    let StrikedTypePayoff_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.StrikedTypePayoff1 
                                                            _Type.cell 
                                                            _strike.cell 
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<StrikedTypePayoff>) l
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                Model.specify 
                    ...
                    ; source =  (fun () -> "(new Cephei.Cell.List<StrikedTypePayoff>(" + (Helper.sourceFoldArray (s) + "))"))

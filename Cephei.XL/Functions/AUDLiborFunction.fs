    ...
    let AUDLibor_clone
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _h = Helper.toHandle<YieldTermStructure> h "h" 
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).Clone
                                                            _h.cell 
                                                       ) :> ICell
    ...
    let AUDLibor_maturityDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).MaturityDate
                                                            _valueDate.cell 
                                                       ) :> ICell
    ...
    let AUDLibor_valueDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).ValueDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let AUDLibor_businessDayConvention
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).BusinessDayConvention
                                                       ) :> ICell
    ...
    let AUDLibor_endOfMonth
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).EndOfMonth
                                                       ) :> ICell
    ...
    let AUDLibor_forecastFixing1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).ForecastFixing1
                                                            _d1.cell 
                                                            _d2.cell 
                                                            _t.cell 
                                                       ) :> ICell
    ...
    let AUDLibor_forecastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).ForecastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let AUDLibor_forwardingTermStructure
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).ForwardingTermStructure
                                                       ) :> ICell
    ...
    let AUDLibor_currency
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).Currency
                                                       ) :> ICell
    ...
    let AUDLibor_dayCounter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).DayCounter
                                                       ) :> ICell
    ...
    let AUDLibor_familyName
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).FamilyName
                                                       ) :> ICell
    ...
    let AUDLibor_fixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).Fixing
                                                            _fixingDate.cell 
    ...
    let AUDLibor_fixingCalendar
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).FixingCalendar
                                                       ) :> ICell
    ...
    let AUDLibor_fixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let _valueDate = Helper.toCell<Date> valueDate "valueDate" 
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).FixingDate
                                                            _valueDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let AUDLibor_fixingDays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).FixingDays
                                                       ) :> ICell
    ...
    let AUDLibor_isValidFixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).IsValidFixingDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let AUDLibor_name
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).Name
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let AUDLibor_pastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).PastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
    ...
    let AUDLibor_tenor
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).Tenor
                                                       ) :> ICell
    ...
    let AUDLibor_update
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).Update
                                                       ) :> ICell
    ...
    let AUDLibor_addFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).AddFixing
                                                            _d.cell 
                                                            _v.cell 
    ...
    let AUDLibor_addFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" 
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).AddFixings
                                                            _d.cell 
                                                            _v.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
    ...
    let AUDLibor_addFixings1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" 
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).AddFixings1
                                                            _source.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
                let format (o : AUDLibor) (l:string) = o.ToString() :> obj
    ...
    let AUDLibor_allowsNativeFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).AllowsNativeFixings
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let AUDLibor_clearFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).ClearFixings
                                                       ) :> ICell
                let format (o : AUDLibor) (l:string) = o.ToString() :> obj
    ...
    let AUDLibor_registerWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).RegisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : AUDLibor) (l:string) = o.ToString() :> obj
    ...
    let AUDLibor_timeSeries
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).TimeSeries
                                                       ) :> ICell
    ...
    let AUDLibor_unregisterWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AUDLibor = Helper.toCell<AUDLibor> audlibor "AUDLibor"  
                let _handler = Helper.toCell<Callback> handler "handler" 
                let builder () = withMnemonic mnemonic ((AUDLiborModel.Cast _AUDLibor.cell).UnregisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : AUDLibor) (l:string) = o.ToString() :> obj

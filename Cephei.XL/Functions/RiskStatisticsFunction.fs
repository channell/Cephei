    ...
    *)
    ...
    let RiskStatistics_gaussianAverageShortfall
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifier for Cell")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="RiskStatistics",Description = "RiskStatistics")>] 
         riskstatistics : obj)
    ...
    let RiskStatistics_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<RiskStatistics> (c)) :> ICell
                let format (i : Cephei.Cell.List<RiskStatistics>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<RiskStatistics>(" + (Helper.sourceFoldArray (s) + "))"))

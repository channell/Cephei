    ...
    let NotionalPath_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.NotionalPath ()
                                                       ) :> ICell
    ...
    let NotionalPath_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<NotionalPath> (c)) :> ICell
                let format (i : Cephei.Cell.List<NotionalPath>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<NotionalPath>(" + (Helper.sourceFoldArray (s) + "))"))

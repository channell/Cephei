    ...
    let ConstantDefaultIntensity_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.ConstantDefaultIntensity 
                                                            _constant.cell 
                                                            _recovery.cell 
                                                       ) :> ICell
    ...
    let ConstantDefaultIntensity_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _constant = Helper.toCell<double> constant "constant" 
                let builder (current : ICell) = (Fun.ConstantDefaultIntensity1 
                                                            _constant.cell 
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<ConstantDefaultIntensity>) l
    ...
    let ConstantDefaultIntensity_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<ConstantDefaultIntensity> (c)) :> ICell
                let format (i : Cephei.Cell.List<ConstantDefaultIntensity>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<ConstantDefaultIntensity>(" + (Helper.sourceFoldArray (s) + "))"))

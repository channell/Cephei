    ...
    *)
    ...
    let Actual360_create
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifier for Cell")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="c",Description = "bool or empty")>] 
         c : obj)
        = 
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.Actual360 
                                                            _c.cell 
                                                       ) :> ICell
    ...
    let Actual360_yearFraction
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((Actual360Model.Cast _Actual360.cell).YearFraction
                                                            _d1.cell 
                                                            _d2.cell 
    ...
    let Actual360_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Actual360> (c)) :> ICell
                let format (i : Cephei.Cell.List<Actual360>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Actual360>(" + (Helper.sourceFoldArray (s) + "))"))

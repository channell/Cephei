    ...
    let SegmentIntegral_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.SegmentIntegral 
                                                            _intervals.cell 
                                                       ) :> ICell
    ...
    let SegmentIntegral_value
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((SegmentIntegralModel.Cast _SegmentIntegral.cell).Value
                                                            _f.cell 
                                                            _a.cell 
                                                            _b.cell 
                                                       ) :> ICell
    ...
    let SegmentIntegral_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<SegmentIntegral> (c)) :> ICell
                let format (i : Cephei.Cell.List<SegmentIntegral>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<SegmentIntegral>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let AdditiveEQPBinomialTree_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.AdditiveEQPBinomialTree1 
                                                            _Process.cell 
                                                            _End.cell 
                                                            _steps.cell 
                                                            _strike.cell 
    ...
    let AdditiveEQPBinomialTree_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.AdditiveEQPBinomialTree ()
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<AdditiveEQPBinomialTree>) l
    ...
    let AdditiveEQPBinomialTree_factory
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AdditiveEQPBinomialTree = Helper.toModelReference<AdditiveEQPBinomialTree> additiveeqpbinomialtree "AdditiveEQPBinomialTree"  
                let _Process = Helper.toCell<StochasticProcess1D> Process "Process" 
                let _End = Helper.toCell<double> End "End" 
                let _steps = Helper.toCell<int> steps "steps" 
                let _strike = Helper.toCell<double> strike "strike" 
                let builder (current : ICell) = ((AdditiveEQPBinomialTreeModel.Cast _AdditiveEQPBinomialTree.cell).Factory
                                                            _Process.cell 
                                                            _End.cell 
                                                            _steps.cell 
                                                            _strike.cell 
    ...
    let AdditiveEQPBinomialTree_probability
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((AdditiveEQPBinomialTreeModel.Cast _AdditiveEQPBinomialTree.cell).Probability
                                                            _x.cell 
                                                            _y.cell 
                                                            _z.cell 
                                                       ) :> ICell
    ...
    let AdditiveEQPBinomialTree_underlying
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((AdditiveEQPBinomialTreeModel.Cast _AdditiveEQPBinomialTree.cell).Underlying
                                                            _i.cell 
                                                            _index.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let AdditiveEQPBinomialTree_descendant
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((AdditiveEQPBinomialTreeModel.Cast _AdditiveEQPBinomialTree.cell).Descendant
                                                            _x.cell 
                                                            _index.cell 
                                                            _branch.cell 
                                                       ) :> ICell
    ...
    let AdditiveEQPBinomialTree_size
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AdditiveEQPBinomialTree = Helper.toModelReference<AdditiveEQPBinomialTree> additiveeqpbinomialtree "AdditiveEQPBinomialTree"  
                let _i = Helper.toCell<int> i "i" 
                let builder (current : ICell) = ((AdditiveEQPBinomialTreeModel.Cast _AdditiveEQPBinomialTree.cell).Size
                                                            _i.cell 
                                                       ) :> ICell
                let format (o : int) (l:string) = o :> obj
    ...
    let AdditiveEQPBinomialTree_columns
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AdditiveEQPBinomialTree = Helper.toModelReference<AdditiveEQPBinomialTree> additiveeqpbinomialtree "AdditiveEQPBinomialTree"  
                let builder (current : ICell) = ((AdditiveEQPBinomialTreeModel.Cast _AdditiveEQPBinomialTree.cell).Columns
                                                       ) :> ICell
                let format (o : int) (l:string) = o :> obj
    ...
    let AdditiveEQPBinomialTree_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<AdditiveEQPBinomialTree> (c)) :> ICell
                let format (i : Cephei.Cell.List<AdditiveEQPBinomialTree>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<AdditiveEQPBinomialTree>(" + (Helper.sourceFoldArray (s) + "))"))

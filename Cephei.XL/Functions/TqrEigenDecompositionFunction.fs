    ...
    let TqrEigenDecomposition_eigenvalues
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _TqrEigenDecomposition = Helper.toModelReference<TqrEigenDecomposition> tqreigendecomposition "TqrEigenDecomposition"  
    ...
    let TqrEigenDecomposition_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<TqrEigenDecomposition> (c)) :> ICell
                let format (i : Cephei.Cell.List<TqrEigenDecomposition>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<TqrEigenDecomposition>(" + (Helper.sourceFoldArray (s) + "))"))

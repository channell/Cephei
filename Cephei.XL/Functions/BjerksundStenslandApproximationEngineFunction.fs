        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                Model.specify 
                    ...
                    ; creator = builder
                    ; subscriber = Helper.subscriberModel<BjerksundStenslandApproximationEngine> format
                    ; source = source 
                    ; hash = hash
                    } :?> string
            with
    ...
    let BjerksundStenslandApproximationEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<BjerksundStenslandApproximationEngine> i "value" ) |>
                        Seq.toArray
                let c = a |> Array.map (fun i -> i.cell)

                let s = a |> Array.map (fun i -> i.source)
                let builder (current : ICell) = (new Cephei.Cell.List<BjerksundStenslandApproximationEngine> (c)) :> ICell
                let format (i : Cephei.Cell.List<BjerksundStenslandApproximationEngine>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BjerksundStenslandApproximationEngine>(" + (Helper.sourceFoldArray (s) + "))"))

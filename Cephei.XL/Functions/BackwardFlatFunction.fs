    ...
    let BackwardFlat_global
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _BackwardFlat = Helper.toModelReference<BackwardFlat> backwardflat "BackwardFlat"  
                let builder (current : ICell) = ((BackwardFlatModel.Cast _BackwardFlat.cell).Global
                                                       ) :> ICell
    ...
    let BackwardFlat_interpolate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((BackwardFlatModel.Cast _BackwardFlat.cell).Interpolate
                                                            _xBegin.cell 
                                                            _size.cell 
                                                            _yBegin.cell 
                                                       ) :> ICell
    ...
    let BackwardFlat_requiredPoints
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _BackwardFlat = Helper.toModelReference<BackwardFlat> backwardflat "BackwardFlat"  
                let builder (current : ICell) = ((BackwardFlatModel.Cast _BackwardFlat.cell).RequiredPoints
                                                       ) :> ICell
    ...
    let BackwardFlat_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<BackwardFlat> (c)) :> ICell
                let format (i : Cephei.Cell.List<BackwardFlat>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BackwardFlat>(" + (Helper.sourceFoldArray (s) + "))"))

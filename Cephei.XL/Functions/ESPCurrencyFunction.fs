    ...
    let ESPCurrency_code
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _ESPCurrency = Helper.toModelReference<ESPCurrency> espcurrency "ESPCurrency"  
    ...
    let ESPCurrency_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<ESPCurrency> (c)) :> ICell
                let format (i : Cephei.Cell.List<ESPCurrency>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<ESPCurrency>(" + (Helper.sourceFoldArray (s) + "))"))

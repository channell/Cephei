    ...
    let ExponentialSplinesFitting_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.ExponentialSplinesFitting 
    ...
    let ExponentialSplinesFitting_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<ExponentialSplinesFitting> (c)) :> ICell
                let format (i : Cephei.Cell.List<ExponentialSplinesFitting>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<ExponentialSplinesFitting>(" + (Helper.sourceFoldArray (s) + "))"))

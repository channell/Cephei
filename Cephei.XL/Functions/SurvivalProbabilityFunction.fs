    ...
    let SurvivalProbability_guess
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((SurvivalProbabilityModel.Cast _SurvivalProbability.cell).Guess
                                                            _i.cell 
                                                            _c.cell 
                                                            _validData.cell 
                                                            _f.cell 
    ...
    let SurvivalProbability_maxValueAfter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _validData = Helper.toCell<bool> validData "validData" 
                let _f = Helper.toCell<int> f "f" 
                let builder (current : ICell) = ((SurvivalProbabilityModel.Cast _SurvivalProbability.cell).MaxValueAfter
                                                            _i.cell 
                                                            _c.cell 
                                                            _validData.cell 
                                                            _f.cell 
    ...
    let SurvivalProbability_minValueAfter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _validData = Helper.toCell<bool> validData "validData" 
                let _f = Helper.toCell<int> f "f" 
                let builder (current : ICell) = ((SurvivalProbabilityModel.Cast _SurvivalProbability.cell).MinValueAfter
                                                            _i.cell 
                                                            _c.cell 
                                                            _validData.cell 
                                                            _f.cell 
    ...
    let SurvivalProbability_updateGuess
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _i = Helper.toCell<int> i "i" 
                let builder (current : ICell) = ((SurvivalProbabilityModel.Cast _SurvivalProbability.cell).UpdateGuess
                                                            _data.cell 
                                                            _discount.cell 
                                                            _i.cell 
                                                       ) :> ICell
    ...
    let SurvivalProbability_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<SurvivalProbability> (c)) :> ICell
                let format (i : Cephei.Cell.List<SurvivalProbability>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<SurvivalProbability>(" + (Helper.sourceFoldArray (s) + "))"))

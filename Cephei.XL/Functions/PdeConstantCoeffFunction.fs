    ...
    let PdeConstantCoeff_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _x = Helper.toCell<double> x "x" 
                let builder (current : ICell) = (Fun.PdeConstantCoeff 
                                                            _Process.cell 
                                                            _t.cell 
                                                            _x.cell 
                                                       ) :> ICell
    ...
    let PdeConstantCoeff_generateOperator
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((PdeConstantCoeffModel.Cast _PdeConstantCoeff.cell).GenerateOperator
                                                            _t.cell 
                                                            _tg.cell 
                                                            _L.cell 
                                                       ) :> ICell
    ...
    let PdeConstantCoeff_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<PdeConstantCoeff> (c)) :> ICell
                let format (i : Cephei.Cell.List<PdeConstantCoeff>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<PdeConstantCoeff>(" + (Helper.sourceFoldArray (s) + "))"))

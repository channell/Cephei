    ...
    let InterpolatedForwardCurve_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.InterpolatedForwardCurve 
                                                            _dates.cell 
                                                            _forwards.cell 
                                                            _dayCounter.cell 
                                                            _interpolator.cell 
    ...
    let InterpolatedForwardCurve_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _interpolator = Helper.toDefault<'Interpolator> interpolator "interpolator" default(Interpolator)
                let builder (current : ICell) = (Fun.InterpolatedForwardCurve1 
                                                            _dates.cell 
                                                            _forwards.cell 
                                                            _dayCounter.cell 
                                                            _calendar.cell 
    ...
    let InterpolatedForwardCurve_create2
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _interpolator = Helper.toDefault<'Interpolator> interpolator "interpolator" default(Interpolator)
                let builder (current : ICell) = (Fun.InterpolatedForwardCurve2 
                                                            _dates.cell 
                                                            _forwards.cell 
                                                            _dayCounter.cell 
                                                            _calendar.cell 
    ...
    let InterpolatedForwardCurve_create3
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _jumpDates = Helper.toDefault<Generic.List<Date>> jumpDates "jumpDates" null
                let _interpolator = Helper.toDefault<'Interpolator> interpolator "interpolator" default(Interpolator)
                let builder (current : ICell) = (Fun.InterpolatedForwardCurve3 
    ...
    let InterpolatedForwardCurve_create4
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _jumpDates = Helper.toDefault<Generic.List<Date>> jumpDates "jumpDates" null
                let _interpolator = Helper.toDefault<'Interpolator> interpolator "interpolator" default(Interpolator)
                let builder (current : ICell) = (Fun.InterpolatedForwardCurve4 
                                                            _dayCounter.cell 
                                                            _jumps.cell 
                                                            _jumpDates.cell 
                                                            _interpolator.cell 
    ...
    let InterpolatedForwardCurve_create5
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _jumpDates = Helper.toDefault<Generic.List<Date>> jumpDates "jumpDates" null
                let _interpolator = Helper.toDefault<'Interpolator> interpolator "interpolator" default(Interpolator)
                let builder (current : ICell) = (Fun.InterpolatedForwardCurve5 
                                                            _referenceDate.cell 
                                                            _dayCounter.cell 
                                                            _jumps.cell 
                                                            _jumpDates.cell 
    ...
    let InterpolatedForwardCurve_create6
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _jumpDates = Helper.toDefault<Generic.List<Date>> jumpDates "jumpDates" null
                let _interpolator = Helper.toDefault<'Interpolator> interpolator "interpolator" default(Interpolator)
                let builder (current : ICell) = (Fun.InterpolatedForwardCurve6 
                                                            _dates.cell 
                                                            _forwards.cell 
                                                            _dayCounter.cell 
                                                            _jumps.cell 

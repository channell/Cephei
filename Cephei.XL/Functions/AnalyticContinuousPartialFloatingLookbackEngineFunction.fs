    ...
    let AnalyticContinuousPartialFloatingLookbackEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.AnalyticContinuousPartialFloatingLookbackEngine 
                                                            _Process.cell 
    ...
    let AnalyticContinuousPartialFloatingLookbackEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<AnalyticContinuousPartialFloatingLookbackEngine> i "value" ) |>
                        Seq.toArray
                let c = a |> Array.map (fun i -> i.cell)

                let s = a |> Array.map (fun i -> i.source)
                let builder (current : ICell) = (new Cephei.Cell.List<AnalyticContinuousPartialFloatingLookbackEngine> (c)) :> ICell
                let format (i : Cephei.Cell.List<AnalyticContinuousPartialFloatingLookbackEngine>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<AnalyticContinuousPartialFloatingLookbackEngine>(" + (Helper.sourceFoldArray (s) + "))"))

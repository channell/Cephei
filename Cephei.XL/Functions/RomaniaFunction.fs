    ...
    let Romania_addedHolidays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Romania = Helper.toModelReference<Romania> romania "Romania"  
    ...
    let Romania_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Romania> (c)) :> ICell
                let format (i : Cephei.Cell.List<Romania>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Romania>(" + (Helper.sourceFoldArray (s) + "))"))

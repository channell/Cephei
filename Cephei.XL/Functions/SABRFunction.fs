    ...
    let SABR_interpolate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _SABR = Helper.toModelReference<SABR> sabr "SABR"  
    ...
    let SABR_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<SABR> (c)) :> ICell
                let format (i : Cephei.Cell.List<SABR>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<SABR>(" + (Helper.sourceFoldArray (s) + "))"))

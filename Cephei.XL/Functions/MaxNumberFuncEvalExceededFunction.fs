    ...
    let MaxNumberFuncEvalExceeded_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = withMnemonic mnemonic (Fun.MaxNumberFuncEvalExceeded 
                                                            _message.cell 
                                                            _inner.cell 
                                                       ) :> ICell
                ...
                Model.specify 
                    ...
                    ; creator = builder
                    ; subscriber = Helper.subscriberModel<MaxNumberFuncEvalExceeded> format
                    ; source = source 
                    ; hash = hash
                    } :?> string
            with
    ...
    let MaxNumberFuncEvalExceeded_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _message = Helper.toCell<string> message "message" 
                let builder (current : ICell) = withMnemonic mnemonic (Fun.MaxNumberFuncEvalExceeded1 
                                                            _message.cell 
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<MaxNumberFuncEvalExceeded>) l
    ...
    let MaxNumberFuncEvalExceeded_create2
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = withMnemonic mnemonic (Fun.MaxNumberFuncEvalExceeded2 ()
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<MaxNumberFuncEvalExceeded>) l
    ...
    let MaxNumberFuncEvalExceeded_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let a = values |>
                        Seq.cast<obj> |>
                        Seq.map (fun (i : obj) -> Helper.toCell<MaxNumberFuncEvalExceeded> i "value" ) |>
                        Seq.toArray
                let c = a |> Array.map (fun i -> i.cell)

                let s = a |> Array.map (fun i -> i.source)
                let builder (current : ICell) = (new Cephei.Cell.List<MaxNumberFuncEvalExceeded> (c)) :> ICell
                let format (i : Cephei.Cell.List<MaxNumberFuncEvalExceeded>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<MaxNumberFuncEvalExceeded>(" + (Helper.sourceFoldArray (s) + "))"))

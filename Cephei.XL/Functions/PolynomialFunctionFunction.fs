    ...
    let PolynomialFunction_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.PolynomialFunction 
                                                            _coeff.cell 
                                                       ) :> ICell
    ...
    let PolynomialFunction_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<PolynomialFunction> (c)) :> ICell
                let format (i : Cephei.Cell.List<PolynomialFunction>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<PolynomialFunction>(" + (Helper.sourceFoldArray (s) + "))"))

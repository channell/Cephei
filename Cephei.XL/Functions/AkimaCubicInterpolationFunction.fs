    ...
    let AkimaCubicInterpolation_aCoefficients
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).ACoefficients
                                                       ) :> ICell
    ...
    let AkimaCubicInterpolation_bCoefficients
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).BCoefficients
                                                       ) :> ICell
                let format (i : Generic.List<double>) (l : string) = (Helper.Range.fromArray (i.ToArray()) l)
    ...
    let AkimaCubicInterpolation_cCoefficients
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).CCoefficients
                                                       ) :> ICell
                let format (i : Generic.List<double>) (l : string) = (Helper.Range.fromArray (i.ToArray()) l)
    ...
    let AkimaCubicInterpolation_derivative
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).Derivative
                                                            _x.cell 
    ...
    let AkimaCubicInterpolation_empty
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).Empty
                                                       ) :> ICell
    ...
    let AkimaCubicInterpolation_primitive
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _x = Helper.toCell<double> x "x" 
                let _allowExtrapolation = Helper.toCell<bool> allowExtrapolation "allowExtrapolation" 
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).Primitive
                                                            _x.cell 
                                                            _allowExtrapolation.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let AkimaCubicInterpolation_secondDerivative
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _x = Helper.toCell<double> x "x" 
                let _allowExtrapolation = Helper.toCell<bool> allowExtrapolation "allowExtrapolation" 
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).SecondDerivative
                                                            _x.cell 
                                                            _allowExtrapolation.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let AkimaCubicInterpolation_update
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).Update
                                                       ) :> ICell
    ...
    let AkimaCubicInterpolation_value1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _x = Helper.toCell<double> x "x" 
                let _allowExtrapolation = Helper.toCell<bool> allowExtrapolation "allowExtrapolation" 
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).Value1
                                                            _x.cell 
                                                            _allowExtrapolation.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let AkimaCubicInterpolation_value
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let _x = Helper.toCell<double> x "x" 
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).Value
                                                            _x.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let AkimaCubicInterpolation_xMax
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).XMax
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let AkimaCubicInterpolation_xMin
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).XMin
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let AkimaCubicInterpolation_allowsExtrapolation
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).AllowsExtrapolation
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let AkimaCubicInterpolation_disableExtrapolation
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).DisableExtrapolation
                                                            _b.cell 
                                                       ) :> ICell
                let format (o : AkimaCubicInterpolation) (l:string) = o.ToString() :> obj
    ...
    let AkimaCubicInterpolation_enableExtrapolation
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let _b = Helper.toCell<bool> b "b" 
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).EnableExtrapolation
                                                            _b.cell 
                                                       ) :> ICell
                let format (o : AkimaCubicInterpolation) (l:string) = o.ToString() :> obj
    ...
    let AkimaCubicInterpolation_extrapolate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _AkimaCubicInterpolation = Helper.toCell<AkimaCubicInterpolation> akimacubicinterpolation "AkimaCubicInterpolation"  
                let builder () = withMnemonic mnemonic ((AkimaCubicInterpolationModel.Cast _AkimaCubicInterpolation.cell).Extrapolate
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj

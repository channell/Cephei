    ...
    let VannaVolgaDoubleBarrierEngine_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.VannaVolgaDoubleBarrierEngine 
                                                            _atmVol.cell 
                                                            _vol25Put.cell 
                                                            _vol25Call.cell 
                                                            _spotFX.cell 
    ...
    let VannaVolgaDoubleBarrierEngine_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<VannaVolgaDoubleBarrierEngine> (c)) :> ICell
                let format (i : Cephei.Cell.List<VannaVolgaDoubleBarrierEngine>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<VannaVolgaDoubleBarrierEngine>(" + (Helper.sourceFoldArray (s) + "))"))

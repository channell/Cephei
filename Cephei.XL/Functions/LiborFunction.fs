    ...
    let Libor_clone
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).Clone
                                                            _h.cell 
                                                       ) :> ICell
    ...
    let Libor_maturityDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).MaturityDate
                                                            _valueDate.cell 
                                                       ) :> ICell
    ...
    let Libor_valueDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).ValueDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let Libor_businessDayConvention
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).BusinessDayConvention
                                                       ) :> ICell
    ...
    let Libor_endOfMonth
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).EndOfMonth
                                                       ) :> ICell
    ...
    let Libor_forecastFixing1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).ForecastFixing1
                                                            _d1.cell 
                                                            _d2.cell 
                                                            _t.cell 
                                                       ) :> ICell
    ...
    let Libor_forecastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).ForecastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : double) (l:string) = o :> obj
    ...
    let Libor_forwardingTermStructure
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).ForwardingTermStructure
                                                       ) :> ICell
    ...
    let Libor_currency
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).Currency
                                                       ) :> ICell
    ...
    let Libor_dayCounter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).DayCounter
                                                       ) :> ICell
    ...
    let Libor_familyName
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).FamilyName
                                                       ) :> ICell
    ...
    let Libor_fixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).Fixing
                                                            _fixingDate.cell 
    ...
    let Libor_fixingCalendar
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).FixingCalendar
                                                       ) :> ICell
    ...
    let Libor_fixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let _valueDate = Helper.toCell<Date> valueDate "valueDate" 
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).FixingDate
                                                            _valueDate.cell 
                                                       ) :> ICell
                let format (d : Date) (l:string) = d.serialNumber() :> obj
    ...
    let Libor_fixingDays
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).FixingDays
                                                       ) :> ICell
    ...
    let Libor_isValidFixingDate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).IsValidFixingDate
                                                            _fixingDate.cell 
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let Libor_name
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).Name
                                                       ) :> ICell
                let format (o : string) (l:string) = o.ToString() :> obj
    ...
    let Libor_pastFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let _fixingDate = Helper.toCell<Date> fixingDate "fixingDate" 
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).PastFixing
                                                            _fixingDate.cell 
                                                       ) :> ICell
    ...
    let Libor_tenor
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).Tenor
                                                       ) :> ICell
    ...
    let Libor_update
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).Update
                                                       ) :> ICell
    ...
    let Libor_addFixing
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).AddFixing
                                                            _d.cell 
                                                            _v.cell 
    ...
    let Libor_addFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" 
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).AddFixings
                                                            _d.cell 
                                                            _v.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
    ...
    let Libor_addFixings1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _forceOverwrite = Helper.toCell<bool> forceOverwrite "forceOverwrite" 
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).AddFixings1
                                                            _source.cell 
                                                            _forceOverwrite.cell 
                                                       ) :> ICell
                let format (o : Libor) (l:string) = o.ToString() :> obj
    ...
    let Libor_allowsNativeFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).AllowsNativeFixings
                                                       ) :> ICell
                let format (o : bool) (l:string) = o.ToString() :> obj
    ...
    let Libor_clearFixings
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).ClearFixings
                                                       ) :> ICell
                let format (o : Libor) (l:string) = o.ToString() :> obj
    ...
    let Libor_registerWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).RegisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : Libor) (l:string) = o.ToString() :> obj
    ...
    let Libor_timeSeries
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).TimeSeries
                                                       ) :> ICell
    ...
    let Libor_unregisterWith
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _Libor = Helper.toCell<Libor> libor "Libor"  
                let _handler = Helper.toCell<Callback> handler "handler" 
                let builder () = withMnemonic mnemonic ((LiborModel.Cast _Libor.cell).UnregisterWith
                                                            _handler.cell 
                                                       ) :> ICell
                let format (o : Libor) (l:string) = o.ToString() :> obj

    ...
    let FixedRateBond_dayCounter
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _FixedRateBond = Helper.toModelReference<FixedRateBond> fixedratebond "FixedRateBond"  
    ...
    let FixedRateBond_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<FixedRateBond> (c)) :> ICell
                let format (i : Cephei.Cell.List<FixedRateBond>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<FixedRateBond>(" + (Helper.sourceFoldArray (s) + "))"))

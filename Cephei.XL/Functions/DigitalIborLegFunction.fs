    ...
    let DigitalIborLeg_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.DigitalIborLeg 
                                                            _schedule.cell 
                                                            _index.cell 
    ...
    let DigitalIborLeg_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<DigitalIborLeg> (c)) :> ICell
                let format (i : Cephei.Cell.List<DigitalIborLeg>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<DigitalIborLeg>(" + (Helper.sourceFoldArray (s) + "))"))

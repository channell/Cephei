    ...
    let ATSCurrency_code
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _ATSCurrency = Helper.toModelReference<ATSCurrency> atscurrency "ATSCurrency"  
    ...
    let ATSCurrency_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<ATSCurrency> (c)) :> ICell
                let format (i : Cephei.Cell.List<ATSCurrency>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<ATSCurrency>(" + (Helper.sourceFoldArray (s) + "))"))

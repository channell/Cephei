    ...
    let PTECurrency_code
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _PTECurrency = Helper.toModelReference<PTECurrency> ptecurrency "PTECurrency"  
    ...
    let PTECurrency_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<PTECurrency> (c)) :> ICell
                let format (i : Cephei.Cell.List<PTECurrency>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<PTECurrency>(" + (Helper.sourceFoldArray (s) + "))"))

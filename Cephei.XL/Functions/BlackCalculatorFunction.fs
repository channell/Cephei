    ...
    let BlackCalculator_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.BlackCalculator 
                                                            _payoff.cell 
                                                            _forward.cell 
                                                            _stdDev.cell 
                                                            _discount.cell 
    ...
    let BlackCalculator_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<BlackCalculator> (c)) :> ICell
                let format (i : Cephei.Cell.List<BlackCalculator>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<BlackCalculator>(" + (Helper.sourceFoldArray (s) + "))"))

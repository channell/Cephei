    ...
    let Business252_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.Business252 
                                                            _c.cell 
                                                       ) :> ICell
    ...
    let Business252_yearFraction
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((Business252Model.Cast _Business252.cell).YearFraction
                                                            _d1.cell 
                                                            _d2.cell 
                                                            _d3.cell 
                                                            _d4.cell 
    ...
    let Business252_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Business252> (c)) :> ICell
                let format (i : Cephei.Cell.List<Business252>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Business252>(" + (Helper.sourceFoldArray (s) + "))"))

    ...
    let AverageBasketPayoff_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.AverageBasketPayoff1
                                                            _p.cell 
                                                            _n.cell 
                                                       ) :> ICell
    ...
    let AverageBasketPayoff_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _p = Helper.toCell<Payoff> p "p" 
                let _a = Helper.toCell<Vector> a "a" 
                let builder (current : ICell) = (Fun.AverageBasketPayoff
                                                            _p.cell 
                                                            _a.cell 
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<AverageBasketPayoff>) l
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                Model.specify 
                    ...
                    ; source =  (fun () -> "(new Cephei.Cell.List<AverageBasketPayoff>(" + (Helper.sourceFoldArray (s) + "))"))

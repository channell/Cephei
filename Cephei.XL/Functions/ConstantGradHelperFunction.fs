    ...
    let ConstantGradHelper_fNext
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _ConstantGradHelper = Helper.toModelReference<ConstantGradHelper> constantgradhelper "ConstantGradHelper"  
    ...
    let ConstantGradHelper_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<ConstantGradHelper> (c)) :> ICell
                let format (i : Cephei.Cell.List<ConstantGradHelper>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<ConstantGradHelper>(" + (Helper.sourceFoldArray (s) + "))"))

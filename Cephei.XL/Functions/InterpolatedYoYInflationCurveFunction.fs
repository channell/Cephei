    ...
    let InterpolatedYoYInflationCurve_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.InterpolatedYoYInflationCurve 
                                                            _referenceDate.cell 
                                                            _calendar.cell 
                                                            _dayCounter.cell 
                                                            _lag.cell 
    ...
    let InterpolatedYoYInflationCurve_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _dates = Helper.toCell<Generic.List<Date>> dates "dates" 
                let _rates = Helper.toCell<Generic.List<double>> rates "rates" 
                let builder (current : ICell) = (Fun.InterpolatedYoYInflationCurve1 
                                                            _referenceDate.cell 
                                                            _calendar.cell 
                                                            _dayCounter.cell 
                                                            _lag.cell 
    ...
    let InterpolatedYoYInflationCurve_yoyRate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((InterpolatedYoYInflationCurveModel.Cast _InterpolatedYoYInflationCurve.cell).YoyRate
                                                            _d.cell 
                                                            _instObsLag.cell 
    ...
    let InterpolatedYoYInflationCurve_yoyRate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _instObsLag = Helper.toCell<Period> instObsLag "instObsLag" 
                let _forceLinearInterpolation = Helper.toCell<bool> forceLinearInterpolation "forceLinearInterpolation" 
                let builder (current : ICell) = ((InterpolatedYoYInflationCurveModel.Cast _InterpolatedYoYInflationCurve.cell).YoyRate1
                                                            _d.cell 
                                                            _instObsLag.cell 
                                                            _forceLinearInterpolation.cell 
                                                       ) :> ICell
    ...
    let InterpolatedYoYInflationCurve_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<InterpolatedYoYInflationCurve> (c)) :> ICell
                let format (i : Cephei.Cell.List<InterpolatedYoYInflationCurve>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<InterpolatedYoYInflationCurve>(" + (Helper.sourceFoldArray (s) + "))"))

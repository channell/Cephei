    ...
    let TrapezoidIntegral_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.TrapezoidIntegral 
                                                            _accuracy.cell 
                                                            _maxIterations.cell 
                                                       ) :> ICell
    ...
    let TrapezoidIntegral_value
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((TrapezoidIntegralModel.Cast _TrapezoidIntegral.cell).Value
                                                            _f.cell 
                                                            _a.cell 
                                                            _b.cell 
                                                       ) :> ICell
    ...
    let TrapezoidIntegral_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<TrapezoidIntegral> (c)) :> ICell
                let format (i : Cephei.Cell.List<TrapezoidIntegral>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<TrapezoidIntegral>(" + (Helper.sourceFoldArray (s) + "))"))

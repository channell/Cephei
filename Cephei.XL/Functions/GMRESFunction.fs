    ...
    *)
    ...
    let GMRES_create
        ...
        ([<ExcelArgument(Name="relTol",Description = "double")>] 
         relTol : obj)
        ([<ExcelArgument(Name="preConditioner",Description = "MatrixMult or empty")>] 
         preConditioner : obj)
        = 
    ...
    let GMRES_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<GMRES> (c)) :> ICell
                let format (i : Cephei.Cell.List<GMRES>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<GMRES>(" + (Helper.sourceFoldArray (s) + "))"))

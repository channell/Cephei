...
module IndonesiaFunction =
    ...
    *)
    ...
    let Indonesia_create
        ([<ExcelArgument(Name="Mnemonic",Description = "Indonesia")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="m",Description = "Indonesia.Market")>] 
         m : obj)
        = 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                Model.specify 
                    ...
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "cell Generic.List<Indonesia>(" + (Helper.sourceFoldArray (s) + ")"))

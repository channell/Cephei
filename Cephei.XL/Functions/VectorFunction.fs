...
module VectorFunction =
    ...
    *)
    ...
    let Vector_create1
        ([<ExcelArgument(Name="Mnemonic",Description = "Identifier for Cell")>] 
         mnemonic : string)
        ([<ExcelArgument(Name="from",Description = "double range")>] 
         from : obj)
        = 
        if not (Model.IsInFunctionWizard()) then
    ...
    let Vector_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<Vector> (c)) :> ICell
                let format (i : Cephei.Cell.List<Vector>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<Vector>(" + (Helper.sourceFoldArray (s) + "))"))

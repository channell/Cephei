    ...
    let CumulativeGammaDistribution_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.CumulativeGammaDistribution 
                                                            _a.cell 
                                                       ) :> ICell
    ...
    let CumulativeGammaDistribution_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<CumulativeGammaDistribution> (c)) :> ICell
                let format (i : Cephei.Cell.List<CumulativeGammaDistribution>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<CumulativeGammaDistribution>(" + (Helper.sourceFoldArray (s) + "))"))

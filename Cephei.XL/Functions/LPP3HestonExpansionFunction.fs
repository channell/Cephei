    ...
    let LPP3HestonExpansion_impliedVolatility
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = withMnemonic mnemonic ((LPP3HestonExpansionModel.Cast _LPP3HestonExpansion.cell).ImpliedVolatility
                                                            _strike.cell 
                                                            _forward.cell 
                                                       ) :> ICell
    ...
    let LPP3HestonExpansion_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = withMnemonic mnemonic (Fun.LPP3HestonExpansion 
                                                            _kappa.cell 
                                                            _theta.cell 
                                                            _sigma.cell 
                                                            _v0.cell 
    ...
    let LPP3HestonExpansion_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<LPP3HestonExpansion> (c)) :> ICell
                let format (i : Cephei.Cell.List<LPP3HestonExpansion>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<LPP3HestonExpansion>(" + (Helper.sourceFoldArray (s) + "))"))

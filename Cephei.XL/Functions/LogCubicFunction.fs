    ...
    let LogCubic_interpolate
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = ((LogCubicModel.Cast _LogCubic.cell).Interpolate
                                                            _xBegin.cell 
                                                            _size.cell 
                                                            _yBegin.cell 
                                                       ) :> ICell
    ...
    let LogCubic_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.LogCubic 
                                                            _da.cell 
                                                            _monotonic.cell 
                                                            _leftCondition.cell 
    ...
    let LogCubic_create1
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.LogCubic1 ()
                                                       ) :> ICell
                let format (i : ICell) (l:string) = Helper.Range.fromModel (i :?> ICell<LogCubic>) l
    ...
    let LogCubic_Range 
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (new Cephei.Cell.List<LogCubic> (c)) :> ICell
                let format (i : Cephei.Cell.List<LogCubic>) (l : string) = Helper.Range.fromModelList i l

                Model.specify 
                    { mnemonic = Model.formatMnemonic mnemonic
                    ; creator = builder
                    ; subscriber = Helper.subscriberModelRange format
                    ; source =  (fun () -> "(new Cephei.Cell.List<LogCubic>(" + (Helper.sourceFoldArray (s) + "))"))

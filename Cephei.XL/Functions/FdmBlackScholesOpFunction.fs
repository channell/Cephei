    ...
    let FdmBlackScholesOp_create
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let builder (current : ICell) = (Fun.FdmBlackScholesOp 
                                                            _mesher.cell 
                                                            _bsProcess.cell 
                                                            _strike.cell 
                                                            _localVol.cell 
    ...
    let FdmBlackScholesOp_solve_splitting
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                let _r = Helper.toCell<Vector> r "r" 
                let _dt = Helper.toCell<double> dt "dt" 
                let builder (current : ICell) = ((FdmBlackScholesOpModel.Cast _FdmBlackScholesOp.cell).Solve_splitting
                                                            _direction.cell 
                                                            _r.cell 
                                                            _dt.cell 
                                                       ) :> ICell
        ...
        if not (Model.IsInFunctionWizard()) then
            ...
            try
                ...
                Model.specify 
                    ...
                    ; source =  (fun () -> "(new Cephei.Cell.List<FdmBlackScholesOp>(" + (Helper.sourceFoldArray (s) + "))"))

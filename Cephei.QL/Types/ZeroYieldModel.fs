...
*)
    ...
    let _discountImpl                              (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _ZeroYield.Value.discountImpl(i.Value, t.Value))
    let _forwardImpl                               (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _ZeroYield.Value.forwardImpl(i.Value, t.Value))
    let _guess                                     (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _ZeroYield.Value.guess(i.Value, c.Value, validData.Value, f.Value))
    let _initialDate                               (c : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _ZeroYield.Value.initialDate(c.Value))
    let _initialValue                              (c : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _ZeroYield.Value.initialValue(c.Value))
    let _maxIterations                             = cell (fun () -> _ZeroYield.Value.maxIterations())
    let _maxValueAfter                             (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _ZeroYield.Value.maxValueAfter(i.Value, c.Value, validData.Value, f.Value))
    let _minValueAfter                             (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _ZeroYield.Value.minValueAfter(i.Value, c.Value, validData.Value, f.Value))
    let _updateGuess                               (data : ICell<Generic.List<double>>) (rate : ICell<double>) (i : ICell<int>)   
                                                   = cell (fun () -> _ZeroYield.Value.updateGuess(data.Value, rate.Value, i.Value)
                                                                     _ZeroYield.Value)
    let _zeroYieldImpl                             (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _ZeroYield.Value.zeroYieldImpl(i.Value, t.Value))

...
*)
    ...
    let _covariance                                (t : ICell<double>) (x : ICell<Vector>)   
                                                   = triv (fun () -> _LfmCovarianceProxy.Value.covariance(t.Value, x.Value))
    let _diffusion                                 (t : ICell<double>)   
                                                   = triv (fun () -> _LfmCovarianceProxy.Value.diffusion(t.Value))
    let _diffusion1                                (t : ICell<double>) (x : ICell<Vector>)   
                                                   = triv (fun () -> _LfmCovarianceProxy.Value.diffusion(t.Value, x.Value))
    let _integratedCovariance                      (i : ICell<int>) (j : ICell<int>) (t : ICell<double>)   
                                                   = triv (fun () -> _LfmCovarianceProxy.Value.integratedCovariance(i.Value, j.Value, t.Value))
    let _integratedCovariance1                     (i : ICell<int>) (j : ICell<int>) (t : ICell<double>) (x : ICell<Vector>)   
                                                   = triv (fun () -> _LfmCovarianceProxy.Value.integratedCovariance(i.Value, j.Value, t.Value, x.Value))
    let _volaModel                                 = triv (fun () -> _LfmCovarianceProxy.Value.volaModel)
    let _volaModel_                                = triv (fun () -> _LfmCovarianceProxy.Value.volaModel_)
    let _volatilityModel                           = triv (fun () -> _LfmCovarianceProxy.Value.volatilityModel())

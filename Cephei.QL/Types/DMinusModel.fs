...
*)
    let mutable
        _DMinus                                    = make (fun () -> new DMinus (gridPoints.Value, h.Value))
    let _add                                       (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.add(A.Value, B.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.applyTo(v.Value))
    let _Clone                                     = triv _DMinus (fun () -> _DMinus.Value.Clone())
    let _diagonal                                  = triv _DMinus (fun () -> _DMinus.Value.diagonal())
    let _identity                                  (size : ICell<int>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.identity(size.Value))
    let _isTimeDependent                           = triv _DMinus (fun () -> _DMinus.Value.isTimeDependent())
    let _lowerDiagonal                             = triv _DMinus (fun () -> _DMinus.Value.lowerDiagonal())
    let _multiply                                  (a : ICell<double>) (o : ICell<IOperator>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.multiply(a.Value, o.Value))
    let _setFirstRow                               (valB : ICell<double>) (valC : ICell<double>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.setFirstRow(valB.Value, valC.Value)
                                                                             _DMinus.Value)
    let _setLastRow                                (valA : ICell<double>) (valB : ICell<double>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.setLastRow(valA.Value, valB.Value)
                                                                             _DMinus.Value)
    let _setMidRow                                 (i : ICell<int>) (valA : ICell<double>) (valB : ICell<double>) (valC : ICell<double>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.setMidRow(i.Value, valA.Value, valB.Value, valC.Value)
                                                                             _DMinus.Value)
    let _setMidRows                                (valA : ICell<double>) (valB : ICell<double>) (valC : ICell<double>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.setMidRows(valA.Value, valB.Value, valC.Value)
                                                                             _DMinus.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.setTime(t.Value)
                                                                             _DMinus.Value)
    let _size                                      = triv _DMinus (fun () -> _DMinus.Value.size())
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.solveFor(rhs.Value))
    let _SOR                                       (rhs : ICell<Vector>) (tol : ICell<double>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.SOR(rhs.Value, tol.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _DMinus (fun () -> _DMinus.Value.subtract(A.Value, B.Value))
    let _upperDiagonal                             = triv _DMinus (fun () -> _DMinus.Value.upperDiagonal())

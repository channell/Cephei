...
*)
    let mutable
        _HestonExpansionEngine                     = make (fun () -> (createEvaluationDate _evaluationDate (fun () ->new HestonExpansionEngine (model.Value, formula.Value))))
    let _setModel                                  (model : ICell<Handle<HestonModel>>)   
                                                   = triv _HestonExpansionEngine (fun () -> (curryEvaluationDate _evaluationDate _HestonExpansionEngine).Value.setModel(model.Value)
                                                                                            _HestonExpansionEngine.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _HestonExpansionEngine (fun () -> (curryEvaluationDate _evaluationDate _HestonExpansionEngine).Value.registerWith(handler.Value)
                                                                                            _HestonExpansionEngine.Value)
    let _reset                                     = triv _HestonExpansionEngine (fun () -> (curryEvaluationDate _evaluationDate _HestonExpansionEngine).Value.reset()
                                                                                            _HestonExpansionEngine.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _HestonExpansionEngine (fun () -> (curryEvaluationDate _evaluationDate _HestonExpansionEngine).Value.unregisterWith(handler.Value)
                                                                                            _HestonExpansionEngine.Value)
    let _update                                    = triv _HestonExpansionEngine (fun () -> (curryEvaluationDate _evaluationDate _HestonExpansionEngine).Value.update()
                                                                                            _HestonExpansionEngine.Value)

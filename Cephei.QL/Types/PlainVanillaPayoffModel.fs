...
*)
    ...
    let _name                                      = cell (fun () -> _PlainVanillaPayoff.Value.name())
    let _value                                     (price : ICell<double>)   
                                                   = cell (fun () -> _PlainVanillaPayoff.Value.value(price.Value))
    let _description                               = cell (fun () -> _PlainVanillaPayoff.Value.description())
    let _strike                                    = cell (fun () -> _PlainVanillaPayoff.Value.strike())
    let _optionType                                = cell (fun () -> _PlainVanillaPayoff.Value.optionType())
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _PlainVanillaPayoff.Value.accept(v.Value)

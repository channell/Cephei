...
*)
    ...
    let _seasonalityFactor                         (To : ICell<Date>)   
                                                   = cell (fun () -> _KerkhofSeasonality.Value.seasonalityFactor(To.Value))
    let _correctYoYRate                            (d : ICell<Date>) (r : ICell<double>) (iTS : ICell<InflationTermStructure>)   
                                                   = cell (fun () -> _KerkhofSeasonality.Value.correctYoYRate(d.Value, r.Value, iTS.Value))
    let _correctZeroRate                           (d : ICell<Date>) (r : ICell<double>) (iTS : ICell<InflationTermStructure>)   
                                                   = cell (fun () -> _KerkhofSeasonality.Value.correctZeroRate(d.Value, r.Value, iTS.Value))
    let _frequency                                 = cell (fun () -> _KerkhofSeasonality.Value.frequency())
    let _isConsistent                              (iTS : ICell<InflationTermStructure>)   
                                                   = cell (fun () -> _KerkhofSeasonality.Value.isConsistent(iTS.Value))
    let _seasonalityBaseDate                       = cell (fun () -> _KerkhofSeasonality.Value.seasonalityBaseDate())
    let _seasonalityFactors                        = cell (fun () -> _KerkhofSeasonality.Value.seasonalityFactors())
    let _set                                       (seasonalityBaseDate : ICell<Date>) (frequency : ICell<Frequency>) (seasonalityFactors : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _KerkhofSeasonality.Value.set(seasonalityBaseDate.Value, frequency.Value, seasonalityFactors.Value)

...
(*
    Functions
*)
    let _CallableZeroCouponBond                    = triv (fun () -> withEngine _pricingEngine.Value (new CallableZeroCouponBond (settlementDays.Value, faceAmount.Value, calendar.Value, maturityDate.Value, dayCounter.Value, paymentConvention.Value, redemption.Value, issueDate.Value, putCallSchedule.Value)))

...
*)
    ...
    let _correctZeroRate                           (d : ICell<Date>) (r : ICell<double>) (iTS : ICell<InflationTermStructure>)   
                                                   = triv (fun () -> _MultiplicativePriceSeasonality.Value.correctZeroRate(d.Value, r.Value, iTS.Value))
    let _frequency                                 = triv (fun () -> _MultiplicativePriceSeasonality.Value.frequency())
    let _isConsistent                              (iTS : ICell<InflationTermStructure>)   
                                                   = triv (fun () -> _MultiplicativePriceSeasonality.Value.isConsistent(iTS.Value))
    let _seasonalityBaseDate                       = triv (fun () -> _MultiplicativePriceSeasonality.Value.seasonalityBaseDate())
    let _seasonalityFactor                         (To : ICell<Date>)   
                                                   = triv (fun () -> _MultiplicativePriceSeasonality.Value.seasonalityFactor(To.Value))
    let _seasonalityFactors                        = triv (fun () -> _MultiplicativePriceSeasonality.Value.seasonalityFactors())

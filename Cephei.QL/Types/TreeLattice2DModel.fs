...
*)
    let mutable
        _TreeLattice2D                             = make (fun () -> new TreeLattice2D<'T,'Tl> (tree1.Value, tree2.Value, correlation.Value))
    let _descendant                                (i : ICell<int>) (index : ICell<int>) (branch : ICell<int>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.descendant(i.Value, index.Value, branch.Value))
    let _grid                                      (t : ICell<double>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.grid(t.Value))
    let _probability                               (i : ICell<int>) (index : ICell<int>) (branch : ICell<int>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.probability(i.Value, index.Value, branch.Value))
    let _size                                      (i : ICell<int>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.size(i.Value))
    let _initialize                                (asset : ICell<DiscretizedAsset>) (t : ICell<double>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.initialize(asset.Value, t.Value)
                                                                                    _TreeLattice2D.Value)
    let _partialRollback                           (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.partialRollback(asset.Value, To.Value)
                                                                                    _TreeLattice2D.Value)
    let _presentValue                              (asset : ICell<DiscretizedAsset>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.presentValue(asset.Value))
    let _rollback                                  (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.rollback(asset.Value, To.Value)
                                                                                    _TreeLattice2D.Value)
    let _statePrices                               (i : ICell<int>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.statePrices(i.Value))
    let _stepback                                  (i : ICell<int>) (values : ICell<Vector>) (newValues : ICell<Vector>)   
                                                   = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.stepback(i.Value, values.Value, newValues.Value)
                                                                                    _TreeLattice2D.Value)
    let _timeGrid                                  = triv _TreeLattice2D (fun () -> _TreeLattice2D.Value.timeGrid())

...
*)
    let mutable
        _VarProxy_Helper                           = make (fun () -> new VarProxy_Helper (proxy.Value, i.Value, j.Value))
    let _corrModel_                                = triv _VarProxy_Helper (fun () -> _VarProxy_Helper.Value.corrModel_)
    let _value                                     (t : ICell<double>)   
                                                   = triv _VarProxy_Helper (fun () -> _VarProxy_Helper.Value.value(t.Value))
    let _volaModel_                                = triv _VarProxy_Helper (fun () -> _VarProxy_Helper.Value.volaModel_)

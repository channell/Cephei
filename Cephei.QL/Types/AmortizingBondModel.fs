...
(*
    Functions
*)
    let _AmortizingBond                            = triv (fun () -> withEngine _pricingEngine.Value (new AmortizingBond (FaceValue.Value, MarketValue.Value, CouponRate.Value, IssueDate.Value, MaturityDate.Value, TradeDate.Value, payFrequency.Value, dCounter.Value, Method.Value, calendar.Value, gYield.Value)))

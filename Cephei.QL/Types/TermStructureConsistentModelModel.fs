...
*)
    ...
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _TermStructureConsistentModel.Value.registerWith(handler.Value)
                                                                     _TermStructureConsistentModel.Value)
    let _termStructure                             = cell (fun () -> _TermStructureConsistentModel.Value.termStructure())
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _TermStructureConsistentModel.Value.unregisterWith(handler.Value)
                                                                     _TermStructureConsistentModel.Value)

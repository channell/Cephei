...
*)
    ...
    let _name                                      = cell (fun () -> _PercentageStrikePayoff.Value.name())
    let _value                                     (price : ICell<double>)   
                                                   = cell (fun () -> _PercentageStrikePayoff.Value.value(price.Value))
    let _description                               = cell (fun () -> _PercentageStrikePayoff.Value.description())
    let _strike                                    = cell (fun () -> _PercentageStrikePayoff.Value.strike())
    let _optionType                                = cell (fun () -> _PercentageStrikePayoff.Value.optionType())
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _PercentageStrikePayoff.Value.accept(v.Value)

...
*)
    let mutable
        _Australia                                 = make (fun () -> new Australia ())
    let _addedHolidays                             = triv _Australia (fun () -> _Australia.Value.addedHolidays)
    let _addHoliday                                (d : ICell<Date>)   
                                                   = triv _Australia (fun () -> _Australia.Value.addHoliday(d.Value)
                                                                                _Australia.Value)
    let _adjust                                    (d : ICell<Date>) (c : ICell<BusinessDayConvention>)   
                                                   = triv _Australia (fun () -> _Australia.Value.adjust(d.Value, c.Value))
    let _advance                                   (d : ICell<Date>) (p : ICell<Period>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Australia (fun () -> _Australia.Value.advance(d.Value, p.Value, c.Value, endOfMonth.Value))
    let _advance1                                  (d : ICell<Date>) (n : ICell<int>) (unit : ICell<TimeUnit>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Australia (fun () -> _Australia.Value.advance(d.Value, n.Value, unit.Value, c.Value, endOfMonth.Value))
    let _businessDaysBetween                       (from : ICell<Date>) (To : ICell<Date>) (includeFirst : ICell<bool>) (includeLast : ICell<bool>)   
                                                   = triv _Australia (fun () -> _Australia.Value.businessDaysBetween(from.Value, To.Value, includeFirst.Value, includeLast.Value))
    let _calendar                                  = triv _Australia (fun () -> _Australia.Value.calendar)
    let _empty                                     = triv _Australia (fun () -> _Australia.Value.empty())
    let _endOfMonth                                (d : ICell<Date>)   
                                                   = triv _Australia (fun () -> _Australia.Value.endOfMonth(d.Value))
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Australia (fun () -> _Australia.Value.Equals(o.Value))
    let _isBusinessDay                             (d : ICell<Date>)   
                                                   = triv _Australia (fun () -> _Australia.Value.isBusinessDay(d.Value))
    let _isEndOfMonth                              (d : ICell<Date>)   
                                                   = triv _Australia (fun () -> _Australia.Value.isEndOfMonth(d.Value))
    let _isHoliday                                 (d : ICell<Date>)   
                                                   = triv _Australia (fun () -> _Australia.Value.isHoliday(d.Value))
    let _isWeekend                                 (w : ICell<DayOfWeek>)   
                                                   = triv _Australia (fun () -> _Australia.Value.isWeekend(w.Value))
    let _name                                      = triv _Australia (fun () -> _Australia.Value.name())
    let _removedHolidays                           = triv _Australia (fun () -> _Australia.Value.removedHolidays)
    let _removeHoliday                             (d : ICell<Date>)   
                                                   = triv _Australia (fun () -> _Australia.Value.removeHoliday(d.Value)
                                                                                _Australia.Value)
    do this.Bind(_Australia)
(* 
    casting 
*)
    
    member internal this.Inject v = _Australia <- v
    static member Cast (p : ICell<Australia>) = 
        if p :? AustraliaModel then 
            p :?> AustraliaModel
        else
            let o = new AustraliaModel ()
            o.Inject p
            o.Bind p
            o

...
*)
    ...
    let _cashPayoff                                = cell (fun () -> _CashOrNothingPayoff.Value.cashPayoff())
    let _description                               = cell (fun () -> _CashOrNothingPayoff.Value.description())
    let _name                                      = cell (fun () -> _CashOrNothingPayoff.Value.name())
    let _value                                     (price : ICell<double>)   
                                                   = cell (fun () -> _CashOrNothingPayoff.Value.value(price.Value))
    let _strike                                    = cell (fun () -> _CashOrNothingPayoff.Value.strike())
    let _optionType                                = cell (fun () -> _CashOrNothingPayoff.Value.optionType())
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _CashOrNothingPayoff.Value.accept(v.Value)

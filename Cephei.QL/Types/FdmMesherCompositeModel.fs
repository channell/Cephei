...
*)
    ...
    let _dminus                                    (iter : ICell<FdmLinearOpIterator>) (direction : ICell<int>)   
                                                   = cell (fun () -> _FdmMesherComposite.Value.dminus(iter.Value, direction.Value))
    let _dplus                                     (iter : ICell<FdmLinearOpIterator>) (direction : ICell<int>)   
                                                   = cell (fun () -> _FdmMesherComposite.Value.dplus(iter.Value, direction.Value))
    let _getFdm1dMeshers                           = cell (fun () -> _FdmMesherComposite.Value.getFdm1dMeshers())
    let _location                                  (iter : ICell<FdmLinearOpIterator>) (direction : ICell<int>)   
                                                   = cell (fun () -> _FdmMesherComposite.Value.location(iter.Value, direction.Value))
    let _locations                                 (direction : ICell<int>)   
                                                   = cell (fun () -> _FdmMesherComposite.Value.locations(direction.Value))
    let _layout                                    = cell (fun () -> _FdmMesherComposite.Value.layout())

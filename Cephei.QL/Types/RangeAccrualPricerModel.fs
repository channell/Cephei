...
*)
    ...
    let _capletRate                                (effectiveCap : ICell<double>)   
                                                   = triv (fun () -> _RangeAccrualPricer.Value.capletRate(effectiveCap.Value))
    let _floorletPrice                             (effectiveFloor : ICell<double>)   
                                                   = triv (fun () -> _RangeAccrualPricer.Value.floorletPrice(effectiveFloor.Value))
    let _floorletRate                              (effectiveFloor : ICell<double>)   
                                                   = triv (fun () -> _RangeAccrualPricer.Value.floorletRate(effectiveFloor.Value))
    let _initialize                                (coupon : ICell<FloatingRateCoupon>)   
                                                   = triv (fun () -> _RangeAccrualPricer.Value.initialize(coupon.Value)
                                                                     _RangeAccrualPricer.Value)
    let _swapletPrice                              = triv (fun () -> _RangeAccrualPricer.Value.swapletPrice())
    let _swapletRate                               = triv (fun () -> _RangeAccrualPricer.Value.swapletRate())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _RangeAccrualPricer.Value.registerWith(handler.Value)
                                                                     _RangeAccrualPricer.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv (fun () -> _RangeAccrualPricer.Value.unregisterWith(handler.Value)
                                                                     _RangeAccrualPricer.Value)
    let _update                                    = triv (fun () -> _RangeAccrualPricer.Value.update()
                                                                     _RangeAccrualPricer.Value)

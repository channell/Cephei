...
*)
    let mutable
        _Finland                                   = make (fun () -> new Finland ())
    let _addedHolidays                             = triv _Finland (fun () -> _Finland.Value.addedHolidays)
    let _addHoliday                                (d : ICell<Date>)   
                                                   = triv _Finland (fun () -> _Finland.Value.addHoliday(d.Value)
                                                                              _Finland.Value)
    let _adjust                                    (d : ICell<Date>) (c : ICell<BusinessDayConvention>)   
                                                   = triv _Finland (fun () -> _Finland.Value.adjust(d.Value, c.Value))
    let _advance                                   (d : ICell<Date>) (p : ICell<Period>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Finland (fun () -> _Finland.Value.advance(d.Value, p.Value, c.Value, endOfMonth.Value))
    let _advance1                                  (d : ICell<Date>) (n : ICell<int>) (unit : ICell<TimeUnit>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Finland (fun () -> _Finland.Value.advance(d.Value, n.Value, unit.Value, c.Value, endOfMonth.Value))
    let _businessDaysBetween                       (from : ICell<Date>) (To : ICell<Date>) (includeFirst : ICell<bool>) (includeLast : ICell<bool>)   
                                                   = triv _Finland (fun () -> _Finland.Value.businessDaysBetween(from.Value, To.Value, includeFirst.Value, includeLast.Value))
    let _calendar                                  = triv _Finland (fun () -> _Finland.Value.calendar)
    let _empty                                     = triv _Finland (fun () -> _Finland.Value.empty())
    let _endOfMonth                                (d : ICell<Date>)   
                                                   = triv _Finland (fun () -> _Finland.Value.endOfMonth(d.Value))
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Finland (fun () -> _Finland.Value.Equals(o.Value))
    let _isBusinessDay                             (d : ICell<Date>)   
                                                   = triv _Finland (fun () -> _Finland.Value.isBusinessDay(d.Value))
    let _isEndOfMonth                              (d : ICell<Date>)   
                                                   = triv _Finland (fun () -> _Finland.Value.isEndOfMonth(d.Value))
    let _isHoliday                                 (d : ICell<Date>)   
                                                   = triv _Finland (fun () -> _Finland.Value.isHoliday(d.Value))
    let _isWeekend                                 (w : ICell<DayOfWeek>)   
                                                   = triv _Finland (fun () -> _Finland.Value.isWeekend(w.Value))
    let _name                                      = triv _Finland (fun () -> _Finland.Value.name())
    let _removedHolidays                           = triv _Finland (fun () -> _Finland.Value.removedHolidays)
    let _removeHoliday                             (d : ICell<Date>)   
                                                   = triv _Finland (fun () -> _Finland.Value.removeHoliday(d.Value)
                                                                              _Finland.Value)
...
*)
    ...
    member internal this.Inject v = _Finland <- v
    static member Cast (p : ICell<Finland>) = 
        if p :? FinlandModel then 
            p :?> FinlandModel
        else
            let o = new FinlandModel ()
            o.Inject p
            o.Bind p
            o

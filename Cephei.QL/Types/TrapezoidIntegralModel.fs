...
*)
    let mutable
        _TrapezoidIntegral                         = make (fun () -> new TrapezoidIntegral<'IntegrationPolicy> (accuracy.Value, maxIterations.Value))
    let _absoluteAccuracy                          = triv _TrapezoidIntegral (fun () -> _TrapezoidIntegral.Value.absoluteAccuracy())
    let _absoluteError                             = triv _TrapezoidIntegral (fun () -> _TrapezoidIntegral.Value.absoluteError())
    let _integrationSuccess                        = triv _TrapezoidIntegral (fun () -> _TrapezoidIntegral.Value.integrationSuccess())
    let _maxEvaluations                            = triv _TrapezoidIntegral (fun () -> _TrapezoidIntegral.Value.maxEvaluations())
    let _numberOfEvaluations                       = triv _TrapezoidIntegral (fun () -> _TrapezoidIntegral.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = triv _TrapezoidIntegral (fun () -> _TrapezoidIntegral.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                                        _TrapezoidIntegral.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = triv _TrapezoidIntegral (fun () -> _TrapezoidIntegral.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                                        _TrapezoidIntegral.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = triv _TrapezoidIntegral (fun () -> _TrapezoidIntegral.Value.value(f.Value, a.Value, b.Value))

...
*)
    ...
    let _absoluteAccuracy                          = cell (fun () -> _TrapezoidIntegral.Value.absoluteAccuracy())
    let _absoluteError                             = cell (fun () -> _TrapezoidIntegral.Value.absoluteError())
    let _integrationSuccess                        = cell (fun () -> _TrapezoidIntegral.Value.integrationSuccess())
    let _maxEvaluations                            = cell (fun () -> _TrapezoidIntegral.Value.maxEvaluations())
    let _numberOfEvaluations                       = cell (fun () -> _TrapezoidIntegral.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = cell (fun () -> _TrapezoidIntegral.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                     _TrapezoidIntegral.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = cell (fun () -> _TrapezoidIntegral.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                     _TrapezoidIntegral.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = cell (fun () -> _TrapezoidIntegral.Value.value(f.Value, a.Value, b.Value))

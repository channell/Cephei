...
*)
    let mutable
        _Germany                                   = make (fun () -> new Germany (m.Value))
    let _addedHolidays                             = triv _Germany (fun () -> _Germany.Value.addedHolidays)
    let _addHoliday                                (d : ICell<Date>)   
                                                   = triv _Germany (fun () -> _Germany.Value.addHoliday(d.Value)
                                                                              _Germany.Value)
    let _adjust                                    (d : ICell<Date>) (c : ICell<BusinessDayConvention>)   
                                                   = triv _Germany (fun () -> _Germany.Value.adjust(d.Value, c.Value))
    let _advance                                   (d : ICell<Date>) (p : ICell<Period>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Germany (fun () -> _Germany.Value.advance(d.Value, p.Value, c.Value, endOfMonth.Value))
    let _advance1                                  (d : ICell<Date>) (n : ICell<int>) (unit : ICell<TimeUnit>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Germany (fun () -> _Germany.Value.advance(d.Value, n.Value, unit.Value, c.Value, endOfMonth.Value))
    let _businessDaysBetween                       (from : ICell<Date>) (To : ICell<Date>) (includeFirst : ICell<bool>) (includeLast : ICell<bool>)   
                                                   = triv _Germany (fun () -> _Germany.Value.businessDaysBetween(from.Value, To.Value, includeFirst.Value, includeLast.Value))
    let _calendar                                  = triv _Germany (fun () -> _Germany.Value.calendar)
    let _empty                                     = triv _Germany (fun () -> _Germany.Value.empty())
    let _endOfMonth                                (d : ICell<Date>)   
                                                   = triv _Germany (fun () -> _Germany.Value.endOfMonth(d.Value))
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Germany (fun () -> _Germany.Value.Equals(o.Value))
    let _isBusinessDay                             (d : ICell<Date>)   
                                                   = triv _Germany (fun () -> _Germany.Value.isBusinessDay(d.Value))
    let _isEndOfMonth                              (d : ICell<Date>)   
                                                   = triv _Germany (fun () -> _Germany.Value.isEndOfMonth(d.Value))
    let _isHoliday                                 (d : ICell<Date>)   
                                                   = triv _Germany (fun () -> _Germany.Value.isHoliday(d.Value))
    let _isWeekend                                 (w : ICell<DayOfWeek>)   
                                                   = triv _Germany (fun () -> _Germany.Value.isWeekend(w.Value))
    let _name                                      = triv _Germany (fun () -> _Germany.Value.name())
    let _removedHolidays                           = triv _Germany (fun () -> _Germany.Value.removedHolidays)
    let _removeHoliday                             (d : ICell<Date>)   
                                                   = triv _Germany (fun () -> _Germany.Value.removeHoliday(d.Value)
                                                                              _Germany.Value)
    do this.Bind(_Germany)
(* 
    casting 
*)
    internal new () = new GermanyModel(null)
    member internal this.Inject v = _Germany <- v

...
*)
    ...
    let _equityFxCorrelation                       = cell (fun () -> _FdmQuantoHelper.Value.equityFxCorrelation())
    let _exchRateATMlevel                          = cell (fun () -> _FdmQuantoHelper.Value.exchRateATMlevel())
    let _foreignTermStructure                      = cell (fun () -> _FdmQuantoHelper.Value.foreignTermStructure())
    let _fxVolatilityTermStructure                 = cell (fun () -> _FdmQuantoHelper.Value.fxVolatilityTermStructure())
    let _quantoAdjustment                          (equityVol : ICell<double>) (t1 : ICell<double>) (t2 : ICell<double>)   
                                                   = cell (fun () -> _FdmQuantoHelper.Value.quantoAdjustment(equityVol.Value, t1.Value, t2.Value))
    let _quantoAdjustment1                         (equityVol : ICell<Vector>) (t1 : ICell<double>) (t2 : ICell<double>)   
                                                   = cell (fun () -> _FdmQuantoHelper.Value.quantoAdjustment(equityVol.Value, t1.Value, t2.Value))
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _FdmQuantoHelper.Value.registerWith(handler.Value)
                                                                     _FdmQuantoHelper.Value)
    let _riskFreeTermStructure                     = cell (fun () -> _FdmQuantoHelper.Value.riskFreeTermStructure())
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _FdmQuantoHelper.Value.unregisterWith(handler.Value)
                                                                     _FdmQuantoHelper.Value)
    let _update                                    = cell (fun () -> _FdmQuantoHelper.Value.update()
                                                                     _FdmQuantoHelper.Value)

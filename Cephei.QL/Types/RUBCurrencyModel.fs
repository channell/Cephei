...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _RUBCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _RUBCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _RUBCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _RUBCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _RUBCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _RUBCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _RUBCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _RUBCurrency.Value.symbol)

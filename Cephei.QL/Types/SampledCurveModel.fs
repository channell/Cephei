...
*)
    let mutable
        _SampledCurve                              = make (fun () -> new SampledCurve (grid.Value))
    let _Clone                                     = triv _SampledCurve (fun () -> _SampledCurve.Value.Clone())
    let _empty                                     = triv _SampledCurve (fun () -> _SampledCurve.Value.empty())
    let _firstDerivativeAtCenter                   = triv _SampledCurve (fun () -> _SampledCurve.Value.firstDerivativeAtCenter())
    let _grid                                      = triv _SampledCurve (fun () -> _SampledCurve.Value.grid())
    let _gridValue                                 (i : ICell<int>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.gridValue(i.Value))
    let _regrid                                    (new_grid : ICell<Vector>) (func : ICell<Func<double,double>>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.regrid(new_grid.Value, func.Value)
                                                                                   _SampledCurve.Value)
    let _regrid1                                   (new_grid : ICell<Vector>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.regrid(new_grid.Value)
                                                                                   _SampledCurve.Value)
    let _regridLogGrid                             (min : ICell<double>) (max : ICell<double>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.regridLogGrid(min.Value, max.Value)
                                                                                   _SampledCurve.Value)
    let _sample                                    (f : ICell<Func<double,double>>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.sample(f.Value)
                                                                                   _SampledCurve.Value)
    let _scaleGrid                                 (s : ICell<double>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.scaleGrid(s.Value)
                                                                                   _SampledCurve.Value)
    let _secondDerivativeAtCenter                  = triv _SampledCurve (fun () -> _SampledCurve.Value.secondDerivativeAtCenter())
    let _setGrid                                   (g : ICell<Vector>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.setGrid(g.Value)
                                                                                   _SampledCurve.Value)
    let _setLogGrid                                (min : ICell<double>) (max : ICell<double>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.setLogGrid(min.Value, max.Value)
                                                                                   _SampledCurve.Value)
    let _setValue                                  (i : ICell<int>) (v : ICell<double>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.setValue(i.Value, v.Value)
                                                                                   _SampledCurve.Value)
    let _setValues                                 (g : ICell<Vector>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.setValues(g.Value)
                                                                                   _SampledCurve.Value)
    let _shiftGrid                                 (s : ICell<double>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.shiftGrid(s.Value)
                                                                                   _SampledCurve.Value)
    let _size                                      = triv _SampledCurve (fun () -> _SampledCurve.Value.size())
    let _transform                                 (x : ICell<Func<double,double>>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.transform(x.Value))
    let _transformGrid                             (x : ICell<Func<double,double>>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.transformGrid(x.Value))
    let _value                                     (i : ICell<int>)   
                                                   = triv _SampledCurve (fun () -> _SampledCurve.Value.value(i.Value))
    let _valueAtCenter                             = triv _SampledCurve (fun () -> _SampledCurve.Value.valueAtCenter())
    let _values                                    = triv _SampledCurve (fun () -> _SampledCurve.Value.values())
...
*)
    let mutable
        _SampledCurve                              = make (fun () -> new SampledCurve (gridSize.Value))
    let _Clone                                     = triv _SampledCurve (fun () -> _SampledCurve.Value.Clone())
    let _empty                                     = triv _SampledCurve (fun () -> _SampledCurve.Value.empty())
    let _firstDerivativeAtCenter                   = triv _SampledCurve (fun () -> _SampledCurve.Value.firstDerivativeAtCenter())
    let _grid                                      = triv _SampledCurve (fun () -> _SampledCurve.Value.grid())

...
*)
    ...
    let _primitive                                 (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _KernelInterpolation.Value.primitive(x.Value, allowExtrapolation.Value))
    let _secondDerivative                          (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _KernelInterpolation.Value.secondDerivative(x.Value, allowExtrapolation.Value))
    let _update                                    = triv (fun () -> _KernelInterpolation.Value.update()
                                                                     _KernelInterpolation.Value)
    let _value                                     (x : ICell<double>)   
                                                   = triv (fun () -> _KernelInterpolation.Value.value(x.Value))
    let _value1                                    (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _KernelInterpolation.Value.value(x.Value, allowExtrapolation.Value))
    let _xMax                                      = triv (fun () -> _KernelInterpolation.Value.xMax())
    let _xMin                                      = triv (fun () -> _KernelInterpolation.Value.xMin())
    let _allowsExtrapolation                       = triv (fun () -> _KernelInterpolation.Value.allowsExtrapolation())
    let _disableExtrapolation                      (b : ICell<bool>)   
                                                   = triv (fun () -> _KernelInterpolation.Value.disableExtrapolation(b.Value)
                                                                     _KernelInterpolation.Value)
    let _enableExtrapolation                       (b : ICell<bool>)   
                                                   = triv (fun () -> _KernelInterpolation.Value.enableExtrapolation(b.Value)
                                                                     _KernelInterpolation.Value)

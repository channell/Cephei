...
*)
    ...
    let _clone                                     = cell (fun () -> _NelsonSiegelFitting.Value.clone())
    let _size                                      = cell (fun () -> _NelsonSiegelFitting.Value.size())
    let _constrainAtZero                           = cell (fun () -> _NelsonSiegelFitting.Value.constrainAtZero())
    let _discount                                  (x : ICell<Vector>) (t : ICell<double>)   
                                                   = cell (fun () -> _NelsonSiegelFitting.Value.discount(x.Value, t.Value))
    let _minimumCostValue                          = cell (fun () -> _NelsonSiegelFitting.Value.minimumCostValue())
    let _numberOfIterations                        = cell (fun () -> _NelsonSiegelFitting.Value.numberOfIterations())
    let _optimizationMethod                        = cell (fun () -> _NelsonSiegelFitting.Value.optimizationMethod())
    let _solution                                  = cell (fun () -> _NelsonSiegelFitting.Value.solution())
    let _weights                                   = cell (fun () -> _NelsonSiegelFitting.Value.weights())

...
*)
    ...
    let _Add                                       (item : ICell<KeyValuePair<Date,'T>>)   
                                                   = cell (fun () -> _TimeSeries.Value.Add(item.Value)
                                                                     _TimeSeries.Value)
    let _Add1                                      (key : ICell<Date>) (value : ICell<'T>)   
                                                   = cell (fun () -> _TimeSeries.Value.Add(key.Value, value.Value)
                                                                     _TimeSeries.Value)
    let _Clear                                     = cell (fun () -> _TimeSeries.Value.Clear()
                                                                     _TimeSeries.Value)
    let _Contains                                  (item : ICell<KeyValuePair<Date,'T>>)   
                                                   = cell (fun () -> _TimeSeries.Value.Contains(item.Value))
    let _ContainsKey                               (key : ICell<Date>)   
                                                   = cell (fun () -> _TimeSeries.Value.ContainsKey(key.Value))
    let _CopyTo                                    (array : ICell<KeyValuePair<Date,'T>[]>) (arrayIndex : ICell<int>)   
                                                   = cell (fun () -> _TimeSeries.Value.CopyTo(array.Value, arrayIndex.Value)
                                                                     _TimeSeries.Value)
    let _Count                                     = cell (fun () -> _TimeSeries.Value.Count)
    let _GetEnumerator                             = cell (fun () -> _TimeSeries.Value.GetEnumerator())
    let _IsReadOnly                                = cell (fun () -> _TimeSeries.Value.IsReadOnly)
    let _Keys                                      = cell (fun () -> _TimeSeries.Value.Keys)
    let _Remove                                    (item : ICell<KeyValuePair<Date,'T>>)   
                                                   = cell (fun () -> _TimeSeries.Value.Remove(item.Value))
    let _Remove1                                   (key : ICell<Date>)   
                                                   = cell (fun () -> _TimeSeries.Value.Remove(key.Value))
    let _this                                      (key : ICell<Date>)   
                                                   = cell (fun () -> _TimeSeries.Value.[key.Value])
    let _TryGetValue                               (key : ICell<Date>) (value : ICell<'T ref>)   
                                                   = cell (fun () -> _TimeSeries.Value.TryGetValue(key.Value, value.Value))
    let _Values                                    = cell (fun () -> _TimeSeries.Value.Values)

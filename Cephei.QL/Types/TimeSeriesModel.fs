...
*)
    ...
    let _Add1                                      (key : ICell<Date>) (value : ICell<'T>)   
                                                   = triv (fun () -> _TimeSeries.Value.Add(key.Value, value.Value)
                                                                     _TimeSeries.Value)
    let _Clear                                     = triv (fun () -> _TimeSeries.Value.Clear()
                                                                     _TimeSeries.Value)
    let _Contains                                  (item : ICell<KeyValuePair<Date,'T>>)   
                                                   = triv (fun () -> _TimeSeries.Value.Contains(item.Value))
    let _ContainsKey                               (key : ICell<Date>)   
                                                   = triv (fun () -> _TimeSeries.Value.ContainsKey(key.Value))
    let _CopyTo                                    (array : ICell<KeyValuePair<Date,'T>[]>) (arrayIndex : ICell<int>)   
                                                   = triv (fun () -> _TimeSeries.Value.CopyTo(array.Value, arrayIndex.Value)
                                                                     _TimeSeries.Value)
    let _Count                                     = triv (fun () -> _TimeSeries.Value.Count)
    let _GetEnumerator                             = triv (fun () -> _TimeSeries.Value.GetEnumerator())
    let _IsReadOnly                                = triv (fun () -> _TimeSeries.Value.IsReadOnly)
    let _Keys                                      = triv (fun () -> _TimeSeries.Value.Keys)
    let _Remove                                    (item : ICell<KeyValuePair<Date,'T>>)   
                                                   = triv (fun () -> _TimeSeries.Value.Remove(item.Value))

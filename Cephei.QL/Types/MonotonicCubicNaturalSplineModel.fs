...
*)
    ...
    let _derivative                                (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _MonotonicCubicNaturalSpline.Value.derivative(x.Value, allowExtrapolation.Value))
    let _empty                                     = triv (fun () -> _MonotonicCubicNaturalSpline.Value.empty())
    let _primitive                                 (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _MonotonicCubicNaturalSpline.Value.primitive(x.Value, allowExtrapolation.Value))
    let _secondDerivative                          (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _MonotonicCubicNaturalSpline.Value.secondDerivative(x.Value, allowExtrapolation.Value))
    let _update                                    = triv (fun () -> _MonotonicCubicNaturalSpline.Value.update()
                                                                     _MonotonicCubicNaturalSpline.Value)
    let _value                                     (x : ICell<double>)   
                                                   = triv (fun () -> _MonotonicCubicNaturalSpline.Value.value(x.Value))
    let _value1                                    (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _MonotonicCubicNaturalSpline.Value.value(x.Value, allowExtrapolation.Value))
    let _xMax                                      = triv (fun () -> _MonotonicCubicNaturalSpline.Value.xMax())
    let _xMin                                      = triv (fun () -> _MonotonicCubicNaturalSpline.Value.xMin())
    let _allowsExtrapolation                       = triv (fun () -> _MonotonicCubicNaturalSpline.Value.allowsExtrapolation())
    let _disableExtrapolation                      (b : ICell<bool>)   
                                                   = triv (fun () -> _MonotonicCubicNaturalSpline.Value.disableExtrapolation(b.Value)
                                                                     _MonotonicCubicNaturalSpline.Value)
    let _enableExtrapolation                       (b : ICell<bool>)   
                                                   = triv (fun () -> _MonotonicCubicNaturalSpline.Value.enableExtrapolation(b.Value)
                                                                     _MonotonicCubicNaturalSpline.Value)

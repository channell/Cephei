...
*)
    ...
    let _dayCount                                  (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = cell (fun () -> _ActualActual.Value.dayCount(d1.Value, d2.Value))
    let _dayCounter                                = cell (fun () -> _ActualActual.Value.dayCounter)
    let _empty                                     = cell (fun () -> _ActualActual.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = cell (fun () -> _ActualActual.Value.Equals(o.Value))
    let _name                                      = cell (fun () -> _ActualActual.Value.name())
    let _ToString                                  = cell (fun () -> _ActualActual.Value.ToString())
    let _yearFraction                              (d1 : ICell<Date>) (d2 : ICell<Date>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>)   
                                                   = cell (fun () -> _ActualActual.Value.yearFraction(d1.Value, d2.Value, refPeriodStart.Value, refPeriodEnd.Value))
    let _yearFraction1                             (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = cell (fun () -> _ActualActual.Value.yearFraction(d1.Value, d2.Value))

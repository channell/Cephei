...
*)
    let mutable
        _DefaultDensity                            = make (fun () -> new DefaultDensity ())
    let _discountImpl                              (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.discountImpl(i.Value, t.Value))
    let _forwardImpl                               (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.forwardImpl(i.Value, t.Value))
    let _guess                                     (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.guess(i.Value, c.Value, validData.Value, f.Value))
    let _initialDate                               (c : ICell<DefaultProbabilityTermStructure>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.initialDate(c.Value))
    let _initialValue                              (c : ICell<DefaultProbabilityTermStructure>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.initialValue(c.Value))
    let _maxIterations                             = triv _DefaultDensity (fun () -> _DefaultDensity.Value.maxIterations())
    let _maxValueAfter                             (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.maxValueAfter(i.Value, c.Value, validData.Value, f.Value))
    let _minValueAfter                             (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.minValueAfter(i.Value, c.Value, validData.Value, f.Value))
    let _updateGuess                               (data : ICell<Generic.List<double>>) (density : ICell<double>) (i : ICell<int>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.updateGuess(data.Value, density.Value, i.Value)
                                                                                     _DefaultDensity.Value)
    let _zeroYieldImpl                             (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = triv _DefaultDensity (fun () -> _DefaultDensity.Value.zeroYieldImpl(i.Value, t.Value))

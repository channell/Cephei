...
*)
    ...
    let _calculate                                 (r : ICell<IPricingEngineResults>)   
                                                   = triv (fun () -> _FDShoutCondition.Value.calculate(r.Value)
                                                                     _FDShoutCondition.Value)
    let _setStepCondition                          (impl : ICell<Func<IStepCondition<Vector>>>)   
                                                   = triv (fun () -> _FDShoutCondition.Value.setStepCondition(impl.Value)
                                                                     _FDShoutCondition.Value)
    let _ensureStrikeInGrid                        = triv (fun () -> _FDShoutCondition.Value.ensureStrikeInGrid()
                                                                     _FDShoutCondition.Value)
    let _getResidualTime                           = triv (fun () -> _FDShoutCondition.Value.getResidualTime())
    let _grid                                      = triv (fun () -> _FDShoutCondition.Value.grid())
    let _intrinsicValues_                          = triv (fun () -> _FDShoutCondition.Value.intrinsicValues_)

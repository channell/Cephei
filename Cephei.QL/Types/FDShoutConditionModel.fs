...
*)
    ...
    let _factory                                   (Process : ICell<GeneralizedBlackScholesProcess>) (timeSteps : ICell<int>) (gridPoints : ICell<int>) (timeDependent : ICell<bool>)   
                                                   = cell (fun () -> _FDShoutCondition.Value.factory(Process.Value, timeSteps.Value, gridPoints.Value, timeDependent.Value))
    let _calculate                                 (r : ICell<IPricingEngineResults>)   
                                                   = cell (fun () -> _FDShoutCondition.Value.calculate(r.Value)
                                                                     _FDShoutCondition.Value)
    let _setStepCondition                          (impl : ICell<Func<IStepCondition<Vector>>>)   
                                                   = cell (fun () -> _FDShoutCondition.Value.setStepCondition(impl.Value)
                                                                     _FDShoutCondition.Value)
    let _ensureStrikeInGrid                        = cell (fun () -> _FDShoutCondition.Value.ensureStrikeInGrid()
                                                                     _FDShoutCondition.Value)
    let _getResidualTime                           = cell (fun () -> _FDShoutCondition.Value.getResidualTime())
    let _grid                                      = cell (fun () -> _FDShoutCondition.Value.grid())
    let _intrinsicValues_                          = cell (fun () -> _FDShoutCondition.Value.intrinsicValues_)

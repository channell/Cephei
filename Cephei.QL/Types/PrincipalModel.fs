...
*)
    let mutable
        _Principal                                 = make (fun () -> new Principal ())
    let _accrualEndDate                            = triv _Principal (fun () -> _Principal.Value.accrualEndDate())
    let _accrualStartDate                          = triv _Principal (fun () -> _Principal.Value.accrualStartDate())
    let _amount                                    = triv _Principal (fun () -> _Principal.Value.amount())
    let _date                                      = triv _Principal (fun () -> _Principal.Value.date())
    let _dayCounter                                = triv _Principal (fun () -> _Principal.Value.dayCounter())
    let _nominal                                   = triv _Principal (fun () -> _Principal.Value.nominal())
    let _refPeriodEnd                              = triv _Principal (fun () -> _Principal.Value.refPeriodEnd)
    let _refPeriodStart                            = triv _Principal (fun () -> _Principal.Value.refPeriodStart)
    let _setAmount                                 (amount : ICell<double>)   
                                                   = triv _Principal (fun () -> _Principal.Value.setAmount(amount.Value)
                                                                                _Principal.Value)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = triv _Principal (fun () -> _Principal.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = triv _Principal (fun () -> _Principal.Value.Equals(cf.Value))
    let _exCouponDate                              = triv _Principal (fun () -> _Principal.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = triv _Principal (fun () -> _Principal.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = triv _Principal (fun () -> _Principal.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = triv _Principal (fun () -> _Principal.Value.accept(v.Value)
                                                                                _Principal.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _Principal (fun () -> _Principal.Value.registerWith(handler.Value)
                                                                                _Principal.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _Principal (fun () -> _Principal.Value.unregisterWith(handler.Value)
                                                                                _Principal.Value)
...
*)
    let mutable
        _Principal                                 = make (fun () -> new Principal (amount.Value, nominal.Value, paymentDate.Value, accrualStartDate.Value, accrualEndDate.Value, dayCounter.Value, refPeriodStart.Value, refPeriodEnd.Value))
    let _accrualEndDate                            = triv _Principal (fun () -> _Principal.Value.accrualEndDate())
    let _accrualStartDate                          = triv _Principal (fun () -> _Principal.Value.accrualStartDate())
    let _amount                                    = triv _Principal (fun () -> _Principal.Value.amount())
    let _date                                      = triv _Principal (fun () -> _Principal.Value.date())

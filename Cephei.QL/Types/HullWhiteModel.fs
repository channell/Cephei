...
*)
    ...
    let _termStructure                             = triv (fun () -> _HullWhite.Value.termStructure())
    let _termStructure_                            = triv (fun () -> _HullWhite.Value.termStructure_)
    let _tree                                      (grid : ICell<TimeGrid>)   
                                                   = triv (fun () -> _HullWhite.Value.tree(grid.Value))
    let _a                                         = triv (fun () -> _HullWhite.Value.a())
    let _b                                         = triv (fun () -> _HullWhite.Value.b())
    let _lambda                                    = triv (fun () -> _HullWhite.Value.lambda())
    let _sigma                                     = triv (fun () -> _HullWhite.Value.sigma())
    let _discount                                  (t : ICell<double>)   
                                                   = triv (fun () -> _HullWhite.Value.discount(t.Value))
    let _discountBond                              (now : ICell<double>) (maturity : ICell<double>) (factors : ICell<Vector>)   
                                                   = triv (fun () -> _HullWhite.Value.discountBond(now.Value, maturity.Value, factors.Value))
    let _discountBond1                             (now : ICell<double>) (maturity : ICell<double>) (rate : ICell<double>)   
                                                   = triv (fun () -> _HullWhite.Value.discountBond(now.Value, maturity.Value, rate.Value))
    let _calibrate                                 (instruments : ICell<Generic.List<CalibrationHelper>>) (Method : ICell<OptimizationMethod>) (endCriteria : ICell<EndCriteria>) (additionalConstraint : ICell<Constraint>) (weights : ICell<Generic.List<double>>) (fixParameters : ICell<Generic.List<bool>>)   
                                                   = triv (fun () -> _HullWhite.Value.calibrate(instruments.Value, Method.Value, endCriteria.Value, additionalConstraint.Value, weights.Value, fixParameters.Value)
                                                                     _HullWhite.Value)
    let _constraint                                = triv (fun () -> _HullWhite.Value.CONSTRAINT())
    let _endCriteria                               = triv (fun () -> _HullWhite.Value.endCriteria())
    let _notifyObservers                           = triv (fun () -> _HullWhite.Value.notifyObservers()
                                                                     _HullWhite.Value)
    let _parameters                                = triv (fun () -> _HullWhite.Value.parameters())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _HullWhite.Value.registerWith(handler.Value)
                                                                     _HullWhite.Value)
    let _setParams                                 (parameters : ICell<Vector>)   
                                                   = triv (fun () -> _HullWhite.Value.setParams(parameters.Value)
                                                                     _HullWhite.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv (fun () -> _HullWhite.Value.unregisterWith(handler.Value)
                                                                     _HullWhite.Value)
    let _update                                    = triv (fun () -> _HullWhite.Value.update()
                                                                     _HullWhite.Value)

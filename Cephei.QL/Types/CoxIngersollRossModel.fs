...
*)
    ...
    let _tree                                      (grid : ICell<TimeGrid>)   
                                                   = triv (fun () -> _CoxIngersollRoss.Value.tree(grid.Value))
    let _discount                                  (t : ICell<double>)   
                                                   = triv (fun () -> _CoxIngersollRoss.Value.discount(t.Value))
    let _discountBond                              (now : ICell<double>) (maturity : ICell<double>) (factors : ICell<Vector>)   
                                                   = triv (fun () -> _CoxIngersollRoss.Value.discountBond(now.Value, maturity.Value, factors.Value))
    let _discountBond1                             (now : ICell<double>) (maturity : ICell<double>) (rate : ICell<double>)   
                                                   = triv (fun () -> _CoxIngersollRoss.Value.discountBond(now.Value, maturity.Value, rate.Value))
    let _calibrate                                 (instruments : ICell<Generic.List<CalibrationHelper>>) (Method : ICell<OptimizationMethod>) (endCriteria : ICell<EndCriteria>) (additionalConstraint : ICell<Constraint>) (weights : ICell<Generic.List<double>>) (fixParameters : ICell<Generic.List<bool>>)   
                                                   = triv (fun () -> _CoxIngersollRoss.Value.calibrate(instruments.Value, Method.Value, endCriteria.Value, additionalConstraint.Value, weights.Value, fixParameters.Value)
                                                                     _CoxIngersollRoss.Value)
    let _constraint                                = triv (fun () -> _CoxIngersollRoss.Value.CONSTRAINT())
    let _endCriteria                               = triv (fun () -> _CoxIngersollRoss.Value.endCriteria())
    let _notifyObservers                           = triv (fun () -> _CoxIngersollRoss.Value.notifyObservers()
                                                                     _CoxIngersollRoss.Value)
    let _parameters                                = triv (fun () -> _CoxIngersollRoss.Value.parameters())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _CoxIngersollRoss.Value.registerWith(handler.Value)
                                                                     _CoxIngersollRoss.Value)
    let _setParams                                 (parameters : ICell<Vector>)   
                                                   = triv (fun () -> _CoxIngersollRoss.Value.setParams(parameters.Value)
                                                                     _CoxIngersollRoss.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv (fun () -> _CoxIngersollRoss.Value.unregisterWith(handler.Value)
                                                                     _CoxIngersollRoss.Value)
    let _update                                    = triv (fun () -> _CoxIngersollRoss.Value.update()
                                                                     _CoxIngersollRoss.Value)

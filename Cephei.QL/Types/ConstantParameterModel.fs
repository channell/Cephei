...
*)
    let mutable
        _ConstantParameter                         = make (fun () -> new ConstantParameter (Constraint.Value))
    let _constraint                                = triv _ConstantParameter (fun () -> _ConstantParameter.Value.CONSTRAINT())
    let _implementation                            = triv _ConstantParameter (fun () -> _ConstantParameter.Value.implementation())
    let _parameters                                = triv _ConstantParameter (fun () -> _ConstantParameter.Value.parameters())
    let _setParam                                  (i : ICell<int>) (x : ICell<double>)   
                                                   = triv _ConstantParameter (fun () -> _ConstantParameter.Value.setParam(i.Value, x.Value)
                                                                                        _ConstantParameter.Value)
    let _size                                      = triv _ConstantParameter (fun () -> _ConstantParameter.Value.size())
    let _testParams                                (p : ICell<Vector>)   
                                                   = triv _ConstantParameter (fun () -> _ConstantParameter.Value.testParams(p.Value))
    let _value                                     (t : ICell<double>)   
                                                   = triv _ConstantParameter (fun () -> _ConstantParameter.Value.value(t.Value))
...
*)
    let mutable
        _ConstantParameter                         = make (fun () -> new ConstantParameter (value.Value, Constraint.Value))
    let _constraint                                = triv _ConstantParameter (fun () -> _ConstantParameter.Value.CONSTRAINT())
    let _implementation                            = triv _ConstantParameter (fun () -> _ConstantParameter.Value.implementation())
    let _parameters                                = triv _ConstantParameter (fun () -> _ConstantParameter.Value.parameters())
    let _setParam                                  (i : ICell<int>) (x : ICell<double>)   

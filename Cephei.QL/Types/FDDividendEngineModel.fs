...
*)
    ...
    let _factory2                                  (Process : ICell<GeneralizedBlackScholesProcess>) (timeSteps : ICell<int>) (gridPoints : ICell<int>) (timeDependent : ICell<bool>)   
                                                   = cell (fun () -> _FDDividendEngine.Value.factory2(Process.Value, timeSteps.Value, gridPoints.Value, timeDependent.Value))
    let _factory                                   (Process : ICell<GeneralizedBlackScholesProcess>) (timeSteps : ICell<int>) (gridPoints : ICell<int>) (timeDependent : ICell<bool>)   
                                                   = cell (fun () -> _FDDividendEngine.Value.factory(Process.Value, timeSteps.Value, gridPoints.Value, timeDependent.Value))
    let _calculate                                 (r : ICell<IPricingEngineResults>)   
                                                   = cell (fun () -> _FDDividendEngine.Value.calculate(r.Value)
                                                                     _FDDividendEngine.Value)
    let _setStepCondition                          (impl : ICell<Func<IStepCondition<Vector>>>)   
                                                   = cell (fun () -> _FDDividendEngine.Value.setStepCondition(impl.Value)
                                                                     _FDDividendEngine.Value)
    let _ensureStrikeInGrid                        = cell (fun () -> _FDDividendEngine.Value.ensureStrikeInGrid()
                                                                     _FDDividendEngine.Value)
    let _getResidualTime                           = cell (fun () -> _FDDividendEngine.Value.getResidualTime())
    let _grid                                      = cell (fun () -> _FDDividendEngine.Value.grid())
    let _intrinsicValues_                          = cell (fun () -> _FDDividendEngine.Value.intrinsicValues_)

...
*)
    ...
    let _discount                                  (t : ICell<double>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.discount(t.Value))
    let _discountBond                              (t : ICell<double>) (maturity : ICell<double>) (v : ICell<Vector>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.discountBond(t.Value, maturity.Value, v.Value))
    let _discountBondOption                        (Type : ICell<Option.Type>) (strike : ICell<double>) (maturity : ICell<double>) (bondMaturity : ICell<double>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.discountBondOption(Type.Value, strike.Value, maturity.Value, bondMaturity.Value))
    let _getSwaptionVolatilityMatrix               = cell (fun () -> _LiborForwardModel.Value.getSwaptionVolatilityMatrix())
    let _S_0                                       (alpha : ICell<int>) (beta : ICell<int>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.S_0(alpha.Value, beta.Value))
    let _setParams                                 (parameters : ICell<Vector>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.setParams(parameters.Value)
                                                                     _LiborForwardModel.Value)
    let _w_0                                       (alpha : ICell<int>) (beta : ICell<int>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.w_0(alpha.Value, beta.Value))
    let _calibrate                                 (instruments : ICell<Generic.List<CalibrationHelper>>) (Method : ICell<OptimizationMethod>) (endCriteria : ICell<EndCriteria>) (additionalConstraint : ICell<Constraint>) (weights : ICell<Generic.List<double>>) (fixParameters : ICell<Generic.List<bool>>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.calibrate(instruments.Value, Method.Value, endCriteria.Value, additionalConstraint.Value, weights.Value, fixParameters.Value)
                                                                     _LiborForwardModel.Value)
    let _constraint                                = cell (fun () -> _LiborForwardModel.Value.CONSTRAINT())
    let _endCriteria                               = cell (fun () -> _LiborForwardModel.Value.endCriteria())
    let _notifyObservers                           = cell (fun () -> _LiborForwardModel.Value.notifyObservers()
                                                                     _LiborForwardModel.Value)
    let _parameters                                = cell (fun () -> _LiborForwardModel.Value.parameters())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.registerWith(handler.Value)
                                                                     _LiborForwardModel.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.unregisterWith(handler.Value)
                                                                     _LiborForwardModel.Value)
    let _update                                    = cell (fun () -> _LiborForwardModel.Value.update()
                                                                     _LiborForwardModel.Value)
    let _value                                     (parameters : ICell<Vector>) (instruments : ICell<Generic.List<CalibrationHelper>>)   
                                                   = cell (fun () -> _LiborForwardModel.Value.value(parameters.Value, instruments.Value))

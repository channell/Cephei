...
*)
    let mutable
        _CPILeg                                    = make (fun () -> new CPILeg (schedule.Value, index.Value, baseCPI.Value, observationLag.Value))
    let _value                                     = triv _CPILeg (fun () -> _CPILeg.Value.value())
    let _withCaps                                  (cap : ICell<double>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withCaps(cap.Value))
    ...
    let _withExCouponPeriod                        (period : ICell<Period>) (cal : ICell<Calendar>) (convention : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withExCouponPeriod(period.Value, cal.Value, convention.Value, endOfMonth.Value))
    let _withFixedRates                            (fixedRate : ICell<double>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withFixedRates(fixedRate.Value))
    let _withFixedRates1                           (fixedRates : ICell<Generic.List<double>>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withFixedRates(fixedRates.Value))
    let _withFixingDays                            (fixingDays : ICell<Generic.List<int>>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withFixingDays(fixingDays.Value))
    let _withFixingDays1                           (fixingDays : ICell<int>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withFixingDays(fixingDays.Value))
    let _withFloors                                (floors : ICell<Generic.List<Nullable<double>>>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withFloors(floors.Value))
    let _withFloors1                               (floors : ICell<double>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withFloors(floors.Value))
    let _withObservationInterpolation              (interp : ICell<InterpolationType>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withObservationInterpolation(interp.Value))
    let _withPaymentCalendar                       (cal : ICell<Calendar>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withPaymentCalendar(cal.Value))
    let _withPaymentDayCounter                     (dayCounter : ICell<DayCounter>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withPaymentDayCounter(dayCounter.Value))
    let _withSpreads                               (spreads : ICell<Generic.List<double>>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withSpreads(spreads.Value))
    let _withSpreads1                              (spread : ICell<double>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withSpreads(spread.Value))
    let _withSubtractInflationNominal              (growthOnly : ICell<bool>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withSubtractInflationNominal(growthOnly.Value))
    let _withNotionals                             (notional : ICell<double>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withNotionals(notional.Value))
    let _withNotionals1                            (notionals : ICell<Generic.List<double>>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withNotionals(notionals.Value))
    let _withPaymentAdjustment                     (convention : ICell<BusinessDayConvention>)   
                                                   = triv _CPILeg (fun () -> _CPILeg.Value.withPaymentAdjustment(convention.Value))

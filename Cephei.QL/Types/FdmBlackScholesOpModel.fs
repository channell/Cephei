...
*)
    let mutable
        _FdmBlackScholesOp                         = make (fun () -> new FdmBlackScholesOp (mesher.Value, bsProcess.Value, strike.Value, localVol.Value, illegalLocalVolOverwrite.Value, direction.Value, quantoHelper.Value))
    let _add                                       (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.apply(r.Value))
    let _apply_direction                           (direction : ICell<int>) (r : ICell<Vector>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.apply_direction(direction.Value, r.Value))
    let _apply_mixed                               (r : ICell<Vector>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.apply_mixed(r.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.applyTo(v.Value))
    let _Clone                                     = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.Clone())
    let _identity                                  (size : ICell<int>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.identity(size.Value))
    let _isTimeDependent                           = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.isTimeDependent())
    let _multiply                                  (a : ICell<double>) (D : ICell<IOperator>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.multiply(a.Value, D.Value))
    let _preconditioner                            (r : ICell<Vector>) (dt : ICell<double>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.preconditioner(r.Value, dt.Value))
    let _setTime                                   (t : ICell<double>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.setTime(t.Value)
                                                                                        _FdmBlackScholesOp.Value)
    let _setTime1                                  (t1 : ICell<double>) (t2 : ICell<double>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.setTime(t1.Value, t2.Value)
                                                                                        _FdmBlackScholesOp.Value)
    let _size                                      = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.size())
    let _solve_splitting                           (direction : ICell<int>) (r : ICell<Vector>) (dt : ICell<double>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.solve_splitting(direction.Value, r.Value, dt.Value))
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.solveFor(rhs.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.subtract(A.Value, B.Value))
    let _toMatrixDecomp                            = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.toMatrixDecomp())
    let _toMatrix                                  = triv _FdmBlackScholesOp (fun () -> _FdmBlackScholesOp.Value.toMatrix())

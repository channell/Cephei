...
*)
    ...
    let _fixedRate                                 = triv (fun () -> _CPICoupon.Value.fixedRate())
    let _indexFixing                               = triv (fun () -> _CPICoupon.Value.indexFixing())
    let _indexObservation                          (onDate : ICell<Date>)   
                                                   = triv (fun () -> _CPICoupon.Value.indexObservation(onDate.Value))
    let _observationInterpolation                  = triv (fun () -> _CPICoupon.Value.observationInterpolation())
    let _spread                                    = triv (fun () -> _CPICoupon.Value.spread())
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = triv (fun () -> _CPICoupon.Value.accruedAmount(d.Value))
    let _amount                                    = triv (fun () -> _CPICoupon.Value.amount())
    let _dayCounter                                = triv (fun () -> _CPICoupon.Value.dayCounter())
    let _fixingDate                                = triv (fun () -> _CPICoupon.Value.fixingDate())
    let _fixingDays                                = triv (fun () -> _CPICoupon.Value.fixingDays())
    let _index                                     = triv (fun () -> _CPICoupon.Value.index())
    let _observationLag                            = triv (fun () -> _CPICoupon.Value.observationLag())
    let _price                                     (discountingCurve : ICell<Handle<YieldTermStructure>>)   
                                                   = triv (fun () -> _CPICoupon.Value.price(discountingCurve.Value))
    let _pricer                                    = triv (fun () -> _CPICoupon.Value.pricer())
    let _rate                                      = triv (fun () -> _CPICoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<InflationCouponPricer>)   
                                                   = triv (fun () -> _CPICoupon.Value.setPricer(pricer.Value)
                                                                     _CPICoupon.Value)
    let _update                                    = triv (fun () -> _CPICoupon.Value.update()
                                                                     _CPICoupon.Value)
    let _accrualDays                               = triv (fun () -> _CPICoupon.Value.accrualDays())
    let _accrualEndDate                            = triv (fun () -> _CPICoupon.Value.accrualEndDate())
    let _accrualPeriod                             = triv (fun () -> _CPICoupon.Value.accrualPeriod())
    let _accrualStartDate                          = triv (fun () -> _CPICoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = triv (fun () -> _CPICoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = triv (fun () -> _CPICoupon.Value.accruedPeriod(d.Value))
    let _date                                      = triv (fun () -> _CPICoupon.Value.date())
    let _exCouponDate                              = triv (fun () -> _CPICoupon.Value.exCouponDate())
    let _nominal                                   = triv (fun () -> _CPICoupon.Value.nominal())
    let _referencePeriodEnd                        = triv (fun () -> _CPICoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = triv (fun () -> _CPICoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = triv (fun () -> _CPICoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = triv (fun () -> _CPICoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = triv (fun () -> _CPICoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = triv (fun () -> _CPICoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = triv (fun () -> _CPICoupon.Value.accept(v.Value)
                                                                     _CPICoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _CPICoupon.Value.registerWith(handler.Value)
                                                                     _CPICoupon.Value)

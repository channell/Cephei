...
*)
    ...
    let _adjustedFixing                            = cell (fun () -> _CPICoupon.Value.adjustedFixing())
    let _baseCPI                                   = cell (fun () -> _CPICoupon.Value.baseCPI())
    let _cpiIndex                                  = cell (fun () -> _CPICoupon.Value.cpiIndex())
    let _fixedRate                                 = cell (fun () -> _CPICoupon.Value.fixedRate())
    let _indexFixing                               = cell (fun () -> _CPICoupon.Value.indexFixing())
    let _indexObservation                          (onDate : ICell<Date>)   
                                                   = cell (fun () -> _CPICoupon.Value.indexObservation(onDate.Value))
    let _observationInterpolation                  = cell (fun () -> _CPICoupon.Value.observationInterpolation())
    let _spread                                    = cell (fun () -> _CPICoupon.Value.spread())
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CPICoupon.Value.accruedAmount(d.Value))
    let _amount                                    = cell (fun () -> _CPICoupon.Value.amount())
    let _dayCounter                                = cell (fun () -> _CPICoupon.Value.dayCounter())
    let _fixingDate                                = cell (fun () -> _CPICoupon.Value.fixingDate())
    let _fixingDays                                = cell (fun () -> _CPICoupon.Value.fixingDays())
    let _index                                     = cell (fun () -> _CPICoupon.Value.index())
    let _observationLag                            = cell (fun () -> _CPICoupon.Value.observationLag())
    let _price                                     (discountingCurve : ICell<Handle<YieldTermStructure>>)   
                                                   = cell (fun () -> _CPICoupon.Value.price(discountingCurve.Value))
    let _pricer                                    = cell (fun () -> _CPICoupon.Value.pricer())
    let _rate                                      = cell (fun () -> _CPICoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<InflationCouponPricer>)   
                                                   = cell (fun () -> _CPICoupon.Value.setPricer(pricer.Value)
                                                                     _CPICoupon.Value)
    let _update                                    = cell (fun () -> _CPICoupon.Value.update()
                                                                     _CPICoupon.Value)
    let _accrualDays                               = cell (fun () -> _CPICoupon.Value.accrualDays())
    let _accrualEndDate                            = cell (fun () -> _CPICoupon.Value.accrualEndDate())
    let _accrualPeriod                             = cell (fun () -> _CPICoupon.Value.accrualPeriod())
    let _accrualStartDate                          = cell (fun () -> _CPICoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = cell (fun () -> _CPICoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CPICoupon.Value.accruedPeriod(d.Value))
    let _date                                      = cell (fun () -> _CPICoupon.Value.date())
    let _exCouponDate                              = cell (fun () -> _CPICoupon.Value.exCouponDate())
    let _nominal                                   = cell (fun () -> _CPICoupon.Value.nominal())
    let _referencePeriodEnd                        = cell (fun () -> _CPICoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = cell (fun () -> _CPICoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _CPICoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _CPICoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _CPICoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _CPICoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _CPICoupon.Value.accept(v.Value)
                                                                     _CPICoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CPICoupon.Value.registerWith(handler.Value)
                                                                     _CPICoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CPICoupon.Value.unregisterWith(handler.Value)
                                                                     _CPICoupon.Value)

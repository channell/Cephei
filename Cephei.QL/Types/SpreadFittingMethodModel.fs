...
*)
    ...
    let _clone                                     = cell (fun () -> _SpreadFittingMethod.Value.clone())
    let _size                                      = cell (fun () -> _SpreadFittingMethod.Value.size())
    let _constrainAtZero                           = cell (fun () -> _SpreadFittingMethod.Value.constrainAtZero())
    let _discount                                  (x : ICell<Vector>) (t : ICell<double>)   
                                                   = cell (fun () -> _SpreadFittingMethod.Value.discount(x.Value, t.Value))
    let _minimumCostValue                          = cell (fun () -> _SpreadFittingMethod.Value.minimumCostValue())
    let _numberOfIterations                        = cell (fun () -> _SpreadFittingMethod.Value.numberOfIterations())
    let _optimizationMethod                        = cell (fun () -> _SpreadFittingMethod.Value.optimizationMethod())
    let _solution                                  = cell (fun () -> _SpreadFittingMethod.Value.solution())
    let _weights                                   = cell (fun () -> _SpreadFittingMethod.Value.weights())

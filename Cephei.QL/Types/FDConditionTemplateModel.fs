...
*)
    ...
    let _setStepCondition                          (impl : ICell<Func<IStepCondition<Vector>>>)   
                                                   = triv (fun () -> _FDConditionTemplate.Value.setStepCondition(impl.Value)
                                                                     _FDConditionTemplate.Value)
    let _ensureStrikeInGrid                        = triv (fun () -> _FDConditionTemplate.Value.ensureStrikeInGrid()
                                                                     _FDConditionTemplate.Value)
    let _factory                                   (Process : ICell<GeneralizedBlackScholesProcess>) (timeSteps : ICell<int>) (gridPoints : ICell<int>) (timeDependent : ICell<bool>)   
                                                   = triv (fun () -> _FDConditionTemplate.Value.factory(Process.Value, timeSteps.Value, gridPoints.Value, timeDependent.Value))
    let _getResidualTime                           = triv (fun () -> _FDConditionTemplate.Value.getResidualTime())
    let _grid                                      = triv (fun () -> _FDConditionTemplate.Value.grid())
    let _intrinsicValues_                          = triv (fun () -> _FDConditionTemplate.Value.intrinsicValues_)

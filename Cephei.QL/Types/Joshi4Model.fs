...
*)
    ...
    let _factory                                   (Process : ICell<StochasticProcess1D>) (End : ICell<double>) (steps : ICell<int>) (strike : ICell<double>)   
                                                   = cell (fun () -> _Joshi4.Value.factory(Process.Value, End.Value, steps.Value, strike.Value))
    let _probability                               (x : ICell<int>) (y : ICell<int>) (branch : ICell<int>)   
                                                   = cell (fun () -> _Joshi4.Value.probability(x.Value, y.Value, branch.Value))
    let _underlying                                (i : ICell<int>) (index : ICell<int>)   
                                                   = cell (fun () -> _Joshi4.Value.underlying(i.Value, index.Value))
    let _descendant                                (x : ICell<int>) (index : ICell<int>) (branch : ICell<int>)   
                                                   = cell (fun () -> _Joshi4.Value.descendant(x.Value, index.Value, branch.Value))
    let _size                                      (i : ICell<int>)   
                                                   = cell (fun () -> _Joshi4.Value.size(i.Value))
    let _columns                                   = cell (fun () -> _Joshi4.Value.columns())

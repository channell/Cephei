...
*)
    let mutable
        _InterestRate                              = make (fun () -> new InterestRate (r.Value, dc.Value, comp.Value, freq.Value))
    let _compoundFactor                            (t : ICell<double>)   
                                                   = triv _InterestRate (fun () -> _InterestRate.Value.compoundFactor(t.Value))
    let _compoundFactor1                           (d1 : ICell<Date>) (d2 : ICell<Date>) (refStart : ICell<Date>) (refEnd : ICell<Date>)   
                                                   = triv _InterestRate (fun () -> _InterestRate.Value.compoundFactor(d1.Value, d2.Value, refStart.Value, refEnd.Value))
    let _compounding                               = triv _InterestRate (fun () -> _InterestRate.Value.compounding())
    let _dayCounter                                = triv _InterestRate (fun () -> _InterestRate.Value.dayCounter())
    let _discountFactor                            (d1 : ICell<Date>) (d2 : ICell<Date>) (refStart : ICell<Date>) (refEnd : ICell<Date>)   
                                                   = triv _InterestRate (fun () -> _InterestRate.Value.discountFactor(d1.Value, d2.Value, refStart.Value, refEnd.Value))
    let _discountFactor1                           (t : ICell<double>)   
                                                   = triv _InterestRate (fun () -> _InterestRate.Value.discountFactor(t.Value))
    let _equivalentRate                            (resultDC : ICell<DayCounter>) (comp : ICell<Compounding>) (freq : ICell<Frequency>) (d1 : ICell<Date>) (d2 : ICell<Date>) (refStart : ICell<Date>) (refEnd : ICell<Date>)   
                                                   = triv _InterestRate (fun () -> _InterestRate.Value.equivalentRate(resultDC.Value, comp.Value, freq.Value, d1.Value, d2.Value, refStart.Value, refEnd.Value))
    let _equivalentRate1                           (comp : ICell<Compounding>) (freq : ICell<Frequency>) (t : ICell<double>)   
                                                   = triv _InterestRate (fun () -> _InterestRate.Value.equivalentRate(comp.Value, freq.Value, t.Value))
    let _frequency                                 = triv _InterestRate (fun () -> _InterestRate.Value.frequency())
    let _rate                                      = triv _InterestRate (fun () -> _InterestRate.Value.rate())
    let _ToString                                  = triv _InterestRate (fun () -> _InterestRate.Value.ToString())
    let _value                                     = triv _InterestRate (fun () -> _InterestRate.Value.value())
...
*)
    let mutable
        _InterestRate                              = make (fun () -> new InterestRate ())
    let _compoundFactor                            (t : ICell<double>)   
                                                   = triv _InterestRate (fun () -> _InterestRate.Value.compoundFactor(t.Value))
    let _compoundFactor1                           (d1 : ICell<Date>) (d2 : ICell<Date>) (refStart : ICell<Date>) (refEnd : ICell<Date>)   
                                                   = triv _InterestRate (fun () -> _InterestRate.Value.compoundFactor(d1.Value, d2.Value, refStart.Value, refEnd.Value))

...
*)
    ...
    let _apply                                     (x0 : ICell<Vector>) (dx : ICell<Vector>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.apply(x0.Value, dx.Value))
    let _correlation                               = cell (fun () -> _StochasticProcessArray.Value.correlation())
    let _covariance                                (t0 : ICell<double>) (x0 : ICell<Vector>) (dt : ICell<double>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.covariance(t0.Value, x0.Value, dt.Value))
    let _diffusion                                 (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.diffusion(t.Value, x.Value))
    let _drift                                     (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.drift(t.Value, x.Value))
    let _evolve                                    (t0 : ICell<double>) (x0 : ICell<Vector>) (dt : ICell<double>) (dw : ICell<Vector>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.evolve(t0.Value, x0.Value, dt.Value, dw.Value))
    let _expectation                               (t0 : ICell<double>) (x0 : ICell<Vector>) (dt : ICell<double>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.expectation(t0.Value, x0.Value, dt.Value))
    let _initialValues                             = cell (fun () -> _StochasticProcessArray.Value.initialValues())
    let _process                                   (i : ICell<int>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.PROCESS(i.Value))
    let _size                                      = cell (fun () -> _StochasticProcessArray.Value.size())
    let _stdDeviation                              (t0 : ICell<double>) (x0 : ICell<Vector>) (dt : ICell<double>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.stdDeviation(t0.Value, x0.Value, dt.Value))
    let _time                                      (d : ICell<Date>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.time(d.Value))
    let _factors                                   = cell (fun () -> _StochasticProcessArray.Value.factors())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.registerWith(handler.Value)
                                                                     _StochasticProcessArray.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _StochasticProcessArray.Value.unregisterWith(handler.Value)
                                                                     _StochasticProcessArray.Value)
    let _update                                    = cell (fun () -> _StochasticProcessArray.Value.update()
                                                                     _StochasticProcessArray.Value)

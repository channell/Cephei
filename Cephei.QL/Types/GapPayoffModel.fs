...
*)
    ...
    let _description                               = cell (fun () -> _GapPayoff.Value.description())
    let _name                                      = cell (fun () -> _GapPayoff.Value.name())
    let _secondStrike                              = cell (fun () -> _GapPayoff.Value.secondStrike())
    let _value                                     (price : ICell<double>)   
                                                   = cell (fun () -> _GapPayoff.Value.value(price.Value))
    let _strike                                    = cell (fun () -> _GapPayoff.Value.strike())
    let _optionType                                = cell (fun () -> _GapPayoff.Value.optionType())
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _GapPayoff.Value.accept(v.Value)
                                                                     _GapPayoff.Value)

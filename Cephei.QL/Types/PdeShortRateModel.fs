...
*)
    ...
    let _diffusion                                 (t : ICell<double>) (x : ICell<double>)   
                                                   = cell (fun () -> _PdeShortRate.Value.diffusion(t.Value, x.Value))
    let _discount                                  (t : ICell<double>) (x : ICell<double>)   
                                                   = cell (fun () -> _PdeShortRate.Value.discount(t.Value, x.Value))
    let _drift                                     (t : ICell<double>) (x : ICell<double>)   
                                                   = cell (fun () -> _PdeShortRate.Value.drift(t.Value, x.Value))
    let _factory                                   (Process : ICell<GeneralizedBlackScholesProcess>)   
                                                   = cell (fun () -> _PdeShortRate.Value.factory(Process.Value))
    let _generateOperator                          (t : ICell<double>) (tg : ICell<TransformedGrid>) (L : ICell<TridiagonalOperator>)   
                                                   = cell (fun () -> _PdeShortRate.Value.generateOperator(t.Value, tg.Value, L.Value)
                                                                     _PdeShortRate.Value)

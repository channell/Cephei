...
*)
    ...
    let _amount                                    (d : ICell<Date>) (notional : ICell<double>) (recoveryRate : ICell<double>)   
                                                   = cell (fun () -> _FaceValueClaim.Value.amount(d.Value, notional.Value, recoveryRate.Value))
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _FaceValueClaim.Value.registerWith(handler.Value)
                                                                     _FaceValueClaim.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _FaceValueClaim.Value.unregisterWith(handler.Value)
                                                                     _FaceValueClaim.Value)
    let _update                                    = cell (fun () -> _FaceValueClaim.Value.update()
                                                                     _FaceValueClaim.Value)

...
*)
    ...
    let _discountImpl                              (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _HazardRate.Value.discountImpl(i.Value, t.Value))
    let _forwardImpl                               (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _HazardRate.Value.forwardImpl(i.Value, t.Value))
    let _guess                                     (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _HazardRate.Value.guess(i.Value, c.Value, validData.Value, f.Value))
    let _initialDate                               (c : ICell<DefaultProbabilityTermStructure>)   
                                                   = cell (fun () -> _HazardRate.Value.initialDate(c.Value))
    let _initialValue                              (c : ICell<DefaultProbabilityTermStructure>)   
                                                   = cell (fun () -> _HazardRate.Value.initialValue(c.Value))
    let _maxIterations                             = cell (fun () -> _HazardRate.Value.maxIterations())
    let _maxValueAfter                             (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _HazardRate.Value.maxValueAfter(i.Value, c.Value, validData.Value, f.Value))
    let _minValueAfter                             (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _HazardRate.Value.minValueAfter(i.Value, c.Value, validData.Value, f.Value))
    let _updateGuess                               (data : ICell<Generic.List<double>>) (rate : ICell<double>) (i : ICell<int>)   
                                                   = cell (fun () -> _HazardRate.Value.updateGuess(data.Value, rate.Value, i.Value)
                                                                     _HazardRate.Value)
    let _zeroYieldImpl                             (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _HazardRate.Value.zeroYieldImpl(i.Value, t.Value))

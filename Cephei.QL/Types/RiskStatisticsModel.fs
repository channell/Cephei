...
*)
    ...
    let _gaussianExpectedShortfall                 (value : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.gaussianExpectedShortfall(value.Value))
    let _gaussianPercentile                        (value : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.gaussianPercentile(value.Value))
    let _gaussianPotentialUpside                   (value : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.gaussianPotentialUpside(value.Value))
    let _gaussianRegret                            (value : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.gaussianRegret(value.Value))
    let _gaussianShortfall                         (value : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.gaussianShortfall(value.Value))
    let _gaussianValueAtRisk                       (value : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.gaussianValueAtRisk(value.Value))
    let _add                                       (value : ICell<double>) (weight : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.add(value.Value, weight.Value)
                                                                     _RiskStatistics.Value)
    let _addSequence                               (data : ICell<Generic.List<double>>) (weight : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _RiskStatistics.Value.addSequence(data.Value, weight.Value)
                                                                     _RiskStatistics.Value)
    let _averageShortfall                          (target : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.averageShortfall(target.Value))
    let _downsideDeviation                         = triv (fun () -> _RiskStatistics.Value.downsideDeviation())
    let _downsideVariance                          = triv (fun () -> _RiskStatistics.Value.downsideVariance())
    let _errorEstimate                             = triv (fun () -> _RiskStatistics.Value.errorEstimate())
    let _expectationValue                          (f : ICell<Func<KeyValuePair<double,double>,double>>) (inRange : ICell<Func<KeyValuePair<double,double>,bool>>)   
                                                   = triv (fun () -> _RiskStatistics.Value.expectationValue(f.Value, inRange.Value))
    let _expectedShortfall                         (centile : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.expectedShortfall(centile.Value))
    let _kurtosis                                  = triv (fun () -> _RiskStatistics.Value.kurtosis())
    let _max                                       = triv (fun () -> _RiskStatistics.Value.max())
    let _mean                                      = triv (fun () -> _RiskStatistics.Value.mean())
    let _min                                       = triv (fun () -> _RiskStatistics.Value.min())
    let _percentile                                (percent : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.percentile(percent.Value))
    let _potentialUpside                           (centile : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.potentialUpside(centile.Value))
    let _regret                                    (target : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.regret(target.Value))
    let _reset                                     = triv (fun () -> _RiskStatistics.Value.reset()
                                                                     _RiskStatistics.Value)
    let _samples                                   = triv (fun () -> _RiskStatistics.Value.samples())
    let _semiDeviation                             = triv (fun () -> _RiskStatistics.Value.semiDeviation())
    let _semiVariance                              = triv (fun () -> _RiskStatistics.Value.semiVariance())
    let _shortfall                                 (target : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.shortfall(target.Value))
    let _skewness                                  = triv (fun () -> _RiskStatistics.Value.skewness())
    let _standardDeviation                         = triv (fun () -> _RiskStatistics.Value.standardDeviation())
    let _valueAtRisk                               (centile : ICell<double>)   
                                                   = triv (fun () -> _RiskStatistics.Value.valueAtRisk(centile.Value))

...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _FIMCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _FIMCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _FIMCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _FIMCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _FIMCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _FIMCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _FIMCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _FIMCurrency.Value.symbol)

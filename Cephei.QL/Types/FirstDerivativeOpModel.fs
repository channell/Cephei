...
*)
    ...
    let _add                                       (m : ICell<TripleBandLinearOp>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.add(m.Value))
    let _add1                                      (u : ICell<Vector>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.add(u.Value))
    let _add2                                      (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.apply(r.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.applyTo(v.Value))
    let _axpyb                                     (a : ICell<Vector>) (x : ICell<TripleBandLinearOp>) (y : ICell<TripleBandLinearOp>) (b : ICell<Vector>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.axpyb(a.Value, x.Value, y.Value, b.Value)
                                                                     _FirstDerivativeOp.Value)
    let _Clone                                     = cell (fun () -> _FirstDerivativeOp.Value.Clone())
    let _identity                                  (size : ICell<int>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.identity(size.Value))
    let _isTimeDependent                           = cell (fun () -> _FirstDerivativeOp.Value.isTimeDependent())
    let _mult                                      (u : ICell<Vector>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.mult(u.Value))
    let _multiply                                  (a : ICell<double>) (D : ICell<IOperator>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.multiply(a.Value, D.Value))
    let _multR                                     (u : ICell<Vector>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.multR(u.Value))
    let _setTime                                   (t : ICell<double>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.setTime(t.Value)
                                                                     _FirstDerivativeOp.Value)
    let _size                                      = cell (fun () -> _FirstDerivativeOp.Value.size())
    let _solve_splitting                           (r : ICell<Vector>) (a : ICell<double>) (b : ICell<double>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.solve_splitting(r.Value, a.Value, b.Value))
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.solveFor(rhs.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.subtract(A.Value, B.Value))
    let _swap                                      (m : ICell<TripleBandLinearOp>)   
                                                   = cell (fun () -> _FirstDerivativeOp.Value.swap(m.Value)
                                                                     _FirstDerivativeOp.Value)
    let _toMatrix                                  = cell (fun () -> _FirstDerivativeOp.Value.toMatrix())

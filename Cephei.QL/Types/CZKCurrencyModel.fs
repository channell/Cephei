...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _CZKCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _CZKCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _CZKCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _CZKCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _CZKCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _CZKCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _CZKCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _CZKCurrency.Value.symbol)

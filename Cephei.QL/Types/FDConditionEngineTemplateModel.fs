...
*)
    ...
    let _setStepCondition                          (impl : ICell<Func<IStepCondition<Vector>>>)   
                                                   = cell (fun () -> _FDConditionEngineTemplate.Value.setStepCondition(impl.Value)
                                                                     _FDConditionEngineTemplate.Value)
    let _calculate                                 (r : ICell<IPricingEngineResults>)   
                                                   = cell (fun () -> _FDConditionEngineTemplate.Value.calculate(r.Value)
                                                                     _FDConditionEngineTemplate.Value)
    let _ensureStrikeInGrid                        = cell (fun () -> _FDConditionEngineTemplate.Value.ensureStrikeInGrid()
                                                                     _FDConditionEngineTemplate.Value)
    let _factory                                   (Process : ICell<GeneralizedBlackScholesProcess>) (timeSteps : ICell<int>) (gridPoints : ICell<int>) (timeDependent : ICell<bool>)   
                                                   = cell (fun () -> _FDConditionEngineTemplate.Value.factory(Process.Value, timeSteps.Value, gridPoints.Value, timeDependent.Value))
    let _getResidualTime                           = cell (fun () -> _FDConditionEngineTemplate.Value.getResidualTime())
    let _grid                                      = cell (fun () -> _FDConditionEngineTemplate.Value.grid())
    let _intrinsicValues_                          = cell (fun () -> _FDConditionEngineTemplate.Value.intrinsicValues_)

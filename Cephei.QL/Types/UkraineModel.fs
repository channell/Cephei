...
*)
    let mutable
        _Ukraine                                   = make (fun () -> new Ukraine (m.Value))
    let _addedHolidays                             = triv _Ukraine (fun () -> _Ukraine.Value.addedHolidays)
    let _addHoliday                                (d : ICell<Date>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.addHoliday(d.Value)
                                                                              _Ukraine.Value)
    let _adjust                                    (d : ICell<Date>) (c : ICell<BusinessDayConvention>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.adjust(d.Value, c.Value))
    let _advance                                   (d : ICell<Date>) (p : ICell<Period>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.advance(d.Value, p.Value, c.Value, endOfMonth.Value))
    let _advance1                                  (d : ICell<Date>) (n : ICell<int>) (unit : ICell<TimeUnit>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.advance(d.Value, n.Value, unit.Value, c.Value, endOfMonth.Value))
    let _businessDaysBetween                       (from : ICell<Date>) (To : ICell<Date>) (includeFirst : ICell<bool>) (includeLast : ICell<bool>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.businessDaysBetween(from.Value, To.Value, includeFirst.Value, includeLast.Value))
    let _calendar                                  = triv _Ukraine (fun () -> _Ukraine.Value.calendar)
    let _empty                                     = triv _Ukraine (fun () -> _Ukraine.Value.empty())
    let _endOfMonth                                (d : ICell<Date>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.endOfMonth(d.Value))
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.Equals(o.Value))
    let _isBusinessDay                             (d : ICell<Date>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.isBusinessDay(d.Value))
    let _isEndOfMonth                              (d : ICell<Date>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.isEndOfMonth(d.Value))
    let _isHoliday                                 (d : ICell<Date>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.isHoliday(d.Value))
    let _isWeekend                                 (w : ICell<DayOfWeek>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.isWeekend(w.Value))
    let _name                                      = triv _Ukraine (fun () -> _Ukraine.Value.name())
    let _removedHolidays                           = triv _Ukraine (fun () -> _Ukraine.Value.removedHolidays)
    let _removeHoliday                             (d : ICell<Date>)   
                                                   = triv _Ukraine (fun () -> _Ukraine.Value.removeHoliday(d.Value)
                                                                              _Ukraine.Value)

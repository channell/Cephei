...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _ISKCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _ISKCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _ISKCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _ISKCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _ISKCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _ISKCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _ISKCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _ISKCurrency.Value.symbol)

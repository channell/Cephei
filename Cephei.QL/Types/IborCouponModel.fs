...
*)
    ...
    let _factory                                   (nominal : ICell<double>) (paymentDate : ICell<Date>) (startDate : ICell<Date>) (endDate : ICell<Date>) (fixingDays : ICell<int>) (index : ICell<InterestRateIndex>) (gearing : ICell<double>) (spread : ICell<double>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>) (dayCounter : ICell<DayCounter>) (isInArrears : ICell<bool>)   
                                                   = cell (fun () -> _IborCoupon.Value.factory(nominal.Value, paymentDate.Value, startDate.Value, endDate.Value, fixingDays.Value, index.Value, gearing.Value, spread.Value, refPeriodStart.Value, refPeriodEnd.Value, dayCounter.Value, isInArrears.Value))
    let _iborIndex                                 = cell (fun () -> _IborCoupon.Value.iborIndex())
    let _indexFixing                               = cell (fun () -> _IborCoupon.Value.indexFixing())
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = cell (fun () -> _IborCoupon.Value.accruedAmount(d.Value))
    let _adjustedFixing                            = cell (fun () -> _IborCoupon.Value.adjustedFixing)
    let _amount                                    = cell (fun () -> _IborCoupon.Value.amount())
    let _convexityAdjustment                       = cell (fun () -> _IborCoupon.Value.convexityAdjustment())
    let _dayCounter                                = cell (fun () -> _IborCoupon.Value.dayCounter())
    let _fixingDate                                = cell (fun () -> _IborCoupon.Value.fixingDate())
    let _fixingDays                                = cell (fun () -> _IborCoupon.Value.fixingDays)
    let _gearing                                   = cell (fun () -> _IborCoupon.Value.gearing())
    let _index                                     = cell (fun () -> _IborCoupon.Value.index())
    let _isInArrears                               = cell (fun () -> _IborCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _IborCoupon.Value.price(yts.Value))
    let _pricer                                    = cell (fun () -> _IborCoupon.Value.pricer())
    let _rate                                      = cell (fun () -> _IborCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = cell (fun () -> _IborCoupon.Value.setPricer(pricer.Value)
                                                                     _IborCoupon.Value)
    let _spread                                    = cell (fun () -> _IborCoupon.Value.spread())
    let _update                                    = cell (fun () -> _IborCoupon.Value.update()
                                                                     _IborCoupon.Value)
    let _accrualDays                               = cell (fun () -> _IborCoupon.Value.accrualDays())
    let _accrualEndDate                            = cell (fun () -> _IborCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = cell (fun () -> _IborCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = cell (fun () -> _IborCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = cell (fun () -> _IborCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = cell (fun () -> _IborCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = cell (fun () -> _IborCoupon.Value.date())
    let _exCouponDate                              = cell (fun () -> _IborCoupon.Value.exCouponDate())
    let _nominal                                   = cell (fun () -> _IborCoupon.Value.nominal())
    let _referencePeriodEnd                        = cell (fun () -> _IborCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = cell (fun () -> _IborCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _IborCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _IborCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _IborCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _IborCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _IborCoupon.Value.accept(v.Value)
                                                                     _IborCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _IborCoupon.Value.registerWith(handler.Value)
                                                                     _IborCoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _IborCoupon.Value.unregisterWith(handler.Value)
                                                                     _IborCoupon.Value)

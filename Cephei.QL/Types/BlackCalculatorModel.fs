...
*)
    ...
    let _alpha                                     = cell (fun () -> _BlackCalculator.Value.alpha())
    let _beta                                      = cell (fun () -> _BlackCalculator.Value.beta())
    let _delta                                     (spot : ICell<double>)   
                                                   = cell (fun () -> _BlackCalculator.Value.delta(spot.Value))
    let _deltaForward                              = cell (fun () -> _BlackCalculator.Value.deltaForward())
    let _dividendRho                               (maturity : ICell<double>)   
                                                   = cell (fun () -> _BlackCalculator.Value.dividendRho(maturity.Value))
    let _elasticity                                (spot : ICell<double>)   
                                                   = cell (fun () -> _BlackCalculator.Value.elasticity(spot.Value))
    let _elasticityForward                         = cell (fun () -> _BlackCalculator.Value.elasticityForward())
    let _gamma                                     (spot : ICell<double>)   
                                                   = cell (fun () -> _BlackCalculator.Value.gamma(spot.Value))
    let _gammaForward                              = cell (fun () -> _BlackCalculator.Value.gammaForward())
    let _itmAssetProbability                       = cell (fun () -> _BlackCalculator.Value.itmAssetProbability())
    let _itmCashProbability                        = cell (fun () -> _BlackCalculator.Value.itmCashProbability())
    let _rho                                       (maturity : ICell<double>)   
                                                   = cell (fun () -> _BlackCalculator.Value.rho(maturity.Value))
    let _strikeSensitivity                         = cell (fun () -> _BlackCalculator.Value.strikeSensitivity())
    let _theta                                     (spot : ICell<double>) (maturity : ICell<double>)   
                                                   = cell (fun () -> _BlackCalculator.Value.theta(spot.Value, maturity.Value))
    let _thetaPerDay                               (spot : ICell<double>) (maturity : ICell<double>)   
                                                   = cell (fun () -> _BlackCalculator.Value.thetaPerDay(spot.Value, maturity.Value))
    let _value                                     = cell (fun () -> _BlackCalculator.Value.value())
    let _vega                                      (maturity : ICell<double>)   
                                                   = cell (fun () -> _BlackCalculator.Value.vega(maturity.Value))

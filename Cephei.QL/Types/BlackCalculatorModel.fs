...
*)
    let mutable
        _BlackCalculator                           = make (fun () -> new BlackCalculator (payoff.Value, forward.Value, stdDev.Value, discount.Value))
    let _alpha                                     = triv _BlackCalculator (fun () -> _BlackCalculator.Value.alpha())
    let _beta                                      = triv _BlackCalculator (fun () -> _BlackCalculator.Value.beta())
    let _delta                                     (spot : ICell<double>)   
                                                   = triv _BlackCalculator (fun () -> _BlackCalculator.Value.delta(spot.Value))
    let _deltaForward                              = triv _BlackCalculator (fun () -> _BlackCalculator.Value.deltaForward())
    let _dividendRho                               (maturity : ICell<double>)   
                                                   = triv _BlackCalculator (fun () -> _BlackCalculator.Value.dividendRho(maturity.Value))
    let _elasticity                                (spot : ICell<double>)   
                                                   = triv _BlackCalculator (fun () -> _BlackCalculator.Value.elasticity(spot.Value))
    let _elasticityForward                         = triv _BlackCalculator (fun () -> _BlackCalculator.Value.elasticityForward())
    let _gamma                                     (spot : ICell<double>)   
                                                   = triv _BlackCalculator (fun () -> _BlackCalculator.Value.gamma(spot.Value))
    let _gammaForward                              = triv _BlackCalculator (fun () -> _BlackCalculator.Value.gammaForward())
    let _itmAssetProbability                       = triv _BlackCalculator (fun () -> _BlackCalculator.Value.itmAssetProbability())
    let _itmCashProbability                        = triv _BlackCalculator (fun () -> _BlackCalculator.Value.itmCashProbability())
    let _rho                                       (maturity : ICell<double>)   
                                                   = triv _BlackCalculator (fun () -> _BlackCalculator.Value.rho(maturity.Value))
    let _strikeSensitivity                         = triv _BlackCalculator (fun () -> _BlackCalculator.Value.strikeSensitivity())
    let _theta                                     (spot : ICell<double>) (maturity : ICell<double>)   
                                                   = triv _BlackCalculator (fun () -> _BlackCalculator.Value.theta(spot.Value, maturity.Value))
    let _thetaPerDay                               (spot : ICell<double>) (maturity : ICell<double>)   
                                                   = triv _BlackCalculator (fun () -> _BlackCalculator.Value.thetaPerDay(spot.Value, maturity.Value))
    let _value                                     = triv _BlackCalculator (fun () -> _BlackCalculator.Value.value())
    let _vega                                      (maturity : ICell<double>)   
                                                   = triv _BlackCalculator (fun () -> _BlackCalculator.Value.vega(maturity.Value))

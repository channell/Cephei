...
*)
    let mutable
        _CmsLeg                                    = make (fun () -> new CmsLeg (schedule.Value, swapIndex.Value))
    let _value                                     = triv _CmsLeg (fun () -> _CmsLeg.Value.value())
    let _inArrears                                 = triv _CmsLeg (fun () -> _CmsLeg.Value.inArrears())
    let _inArrears1                                (flag : ICell<bool>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.inArrears(flag.Value))
    let _withCaps                                  (caps : ICell<Generic.List<Nullable<double>>>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withCaps(caps.Value))
    let _withCaps1                                 (cap : ICell<Nullable<double>>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withCaps(cap.Value))
    let _withFixingDays                            (fixingDays : ICell<int>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withFixingDays(fixingDays.Value))
    let _withFixingDays1                           (fixingDays : ICell<Generic.List<int>>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withFixingDays(fixingDays.Value))
    let _withFloors                                (floor : ICell<Nullable<double>>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withFloors(floor.Value))
    let _withFloors1                               (floors : ICell<Generic.List<Nullable<double>>>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withFloors(floors.Value))
    let _withGearings                              (gearing : ICell<double>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withGearings(gearing.Value))
    let _withGearings1                             (gearings : ICell<Generic.List<double>>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withGearings(gearings.Value))
    let _withPaymentDayCounter                     (dayCounter : ICell<DayCounter>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withPaymentDayCounter(dayCounter.Value))
    let _withSpreads                               (spread : ICell<double>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withSpreads(spread.Value))
    let _withSpreads1                              (spreads : ICell<Generic.List<double>>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withSpreads(spreads.Value))
    let _withZeroPayments                          = triv _CmsLeg (fun () -> _CmsLeg.Value.withZeroPayments())
    let _withZeroPayments1                         (flag : ICell<bool>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withZeroPayments(flag.Value))
    let _withNotionals                             (notional : ICell<double>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withNotionals(notional.Value))
    let _withNotionals1                            (notionals : ICell<Generic.List<double>>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withNotionals(notionals.Value))
    let _withPaymentAdjustment                     (convention : ICell<BusinessDayConvention>)   
                                                   = triv _CmsLeg (fun () -> _CmsLeg.Value.withPaymentAdjustment(convention.Value))

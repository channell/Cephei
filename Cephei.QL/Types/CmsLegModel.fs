...
*)
    ...
    let _value                                     = cell (fun () -> _CmsLeg.Value.value())
    let _inArrears                                 = cell (fun () -> _CmsLeg.Value.inArrears())
    let _inArrears1                                (flag : ICell<bool>)   
                                                   = cell (fun () -> _CmsLeg.Value.inArrears(flag.Value))
    let _withCaps                                  (caps : ICell<List<Nullable<double>>>)   
                                                   = cell (fun () -> _CmsLeg.Value.withCaps(caps.Value))
    let _withCaps1                                 (cap : ICell<Nullable<double>>)   
                                                   = cell (fun () -> _CmsLeg.Value.withCaps(cap.Value))
    let _withFixingDays                            (fixingDays : ICell<int>)   
                                                   = cell (fun () -> _CmsLeg.Value.withFixingDays(fixingDays.Value))
    let _withFixingDays1                           (fixingDays : ICell<Generic.List<int>>)   
                                                   = cell (fun () -> _CmsLeg.Value.withFixingDays(fixingDays.Value))
    let _withFloors                                (floor : ICell<Nullable<double>>)   
                                                   = cell (fun () -> _CmsLeg.Value.withFloors(floor.Value))
    let _withFloors1                               (floors : ICell<List<Nullable<double>>>)   
                                                   = cell (fun () -> _CmsLeg.Value.withFloors(floors.Value))
    let _withGearings                              (gearing : ICell<double>)   
                                                   = cell (fun () -> _CmsLeg.Value.withGearings(gearing.Value))
    let _withGearings1                             (gearings : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _CmsLeg.Value.withGearings(gearings.Value))
    let _withPaymentDayCounter                     (dayCounter : ICell<DayCounter>)   
                                                   = cell (fun () -> _CmsLeg.Value.withPaymentDayCounter(dayCounter.Value))
    let _withSpreads                               (spread : ICell<double>)   
                                                   = cell (fun () -> _CmsLeg.Value.withSpreads(spread.Value))
    let _withSpreads1                              (spreads : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _CmsLeg.Value.withSpreads(spreads.Value))
    let _withZeroPayments                          = cell (fun () -> _CmsLeg.Value.withZeroPayments())
    let _withZeroPayments1                         (flag : ICell<bool>)   
                                                   = cell (fun () -> _CmsLeg.Value.withZeroPayments(flag.Value))
    let _withNotionals                             (notional : ICell<double>)   
                                                   = cell (fun () -> _CmsLeg.Value.withNotionals(notional.Value))
    let _withNotionals1                            (notionals : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _CmsLeg.Value.withNotionals(notionals.Value))
    let _withPaymentAdjustment                     (convention : ICell<BusinessDayConvention>)   
                                                   = cell (fun () -> _CmsLeg.Value.withPaymentAdjustment(convention.Value))

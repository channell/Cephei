...
*)
    ...
    let _a                                         = cell (fun () -> _Vasicek.Value.a())
    let _b                                         = cell (fun () -> _Vasicek.Value.b())
    let _discountBondOption                        (Type : ICell<Option.Type>) (strike : ICell<double>) (maturity : ICell<double>) (bondMaturity : ICell<double>)   
                                                   = cell (fun () -> _Vasicek.Value.discountBondOption(Type.Value, strike.Value, maturity.Value, bondMaturity.Value))
    let _dynamics                                  = cell (fun () -> _Vasicek.Value.dynamics())
    let _lambda                                    = cell (fun () -> _Vasicek.Value.lambda())
    let _sigma                                     = cell (fun () -> _Vasicek.Value.sigma())
    let _discount                                  (t : ICell<double>)   
                                                   = cell (fun () -> _Vasicek.Value.discount(t.Value))
    let _discountBond                              (now : ICell<double>) (maturity : ICell<double>) (factors : ICell<Vector>)   
                                                   = cell (fun () -> _Vasicek.Value.discountBond(now.Value, maturity.Value, factors.Value))
    let _discountBond1                             (now : ICell<double>) (maturity : ICell<double>) (rate : ICell<double>)   
                                                   = cell (fun () -> _Vasicek.Value.discountBond(now.Value, maturity.Value, rate.Value))
    let _tree                                      (grid : ICell<TimeGrid>)   
                                                   = cell (fun () -> _Vasicek.Value.tree(grid.Value))
    let _calibrate                                 (instruments : ICell<Generic.List<CalibrationHelper>>) (Method : ICell<OptimizationMethod>) (endCriteria : ICell<EndCriteria>) (additionalConstraint : ICell<Constraint>) (weights : ICell<Generic.List<double>>) (fixParameters : ICell<Generic.List<bool>>)   
                                                   = cell (fun () -> _Vasicek.Value.calibrate(instruments.Value, Method.Value, endCriteria.Value, additionalConstraint.Value, weights.Value, fixParameters.Value)
                                                                     _Vasicek.Value)
    let _constraint                                = cell (fun () -> _Vasicek.Value.CONSTRAINT())
    let _endCriteria                               = cell (fun () -> _Vasicek.Value.endCriteria())
    let _notifyObservers                           = cell (fun () -> _Vasicek.Value.notifyObservers()
                                                                     _Vasicek.Value)
    let _parameters                                = cell (fun () -> _Vasicek.Value.parameters())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _Vasicek.Value.registerWith(handler.Value)
                                                                     _Vasicek.Value)
    let _setParams                                 (parameters : ICell<Vector>)   
                                                   = cell (fun () -> _Vasicek.Value.setParams(parameters.Value)
                                                                     _Vasicek.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _Vasicek.Value.unregisterWith(handler.Value)
                                                                     _Vasicek.Value)
    let _update                                    = cell (fun () -> _Vasicek.Value.update()
                                                                     _Vasicek.Value)
    let _value                                     (parameters : ICell<Vector>) (instruments : ICell<Generic.List<CalibrationHelper>>)   
                                                   = cell (fun () -> _Vasicek.Value.value(parameters.Value, instruments.Value))

...
*)
    let mutable
        _Vasicek                                   = make (fun () -> new Vasicek (r0.Value, a.Value, b.Value, sigma.Value, lambda.Value))
    let _a                                         = triv _Vasicek (fun () -> _Vasicek.Value.a())
    let _b                                         = triv _Vasicek (fun () -> _Vasicek.Value.b())
    let _discountBondOption                        (Type : ICell<Option.Type>) (strike : ICell<double>) (maturity : ICell<double>) (bondMaturity : ICell<double>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.discountBondOption(Type.Value, strike.Value, maturity.Value, bondMaturity.Value))
    let _dynamics                                  = triv _Vasicek (fun () -> _Vasicek.Value.dynamics())
    let _lambda                                    = triv _Vasicek (fun () -> _Vasicek.Value.lambda())
    let _sigma                                     = triv _Vasicek (fun () -> _Vasicek.Value.sigma())
    let _discount                                  (t : ICell<double>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.discount(t.Value))
    let _discountBond                              (now : ICell<double>) (maturity : ICell<double>) (factors : ICell<Vector>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.discountBond(now.Value, maturity.Value, factors.Value))
    let _discountBond1                             (now : ICell<double>) (maturity : ICell<double>) (rate : ICell<double>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.discountBond(now.Value, maturity.Value, rate.Value))
    let _tree                                      (grid : ICell<TimeGrid>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.tree(grid.Value))
    let _calibrate                                 (instruments : ICell<Generic.List<CalibrationHelper>>) (Method : ICell<OptimizationMethod>) (endCriteria : ICell<EndCriteria>) (additionalConstraint : ICell<Constraint>) (weights : ICell<Generic.List<double>>) (fixParameters : ICell<Generic.List<bool>>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.calibrate(instruments.Value, Method.Value, endCriteria.Value, additionalConstraint.Value, weights.Value, fixParameters.Value)
                                                                              _Vasicek.Value)
    let _constraint                                = triv _Vasicek (fun () -> _Vasicek.Value.CONSTRAINT())
    let _endCriteria                               = triv _Vasicek (fun () -> _Vasicek.Value.endCriteria())
    let _notifyObservers                           = triv _Vasicek (fun () -> _Vasicek.Value.notifyObservers()
                                                                              _Vasicek.Value)
    let _parameters                                = triv _Vasicek (fun () -> _Vasicek.Value.parameters())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.registerWith(handler.Value)
                                                                              _Vasicek.Value)
    let _setParams                                 (parameters : ICell<Vector>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.setParams(parameters.Value)
                                                                              _Vasicek.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.unregisterWith(handler.Value)
                                                                              _Vasicek.Value)
    let _update                                    = triv _Vasicek (fun () -> _Vasicek.Value.update()
                                                                              _Vasicek.Value)
    let _value                                     (parameters : ICell<Vector>) (instruments : ICell<Generic.List<CalibrationHelper>>)   
                                                   = triv _Vasicek (fun () -> _Vasicek.Value.value(parameters.Value, instruments.Value))

...
*)
    ...
    let _factory                                   (L : ICell<Object>) (bcs : ICell<Object>) (additionalInputs : ICell<Object[]>)   
                                                   = cell (fun () -> _ExplicitEulerScheme.Value.factory(L.Value, bcs.Value, additionalInputs.Value))
    let _setStep                                   (dt : ICell<double>)   
                                                   = cell (fun () -> _ExplicitEulerScheme.Value.setStep(dt.Value)
                                                                     _ExplicitEulerScheme.Value)
    let _step                                      (a : ICell<Object ref>) (t : ICell<double>) (theta : ICell<double>)   
                                                   = cell (fun () -> _ExplicitEulerScheme.Value.step(a.Value, t.Value, theta.Value)
                                                                     _ExplicitEulerScheme.Value)

...
*)
    ...
    let _factory2                                  (Process : ICell<GeneralizedBlackScholesProcess>) (timeSteps : ICell<int>) (gridPoints : ICell<int>) (timeDependent : ICell<bool>)   
                                                   = cell (fun () -> _FDDividendEngineShiftScale.Value.factory2(Process.Value, timeSteps.Value, gridPoints.Value, timeDependent.Value))
    let _factory                                   (Process : ICell<GeneralizedBlackScholesProcess>) (timeSteps : ICell<int>) (gridPoints : ICell<int>) (timeDependent : ICell<bool>)   
                                                   = cell (fun () -> _FDDividendEngineShiftScale.Value.factory(Process.Value, timeSteps.Value, gridPoints.Value, timeDependent.Value))
    let _calculate                                 (r : ICell<IPricingEngineResults>)   
                                                   = cell (fun () -> _FDDividendEngineShiftScale.Value.calculate(r.Value)
                                                                     _FDDividendEngineShiftScale.Value)
    let _setStepCondition                          (impl : ICell<Func<IStepCondition<Vector>>>)   
                                                   = cell (fun () -> _FDDividendEngineShiftScale.Value.setStepCondition(impl.Value)
                                                                     _FDDividendEngineShiftScale.Value)
    let _ensureStrikeInGrid                        = cell (fun () -> _FDDividendEngineShiftScale.Value.ensureStrikeInGrid()
                                                                     _FDDividendEngineShiftScale.Value)
    let _getResidualTime                           = cell (fun () -> _FDDividendEngineShiftScale.Value.getResidualTime())
    let _grid                                      = cell (fun () -> _FDDividendEngineShiftScale.Value.grid())
    let _intrinsicValues_                          = cell (fun () -> _FDDividendEngineShiftScale.Value.intrinsicValues_)

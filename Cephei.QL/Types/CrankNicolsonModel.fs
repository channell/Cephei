...
*)
    ...
    let _factory                                   (L : ICell<Object>) (bcs : ICell<Object>) (additionalFields : ICell<Object[]>)   
                                                   = cell (fun () -> _CrankNicolson.Value.factory(L.Value, bcs.Value, additionalFields.Value))
    let _setStep                                   (dt : ICell<double>)   
                                                   = cell (fun () -> _CrankNicolson.Value.setStep(dt.Value)
                                                                     _CrankNicolson.Value)
    let _step                                      (o : ICell<Object ref>) (t : ICell<double>) (theta : ICell<double>)   
                                                   = cell (fun () -> _CrankNicolson.Value.step(o.Value, t.Value, theta.Value)
                                                                     _CrankNicolson.Value)

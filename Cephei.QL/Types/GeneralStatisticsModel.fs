...
*)
    ...
    let _add                                       (value : ICell<double>) (weight : ICell<double>)   
                                                   = cell (fun () -> _GeneralStatistics.Value.add(value.Value, weight.Value)
                                                                     _GeneralStatistics.Value)
    let _add1                                      (value : ICell<double>)   
                                                   = cell (fun () -> _GeneralStatistics.Value.add(value.Value)
                                                                     _GeneralStatistics.Value)
    let _addSequence                               (list : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _GeneralStatistics.Value.addSequence(list.Value)
                                                                     _GeneralStatistics.Value)
    let _addSequence1                              (data : ICell<Generic.List<double>>) (weight : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _GeneralStatistics.Value.addSequence(data.Value, weight.Value)
                                                                     _GeneralStatistics.Value)
    let _data                                      = cell (fun () -> _GeneralStatistics.Value.data())
    let _errorEstimate                             = cell (fun () -> _GeneralStatistics.Value.errorEstimate())
    let _expectationValue                          (f : ICell<Func<KeyValuePair<double,double>,double>>) (inRange : ICell<Func<KeyValuePair<double,double>,bool>>)   
                                                   = cell (fun () -> _GeneralStatistics.Value.expectationValue(f.Value, inRange.Value))
    let _kurtosis                                  = cell (fun () -> _GeneralStatistics.Value.kurtosis())
    let _max                                       = cell (fun () -> _GeneralStatistics.Value.max())
    let _mean                                      = cell (fun () -> _GeneralStatistics.Value.mean())
    let _min                                       = cell (fun () -> _GeneralStatistics.Value.min())
    let _percentile                                (percent : ICell<double>)   
                                                   = cell (fun () -> _GeneralStatistics.Value.percentile(percent.Value))
    let _reset                                     = cell (fun () -> _GeneralStatistics.Value.reset()
                                                                     _GeneralStatistics.Value)
    let _samples                                   = cell (fun () -> _GeneralStatistics.Value.samples())
    let _skewness                                  = cell (fun () -> _GeneralStatistics.Value.skewness())
    let _sort                                      = cell (fun () -> _GeneralStatistics.Value.sort()
                                                                     _GeneralStatistics.Value)
    let _standardDeviation                         = cell (fun () -> _GeneralStatistics.Value.standardDeviation())
    let _topPercentile                             (percent : ICell<double>)   
                                                   = cell (fun () -> _GeneralStatistics.Value.topPercentile(percent.Value))
    let _variance                                  = cell (fun () -> _GeneralStatistics.Value.variance())
    let _weightSum                                 = cell (fun () -> _GeneralStatistics.Value.weightSum())

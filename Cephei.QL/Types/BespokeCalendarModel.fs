...
*)
    ...
    let _addWeekend                                (w : ICell<DayOfWeek>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.addWeekend(w.Value)
                                                                     _BespokeCalendar.Value)
    let _name                                      = cell (fun () -> _BespokeCalendar.Value.name())
    let _addedHolidays                             = cell (fun () -> _BespokeCalendar.Value.addedHolidays)
    let _addHoliday                                (d : ICell<Date>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.addHoliday(d.Value)
                                                                     _BespokeCalendar.Value)
    let _adjust                                    (d : ICell<Date>) (c : ICell<BusinessDayConvention>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.adjust(d.Value, c.Value))
    let _advance                                   (d : ICell<Date>) (p : ICell<Period>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.advance(d.Value, p.Value, c.Value, endOfMonth.Value))
    let _advance1                                  (d : ICell<Date>) (n : ICell<int>) (unit : ICell<TimeUnit>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.advance(d.Value, n.Value, unit.Value, c.Value, endOfMonth.Value))
    let _businessDaysBetween                       (from : ICell<Date>) (To : ICell<Date>) (includeFirst : ICell<bool>) (includeLast : ICell<bool>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.businessDaysBetween(from.Value, To.Value, includeFirst.Value, includeLast.Value))
    let _calendar                                  = cell (fun () -> _BespokeCalendar.Value.calendar)
    let _empty                                     = cell (fun () -> _BespokeCalendar.Value.empty())
    let _endOfMonth                                (d : ICell<Date>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.endOfMonth(d.Value))
    let _Equals                                    (o : ICell<Object>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.Equals(o.Value))
    let _isBusinessDay                             (d : ICell<Date>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.isBusinessDay(d.Value))
    let _isEndOfMonth                              (d : ICell<Date>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.isEndOfMonth(d.Value))
    let _isHoliday                                 (d : ICell<Date>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.isHoliday(d.Value))
    let _isWeekend                                 (w : ICell<DayOfWeek>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.isWeekend(w.Value))
    let _removedHolidays                           = cell (fun () -> _BespokeCalendar.Value.removedHolidays)
    let _removeHoliday                             (d : ICell<Date>)   
                                                   = cell (fun () -> _BespokeCalendar.Value.removeHoliday(d.Value)
                                                                     _BespokeCalendar.Value)

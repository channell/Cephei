...
*)
    ...
    let _calculate                                 (r : ICell<IPricingEngineResults>)   
                                                   = triv (fun () -> _FDAmericanCondition.Value.calculate(r.Value)
                                                                     _FDAmericanCondition.Value)
    let _setStepCondition                          (impl : ICell<Func<IStepCondition<Vector>>>)   
                                                   = triv (fun () -> _FDAmericanCondition.Value.setStepCondition(impl.Value)
                                                                     _FDAmericanCondition.Value)
    let _ensureStrikeInGrid                        = triv (fun () -> _FDAmericanCondition.Value.ensureStrikeInGrid()
                                                                     _FDAmericanCondition.Value)
    let _getResidualTime                           = triv (fun () -> _FDAmericanCondition.Value.getResidualTime())
    let _grid                                      = triv (fun () -> _FDAmericanCondition.Value.grid())
    let _intrinsicValues_                          = triv (fun () -> _FDAmericanCondition.Value.intrinsicValues_)

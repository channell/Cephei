...
*)
    ...
    let _dminus                                    (iter : ICell<FdmLinearOpIterator>) (direction : ICell<int>)   
                                                   = cell (fun () -> _UniformGridMesher.Value.dminus(iter.Value, direction.Value))
    let _dplus                                     (iter : ICell<FdmLinearOpIterator>) (direction : ICell<int>)   
                                                   = cell (fun () -> _UniformGridMesher.Value.dplus(iter.Value, direction.Value))
    let _location                                  (iter : ICell<FdmLinearOpIterator>) (direction : ICell<int>)   
                                                   = cell (fun () -> _UniformGridMesher.Value.location(iter.Value, direction.Value))
    let _locations                                 (direction : ICell<int>)   
                                                   = cell (fun () -> _UniformGridMesher.Value.locations(direction.Value))
    let _layout                                    = cell (fun () -> _UniformGridMesher.Value.layout())

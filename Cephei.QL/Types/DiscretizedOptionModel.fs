...
*)
    ...
    let _mandatoryTimes                            = cell (fun () -> _DiscretizedOption.Value.mandatoryTimes())
    let _reset                                     (size : ICell<int>)   
                                                   = cell (fun () -> _DiscretizedOption.Value.reset(size.Value)
                                                                     _DiscretizedOption.Value)
    let _adjustValues                              = cell (fun () -> _DiscretizedOption.Value.adjustValues()
                                                                     _DiscretizedOption.Value)
    let _initialize                                (Method : ICell<Lattice>) (t : ICell<double>)   
                                                   = cell (fun () -> _DiscretizedOption.Value.initialize(Method.Value, t.Value)
                                                                     _DiscretizedOption.Value)
    let _method                                    = cell (fun () -> _DiscretizedOption.Value.METHOD())
    let _partialRollback                           (To : ICell<double>)   
                                                   = cell (fun () -> _DiscretizedOption.Value.partialRollback(To.Value)
                                                                     _DiscretizedOption.Value)
    let _postAdjustValues                          = cell (fun () -> _DiscretizedOption.Value.postAdjustValues()
                                                                     _DiscretizedOption.Value)
    let _preAdjustValues                           = cell (fun () -> _DiscretizedOption.Value.preAdjustValues()
                                                                     _DiscretizedOption.Value)
    let _presentValue                              = cell (fun () -> _DiscretizedOption.Value.presentValue())
    let _rollback                                  (To : ICell<double>)   
                                                   = cell (fun () -> _DiscretizedOption.Value.rollback(To.Value)
                                                                     _DiscretizedOption.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = cell (fun () -> _DiscretizedOption.Value.setTime(t.Value)
                                                                     _DiscretizedOption.Value)
    let _setValues                                 (v : ICell<Vector>)   
                                                   = cell (fun () -> _DiscretizedOption.Value.setValues(v.Value)
                                                                     _DiscretizedOption.Value)
    let _time                                      = cell (fun () -> _DiscretizedOption.Value.time())
    let _values                                    = cell (fun () -> _DiscretizedOption.Value.values())

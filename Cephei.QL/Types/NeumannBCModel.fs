...
*)
    ...
    let _applyAfterApplying                        (u : ICell<Vector>)   
                                                   = cell (fun () -> _NeumannBC.Value.applyAfterApplying(u.Value)
                                                                     _NeumannBC.Value)
    let _applyAfterSolving                         (v : ICell<Vector>)   
                                                   = cell (fun () -> _NeumannBC.Value.applyAfterSolving(v.Value)
                                                                     _NeumannBC.Value)
    let _applyBeforeApplying                       (o : ICell<IOperator>)   
                                                   = cell (fun () -> _NeumannBC.Value.applyBeforeApplying(o.Value)
                                                                     _NeumannBC.Value)
    let _applyBeforeSolving                        (o : ICell<IOperator>) (rhs : ICell<Vector>)   
                                                   = cell (fun () -> _NeumannBC.Value.applyBeforeSolving(o.Value, rhs.Value)
                                                                     _NeumannBC.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = cell (fun () -> _NeumannBC.Value.setTime(t.Value)
                                                                     _NeumannBC.Value)

...
*)
    let mutable
        _NeumannBC                                 = make (fun () -> new NeumannBC (value.Value, side.Value))
    let _applyAfterApplying                        (u : ICell<Vector>)   
                                                   = triv _NeumannBC (fun () -> _NeumannBC.Value.applyAfterApplying(u.Value)
                                                                                _NeumannBC.Value)
    let _applyAfterSolving                         (v : ICell<Vector>)   
                                                   = triv _NeumannBC (fun () -> _NeumannBC.Value.applyAfterSolving(v.Value)
                                                                                _NeumannBC.Value)
    let _applyBeforeApplying                       (o : ICell<IOperator>)   
                                                   = triv _NeumannBC (fun () -> _NeumannBC.Value.applyBeforeApplying(o.Value)
                                                                                _NeumannBC.Value)
    let _applyBeforeSolving                        (o : ICell<IOperator>) (rhs : ICell<Vector>)   
                                                   = triv _NeumannBC (fun () -> _NeumannBC.Value.applyBeforeSolving(o.Value, rhs.Value)
                                                                                _NeumannBC.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = triv _NeumannBC (fun () -> _NeumannBC.Value.setTime(t.Value)
                                                                                _NeumannBC.Value)

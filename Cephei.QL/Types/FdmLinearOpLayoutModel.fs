...
*)
    ...
    let _begin                                     = cell (fun () -> _FdmLinearOpLayout.Value.BEGIN())
    let _dim                                       = cell (fun () -> _FdmLinearOpLayout.Value.dim())
    let _end                                       = cell (fun () -> _FdmLinearOpLayout.Value.END())
    let _index                                     (coordinates : ICell<Generic.List<int>>)   
                                                   = cell (fun () -> _FdmLinearOpLayout.Value.index(coordinates.Value))
    let _iter_neighbourhood                        (iterator : ICell<FdmLinearOpIterator>) (i : ICell<int>) (offset : ICell<int>)   
                                                   = cell (fun () -> _FdmLinearOpLayout.Value.iter_neighbourhood(iterator.Value, i.Value, offset.Value))
    let _neighbourhood                             (iterator : ICell<FdmLinearOpIterator>) (i1 : ICell<int>) (offset1 : ICell<int>) (i2 : ICell<int>) (offset2 : ICell<int>)   
                                                   = cell (fun () -> _FdmLinearOpLayout.Value.neighbourhood(iterator.Value, i1.Value, offset1.Value, i2.Value, offset2.Value))
    let _neighbourhood1                            (iterator : ICell<FdmLinearOpIterator>) (i : ICell<int>) (offset : ICell<int>)   
                                                   = cell (fun () -> _FdmLinearOpLayout.Value.neighbourhood(iterator.Value, i.Value, offset.Value))
    let _size                                      = cell (fun () -> _FdmLinearOpLayout.Value.size())
    let _spacing                                   = cell (fun () -> _FdmLinearOpLayout.Value.spacing())

...
*)
    let mutable
        _FdmLinearOpLayout                         = make (fun () -> new FdmLinearOpLayout (dim.Value))
    let _begin                                     = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.BEGIN())
    let _dim                                       = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.dim())
    let _end                                       = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.END())
    let _index                                     (coordinates : ICell<Generic.List<int>>)   
                                                   = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.index(coordinates.Value))
    let _iter_neighbourhood                        (iterator : ICell<FdmLinearOpIterator>) (i : ICell<int>) (offset : ICell<int>)   
                                                   = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.iter_neighbourhood(iterator.Value, i.Value, offset.Value))
    let _neighbourhood                             (iterator : ICell<FdmLinearOpIterator>) (i1 : ICell<int>) (offset1 : ICell<int>) (i2 : ICell<int>) (offset2 : ICell<int>)   
                                                   = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.neighbourhood(iterator.Value, i1.Value, offset1.Value, i2.Value, offset2.Value))
    let _neighbourhood1                            (iterator : ICell<FdmLinearOpIterator>) (i : ICell<int>) (offset : ICell<int>)   
                                                   = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.neighbourhood(iterator.Value, i.Value, offset.Value))
    let _size                                      = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.size())
    let _spacing                                   = triv _FdmLinearOpLayout (fun () -> _FdmLinearOpLayout.Value.spacing())

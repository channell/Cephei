...
*)
    let mutable
        _DayCounter                                = make (fun () -> new DayCounter (d.Value))
    let _dayCount                                  (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = triv _DayCounter (fun () -> _DayCounter.Value.dayCount(d1.Value, d2.Value))
    let _dayCounter                                = triv _DayCounter (fun () -> _DayCounter.Value.dayCounter)
    let _empty                                     = triv _DayCounter (fun () -> _DayCounter.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _DayCounter (fun () -> _DayCounter.Value.Equals(o.Value))
    let _name                                      = triv _DayCounter (fun () -> _DayCounter.Value.name())
    let _ToString                                  = triv _DayCounter (fun () -> _DayCounter.Value.ToString())
    let _yearFraction                              (d1 : ICell<Date>) (d2 : ICell<Date>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>)   
                                                   = triv _DayCounter (fun () -> _DayCounter.Value.yearFraction(d1.Value, d2.Value, refPeriodStart.Value, refPeriodEnd.Value))
    let _yearFraction1                             (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = triv _DayCounter (fun () -> _DayCounter.Value.yearFraction(d1.Value, d2.Value))
...
*)
    let mutable
        _DayCounter                                = make (fun () -> new DayCounter ())
    let _dayCount                                  (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = triv _DayCounter (fun () -> _DayCounter.Value.dayCount(d1.Value, d2.Value))
    let _dayCounter                                = triv _DayCounter (fun () -> _DayCounter.Value.dayCounter)
    let _empty                                     = triv _DayCounter (fun () -> _DayCounter.Value.empty())

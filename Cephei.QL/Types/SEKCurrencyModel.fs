...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _SEKCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _SEKCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _SEKCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _SEKCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _SEKCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _SEKCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _SEKCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _SEKCurrency.Value.symbol)
    let _ToString                                  = triv (fun () -> _SEKCurrency.Value.ToString())
    let _triangulationCurrency                     = triv (fun () -> _SEKCurrency.Value.triangulationCurrency)

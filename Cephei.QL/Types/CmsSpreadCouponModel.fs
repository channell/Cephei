...
*)
    ...
    let _swapSpreadIndex                           = cell (fun () -> _CmsSpreadCoupon.Value.swapSpreadIndex())
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.accruedAmount(d.Value))
    let _adjustedFixing                            = cell (fun () -> _CmsSpreadCoupon.Value.adjustedFixing)
    let _amount                                    = cell (fun () -> _CmsSpreadCoupon.Value.amount())
    let _convexityAdjustment                       = cell (fun () -> _CmsSpreadCoupon.Value.convexityAdjustment())
    let _dayCounter                                = cell (fun () -> _CmsSpreadCoupon.Value.dayCounter())
    let _factory                                   (nominal : ICell<double>) (paymentDate : ICell<Date>) (startDate : ICell<Date>) (endDate : ICell<Date>) (fixingDays : ICell<int>) (index : ICell<InterestRateIndex>) (gearing : ICell<double>) (spread : ICell<double>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>) (dayCounter : ICell<DayCounter>) (isInArrears : ICell<bool>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.factory(nominal.Value, paymentDate.Value, startDate.Value, endDate.Value, fixingDays.Value, index.Value, gearing.Value, spread.Value, refPeriodStart.Value, refPeriodEnd.Value, dayCounter.Value, isInArrears.Value))
    let _fixingDate                                = cell (fun () -> _CmsSpreadCoupon.Value.fixingDate())
    let _fixingDays                                = cell (fun () -> _CmsSpreadCoupon.Value.fixingDays)
    let _gearing                                   = cell (fun () -> _CmsSpreadCoupon.Value.gearing())
    let _index                                     = cell (fun () -> _CmsSpreadCoupon.Value.index())
    let _indexFixing                               = cell (fun () -> _CmsSpreadCoupon.Value.indexFixing())
    let _isInArrears                               = cell (fun () -> _CmsSpreadCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.price(yts.Value))
    let _pricer                                    = cell (fun () -> _CmsSpreadCoupon.Value.pricer())
    let _rate                                      = cell (fun () -> _CmsSpreadCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.setPricer(pricer.Value)
                                                                     _CmsSpreadCoupon.Value)
    let _spread                                    = cell (fun () -> _CmsSpreadCoupon.Value.spread())
    let _update                                    = cell (fun () -> _CmsSpreadCoupon.Value.update()
                                                                     _CmsSpreadCoupon.Value)
    let _accrualDays                               = cell (fun () -> _CmsSpreadCoupon.Value.accrualDays())
    let _accrualEndDate                            = cell (fun () -> _CmsSpreadCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = cell (fun () -> _CmsSpreadCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = cell (fun () -> _CmsSpreadCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = cell (fun () -> _CmsSpreadCoupon.Value.date())
    let _exCouponDate                              = cell (fun () -> _CmsSpreadCoupon.Value.exCouponDate())
    let _nominal                                   = cell (fun () -> _CmsSpreadCoupon.Value.nominal())
    let _referencePeriodEnd                        = cell (fun () -> _CmsSpreadCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = cell (fun () -> _CmsSpreadCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.accept(v.Value)
                                                                     _CmsSpreadCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.registerWith(handler.Value)
                                                                     _CmsSpreadCoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CmsSpreadCoupon.Value.unregisterWith(handler.Value)
                                                                     _CmsSpreadCoupon.Value)

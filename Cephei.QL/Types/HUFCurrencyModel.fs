...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _HUFCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _HUFCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _HUFCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _HUFCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _HUFCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _HUFCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _HUFCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _HUFCurrency.Value.symbol)

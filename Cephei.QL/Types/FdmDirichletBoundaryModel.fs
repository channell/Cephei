...
*)
    let mutable
        _FdmDirichletBoundary                      = make (fun () -> new FdmDirichletBoundary (mesher.Value, valueOnBoundary.Value, direction.Value, side.Value))
    let _applyAfterApplying                        (v : ICell<Vector>)   
                                                   = triv _FdmDirichletBoundary (fun () -> _FdmDirichletBoundary.Value.applyAfterApplying(v.Value)
                                                                                           _FdmDirichletBoundary.Value)
    let _applyAfterApplying1                       (x : ICell<double>) (value : ICell<double>)   
                                                   = triv _FdmDirichletBoundary (fun () -> _FdmDirichletBoundary.Value.applyAfterApplying(x.Value, value.Value))
    let _applyAfterSolving                         (v : ICell<Vector>)   
                                                   = triv _FdmDirichletBoundary (fun () -> _FdmDirichletBoundary.Value.applyAfterSolving(v.Value)
                                                                                           _FdmDirichletBoundary.Value)
    let _applyBeforeApplying                       (o : ICell<IOperator>)   
                                                   = triv _FdmDirichletBoundary (fun () -> _FdmDirichletBoundary.Value.applyBeforeApplying(o.Value)
                                                                                           _FdmDirichletBoundary.Value)
    let _applyBeforeSolving                        (o : ICell<IOperator>) (v : ICell<Vector>)   
                                                   = triv _FdmDirichletBoundary (fun () -> _FdmDirichletBoundary.Value.applyBeforeSolving(o.Value, v.Value)
                                                                                           _FdmDirichletBoundary.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = triv _FdmDirichletBoundary (fun () -> _FdmDirichletBoundary.Value.setTime(t.Value)
                                                                                           _FdmDirichletBoundary.Value)

...
*)
    ...
    let _applyAfterApplying                        (v : ICell<Vector>)   
                                                   = cell (fun () -> _FdmDirichletBoundary.Value.applyAfterApplying(v.Value)
                                                                     _FdmDirichletBoundary.Value)
    let _applyAfterApplying1                       (x : ICell<double>) (value : ICell<double>)   
                                                   = cell (fun () -> _FdmDirichletBoundary.Value.applyAfterApplying(x.Value, value.Value))
    let _applyAfterSolving                         (v : ICell<Vector>)   
                                                   = cell (fun () -> _FdmDirichletBoundary.Value.applyAfterSolving(v.Value)
                                                                     _FdmDirichletBoundary.Value)
    let _applyBeforeApplying                       (o : ICell<IOperator>)   
                                                   = cell (fun () -> _FdmDirichletBoundary.Value.applyBeforeApplying(o.Value)
                                                                     _FdmDirichletBoundary.Value)
    let _applyBeforeSolving                        (o : ICell<IOperator>) (v : ICell<Vector>)   
                                                   = cell (fun () -> _FdmDirichletBoundary.Value.applyBeforeSolving(o.Value, v.Value)
                                                                     _FdmDirichletBoundary.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = cell (fun () -> _FdmDirichletBoundary.Value.setTime(t.Value)
                                                                     _FdmDirichletBoundary.Value)

...
*)
    ...
    let _cap                                       = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.cap())
    let _convexityAdjustment                       = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.convexityAdjustment())
    let _effectiveCap                              = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.effectiveCap())
    let _effectiveFloor                            = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.effectiveFloor())
    let _factory                                   (nominal : ICell<double>) (paymentDate : ICell<Date>) (startDate : ICell<Date>) (endDate : ICell<Date>) (fixingDays : ICell<int>) (index : ICell<InterestRateIndex>) (gearing : ICell<double>) (spread : ICell<double>) (cap : ICell<Nullable<double>>) (floor : ICell<Nullable<double>>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>) (dayCounter : ICell<DayCounter>) (isInArrears : ICell<bool>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.factory(nominal.Value, paymentDate.Value, startDate.Value, endDate.Value, fixingDays.Value, index.Value, gearing.Value, spread.Value, cap.Value, floor.Value, refPeriodStart.Value, refPeriodEnd.Value, dayCounter.Value, isInArrears.Value))
    let _floor                                     = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.floor())
    let _isCapped                                  = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.isCapped())
    let _isFloored                                 = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.isFloored())
    let _rate                                      = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.setPricer(pricer.Value)
                                                                     _CappedFlooredCmsSpreadCoupon.Value)
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.accruedAmount(d.Value))
    let _adjustedFixing                            = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.adjustedFixing)
    let _amount                                    = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.amount())
    let _dayCounter                                = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.dayCounter())
    let _fixingDate                                = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.fixingDate())
    let _fixingDays                                = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.fixingDays)
    let _gearing                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.gearing())
    let _index                                     = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.index())
    let _indexFixing                               = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.indexFixing())
    let _isInArrears                               = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.price(yts.Value))
    let _pricer                                    = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.pricer())
    let _spread                                    = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.spread())
    let _update                                    = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.update()
                                                                     _CappedFlooredCmsSpreadCoupon.Value)
    let _accrualDays                               = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.accrualDays())
    let _accrualEndDate                            = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.date())
    let _exCouponDate                              = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.exCouponDate())
    let _nominal                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.nominal())
    let _referencePeriodEnd                        = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.accept(v.Value)
                                                                     _CappedFlooredCmsSpreadCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.registerWith(handler.Value)
                                                                     _CappedFlooredCmsSpreadCoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CappedFlooredCmsSpreadCoupon.Value.unregisterWith(handler.Value)
                                                                     _CappedFlooredCmsSpreadCoupon.Value)

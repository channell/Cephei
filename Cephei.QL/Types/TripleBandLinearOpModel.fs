...
*)
    ...
    let _add1                                      (u : ICell<Vector>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.add(u.Value))
    let _add2                                      (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.apply(r.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.applyTo(v.Value))
    let _axpyb                                     (a : ICell<Vector>) (x : ICell<TripleBandLinearOp>) (y : ICell<TripleBandLinearOp>) (b : ICell<Vector>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.axpyb(a.Value, x.Value, y.Value, b.Value)
                                                                     _TripleBandLinearOp.Value)
    let _Clone                                     = triv (fun () -> _TripleBandLinearOp.Value.Clone())
    let _identity                                  (size : ICell<int>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.identity(size.Value))
    let _isTimeDependent                           = triv (fun () -> _TripleBandLinearOp.Value.isTimeDependent())
    let _mult                                      (u : ICell<Vector>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.mult(u.Value))
    let _multiply                                  (a : ICell<double>) (D : ICell<IOperator>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.multiply(a.Value, D.Value))
    let _multR                                     (u : ICell<Vector>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.multR(u.Value))
    let _setTime                                   (t : ICell<double>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.setTime(t.Value)
                                                                     _TripleBandLinearOp.Value)
    let _size                                      = triv (fun () -> _TripleBandLinearOp.Value.size())
    let _solve_splitting                           (r : ICell<Vector>) (a : ICell<double>) (b : ICell<double>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.solve_splitting(r.Value, a.Value, b.Value))
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.solveFor(rhs.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.subtract(A.Value, B.Value))
    let _swap                                      (m : ICell<TripleBandLinearOp>)   
                                                   = triv (fun () -> _TripleBandLinearOp.Value.swap(m.Value)
                                                                     _TripleBandLinearOp.Value)

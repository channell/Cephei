...
*)
    ...
    let _covariance                                (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LfmHullWhiteParameterization.Value.covariance(t.Value, x.Value))
    let _covariance1                               (t : ICell<double>)   
                                                   = cell (fun () -> _LfmHullWhiteParameterization.Value.covariance(t.Value))
    let _diffusion                                 (t : ICell<double>)   
                                                   = cell (fun () -> _LfmHullWhiteParameterization.Value.diffusion(t.Value))
    let _diffusion1                                (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LfmHullWhiteParameterization.Value.diffusion(t.Value, x.Value))
    let _integratedCovariance                      (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LfmHullWhiteParameterization.Value.integratedCovariance(t.Value, x.Value))
    let _factors                                   = cell (fun () -> _LfmHullWhiteParameterization.Value.factors())
    let _size                                      = cell (fun () -> _LfmHullWhiteParameterization.Value.size())

...
(*
    Functions
*)
    let _FixedRateBondForward                      = triv (fun () -> withEngine _pricingEngine.Value (new FixedRateBondForward (valueDate.Value, maturityDate.Value, Type.Value, strike.Value, settlementDays.Value, dayCounter.Value, calendar.Value, businessDayConvention.Value, fixedCouponBond.Value, discountCurve.Value, incomeDiscountCurve.Value)))
    ...
    let _spotIncome                                (incomeDiscountCurve : ICell<Handle<YieldTermStructure>>)   
                                                   = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).spotIncome(incomeDiscountCurve.Value))
    let _spotValue                                 = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).spotValue())
    let _forwardValue                              = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).forwardValue())
    let _impliedYield                              (underlyingSpotValue : ICell<double>) (forwardValue : ICell<double>) (settlementDate : ICell<Date>) (compoundingConvention : ICell<Compounding>) (dayCounter : ICell<DayCounter>)   
                                                   = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).impliedYield(underlyingSpotValue.Value, forwardValue.Value, settlementDate.Value, compoundingConvention.Value, dayCounter.Value))
    let _isExpired                                 = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).isExpired())
    let _settlementDate                            = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).settlementDate())
    let _CASH                                      = cell (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).CASH())
    let _errorEstimate                             = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).errorEstimate())
    let _NPV                                       = cell (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).NPV())
    let _result                                    (tag : ICell<string>)   
                                                   = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).result(tag.Value))
    let _setPricingEngine                          (e : ICell<IPricingEngine>)   
                                                   = triv (fun () -> (withEvaluationDate _evaluationDate _FixedRateBondForward).setPricingEngine(e.Value)

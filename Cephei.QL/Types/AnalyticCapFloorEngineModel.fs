...
*)
    let mutable
        _AnalyticCapFloorEngine                    = cell (fun () -> (createEvaluationDate _evaluationDate (fun () ->new AnalyticCapFloorEngine (model.Value))))
    let _setModel                                  (model : ICell<Handle<IAffineModel>>)   
                                                   = triv (fun () -> (curryEvaluationDate _evaluationDate _AnalyticCapFloorEngine).Value.setModel(model.Value)
                                                                     _AnalyticCapFloorEngine.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> (curryEvaluationDate _evaluationDate _AnalyticCapFloorEngine).Value.registerWith(handler.Value)
                                                                     _AnalyticCapFloorEngine.Value)
    let _reset                                     = triv (fun () -> (curryEvaluationDate _evaluationDate _AnalyticCapFloorEngine).Value.reset()
                                                                     _AnalyticCapFloorEngine.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv (fun () -> (curryEvaluationDate _evaluationDate _AnalyticCapFloorEngine).Value.unregisterWith(handler.Value)
                                                                     _AnalyticCapFloorEngine.Value)
    let _update                                    = triv (fun () -> (curryEvaluationDate _evaluationDate _AnalyticCapFloorEngine).Value.update()
                                                                     _AnalyticCapFloorEngine.Value)
...
*)
    let mutable
        _AnalyticCapFloorEngine                    = cell (fun () -> (createEvaluationDate _evaluationDate (fun () ->new AnalyticCapFloorEngine (model.Value, termStructure.Value))))
    let _setModel                                  (model : ICell<Handle<IAffineModel>>)   
                                                   = triv (fun () -> (curryEvaluationDate _evaluationDate _AnalyticCapFloorEngine).Value.setModel(model.Value)
                                                                     _AnalyticCapFloorEngine.Value)
    let _registerWith                              (handler : ICell<Callback>)   

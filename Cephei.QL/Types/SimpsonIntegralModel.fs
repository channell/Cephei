...
*)
    ...
    let _absoluteAccuracy                          = cell (fun () -> _SimpsonIntegral.Value.absoluteAccuracy())
    let _absoluteError                             = cell (fun () -> _SimpsonIntegral.Value.absoluteError())
    let _integrationSuccess                        = cell (fun () -> _SimpsonIntegral.Value.integrationSuccess())
    let _maxEvaluations                            = cell (fun () -> _SimpsonIntegral.Value.maxEvaluations())
    let _numberOfEvaluations                       = cell (fun () -> _SimpsonIntegral.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = cell (fun () -> _SimpsonIntegral.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                     _SimpsonIntegral.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = cell (fun () -> _SimpsonIntegral.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                     _SimpsonIntegral.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = cell (fun () -> _SimpsonIntegral.Value.value(f.Value, a.Value, b.Value))

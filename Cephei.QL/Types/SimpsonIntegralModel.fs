...
*)
    let mutable
        _SimpsonIntegral                           = make (fun () -> new SimpsonIntegral (accuracy.Value, maxIterations.Value))
    let _absoluteAccuracy                          = triv _SimpsonIntegral (fun () -> _SimpsonIntegral.Value.absoluteAccuracy())
    let _absoluteError                             = triv _SimpsonIntegral (fun () -> _SimpsonIntegral.Value.absoluteError())
    let _integrationSuccess                        = triv _SimpsonIntegral (fun () -> _SimpsonIntegral.Value.integrationSuccess())
    let _maxEvaluations                            = triv _SimpsonIntegral (fun () -> _SimpsonIntegral.Value.maxEvaluations())
    let _numberOfEvaluations                       = triv _SimpsonIntegral (fun () -> _SimpsonIntegral.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = triv _SimpsonIntegral (fun () -> _SimpsonIntegral.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                                      _SimpsonIntegral.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = triv _SimpsonIntegral (fun () -> _SimpsonIntegral.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                                      _SimpsonIntegral.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = triv _SimpsonIntegral (fun () -> _SimpsonIntegral.Value.value(f.Value, a.Value, b.Value))

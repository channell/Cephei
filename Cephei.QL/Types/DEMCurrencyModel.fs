...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _DEMCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _DEMCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _DEMCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _DEMCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _DEMCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _DEMCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _DEMCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _DEMCurrency.Value.symbol)
    let _ToString                                  = triv (fun () -> _DEMCurrency.Value.ToString())
    let _triangulationCurrency                     = triv (fun () -> _DEMCurrency.Value.triangulationCurrency)

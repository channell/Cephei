...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _SGDCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _SGDCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _SGDCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _SGDCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _SGDCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _SGDCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _SGDCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _SGDCurrency.Value.symbol)

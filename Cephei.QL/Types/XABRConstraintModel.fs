...
*)
    let mutable
        _XABRConstraint                            = make (fun () -> new XABRConstraint (impl.Value))
    let _config                                    (costFunction : ICell<ProjectedCostFunction>) (coeff : ICell<XABRCoeffHolder<'Model>>) (forward : ICell<double>)   
                                                   = triv _XABRConstraint (fun () -> _XABRConstraint.Value.config(costFunction.Value, coeff.Value, forward.Value)
                                                                                     _XABRConstraint.Value)
    let _empty                                     = triv _XABRConstraint (fun () -> _XABRConstraint.Value.empty())
    let _lowerBound                                (parameters : ICell<Vector>)   
                                                   = triv _XABRConstraint (fun () -> _XABRConstraint.Value.lowerBound(parameters.Value))
    let _test                                      (p : ICell<Vector>)   
                                                   = triv _XABRConstraint (fun () -> _XABRConstraint.Value.test(p.Value))
    let _update                                    (p : ICell<Vector>) (direction : ICell<Vector>) (beta : ICell<double>)   
                                                   = triv _XABRConstraint (fun () -> _XABRConstraint.Value.update(ref p.Value, direction.Value, beta.Value))
    let _upperBound                                (parameters : ICell<Vector>)   
                                                   = triv _XABRConstraint (fun () -> _XABRConstraint.Value.upperBound(parameters.Value))
...
*)
    let mutable
        _XABRConstraint                            = make (fun () -> new XABRConstraint ())
    let _config                                    (costFunction : ICell<ProjectedCostFunction>) (coeff : ICell<XABRCoeffHolder<'Model>>) (forward : ICell<double>)   
                                                   = triv _XABRConstraint (fun () -> _XABRConstraint.Value.config(costFunction.Value, coeff.Value, forward.Value)
                                                                                     _XABRConstraint.Value)
    let _empty                                     = triv _XABRConstraint (fun () -> _XABRConstraint.Value.empty())

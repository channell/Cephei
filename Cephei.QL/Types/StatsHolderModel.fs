...
*)
    ...
    let _addSequence                               (data : ICell<Generic.List<double>>) (weight : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _StatsHolder.Value.addSequence(data.Value, weight.Value)
                                                                     _StatsHolder.Value)
    let _errorEstimate                             = triv (fun () -> _StatsHolder.Value.errorEstimate())
    let _expectationValue                          (f : ICell<Func<KeyValuePair<double,double>,double>>) (inRange : ICell<Func<KeyValuePair<double,double>,bool>>)   
                                                   = triv (fun () -> _StatsHolder.Value.expectationValue(f.Value, inRange.Value))
    let _kurtosis                                  = triv (fun () -> _StatsHolder.Value.kurtosis())
    let _max                                       = triv (fun () -> _StatsHolder.Value.max())
    let _mean                                      = triv (fun () -> _StatsHolder.Value.mean())
    let _min                                       = triv (fun () -> _StatsHolder.Value.min())
    let _percentile                                (percent : ICell<double>)   
                                                   = triv (fun () -> _StatsHolder.Value.percentile(percent.Value))
    let _reset                                     = triv (fun () -> _StatsHolder.Value.reset()
                                                                     _StatsHolder.Value)
    let _samples                                   = triv (fun () -> _StatsHolder.Value.samples())
    let _skewness                                  = triv (fun () -> _StatsHolder.Value.skewness())
    let _standardDeviation                         = triv (fun () -> _StatsHolder.Value.standardDeviation())

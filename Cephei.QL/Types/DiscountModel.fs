...
*)
    ...
    let _discountImpl                              (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _Discount.Value.discountImpl(i.Value, t.Value))
    let _forwardImpl                               (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _Discount.Value.forwardImpl(i.Value, t.Value))
    let _guess                                     (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _Discount.Value.guess(i.Value, c.Value, validData.Value, f.Value))
    let _initialDate                               (c : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _Discount.Value.initialDate(c.Value))
    let _initialValue                              (c : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _Discount.Value.initialValue(c.Value))
    let _maxIterations                             = cell (fun () -> _Discount.Value.maxIterations())
    let _maxValueAfter                             (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _Discount.Value.maxValueAfter(i.Value, c.Value, validData.Value, f.Value))
    let _minValueAfter                             (i : ICell<int>) (c : ICell<InterpolatedCurve>) (validData : ICell<bool>) (f : ICell<int>)   
                                                   = cell (fun () -> _Discount.Value.minValueAfter(i.Value, c.Value, validData.Value, f.Value))
    let _updateGuess                               (data : ICell<Generic.List<double>>) (discount : ICell<double>) (i : ICell<int>)   
                                                   = cell (fun () -> _Discount.Value.updateGuess(data.Value, discount.Value, i.Value)
                                                                     _Discount.Value)
    let _zeroYieldImpl                             (i : ICell<Interpolation>) (t : ICell<double>)   
                                                   = cell (fun () -> _Discount.Value.zeroYieldImpl(i.Value, t.Value))

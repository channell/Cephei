...
*)
    ...
    let _linkTo                                    (h : ICell<'T>) (registerAsObserver : ICell<bool>)   
                                                   = cell (fun () -> _RelinkableHandle.Value.linkTo(h.Value, registerAsObserver.Value)
                                                                     _RelinkableHandle.Value)
    let _linkTo1                                   (h : ICell<'T>)   
                                                   = cell (fun () -> _RelinkableHandle.Value.linkTo(h.Value)
                                                                     _RelinkableHandle.Value)
    let _currentLink                               = cell (fun () -> _RelinkableHandle.Value.currentLink())
    let _empty                                     = cell (fun () -> _RelinkableHandle.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = cell (fun () -> _RelinkableHandle.Value.Equals(o.Value))
    let _link                                      = cell (fun () -> _RelinkableHandle.Value.link)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _RelinkableHandle.Value.registerWith(handler.Value)
                                                                     _RelinkableHandle.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _RelinkableHandle.Value.unregisterWith(handler.Value)
                                                                     _RelinkableHandle.Value)

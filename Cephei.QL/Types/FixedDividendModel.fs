...
*)
    ...
    let _amount                                    = cell (fun () -> _FixedDividend.Value.amount())
    let _amount1                                   (d : ICell<double>)   
                                                   = cell (fun () -> _FixedDividend.Value.amount(d.Value))
    let _date                                      = cell (fun () -> _FixedDividend.Value.date())
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _FixedDividend.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _FixedDividend.Value.Equals(cf.Value))
    let _exCouponDate                              = cell (fun () -> _FixedDividend.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _FixedDividend.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _FixedDividend.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _FixedDividend.Value.accept(v.Value)
                                                                     _FixedDividend.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _FixedDividend.Value.registerWith(handler.Value)
                                                                     _FixedDividend.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _FixedDividend.Value.unregisterWith(handler.Value)
                                                                     _FixedDividend.Value)

...
(*
    Functions
*)
    let _GaussKronrodNonAdaptive                   = cell (fun () -> new GaussKronrodNonAdaptive (absoluteAccuracy.Value, maxEvaluations.Value, relativeAccuracy.Value))
    let _relativeAccuracy                          = triv (fun () -> _GaussKronrodNonAdaptive.Value.relativeAccuracy())
    let _absoluteAccuracy                          = triv (fun () -> _GaussKronrodNonAdaptive.Value.absoluteAccuracy())
    let _absoluteError                             = triv (fun () -> _GaussKronrodNonAdaptive.Value.absoluteError())
    let _integrationSuccess                        = triv (fun () -> _GaussKronrodNonAdaptive.Value.integrationSuccess())
    let _maxEvaluations                            = triv (fun () -> _GaussKronrodNonAdaptive.Value.maxEvaluations())
    let _numberOfEvaluations                       = triv (fun () -> _GaussKronrodNonAdaptive.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = triv (fun () -> _GaussKronrodNonAdaptive.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                     _GaussKronrodNonAdaptive.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = triv (fun () -> _GaussKronrodNonAdaptive.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                     _GaussKronrodNonAdaptive.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = triv (fun () -> _GaussKronrodNonAdaptive.Value.value(f.Value, a.Value, b.Value))

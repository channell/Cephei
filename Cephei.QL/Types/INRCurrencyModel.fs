...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _INRCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _INRCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _INRCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _INRCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _INRCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _INRCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _INRCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _INRCurrency.Value.symbol)

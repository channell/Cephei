...
*)
    ...
    let _amount                                    = cell (fun () -> _SimpleCashFlow.Value.amount())
    let _date                                      = cell (fun () -> _SimpleCashFlow.Value.date())
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _SimpleCashFlow.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _SimpleCashFlow.Value.Equals(cf.Value))
    let _exCouponDate                              = cell (fun () -> _SimpleCashFlow.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _SimpleCashFlow.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _SimpleCashFlow.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _SimpleCashFlow.Value.accept(v.Value)
                                                                     _SimpleCashFlow.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _SimpleCashFlow.Value.registerWith(handler.Value)
                                                                     _SimpleCashFlow.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _SimpleCashFlow.Value.unregisterWith(handler.Value)
                                                                     _SimpleCashFlow.Value)

...
*)
    ...
    let _trigger                                   = cell (fun () -> _SoftCallability.Value.trigger())
    let _date                                      = cell (fun () -> _SoftCallability.Value.date())
    let _price                                     = cell (fun () -> _SoftCallability.Value.price())
    let _type                                      = cell (fun () -> _SoftCallability.Value.TYPE())
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _SoftCallability.Value.accept(v.Value)
                                                                     _SoftCallability.Value)
    let _hasOccurred                               (d : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _SoftCallability.Value.hasOccurred(d.Value, includeRefDate.Value))
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _SoftCallability.Value.registerWith(handler.Value)
                                                                     _SoftCallability.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _SoftCallability.Value.unregisterWith(handler.Value)
                                                                     _SoftCallability.Value)

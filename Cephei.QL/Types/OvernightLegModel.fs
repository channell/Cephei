...
*)
    ...
    let _value                                     = cell (fun () -> _OvernightLeg.Value.value())
    let _withGearings                              (gearings : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _OvernightLeg.Value.withGearings(gearings.Value))
    let _withGearings1                             (gearing : ICell<double>)   
                                                   = cell (fun () -> _OvernightLeg.Value.withGearings(gearing.Value))
    let _withNotionals                             (notionals : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _OvernightLeg.Value.withNotionals(notionals.Value))
    let _withNotionals1                            (notional : ICell<double>)   
                                                   = cell (fun () -> _OvernightLeg.Value.withNotionals(notional.Value))
    let _withPaymentAdjustment                     (convention : ICell<BusinessDayConvention>)   
                                                   = cell (fun () -> _OvernightLeg.Value.withPaymentAdjustment(convention.Value))
    let _withPaymentDayCounter                     (dayCounter : ICell<DayCounter>)   
                                                   = cell (fun () -> _OvernightLeg.Value.withPaymentDayCounter(dayCounter.Value))
    let _withSpreads                               (spread : ICell<double>)   
                                                   = cell (fun () -> _OvernightLeg.Value.withSpreads(spread.Value))
    let _withSpreads1                              (spreads : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _OvernightLeg.Value.withSpreads(spreads.Value))

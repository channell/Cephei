...
*)
    let mutable
        _Callability                               = make (fun () -> new Callability (price.Value, Type.Value, date.Value))
    let _date                                      = triv _Callability (fun () -> _Callability.Value.date())
    let _price                                     = triv _Callability (fun () -> _Callability.Value.price())
    let _type                                      = triv _Callability (fun () -> _Callability.Value.TYPE())
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = triv _Callability (fun () -> _Callability.Value.accept(v.Value)
                                                                                  _Callability.Value)
    let _hasOccurred                               (d : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = triv _Callability (fun () -> _Callability.Value.hasOccurred(d.Value, includeRefDate.Value))
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _Callability (fun () -> _Callability.Value.registerWith(handler.Value)
                                                                                  _Callability.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _Callability (fun () -> _Callability.Value.unregisterWith(handler.Value)
                                                                                  _Callability.Value)

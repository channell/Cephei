...
*)
    ...
    let _date                                      = cell (fun () -> _Callability.Value.date())
    let _price                                     = cell (fun () -> _Callability.Value.price())
    let _type                                      = cell (fun () -> _Callability.Value.TYPE())
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _Callability.Value.accept(v.Value)
                                                                     _Callability.Value)
    let _hasOccurred                               (d : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _Callability.Value.hasOccurred(d.Value, includeRefDate.Value))
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _Callability.Value.registerWith(handler.Value)
                                                                     _Callability.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _Callability.Value.unregisterWith(handler.Value)
                                                                     _Callability.Value)

...
*)
    ...
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = triv (fun () -> _DZero.Value.applyTo(v.Value))
    let _Clone                                     = triv (fun () -> _DZero.Value.Clone())
    let _diagonal                                  = triv (fun () -> _DZero.Value.diagonal())
    let _identity                                  (size : ICell<int>)   
                                                   = triv (fun () -> _DZero.Value.identity(size.Value))
    let _isTimeDependent                           = triv (fun () -> _DZero.Value.isTimeDependent())
    let _lowerDiagonal                             = triv (fun () -> _DZero.Value.lowerDiagonal())
    let _multiply                                  (a : ICell<double>) (o : ICell<IOperator>)   
                                                   = triv (fun () -> _DZero.Value.multiply(a.Value, o.Value))
    let _setFirstRow                               (valB : ICell<double>) (valC : ICell<double>)   
                                                   = triv (fun () -> _DZero.Value.setFirstRow(valB.Value, valC.Value)
                                                                     _DZero.Value)
    let _setLastRow                                (valA : ICell<double>) (valB : ICell<double>)   
                                                   = triv (fun () -> _DZero.Value.setLastRow(valA.Value, valB.Value)
                                                                     _DZero.Value)
    let _setMidRow                                 (i : ICell<int>) (valA : ICell<double>) (valB : ICell<double>) (valC : ICell<double>)   
                                                   = triv (fun () -> _DZero.Value.setMidRow(i.Value, valA.Value, valB.Value, valC.Value)
                                                                     _DZero.Value)
    let _setMidRows                                (valA : ICell<double>) (valB : ICell<double>) (valC : ICell<double>)   
                                                   = triv (fun () -> _DZero.Value.setMidRows(valA.Value, valB.Value, valC.Value)
                                                                     _DZero.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = triv (fun () -> _DZero.Value.setTime(t.Value)
                                                                     _DZero.Value)
    let _size                                      = triv (fun () -> _DZero.Value.size())
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = triv (fun () -> _DZero.Value.solveFor(rhs.Value))
    let _SOR                                       (rhs : ICell<Vector>) (tol : ICell<double>)   
                                                   = triv (fun () -> _DZero.Value.SOR(rhs.Value, tol.Value))

...
*)
    ...
    let _fixing                                    (aFixingDate : ICell<Date>) (forecastTodaysFixing : ICell<bool>)   
                                                   = triv (fun () -> _EUHICP.Value.fixing(aFixingDate.Value, forecastTodaysFixing.Value))
    let _zeroInflationTermStructure                = triv (fun () -> _EUHICP.Value.zeroInflationTermStructure())
    let _addFixing                                 (fixingDate : ICell<Date>) (fixing : ICell<double>) (forceOverwrite : ICell<bool>)   
                                                   = triv (fun () -> _EUHICP.Value.addFixing(fixingDate.Value, fixing.Value, forceOverwrite.Value)
                                                                     _EUHICP.Value)
    let _availabilityLag                           = triv (fun () -> _EUHICP.Value.availabilityLag())
    let _currency                                  = triv (fun () -> _EUHICP.Value.currency())
    let _familyName                                = triv (fun () -> _EUHICP.Value.familyName())
    let _fixingCalendar                            = triv (fun () -> _EUHICP.Value.fixingCalendar())
    let _frequency                                 = triv (fun () -> _EUHICP.Value.frequency())
    let _interpolated                              = triv (fun () -> _EUHICP.Value.interpolated())
    let _isValidFixingDate                         (fixingDate : ICell<Date>)   
                                                   = triv (fun () -> _EUHICP.Value.isValidFixingDate(fixingDate.Value))
    let _name                                      = triv (fun () -> _EUHICP.Value.name())
    let _region                                    = triv (fun () -> _EUHICP.Value.region())
    let _revised                                   = triv (fun () -> _EUHICP.Value.revised())
    let _update                                    = triv (fun () -> _EUHICP.Value.update()
                                                                     _EUHICP.Value)
    let _addFixings                                (d : ICell<Generic.List<Date>>) (v : ICell<Generic.List<double>>) (forceOverwrite : ICell<bool>)   
                                                   = triv (fun () -> _EUHICP.Value.addFixings(d.Value, v.Value, forceOverwrite.Value)
                                                                     _EUHICP.Value)
    let _addFixings1                               (source : ICell<TimeSeries<Nullable<double>>>) (forceOverwrite : ICell<bool>)   
                                                   = triv (fun () -> _EUHICP.Value.addFixings(source.Value, forceOverwrite.Value)
                                                                     _EUHICP.Value)
    let _allowsNativeFixings                       = triv (fun () -> _EUHICP.Value.allowsNativeFixings())
    let _clearFixings                              = triv (fun () -> _EUHICP.Value.clearFixings()
                                                                     _EUHICP.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _EUHICP.Value.registerWith(handler.Value)
                                                                     _EUHICP.Value)
    let _timeSeries                                = triv (fun () -> _EUHICP.Value.timeSeries())

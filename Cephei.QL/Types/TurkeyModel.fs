...
*)
    let mutable
        _Turkey                                    = make (fun () -> new Turkey ())
    let _addedHolidays                             = triv _Turkey (fun () -> _Turkey.Value.addedHolidays)
    let _addHoliday                                (d : ICell<Date>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.addHoliday(d.Value)
                                                                             _Turkey.Value)
    let _adjust                                    (d : ICell<Date>) (c : ICell<BusinessDayConvention>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.adjust(d.Value, c.Value))
    let _advance                                   (d : ICell<Date>) (p : ICell<Period>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.advance(d.Value, p.Value, c.Value, endOfMonth.Value))
    let _advance1                                  (d : ICell<Date>) (n : ICell<int>) (unit : ICell<TimeUnit>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.advance(d.Value, n.Value, unit.Value, c.Value, endOfMonth.Value))
    let _businessDaysBetween                       (from : ICell<Date>) (To : ICell<Date>) (includeFirst : ICell<bool>) (includeLast : ICell<bool>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.businessDaysBetween(from.Value, To.Value, includeFirst.Value, includeLast.Value))
    let _calendar                                  = triv _Turkey (fun () -> _Turkey.Value.calendar)
    let _empty                                     = triv _Turkey (fun () -> _Turkey.Value.empty())
    let _endOfMonth                                (d : ICell<Date>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.endOfMonth(d.Value))
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.Equals(o.Value))
    let _isBusinessDay                             (d : ICell<Date>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.isBusinessDay(d.Value))
    let _isEndOfMonth                              (d : ICell<Date>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.isEndOfMonth(d.Value))
    let _isHoliday                                 (d : ICell<Date>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.isHoliday(d.Value))
    let _isWeekend                                 (w : ICell<DayOfWeek>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.isWeekend(w.Value))
    let _name                                      = triv _Turkey (fun () -> _Turkey.Value.name())
    let _removedHolidays                           = triv _Turkey (fun () -> _Turkey.Value.removedHolidays)
    let _removeHoliday                             (d : ICell<Date>)   
                                                   = triv _Turkey (fun () -> _Turkey.Value.removeHoliday(d.Value)
                                                                             _Turkey.Value)

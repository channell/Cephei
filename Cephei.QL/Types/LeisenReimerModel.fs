...
*)
    ...
    let _factory                                   (Process : ICell<StochasticProcess1D>) (End : ICell<double>) (steps : ICell<int>) (strike : ICell<double>)   
                                                   = cell (fun () -> _LeisenReimer.Value.factory(Process.Value, End.Value, steps.Value, strike.Value))
    let _probability                               (i : ICell<int>) (j : ICell<int>) (branch : ICell<int>)   
                                                   = cell (fun () -> _LeisenReimer.Value.probability(i.Value, j.Value, branch.Value))
    let _underlying                                (i : ICell<int>) (index : ICell<int>)   
                                                   = cell (fun () -> _LeisenReimer.Value.underlying(i.Value, index.Value))
    let _descendant                                (x : ICell<int>) (index : ICell<int>) (branch : ICell<int>)   
                                                   = cell (fun () -> _LeisenReimer.Value.descendant(x.Value, index.Value, branch.Value))
    let _size                                      (i : ICell<int>)   
                                                   = cell (fun () -> _LeisenReimer.Value.size(i.Value))
    let _columns                                   = cell (fun () -> _LeisenReimer.Value.columns())

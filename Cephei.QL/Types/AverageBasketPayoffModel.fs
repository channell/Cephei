...
*)
    ...
    let _accumulate                                (a : ICell<Vector>)   
                                                   = cell (fun () -> _AverageBasketPayoff.Value.accumulate(a.Value))
    let _basePayoff                                = cell (fun () -> _AverageBasketPayoff.Value.basePayoff())
    let _description                               = cell (fun () -> _AverageBasketPayoff.Value.description())
    let _name                                      = cell (fun () -> _AverageBasketPayoff.Value.name())
    let _value                                     (price : ICell<double>)   
                                                   = cell (fun () -> _AverageBasketPayoff.Value.value(price.Value))
    let _value1                                    (a : ICell<Vector>)   
                                                   = cell (fun () -> _AverageBasketPayoff.Value.value(a.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _AverageBasketPayoff.Value.accept(v.Value)

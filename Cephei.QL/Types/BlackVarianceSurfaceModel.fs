...
*)
    ...
    let _maxStrike                                 = triv (fun () -> _BlackVarianceSurface.Value.maxStrike())
    let _minStrike                                 = triv (fun () -> _BlackVarianceSurface.Value.minStrike())
    let _setInterpolation                          (i : ICell<'Interpolator>)   
                                                   = triv (fun () -> _BlackVarianceSurface.Value.setInterpolation(i.Value)
                                                                     _BlackVarianceSurface.Value)
    let _strikes                                   = triv (fun () -> _BlackVarianceSurface.Value.strikes())
    let _times                                     = triv (fun () -> _BlackVarianceSurface.Value.times())

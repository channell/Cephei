...
*)
    ...
    let _gradient                                  (grad_f : ICell<Vector ref>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LeastSquareFunction.Value.gradient(grad_f.Value, x.Value)
                                                                     _LeastSquareFunction.Value)
    let _value                                     (x : ICell<Vector>)   
                                                   = cell (fun () -> _LeastSquareFunction.Value.value(x.Value))
    let _valueAndGradient                          (grad_f : ICell<Vector ref>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LeastSquareFunction.Value.valueAndGradient(grad_f.Value, x.Value))
    let _values                                    (x : ICell<Vector>)   
                                                   = cell (fun () -> _LeastSquareFunction.Value.values(x.Value))
    let _finiteDifferenceEpsilon                   = cell (fun () -> _LeastSquareFunction.Value.finiteDifferenceEpsilon())
    let _jacobian                                  (jac : ICell<Matrix>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LeastSquareFunction.Value.jacobian(jac.Value, x.Value)
                                                                     _LeastSquareFunction.Value)
    let _valuesAndJacobian                         (jac : ICell<Matrix>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LeastSquareFunction.Value.valuesAndJacobian(jac.Value, x.Value))

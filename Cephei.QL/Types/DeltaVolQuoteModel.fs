...
*)
    ...
    let _isValid                                   = triv (fun () -> _DeltaVolQuote.Value.isValid())
    let _maturity                                  = triv (fun () -> _DeltaVolQuote.Value.maturity())
    let _update                                    = triv (fun () -> _DeltaVolQuote.Value.update()
                                                                     _DeltaVolQuote.Value)
    let _value                                     = triv (fun () -> _DeltaVolQuote.Value.value())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _DeltaVolQuote.Value.registerWith(handler.Value)
                                                                     _DeltaVolQuote.Value)

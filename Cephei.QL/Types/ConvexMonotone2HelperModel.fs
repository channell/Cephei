...
*)
    let mutable
        _ConvexMonotone2Helper                     = make (fun () -> new ConvexMonotone2Helper (xPrev.Value, xNext.Value, gPrev.Value, gNext.Value, fAverage.Value, eta2.Value, prevPrimitive.Value))
    let _fNext                                     = triv _ConvexMonotone2Helper (fun () -> _ConvexMonotone2Helper.Value.fNext())
    let _primitive                                 (x : ICell<double>)   
                                                   = triv _ConvexMonotone2Helper (fun () -> _ConvexMonotone2Helper.Value.primitive(x.Value))
    let _value                                     (x : ICell<double>)   
                                                   = triv _ConvexMonotone2Helper (fun () -> _ConvexMonotone2Helper.Value.value(x.Value))

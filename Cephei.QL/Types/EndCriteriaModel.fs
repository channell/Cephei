...
*)
    ...
    let _checkMaxIterations                        (iteration : ICell<int>) (ecType : ICell<EndCriteria.Type ref>)   
                                                   = cell (fun () -> _EndCriteria.Value.checkMaxIterations(iteration.Value, ecType.Value))
    let _checkStationaryFunctionAccuracy           (f : ICell<double>) (positiveOptimization : ICell<bool>) (ecType : ICell<EndCriteria.Type ref>)   
                                                   = cell (fun () -> _EndCriteria.Value.checkStationaryFunctionAccuracy(f.Value, positiveOptimization.Value, ecType.Value))
    ...
    let _checkZeroGradientNorm                     (gradientNorm : ICell<double>) (ecType : ICell<EndCriteria.Type ref>)   
                                                   = cell (fun () -> _EndCriteria.Value.checkZeroGradientNorm(gradientNorm.Value, ecType.Value))
    let _functionEpsilon                           = cell (fun () -> _EndCriteria.Value.functionEpsilon())
    let _gradientNormEpsilon                       = cell (fun () -> _EndCriteria.Value.gradientNormEpsilon())
    let _maxIterations                             = cell (fun () -> _EndCriteria.Value.maxIterations())
    let _maxStationaryStateIterations              = cell (fun () -> _EndCriteria.Value.maxStationaryStateIterations())
    let _rootEpsilon                               = cell (fun () -> _EndCriteria.Value.rootEpsilon())

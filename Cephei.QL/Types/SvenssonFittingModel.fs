...
*)
    ...
    let _clone                                     = cell (fun () -> _SvenssonFitting.Value.clone())
    let _size                                      = cell (fun () -> _SvenssonFitting.Value.size())
    let _constrainAtZero                           = cell (fun () -> _SvenssonFitting.Value.constrainAtZero())
    let _discount                                  (x : ICell<Vector>) (t : ICell<double>)   
                                                   = cell (fun () -> _SvenssonFitting.Value.discount(x.Value, t.Value))
    let _minimumCostValue                          = cell (fun () -> _SvenssonFitting.Value.minimumCostValue())
    let _numberOfIterations                        = cell (fun () -> _SvenssonFitting.Value.numberOfIterations())
    let _optimizationMethod                        = cell (fun () -> _SvenssonFitting.Value.optimizationMethod())
    let _solution                                  = cell (fun () -> _SvenssonFitting.Value.solution())
    let _weights                                   = cell (fun () -> _SvenssonFitting.Value.weights())

...
*)
    ...
    let _addSequence                               (data : ICell<Generic.List<double>>) (weight : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.addSequence(data.Value, weight.Value)
                                                                     _GenericRiskStatistics.Value)
    let _averageShortfall                          (target : ICell<double>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.averageShortfall(target.Value))
    let _downsideDeviation                         = triv (fun () -> _GenericRiskStatistics.Value.downsideDeviation())
    let _downsideVariance                          = triv (fun () -> _GenericRiskStatistics.Value.downsideVariance())
    let _errorEstimate                             = triv (fun () -> _GenericRiskStatistics.Value.errorEstimate())
    let _expectationValue                          (f : ICell<Func<KeyValuePair<double,double>,double>>) (inRange : ICell<Func<KeyValuePair<double,double>,bool>>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.expectationValue(f.Value, inRange.Value))
    let _expectedShortfall                         (centile : ICell<double>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.expectedShortfall(centile.Value))
    let _kurtosis                                  = triv (fun () -> _GenericRiskStatistics.Value.kurtosis())
    let _max                                       = triv (fun () -> _GenericRiskStatistics.Value.max())
    let _mean                                      = triv (fun () -> _GenericRiskStatistics.Value.mean())
    let _min                                       = triv (fun () -> _GenericRiskStatistics.Value.min())
    let _percentile                                (percent : ICell<double>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.percentile(percent.Value))
    let _potentialUpside                           (centile : ICell<double>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.potentialUpside(centile.Value))
    let _regret                                    (target : ICell<double>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.regret(target.Value))
    let _reset                                     = triv (fun () -> _GenericRiskStatistics.Value.reset()
                                                                     _GenericRiskStatistics.Value)
    let _samples                                   = triv (fun () -> _GenericRiskStatistics.Value.samples())
    let _semiDeviation                             = triv (fun () -> _GenericRiskStatistics.Value.semiDeviation())
    let _semiVariance                              = triv (fun () -> _GenericRiskStatistics.Value.semiVariance())
    let _shortfall                                 (target : ICell<double>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.shortfall(target.Value))
    let _skewness                                  = triv (fun () -> _GenericRiskStatistics.Value.skewness())
    let _standardDeviation                         = triv (fun () -> _GenericRiskStatistics.Value.standardDeviation())
    let _valueAtRisk                               (centile : ICell<double>)   
                                                   = triv (fun () -> _GenericRiskStatistics.Value.valueAtRisk(centile.Value))
    let _variance                                  = triv (fun () -> _GenericRiskStatistics.Value.variance())
    let _weightSum                                 = triv (fun () -> _GenericRiskStatistics.Value.weightSum())

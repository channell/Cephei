...
*)
    ...
    let _dt                                        = cell (fun () -> _OvernightIndexedCoupon.Value.dt())
    let _fixingDates                               = cell (fun () -> _OvernightIndexedCoupon.Value.fixingDates())
    let _indexFixings                              = cell (fun () -> _OvernightIndexedCoupon.Value.indexFixings())
    let _valueDates                                = cell (fun () -> _OvernightIndexedCoupon.Value.valueDates())
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.accruedAmount(d.Value))
    let _adjustedFixing                            = cell (fun () -> _OvernightIndexedCoupon.Value.adjustedFixing)
    let _amount                                    = cell (fun () -> _OvernightIndexedCoupon.Value.amount())
    let _convexityAdjustment                       = cell (fun () -> _OvernightIndexedCoupon.Value.convexityAdjustment())
    let _dayCounter                                = cell (fun () -> _OvernightIndexedCoupon.Value.dayCounter())
    let _factory                                   (nominal : ICell<double>) (paymentDate : ICell<Date>) (startDate : ICell<Date>) (endDate : ICell<Date>) (fixingDays : ICell<int>) (index : ICell<InterestRateIndex>) (gearing : ICell<double>) (spread : ICell<double>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>) (dayCounter : ICell<DayCounter>) (isInArrears : ICell<bool>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.factory(nominal.Value, paymentDate.Value, startDate.Value, endDate.Value, fixingDays.Value, index.Value, gearing.Value, spread.Value, refPeriodStart.Value, refPeriodEnd.Value, dayCounter.Value, isInArrears.Value))
    let _fixingDate                                = cell (fun () -> _OvernightIndexedCoupon.Value.fixingDate())
    let _fixingDays                                = cell (fun () -> _OvernightIndexedCoupon.Value.fixingDays)
    let _gearing                                   = cell (fun () -> _OvernightIndexedCoupon.Value.gearing())
    let _index                                     = cell (fun () -> _OvernightIndexedCoupon.Value.index())
    let _indexFixing                               = cell (fun () -> _OvernightIndexedCoupon.Value.indexFixing())
    let _isInArrears                               = cell (fun () -> _OvernightIndexedCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.price(yts.Value))
    let _pricer                                    = cell (fun () -> _OvernightIndexedCoupon.Value.pricer())
    let _rate                                      = cell (fun () -> _OvernightIndexedCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.setPricer(pricer.Value)
                                                                     _OvernightIndexedCoupon.Value)
    let _spread                                    = cell (fun () -> _OvernightIndexedCoupon.Value.spread())
    let _update                                    = cell (fun () -> _OvernightIndexedCoupon.Value.update()
                                                                     _OvernightIndexedCoupon.Value)
    let _accrualDays                               = cell (fun () -> _OvernightIndexedCoupon.Value.accrualDays())
    let _accrualEndDate                            = cell (fun () -> _OvernightIndexedCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = cell (fun () -> _OvernightIndexedCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = cell (fun () -> _OvernightIndexedCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = cell (fun () -> _OvernightIndexedCoupon.Value.date())
    let _exCouponDate                              = cell (fun () -> _OvernightIndexedCoupon.Value.exCouponDate())
    let _nominal                                   = cell (fun () -> _OvernightIndexedCoupon.Value.nominal())
    let _referencePeriodEnd                        = cell (fun () -> _OvernightIndexedCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = cell (fun () -> _OvernightIndexedCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.accept(v.Value)
                                                                     _OvernightIndexedCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.registerWith(handler.Value)
                                                                     _OvernightIndexedCoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _OvernightIndexedCoupon.Value.unregisterWith(handler.Value)
                                                                     _OvernightIndexedCoupon.Value)

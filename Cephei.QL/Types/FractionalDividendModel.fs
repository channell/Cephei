...
*)
    ...
    let _nominal                                   = triv (fun () -> _FractionalDividend.Value.nominal())
    let _rate                                      = triv (fun () -> _FractionalDividend.Value.rate())
    let _date                                      = triv (fun () -> _FractionalDividend.Value.date())
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = triv (fun () -> _FractionalDividend.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = triv (fun () -> _FractionalDividend.Value.Equals(cf.Value))
    let _exCouponDate                              = triv (fun () -> _FractionalDividend.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = triv (fun () -> _FractionalDividend.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = triv (fun () -> _FractionalDividend.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = triv (fun () -> _FractionalDividend.Value.accept(v.Value)
                                                                     _FractionalDividend.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _FractionalDividend.Value.registerWith(handler.Value)
                                                                     _FractionalDividend.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv (fun () -> _FractionalDividend.Value.unregisterWith(handler.Value)
                                                                     _FractionalDividend.Value)

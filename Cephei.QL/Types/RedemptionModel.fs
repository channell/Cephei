...
*)
    let mutable
        _Redemption                                = make (fun () -> new Redemption (amount.Value, date.Value))
    let _amount                                    = triv _Redemption (fun () -> _Redemption.Value.amount())
    let _date                                      = triv _Redemption (fun () -> _Redemption.Value.date())
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = triv _Redemption (fun () -> _Redemption.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = triv _Redemption (fun () -> _Redemption.Value.Equals(cf.Value))
    let _exCouponDate                              = triv _Redemption (fun () -> _Redemption.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = triv _Redemption (fun () -> _Redemption.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = triv _Redemption (fun () -> _Redemption.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = triv _Redemption (fun () -> _Redemption.Value.accept(v.Value)
                                                                                 _Redemption.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _Redemption (fun () -> _Redemption.Value.registerWith(handler.Value)
                                                                                 _Redemption.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _Redemption (fun () -> _Redemption.Value.unregisterWith(handler.Value)
                                                                                 _Redemption.Value)

...
*)
    ...
    let _amount                                    = cell (fun () -> _Redemption.Value.amount())
    let _date                                      = cell (fun () -> _Redemption.Value.date())
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _Redemption.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _Redemption.Value.Equals(cf.Value))
    let _exCouponDate                              = cell (fun () -> _Redemption.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _Redemption.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _Redemption.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _Redemption.Value.accept(v.Value)
                                                                     _Redemption.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _Redemption.Value.registerWith(handler.Value)
                                                                     _Redemption.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _Redemption.Value.unregisterWith(handler.Value)
                                                                     _Redemption.Value)

...
*)
    ...
    let _mandatoryTimes                            = triv (fun () -> _DiscretizedBarrierOption.Value.mandatoryTimes())
    let _reset                                     (size : ICell<int>)   
                                                   = triv (fun () -> _DiscretizedBarrierOption.Value.reset(size.Value)
                                                                     _DiscretizedBarrierOption.Value)
    let _vanilla                                   = triv (fun () -> _DiscretizedBarrierOption.Value.vanilla())
    let _adjustValues                              = triv (fun () -> _DiscretizedBarrierOption.Value.adjustValues()
                                                                     _DiscretizedBarrierOption.Value)
    let _initialize                                (Method : ICell<Lattice>) (t : ICell<double>)   
                                                   = triv (fun () -> _DiscretizedBarrierOption.Value.initialize(Method.Value, t.Value)
                                                                     _DiscretizedBarrierOption.Value)
    let _method                                    = triv (fun () -> _DiscretizedBarrierOption.Value.METHOD())
    let _partialRollback                           (To : ICell<double>)   
                                                   = triv (fun () -> _DiscretizedBarrierOption.Value.partialRollback(To.Value)
                                                                     _DiscretizedBarrierOption.Value)
    let _postAdjustValues                          = triv (fun () -> _DiscretizedBarrierOption.Value.postAdjustValues()
                                                                     _DiscretizedBarrierOption.Value)
    let _preAdjustValues                           = triv (fun () -> _DiscretizedBarrierOption.Value.preAdjustValues()
                                                                     _DiscretizedBarrierOption.Value)
    let _presentValue                              = triv (fun () -> _DiscretizedBarrierOption.Value.presentValue())
    let _rollback                                  (To : ICell<double>)   
                                                   = triv (fun () -> _DiscretizedBarrierOption.Value.rollback(To.Value)
                                                                     _DiscretizedBarrierOption.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = triv (fun () -> _DiscretizedBarrierOption.Value.setTime(t.Value)
                                                                     _DiscretizedBarrierOption.Value)
    let _setValues                                 (v : ICell<Vector>)   
                                                   = triv (fun () -> _DiscretizedBarrierOption.Value.setValues(v.Value)
                                                                     _DiscretizedBarrierOption.Value)
    let _time                                      = triv (fun () -> _DiscretizedBarrierOption.Value.time())
    let _values                                    = triv (fun () -> _DiscretizedBarrierOption.Value.values())
    do this.Bind(_DiscretizedBarrierOption)
(* 
    casting 
*)
    internal new () = DiscretizedBarrierOptionModel(null,null,null)

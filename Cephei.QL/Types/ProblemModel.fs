...
*)
    ...
    let _constraint                                = cell (fun () -> _Problem.Value.CONSTRAINT())
    let _costFunction                              = cell (fun () -> _Problem.Value.costFunction())
    let _currentValue                              = cell (fun () -> _Problem.Value.currentValue())
    let _functionEvaluation                        = cell (fun () -> _Problem.Value.functionEvaluation())
    let _functionValue                             = cell (fun () -> _Problem.Value.functionValue())
    let _gradient                                  (grad_f : ICell<Vector ref>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _Problem.Value.gradient(grad_f.Value, x.Value)
                                                                     _Problem.Value)
    let _gradientEvaluation                        = cell (fun () -> _Problem.Value.gradientEvaluation())
    let _gradientNormValue                         = cell (fun () -> _Problem.Value.gradientNormValue())
    let _reset                                     = cell (fun () -> _Problem.Value.reset()
                                                                     _Problem.Value)
    let _setCurrentValue                           (currentValue : ICell<Vector>)   
                                                   = cell (fun () -> _Problem.Value.setCurrentValue(currentValue.Value)
                                                                     _Problem.Value)
    let _setFunctionValue                          (functionValue : ICell<double>)   
                                                   = cell (fun () -> _Problem.Value.setFunctionValue(functionValue.Value)
                                                                     _Problem.Value)
    let _setGradientNormValue                      (squaredNorm : ICell<double>)   
                                                   = cell (fun () -> _Problem.Value.setGradientNormValue(squaredNorm.Value)
                                                                     _Problem.Value)
    let _value                                     (x : ICell<Vector>)   
                                                   = cell (fun () -> _Problem.Value.value(x.Value))
    let _valueAndGradient                          (grad_f : ICell<Vector ref>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _Problem.Value.valueAndGradient(grad_f.Value, x.Value))
    let _values                                    (x : ICell<Vector>)   
                                                   = cell (fun () -> _Problem.Value.values(x.Value))

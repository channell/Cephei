...
*)
    let mutable
        _Problem                                   = make (fun () -> new Problem (costFunction.Value, Constraint.Value, initialValue.Value))
    let _constraint                                = triv _Problem (fun () -> _Problem.Value.CONSTRAINT())
    let _costFunction                              = triv _Problem (fun () -> _Problem.Value.costFunction())
    let _currentValue                              = triv _Problem (fun () -> _Problem.Value.currentValue())
    let _functionEvaluation                        = triv _Problem (fun () -> _Problem.Value.functionEvaluation())
    let _functionValue                             = triv _Problem (fun () -> _Problem.Value.functionValue())
    let _gradient                                  (grad_f : ICell<Vector>) (x : ICell<Vector>)   
                                                   = triv _Problem (fun () -> _Problem.Value.gradient(ref grad_f.Value, x.Value)
                                                                              _Problem.Value)
    let _gradientEvaluation                        = triv _Problem (fun () -> _Problem.Value.gradientEvaluation())
    let _gradientNormValue                         = triv _Problem (fun () -> _Problem.Value.gradientNormValue())
    let _reset                                     = triv _Problem (fun () -> _Problem.Value.reset()
                                                                              _Problem.Value)
    let _setCurrentValue                           (currentValue : ICell<Vector>)   
                                                   = triv _Problem (fun () -> _Problem.Value.setCurrentValue(currentValue.Value)
                                                                              _Problem.Value)
    let _setFunctionValue                          (functionValue : ICell<double>)   
                                                   = triv _Problem (fun () -> _Problem.Value.setFunctionValue(functionValue.Value)
                                                                              _Problem.Value)
    let _setGradientNormValue                      (squaredNorm : ICell<double>)   
                                                   = triv _Problem (fun () -> _Problem.Value.setGradientNormValue(squaredNorm.Value)
                                                                              _Problem.Value)
    let _value                                     (x : ICell<Vector>)   
                                                   = triv _Problem (fun () -> _Problem.Value.value(x.Value))
    let _valueAndGradient                          (grad_f : ICell<Vector>) (x : ICell<Vector>)   
                                                   = triv _Problem (fun () -> _Problem.Value.valueAndGradient(ref grad_f.Value, x.Value))
    let _values                                    (x : ICell<Vector>)   
                                                   = triv _Problem (fun () -> _Problem.Value.values(x.Value))

...
*)
    ...
    let _clone                                     = cell (fun () -> _SimplePolynomialFitting.Value.clone())
    let _size                                      = cell (fun () -> _SimplePolynomialFitting.Value.size())
    let _constrainAtZero                           = cell (fun () -> _SimplePolynomialFitting.Value.constrainAtZero())
    let _discount                                  (x : ICell<Vector>) (t : ICell<double>)   
                                                   = cell (fun () -> _SimplePolynomialFitting.Value.discount(x.Value, t.Value))
    let _minimumCostValue                          = cell (fun () -> _SimplePolynomialFitting.Value.minimumCostValue())
    let _numberOfIterations                        = cell (fun () -> _SimplePolynomialFitting.Value.numberOfIterations())
    let _optimizationMethod                        = cell (fun () -> _SimplePolynomialFitting.Value.optimizationMethod())
    let _solution                                  = cell (fun () -> _SimplePolynomialFitting.Value.solution())
    let _weights                                   = cell (fun () -> _SimplePolynomialFitting.Value.weights())

...
(*
    Functions
*)
    let _ConvertibleFloatingRateBond               = triv (fun () -> withEngine _pricingEngine.Value (new ConvertibleFloatingRateBond (exercise.Value, conversionRatio.Value, dividends.Value, callability.Value, creditSpread.Value, issueDate.Value, settlementDays.Value, index.Value, fixingDays.Value, spreads.Value, dayCounter.Value, schedule.Value, redemption.Value)))

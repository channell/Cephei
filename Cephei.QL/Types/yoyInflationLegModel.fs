...
*)
    ...
    let _value                                     = cell (fun () -> _yoyInflationLeg.Value.value())
    let _withCaps                                  (cap : ICell<double>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withCaps(cap.Value))
    let _withCaps1                                 (caps : ICell<List<Nullable<double>>>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withCaps(caps.Value))
    let _withFixingDays                            (fixingDays : ICell<Generic.List<int>>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withFixingDays(fixingDays.Value))
    let _withFixingDays1                           (fixingDays : ICell<int>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withFixingDays(fixingDays.Value))
    let _withFloors                                (floor : ICell<double>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withFloors(floor.Value))
    let _withFloors1                               (floors : ICell<List<Nullable<double>>>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withFloors(floors.Value))
    let _withGearings                              (gearings : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withGearings(gearings.Value))
    let _withGearings1                             (gearing : ICell<double>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withGearings(gearing.Value))
    let _withPaymentDayCounter                     (dayCounter : ICell<DayCounter>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withPaymentDayCounter(dayCounter.Value))
    let _withSpreads                               (spread : ICell<double>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withSpreads(spread.Value))
    let _withSpreads1                              (spreads : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withSpreads(spreads.Value))
    let _withNotionals                             (notional : ICell<double>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withNotionals(notional.Value))
    let _withNotionals1                            (notionals : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withNotionals(notionals.Value))
    let _withPaymentAdjustment                     (convention : ICell<BusinessDayConvention>)   
                                                   = cell (fun () -> _yoyInflationLeg.Value.withPaymentAdjustment(convention.Value))

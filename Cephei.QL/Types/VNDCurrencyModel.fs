...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _VNDCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _VNDCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _VNDCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _VNDCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _VNDCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _VNDCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _VNDCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _VNDCurrency.Value.symbol)

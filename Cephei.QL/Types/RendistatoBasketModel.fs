...
*)
    ...
    let _btps                                      = cell (fun () -> _RendistatoBasket.Value.btps())
    let _cleanPriceQuotes                          = cell (fun () -> _RendistatoBasket.Value.cleanPriceQuotes())
    let _outstanding                               = cell (fun () -> _RendistatoBasket.Value.outstanding())
    let _outstandings                              = cell (fun () -> _RendistatoBasket.Value.outstandings())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _RendistatoBasket.Value.registerWith(handler.Value)
                                                                     _RendistatoBasket.Value)
    let _size                                      = cell (fun () -> _RendistatoBasket.Value.size())
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _RendistatoBasket.Value.unregisterWith(handler.Value)
                                                                     _RendistatoBasket.Value)
    let _update                                    = cell (fun () -> _RendistatoBasket.Value.update()
                                                                     _RendistatoBasket.Value)
    let _weights                                   = cell (fun () -> _RendistatoBasket.Value.weights())

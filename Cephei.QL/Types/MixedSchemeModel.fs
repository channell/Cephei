...
*)
    ...
    let _setStep                                   (dt : ICell<double>)   
                                                   = cell (fun () -> _MixedScheme.Value.setStep(dt.Value)
                                                                     _MixedScheme.Value)
    let _step                                      (o : ICell<Object>) (t : ICell<double>) (theta : ICell<double>)   
                                                   = cell (fun () -> _MixedScheme.Value.step(ref o.Value, t.Value, theta.Value)
                                                                     _MixedScheme.Value)
...
*)
    ...
    let _step                                      (o : ICell<Object ref>) (t : ICell<double>) (theta : ICell<double>)   
                                                   = cell (fun () -> _MixedScheme.Value.step(o.Value, t.Value, theta.Value)
                                                                     _MixedScheme.Value)
    do this.Bind(_MixedScheme)

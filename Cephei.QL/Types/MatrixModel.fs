...
*)
    ...
    let _column                                    (c : ICell<int>)   
                                                   = cell (fun () -> _Matrix.Value.column(c.Value))
    let _columns                                   = cell (fun () -> _Matrix.Value.columns())
    let _diagonal                                  = cell (fun () -> _Matrix.Value.diagonal())
    let _empty                                     = cell (fun () -> _Matrix.Value.empty())
    let _fill                                      (value : ICell<double>)   
                                                   = cell (fun () -> _Matrix.Value.fill(value.Value)
                                                                     _Matrix.Value)
    let _GetRange                                  (start : ICell<int>) (length : ICell<int>)   
                                                   = cell (fun () -> _Matrix.Value.GetRange(start.Value, length.Value))
    let _row                                       (r : ICell<int>)   
                                                   = cell (fun () -> _Matrix.Value.row(r.Value))
    let _rows                                      = cell (fun () -> _Matrix.Value.rows())
    let _swap                                      (i1 : ICell<int>) (j1 : ICell<int>) (i2 : ICell<int>) (j2 : ICell<int>)   
                                                   = cell (fun () -> _Matrix.Value.swap(i1.Value, j1.Value, i2.Value, j2.Value)
                                                                     _Matrix.Value)
    let _swapRow                                   (r1 : ICell<int>) (r2 : ICell<int>)   
                                                   = cell (fun () -> _Matrix.Value.swapRow(r1.Value, r2.Value)
                                                                     _Matrix.Value)
    let _this                                      (i : ICell<int>)   
                                                   = cell (fun () -> _Matrix.Value.[i.Value])
    let _this1                                     (i : ICell<int>) (j : ICell<int>)   
                                                   = cell (fun () -> _Matrix.Value.[i.Value, j.Value])
    let _ToString                                  = cell (fun () -> _Matrix.Value.ToString())

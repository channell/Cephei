...
*)
    ...
    let _convexityAdjustment                       = triv (fun () -> _CappedFlooredCmsCoupon.Value.convexityAdjustment())
    let _effectiveCap                              = triv (fun () -> _CappedFlooredCmsCoupon.Value.effectiveCap())
    let _effectiveFloor                            = triv (fun () -> _CappedFlooredCmsCoupon.Value.effectiveFloor())
    let _floor                                     = triv (fun () -> _CappedFlooredCmsCoupon.Value.floor())
    let _isCapped                                  = triv (fun () -> _CappedFlooredCmsCoupon.Value.isCapped())
    let _isFloored                                 = triv (fun () -> _CappedFlooredCmsCoupon.Value.isFloored())
    let _rate                                      = triv (fun () -> _CappedFlooredCmsCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.setPricer(pricer.Value)
                                                                     _CappedFlooredCmsCoupon.Value)
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.accruedAmount(d.Value))
    let _adjustedFixing                            = triv (fun () -> _CappedFlooredCmsCoupon.Value.adjustedFixing)
    let _amount                                    = triv (fun () -> _CappedFlooredCmsCoupon.Value.amount())
    let _dayCounter                                = triv (fun () -> _CappedFlooredCmsCoupon.Value.dayCounter())
    let _fixingDate                                = triv (fun () -> _CappedFlooredCmsCoupon.Value.fixingDate())
    let _fixingDays                                = triv (fun () -> _CappedFlooredCmsCoupon.Value.fixingDays)
    let _gearing                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.gearing())
    let _index                                     = triv (fun () -> _CappedFlooredCmsCoupon.Value.index())
    let _indexFixing                               = triv (fun () -> _CappedFlooredCmsCoupon.Value.indexFixing())
    let _isInArrears                               = triv (fun () -> _CappedFlooredCmsCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.price(yts.Value))
    let _pricer                                    = triv (fun () -> _CappedFlooredCmsCoupon.Value.pricer())
    let _spread                                    = triv (fun () -> _CappedFlooredCmsCoupon.Value.spread())
    let _update                                    = triv (fun () -> _CappedFlooredCmsCoupon.Value.update()
                                                                     _CappedFlooredCmsCoupon.Value)
    let _accrualDays                               = triv (fun () -> _CappedFlooredCmsCoupon.Value.accrualDays())
    let _accrualEndDate                            = triv (fun () -> _CappedFlooredCmsCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = triv (fun () -> _CappedFlooredCmsCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = triv (fun () -> _CappedFlooredCmsCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = triv (fun () -> _CappedFlooredCmsCoupon.Value.date())
    let _exCouponDate                              = triv (fun () -> _CappedFlooredCmsCoupon.Value.exCouponDate())
    let _nominal                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.nominal())
    let _referencePeriodEnd                        = triv (fun () -> _CappedFlooredCmsCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = triv (fun () -> _CappedFlooredCmsCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.accept(v.Value)
                                                                     _CappedFlooredCmsCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.registerWith(handler.Value)
                                                                     _CappedFlooredCmsCoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv (fun () -> _CappedFlooredCmsCoupon.Value.unregisterWith(handler.Value)
                                                                     _CappedFlooredCmsCoupon.Value)

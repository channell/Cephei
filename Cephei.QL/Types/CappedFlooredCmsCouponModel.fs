...
*)
    ...
    let _factory                                   (nominal : ICell<double>) (paymentDate : ICell<Date>) (startDate : ICell<Date>) (endDate : ICell<Date>) (fixingDays : ICell<int>) (index : ICell<InterestRateIndex>) (gearing : ICell<double>) (spread : ICell<double>) (cap : ICell<Nullable<double>>) (floor : ICell<Nullable<double>>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>) (dayCounter : ICell<DayCounter>) (isInArrears : ICell<bool>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.factory(nominal.Value, paymentDate.Value, startDate.Value, endDate.Value, fixingDays.Value, index.Value, gearing.Value, spread.Value, cap.Value, floor.Value, refPeriodStart.Value, refPeriodEnd.Value, dayCounter.Value, isInArrears.Value))
    let _cap                                       = cell (fun () -> _CappedFlooredCmsCoupon.Value.cap())
    let _convexityAdjustment                       = cell (fun () -> _CappedFlooredCmsCoupon.Value.convexityAdjustment())
    let _effectiveCap                              = cell (fun () -> _CappedFlooredCmsCoupon.Value.effectiveCap())
    let _effectiveFloor                            = cell (fun () -> _CappedFlooredCmsCoupon.Value.effectiveFloor())
    let _floor                                     = cell (fun () -> _CappedFlooredCmsCoupon.Value.floor())
    let _isCapped                                  = cell (fun () -> _CappedFlooredCmsCoupon.Value.isCapped())
    let _isFloored                                 = cell (fun () -> _CappedFlooredCmsCoupon.Value.isFloored())
    let _rate                                      = cell (fun () -> _CappedFlooredCmsCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.setPricer(pricer.Value)
                                                                     _CappedFlooredCmsCoupon.Value)
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.accruedAmount(d.Value))
    let _adjustedFixing                            = cell (fun () -> _CappedFlooredCmsCoupon.Value.adjustedFixing)
    let _amount                                    = cell (fun () -> _CappedFlooredCmsCoupon.Value.amount())
    let _dayCounter                                = cell (fun () -> _CappedFlooredCmsCoupon.Value.dayCounter())
    let _fixingDate                                = cell (fun () -> _CappedFlooredCmsCoupon.Value.fixingDate())
    let _fixingDays                                = cell (fun () -> _CappedFlooredCmsCoupon.Value.fixingDays)
    let _gearing                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.gearing())
    let _index                                     = cell (fun () -> _CappedFlooredCmsCoupon.Value.index())
    let _indexFixing                               = cell (fun () -> _CappedFlooredCmsCoupon.Value.indexFixing())
    let _isInArrears                               = cell (fun () -> _CappedFlooredCmsCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.price(yts.Value))
    let _pricer                                    = cell (fun () -> _CappedFlooredCmsCoupon.Value.pricer())
    let _spread                                    = cell (fun () -> _CappedFlooredCmsCoupon.Value.spread())
    let _update                                    = cell (fun () -> _CappedFlooredCmsCoupon.Value.update()
                                                                     _CappedFlooredCmsCoupon.Value)
    let _accrualDays                               = cell (fun () -> _CappedFlooredCmsCoupon.Value.accrualDays())
    let _accrualEndDate                            = cell (fun () -> _CappedFlooredCmsCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = cell (fun () -> _CappedFlooredCmsCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = cell (fun () -> _CappedFlooredCmsCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = cell (fun () -> _CappedFlooredCmsCoupon.Value.date())
    let _exCouponDate                              = cell (fun () -> _CappedFlooredCmsCoupon.Value.exCouponDate())
    let _nominal                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.nominal())
    let _referencePeriodEnd                        = cell (fun () -> _CappedFlooredCmsCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = cell (fun () -> _CappedFlooredCmsCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.accept(v.Value)
                                                                     _CappedFlooredCmsCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.registerWith(handler.Value)
                                                                     _CappedFlooredCmsCoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CappedFlooredCmsCoupon.Value.unregisterWith(handler.Value)
                                                                     _CappedFlooredCmsCoupon.Value)

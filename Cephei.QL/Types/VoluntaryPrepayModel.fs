...
*)
    ...
    let _amount                                    = cell (fun () -> _VoluntaryPrepay.Value.amount())
    let _date                                      = cell (fun () -> _VoluntaryPrepay.Value.date())
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _VoluntaryPrepay.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _VoluntaryPrepay.Value.Equals(cf.Value))
    let _exCouponDate                              = cell (fun () -> _VoluntaryPrepay.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _VoluntaryPrepay.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _VoluntaryPrepay.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _VoluntaryPrepay.Value.accept(v.Value)
                                                                     _VoluntaryPrepay.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _VoluntaryPrepay.Value.registerWith(handler.Value)
                                                                     _VoluntaryPrepay.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _VoluntaryPrepay.Value.unregisterWith(handler.Value)
                                                                     _VoluntaryPrepay.Value)

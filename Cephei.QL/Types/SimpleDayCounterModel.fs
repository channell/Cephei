...
*)
    ...
    let _dayCount                                  (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = cell (fun () -> _SimpleDayCounter.Value.dayCount(d1.Value, d2.Value))
    let _dayCounter                                = cell (fun () -> _SimpleDayCounter.Value.dayCounter)
    let _empty                                     = cell (fun () -> _SimpleDayCounter.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = cell (fun () -> _SimpleDayCounter.Value.Equals(o.Value))
    let _name                                      = cell (fun () -> _SimpleDayCounter.Value.name())
    let _ToString                                  = cell (fun () -> _SimpleDayCounter.Value.ToString())
    let _yearFraction                              (d1 : ICell<Date>) (d2 : ICell<Date>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>)   
                                                   = cell (fun () -> _SimpleDayCounter.Value.yearFraction(d1.Value, d2.Value, refPeriodStart.Value, refPeriodEnd.Value))
    let _yearFraction1                             (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = cell (fun () -> _SimpleDayCounter.Value.yearFraction(d1.Value, d2.Value))

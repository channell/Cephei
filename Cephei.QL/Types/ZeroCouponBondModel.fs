...
(*
    Functions
*)
    let _ZeroCouponBond                            = triv (fun () -> withEngine _pricingEngine.Value (new ZeroCouponBond (settlementDays.Value, calendar.Value, faceAmount.Value, maturityDate.Value, paymentConvention.Value, redemption.Value, issueDate.Value)))

...
*)
    let mutable
        _CompositeInstrument                       = cell (fun () -> withEngine pricingEngine evaluationDate (new CompositeInstrument ()))
    ...
    let _isExpired                                 = triv (fun () -> (withEvaluationDate _evaluationDate _CompositeInstrument).isExpired())
    let _subtract                                  (instrument : ICell<Instrument>) (multiplier : ICell<double>)   
                                                   = triv (fun () -> (withEvaluationDate _evaluationDate _CompositeInstrument).subtract(instrument.Value, multiplier.Value)
                                                                     _CompositeInstrument.Value)
    let _CASH                                      = cell (fun () -> (withEvaluationDate _evaluationDate _CompositeInstrument).CASH())
    let _errorEstimate                             = triv (fun () -> (withEvaluationDate _evaluationDate _CompositeInstrument).errorEstimate())
    let _NPV                                       = cell (fun () -> (withEvaluationDate _evaluationDate _CompositeInstrument).NPV())
    let _result                                    (tag : ICell<string>)   
                                                   = triv (fun () -> (withEvaluationDate _evaluationDate _CompositeInstrument).result(tag.Value))
    let _setPricingEngine                          (e : ICell<IPricingEngine>)   
                                                   = triv (fun () -> (withEvaluationDate _evaluationDate _CompositeInstrument).setPricingEngine(e.Value)
                                                                     _CompositeInstrument.Value)

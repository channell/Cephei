...
*)
    ...
    let _factory                                   (L : ICell<Object>) (bcs : ICell<Object>) (additionalInputs : ICell<Object[]>)   
                                                   = cell (fun () -> _CrankNicolsonScheme.Value.factory(L.Value, bcs.Value, additionalInputs.Value))
    let _numberOfIterations                        = cell (fun () -> _CrankNicolsonScheme.Value.numberOfIterations())
    let _setStep                                   (dt : ICell<double>)   
                                                   = cell (fun () -> _CrankNicolsonScheme.Value.setStep(dt.Value)
                                                                     _CrankNicolsonScheme.Value)
    let _step                                      (a : ICell<Object ref>) (t : ICell<double>) (theta : ICell<double>)   
                                                   = cell (fun () -> _CrankNicolsonScheme.Value.step(a.Value, t.Value, theta.Value)
                                                                     _CrankNicolsonScheme.Value)

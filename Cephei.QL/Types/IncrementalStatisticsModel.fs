...
*)
    ...
    let _add1                                      (value : ICell<double>)   
                                                   = triv (fun () -> _IncrementalStatistics.Value.add(value.Value)
                                                                     _IncrementalStatistics.Value)
    let _addSequence                               (list : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _IncrementalStatistics.Value.addSequence(list.Value)
                                                                     _IncrementalStatistics.Value)
    let _addSequence1                              (data : ICell<Generic.List<double>>) (weight : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _IncrementalStatistics.Value.addSequence(data.Value, weight.Value)
                                                                     _IncrementalStatistics.Value)
    let _downsideDeviation                         = triv (fun () -> _IncrementalStatistics.Value.downsideDeviation())
    let _downsideVariance                          = triv (fun () -> _IncrementalStatistics.Value.downsideVariance())
    let _errorEstimate                             = triv (fun () -> _IncrementalStatistics.Value.errorEstimate())
    let _expectationValue                          (f : ICell<Func<KeyValuePair<double,double>,double>>) (inRange : ICell<Func<KeyValuePair<double,double>,bool>>)   
                                                   = triv (fun () -> _IncrementalStatistics.Value.expectationValue(f.Value, inRange.Value))
    let _kurtosis                                  = triv (fun () -> _IncrementalStatistics.Value.kurtosis())
    let _max                                       = triv (fun () -> _IncrementalStatistics.Value.max())
    let _mean                                      = triv (fun () -> _IncrementalStatistics.Value.mean())
    let _min                                       = triv (fun () -> _IncrementalStatistics.Value.min())
    let _percentile                                (percent : ICell<double>)   
                                                   = triv (fun () -> _IncrementalStatistics.Value.percentile(percent.Value))
    let _reset                                     = triv (fun () -> _IncrementalStatistics.Value.reset()
                                                                     _IncrementalStatistics.Value)
    let _samples                                   = triv (fun () -> _IncrementalStatistics.Value.samples())
    let _skewness                                  = triv (fun () -> _IncrementalStatistics.Value.skewness())
    let _standardDeviation                         = triv (fun () -> _IncrementalStatistics.Value.standardDeviation())

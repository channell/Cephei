...
*)
    let mutable
        _TransformedGrid                           = make (fun () -> new TransformedGrid (grid.Value))
    let _dx                                        (i : ICell<int>)   
                                                   = triv _TransformedGrid (fun () -> _TransformedGrid.Value.dx(i.Value))
    let _dxArray                                   = triv _TransformedGrid (fun () -> _TransformedGrid.Value.dxArray())
    let _dxm                                       (i : ICell<int>)   
                                                   = triv _TransformedGrid (fun () -> _TransformedGrid.Value.dxm(i.Value))
    let _dxmArray                                  = triv _TransformedGrid (fun () -> _TransformedGrid.Value.dxmArray())
    let _dxp                                       (i : ICell<int>)   
                                                   = triv _TransformedGrid (fun () -> _TransformedGrid.Value.dxp(i.Value))
    let _dxpArray                                  = triv _TransformedGrid (fun () -> _TransformedGrid.Value.dxpArray())
    let _grid                                      (i : ICell<int>)   
                                                   = triv _TransformedGrid (fun () -> _TransformedGrid.Value.grid(i.Value))
    let _gridArray                                 = triv _TransformedGrid (fun () -> _TransformedGrid.Value.gridArray())
    let _size                                      = triv _TransformedGrid (fun () -> _TransformedGrid.Value.size())
    let _transformedGrid                           (i : ICell<int>)   
                                                   = triv _TransformedGrid (fun () -> _TransformedGrid.Value.transformedGrid(i.Value))
    let _transformedGridArray                      = triv _TransformedGrid (fun () -> _TransformedGrid.Value.transformedGridArray())
...
*)
    let mutable
        _TransformedGrid                           = make (fun () -> new TransformedGrid (grid.Value, func.Value))
    let _dx                                        (i : ICell<int>)   
                                                   = triv _TransformedGrid (fun () -> _TransformedGrid.Value.dx(i.Value))
    let _dxArray                                   = triv _TransformedGrid (fun () -> _TransformedGrid.Value.dxArray())
    let _dxm                                       (i : ICell<int>)   

...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _MYRCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _MYRCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _MYRCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _MYRCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _MYRCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _MYRCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _MYRCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _MYRCurrency.Value.symbol)
    let _ToString                                  = triv (fun () -> _MYRCurrency.Value.ToString())
    let _triangulationCurrency                     = triv (fun () -> _MYRCurrency.Value.triangulationCurrency)

...
*)
    let mutable
        _TreeLattice                               = make (fun () -> new TreeLattice<'T> (timeGrid.Value, n.Value))
    let _initialize                                (asset : ICell<DiscretizedAsset>) (t : ICell<double>)   
                                                   = triv _TreeLattice (fun () -> _TreeLattice.Value.initialize(asset.Value, t.Value)
                                                                                  _TreeLattice.Value)
    let _partialRollback                           (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = triv _TreeLattice (fun () -> _TreeLattice.Value.partialRollback(asset.Value, To.Value)
                                                                                  _TreeLattice.Value)
    let _presentValue                              (asset : ICell<DiscretizedAsset>)   
                                                   = triv _TreeLattice (fun () -> _TreeLattice.Value.presentValue(asset.Value))
    let _rollback                                  (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = triv _TreeLattice (fun () -> _TreeLattice.Value.rollback(asset.Value, To.Value)
                                                                                  _TreeLattice.Value)
    let _statePrices                               (i : ICell<int>)   
                                                   = triv _TreeLattice (fun () -> _TreeLattice.Value.statePrices(i.Value))
    let _stepback                                  (i : ICell<int>) (values : ICell<Vector>) (newValues : ICell<Vector>)   
                                                   = triv _TreeLattice (fun () -> _TreeLattice.Value.stepback(i.Value, values.Value, newValues.Value)
                                                                                  _TreeLattice.Value)
    let _grid                                      (t : ICell<double>)   
                                                   = triv _TreeLattice (fun () -> _TreeLattice.Value.grid(t.Value))
    let _timeGrid                                  = triv _TreeLattice (fun () -> _TreeLattice.Value.timeGrid())

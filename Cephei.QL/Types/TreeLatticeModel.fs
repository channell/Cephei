...
(*
    Functions
*)
    let _TreeLattice                               = cell (fun () -> new TreeLattice<'T> (timeGrid.Value, n.Value))
    let _initialize                                (asset : ICell<DiscretizedAsset>) (t : ICell<double>)   
                                                   = cell (fun () -> _TreeLattice.Value.initialize(asset.Value, t.Value)
                                                                     _TreeLattice.Value)
    let _partialRollback                           (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = cell (fun () -> _TreeLattice.Value.partialRollback(asset.Value, To.Value)
                                                                     _TreeLattice.Value)
    let _presentValue                              (asset : ICell<DiscretizedAsset>)   
                                                   = cell (fun () -> _TreeLattice.Value.presentValue(asset.Value))
    let _rollback                                  (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = cell (fun () -> _TreeLattice.Value.rollback(asset.Value, To.Value)
                                                                     _TreeLattice.Value)
    let _statePrices                               (i : ICell<int>)   
                                                   = cell (fun () -> _TreeLattice.Value.statePrices(i.Value))
    let _stepback                                  (i : ICell<int>) (values : ICell<Vector>) (newValues : ICell<Vector>)   
                                                   = cell (fun () -> _TreeLattice.Value.stepback(i.Value, values.Value, newValues.Value)
                                                                     _TreeLattice.Value)
    let _grid                                      (t : ICell<double>)   
                                                   = cell (fun () -> _TreeLattice.Value.grid(t.Value))
    let _timeGrid                                  = cell (fun () -> _TreeLattice.Value.timeGrid())

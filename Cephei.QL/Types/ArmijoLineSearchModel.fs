...
*)
    ...
    let _value                                     (P : ICell<Problem>) (ecType : ICell<EndCriteria.Type ref>) (endCriteria : ICell<EndCriteria>) (t_ini : ICell<double>)   
                                                   = cell (fun () -> _ArmijoLineSearch.Value.value(P.Value, ecType.Value, endCriteria.Value, t_ini.Value))
    let _lastFunctionValue                         = cell (fun () -> _ArmijoLineSearch.Value.lastFunctionValue())
    let _lastGradient                              = cell (fun () -> _ArmijoLineSearch.Value.lastGradient())
    let _lastGradientNorm2                         = cell (fun () -> _ArmijoLineSearch.Value.lastGradientNorm2())
    let _lastX                                     = cell (fun () -> _ArmijoLineSearch.Value.lastX())
    let _searchDirection                           = cell (fun () -> _ArmijoLineSearch.Value.searchDirection)
    let _succeed                                   = cell (fun () -> _ArmijoLineSearch.Value.succeed())
    let _update                                    (data : ICell<Vector ref>) (direction : ICell<Vector>) (beta : ICell<double>) (Constraint : ICell<Constraint>)   
                                                   = cell (fun () -> _ArmijoLineSearch.Value.update(data.Value, direction.Value, beta.Value, Constraint.Value))
...
*)
    ...
    let _update                                    (data : ICell<Vector>) (direction : ICell<Vector>) (beta : ICell<double>) (Constraint : ICell<Constraint>)   
                                                   = cell (fun () -> _ArmijoLineSearch.Value.update(ref data.Value, direction.Value, beta.Value, Constraint.Value))
    do this.Bind(_ArmijoLineSearch)
...
*)
    ...
    let _value                                     (P : ICell<Problem>) (ecType : ICell<EndCriteria.Type>) (endCriteria : ICell<EndCriteria>) (t_ini : ICell<double>)   
                                                   = cell (fun () -> _ArmijoLineSearch.Value.value(P.Value, ref ecType.Value, endCriteria.Value, t_ini.Value))
    let _lastFunctionValue                         = cell (fun () -> _ArmijoLineSearch.Value.lastFunctionValue())
    let _lastGradient                              = cell (fun () -> _ArmijoLineSearch.Value.lastGradient())
    let _lastGradientNorm2                         = cell (fun () -> _ArmijoLineSearch.Value.lastGradientNorm2())
    let _lastX                                     = cell (fun () -> _ArmijoLineSearch.Value.lastX())

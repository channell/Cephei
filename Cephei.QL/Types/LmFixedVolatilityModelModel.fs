...
*)
    ...
    let _volatility                                (i : ICell<int>) (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LmFixedVolatilityModel.Value.volatility(i.Value, t.Value, x.Value))
    let _volatility1                               (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LmFixedVolatilityModel.Value.volatility(t.Value, x.Value))
    let _integratedVariance                        (i : ICell<int>) (j : ICell<int>) (u : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LmFixedVolatilityModel.Value.integratedVariance(i.Value, j.Value, u.Value, x.Value))
    let _parameters                                = cell (fun () -> _LmFixedVolatilityModel.Value.parameters())
    let _setParams                                 (arguments : ICell<Generic.List<Parameter>>)   
                                                   = cell (fun () -> _LmFixedVolatilityModel.Value.setParams(arguments.Value)
                                                                     _LmFixedVolatilityModel.Value)
    let _size                                      = cell (fun () -> _LmFixedVolatilityModel.Value.size())

...
*)
    let mutable
        _FixedRateLeg                              = make (fun () -> new FixedRateLeg (schedule.Value))
    let _value                                     = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.value())
    let _withCouponRates                           (couponRate : ICell<double>) (paymentDayCounter : ICell<DayCounter>) (comp : ICell<Compounding>) (freq : ICell<Frequency>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withCouponRates(couponRate.Value, paymentDayCounter.Value, comp.Value, freq.Value))
    let _withCouponRates1                          (couponRate : ICell<double>) (paymentDayCounter : ICell<DayCounter>) (comp : ICell<Compounding>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withCouponRates(couponRate.Value, paymentDayCounter.Value, comp.Value))
    let _withCouponRates2                          (couponRates : ICell<Generic.List<double>>) (paymentDayCounter : ICell<DayCounter>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withCouponRates(couponRates.Value, paymentDayCounter.Value))
    let _withCouponRates3                          (couponRates : ICell<Generic.List<double>>) (paymentDayCounter : ICell<DayCounter>) (comp : ICell<Compounding>) (freq : ICell<Frequency>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withCouponRates(couponRates.Value, paymentDayCounter.Value, comp.Value, freq.Value))
    let _withCouponRates4                          (couponRates : ICell<Generic.List<InterestRate>>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withCouponRates(couponRates.Value))
    let _withCouponRates5                          (couponRate : ICell<InterestRate>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withCouponRates(couponRate.Value))
    let _withCouponRates6                          (couponRate : ICell<double>) (paymentDayCounter : ICell<DayCounter>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withCouponRates(couponRate.Value, paymentDayCounter.Value))
    let _withCouponRates7                          (couponRates : ICell<Generic.List<double>>) (paymentDayCounter : ICell<DayCounter>) (comp : ICell<Compounding>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withCouponRates(couponRates.Value, paymentDayCounter.Value, comp.Value))
    let _withExCouponPeriod                        (period : ICell<Period>) (cal : ICell<Calendar>) (convention : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withExCouponPeriod(period.Value, cal.Value, convention.Value, endOfMonth.Value))
    let _withFirstPeriodDayCounter                 (dayCounter : ICell<DayCounter>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withFirstPeriodDayCounter(dayCounter.Value))
    let _withLastPeriodDayCounter                  (dayCounter : ICell<DayCounter>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withLastPeriodDayCounter(dayCounter.Value))
    let _withPaymentCalendar                       (cal : ICell<Calendar>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withPaymentCalendar(cal.Value))
    let _withNotionals                             (notional : ICell<double>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withNotionals(notional.Value))
    let _withNotionals1                            (notionals : ICell<Generic.List<double>>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withNotionals(notionals.Value))
    let _withPaymentAdjustment                     (convention : ICell<BusinessDayConvention>)   
                                                   = triv _FixedRateLeg (fun () -> _FixedRateLeg.Value.withPaymentAdjustment(convention.Value))

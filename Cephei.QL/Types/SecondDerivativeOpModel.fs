...
*)
    ...
    let _add                                       (m : ICell<TripleBandLinearOp>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.add(m.Value))
    let _add1                                      (u : ICell<Vector>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.add(u.Value))
    let _add2                                      (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.apply(r.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.applyTo(v.Value))
    let _axpyb                                     (a : ICell<Vector>) (x : ICell<TripleBandLinearOp>) (y : ICell<TripleBandLinearOp>) (b : ICell<Vector>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.axpyb(a.Value, x.Value, y.Value, b.Value)
                                                                     _SecondDerivativeOp.Value)
    let _Clone                                     = cell (fun () -> _SecondDerivativeOp.Value.Clone())
    let _identity                                  (size : ICell<int>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.identity(size.Value))
    let _isTimeDependent                           = cell (fun () -> _SecondDerivativeOp.Value.isTimeDependent())
    let _mult                                      (u : ICell<Vector>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.mult(u.Value))
    let _multiply                                  (a : ICell<double>) (D : ICell<IOperator>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.multiply(a.Value, D.Value))
    let _multR                                     (u : ICell<Vector>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.multR(u.Value))
    let _setTime                                   (t : ICell<double>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.setTime(t.Value)
                                                                     _SecondDerivativeOp.Value)
    let _size                                      = cell (fun () -> _SecondDerivativeOp.Value.size())
    let _solve_splitting                           (r : ICell<Vector>) (a : ICell<double>) (b : ICell<double>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.solve_splitting(r.Value, a.Value, b.Value))
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.solveFor(rhs.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.subtract(A.Value, B.Value))
    let _swap                                      (m : ICell<TripleBandLinearOp>)   
                                                   = cell (fun () -> _SecondDerivativeOp.Value.swap(m.Value)
                                                                     _SecondDerivativeOp.Value)
    let _toMatrix                                  = cell (fun () -> _SecondDerivativeOp.Value.toMatrix())

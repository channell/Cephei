...
*)
    let mutable
        _DownRounding                              = make (fun () -> new DownRounding (precision.Value, digit.Value))
    let _Digit                                     = triv _DownRounding (fun () -> _DownRounding.Value.Digit)
    let _getType                                   = triv _DownRounding (fun () -> _DownRounding.Value.getType)
    let _Precision                                 = triv _DownRounding (fun () -> _DownRounding.Value.Precision)
    let _Round                                     (value : ICell<double>)   
                                                   = triv _DownRounding (fun () -> _DownRounding.Value.Round(value.Value))
...
*)
    let mutable
        _DownRounding                              = make (fun () -> new DownRounding (precision.Value))
    let _Digit                                     = triv _DownRounding (fun () -> _DownRounding.Value.Digit)
    let _getType                                   = triv _DownRounding (fun () -> _DownRounding.Value.getType)
    let _Precision                                 = triv _DownRounding (fun () -> _DownRounding.Value.Precision)
    let _Round                                     (value : ICell<double>)   

...
*)
    ...
    let _Clear                                     = cell (fun () -> _SparseMatrix.Value.Clear()
                                                                     _SparseMatrix.Value)
    let _columns                                   = cell (fun () -> _SparseMatrix.Value.columns())
    let _GetAt                                     (row : ICell<int>) (col : ICell<int>)   
                                                   = cell (fun () -> _SparseMatrix.Value.GetAt(row.Value, col.Value))
    let _GetColumnData                             (col : ICell<int>)   
                                                   = cell (fun () -> _SparseMatrix.Value.GetColumnData(col.Value))
    let _GetColumnDataCount                        (col : ICell<int>)   
                                                   = cell (fun () -> _SparseMatrix.Value.GetColumnDataCount(col.Value))
    let _GetRowData                                (row : ICell<int>)   
                                                   = cell (fun () -> _SparseMatrix.Value.GetRowData(row.Value))
    let _GetRowDataCount                           (row : ICell<int>)   
                                                   = cell (fun () -> _SparseMatrix.Value.GetRowDataCount(row.Value))
    let _RemoveAt                                  (row : ICell<int>) (col : ICell<int>)   
                                                   = cell (fun () -> _SparseMatrix.Value.RemoveAt(row.Value, col.Value)
                                                                     _SparseMatrix.Value)
    let _rows                                      = cell (fun () -> _SparseMatrix.Value.rows())
    let _SetAt                                     (row : ICell<int>) (col : ICell<int>) (value : ICell<double>)   
                                                   = cell (fun () -> _SparseMatrix.Value.SetAt(row.Value, col.Value, value.Value)
                                                                     _SparseMatrix.Value)
    let _this                                      (row : ICell<int>) (col : ICell<int>)   
                                                   = cell (fun () -> _SparseMatrix.Value.[row.Value, col.Value])
    let _values                                    = cell (fun () -> _SparseMatrix.Value.values())

...
*)
    ...
    let _covariance1                               (t1 : ICell<double>) (t2 : ICell<double>) (T : ICell<double>) (S : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.covariance(t1.Value, t2.Value, T.Value, S.Value))
    let _instantaneousCovariance                   (u : ICell<double>) (T : ICell<double>) (S : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.instantaneousCovariance(u.Value, T.Value, S.Value))
    let _instantaneousVariance                     (u : ICell<double>) (T : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.instantaneousVariance(u.Value, T.Value))
    let _instantaneousVolatility                   (u : ICell<double>) (T : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.instantaneousVolatility(u.Value, T.Value))
    let _longTermVolatility                        = triv (fun () -> _AbcdFunction.Value.longTermVolatility())
    let _maximumVolatility                         = triv (fun () -> _AbcdFunction.Value.maximumVolatility())
    let _primitive                                 (t : ICell<double>) (T2 : ICell<double>) (S : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.primitive(t.Value, T2.Value, S.Value))
    let _shortTermVolatility                       = triv (fun () -> _AbcdFunction.Value.shortTermVolatility())
    let _variance                                  (tMin : ICell<double>) (tMax : ICell<double>) (T : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.variance(tMin.Value, tMax.Value, T.Value))
    let _volatility                                (tMin : ICell<double>) (tMax : ICell<double>) (T : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.volatility(tMin.Value, tMax.Value, T.Value))
    let _a                                         = triv (fun () -> _AbcdFunction.Value.a())
    let _b                                         = triv (fun () -> _AbcdFunction.Value.b())
    let _c                                         = triv (fun () -> _AbcdFunction.Value.c())
    let _coefficients                              = triv (fun () -> _AbcdFunction.Value.coefficients())
    let _d                                         = triv (fun () -> _AbcdFunction.Value.d())
    let _definiteDerivativeCoefficients            (t : ICell<double>) (t2 : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.definiteDerivativeCoefficients(t.Value, t2.Value))
    let _definiteIntegral                          (t1 : ICell<double>) (t2 : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.definiteIntegral(t1.Value, t2.Value))
    let _definiteIntegralCoefficients              (t : ICell<double>) (t2 : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.definiteIntegralCoefficients(t.Value, t2.Value))
    let _derivative                                (t : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.derivative(t.Value))
    let _derivativeCoefficients                    = triv (fun () -> _AbcdFunction.Value.derivativeCoefficients())
    let _longTermValue                             = triv (fun () -> _AbcdFunction.Value.longTermValue())
    let _maximumLocation                           = triv (fun () -> _AbcdFunction.Value.maximumLocation())
    let _maximumValue                              = triv (fun () -> _AbcdFunction.Value.maximumValue())
    let _value                                     (t : ICell<double>)   
                                                   = triv (fun () -> _AbcdFunction.Value.value(t.Value))
    do this.Bind(_AbcdFunction)
(* 
    casting 
*)
    internal new () = AbcdFunctionModel(null,null,null,null)

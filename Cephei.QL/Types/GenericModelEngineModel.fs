...
*)
    let mutable
        _GenericModelEngine                        = make (fun () -> (createEvaluationDate _evaluationDate (fun () ->new GenericModelEngine<'ModelType,'ArgumentsType,'ResultsType> ())))
    let _setModel                                  (model : ICell<Handle<'ModelType>>)   
                                                   = triv _GenericModelEngine (fun () -> (curryEvaluationDate _evaluationDate _GenericModelEngine).Value.setModel(model.Value)
                                                                                         _GenericModelEngine.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _GenericModelEngine (fun () -> (curryEvaluationDate _evaluationDate _GenericModelEngine).Value.registerWith(handler.Value)
                                                                                         _GenericModelEngine.Value)
    let _reset                                     = triv _GenericModelEngine (fun () -> (curryEvaluationDate _evaluationDate _GenericModelEngine).Value.reset()
                                                                                         _GenericModelEngine.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _GenericModelEngine (fun () -> (curryEvaluationDate _evaluationDate _GenericModelEngine).Value.unregisterWith(handler.Value)
                                                                                         _GenericModelEngine.Value)
    let _update                                    = triv _GenericModelEngine (fun () -> (curryEvaluationDate _evaluationDate _GenericModelEngine).Value.update()
                                                                                         _GenericModelEngine.Value)
...
*)
    let mutable
        _GenericModelEngine                        = make (fun () -> (createEvaluationDate _evaluationDate (fun () ->new GenericModelEngine<'ModelType,'ArgumentsType,'ResultsType> (model.Value))))
    let _setModel                                  (model : ICell<Handle<'ModelType>>)   
                                                   = triv _GenericModelEngine (fun () -> (curryEvaluationDate _evaluationDate _GenericModelEngine).Value.setModel(model.Value)
                                                                                         _GenericModelEngine.Value)
    let _registerWith                              (handler : ICell<Callback>)   

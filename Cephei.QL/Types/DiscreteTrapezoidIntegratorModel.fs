...
*)
    ...
    let _absoluteAccuracy                          = cell (fun () -> _DiscreteTrapezoidIntegrator.Value.absoluteAccuracy())
    let _absoluteError                             = cell (fun () -> _DiscreteTrapezoidIntegrator.Value.absoluteError())
    let _integrationSuccess                        = cell (fun () -> _DiscreteTrapezoidIntegrator.Value.integrationSuccess())
    let _maxEvaluations                            = cell (fun () -> _DiscreteTrapezoidIntegrator.Value.maxEvaluations())
    let _numberOfEvaluations                       = cell (fun () -> _DiscreteTrapezoidIntegrator.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = cell (fun () -> _DiscreteTrapezoidIntegrator.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                     _DiscreteTrapezoidIntegrator.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = cell (fun () -> _DiscreteTrapezoidIntegrator.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                     _DiscreteTrapezoidIntegrator.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = cell (fun () -> _DiscreteTrapezoidIntegrator.Value.value(f.Value, a.Value, b.Value))

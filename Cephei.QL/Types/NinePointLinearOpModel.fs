...
*)
    ...
    let _add                                       (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.apply(r.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.applyTo(v.Value))
    let _Clone                                     = cell (fun () -> _NinePointLinearOp.Value.Clone())
    let _identity                                  (size : ICell<int>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.identity(size.Value))
    let _isTimeDependent                           = cell (fun () -> _NinePointLinearOp.Value.isTimeDependent())
    let _mult                                      (r : ICell<Vector>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.mult(r.Value))
    let _multiply                                  (a : ICell<double>) (D : ICell<IOperator>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.multiply(a.Value, D.Value))
    let _setTime                                   (t : ICell<double>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.setTime(t.Value)
                                                                     _NinePointLinearOp.Value)
    let _size                                      = cell (fun () -> _NinePointLinearOp.Value.size())
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.solveFor(rhs.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.subtract(A.Value, B.Value))
    let _swap                                      (m : ICell<NinePointLinearOp>)   
                                                   = cell (fun () -> _NinePointLinearOp.Value.swap(m.Value)
                                                                     _NinePointLinearOp.Value)
    let _toMatrix                                  = cell (fun () -> _NinePointLinearOp.Value.toMatrix())

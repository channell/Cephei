...
*)
    let mutable
        _NinePointLinearOp                         = make (fun () -> new NinePointLinearOp (m.Value))
    let _add                                       (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.apply(r.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.applyTo(v.Value))
    let _Clone                                     = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.Clone())
    let _identity                                  (size : ICell<int>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.identity(size.Value))
    let _isTimeDependent                           = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.isTimeDependent())
    let _mult                                      (r : ICell<Vector>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.mult(r.Value))
    let _multiply                                  (a : ICell<double>) (D : ICell<IOperator>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.multiply(a.Value, D.Value))
    let _setTime                                   (t : ICell<double>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.setTime(t.Value)
                                                                                        _NinePointLinearOp.Value)
    let _size                                      = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.size())
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.solveFor(rhs.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.subtract(A.Value, B.Value))
    let _swap                                      (m : ICell<NinePointLinearOp>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.swap(m.Value)
                                                                                        _NinePointLinearOp.Value)
    let _toMatrix                                  = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.toMatrix())
...
*)
    let mutable
        _NinePointLinearOp                         = make (fun () -> new NinePointLinearOp (d0.Value, d1.Value, mesher.Value))
    let _add                                       (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = triv _NinePointLinearOp (fun () -> _NinePointLinearOp.Value.apply(r.Value))

...
*)
    ...
    let _Clear                                     = cell (fun () -> _WeakEventSource.Value.Clear()
                                                                     _WeakEventSource.Value)
    let _Raise                                     = cell (fun () -> _WeakEventSource.Value.Raise()
                                                                     _WeakEventSource.Value)
    let _Subscribe                                 (handler : ICell<Callback>)   
                                                   = cell (fun () -> _WeakEventSource.Value.Subscribe(handler.Value)
                                                                     _WeakEventSource.Value)
    let _Unsubscribe                               (handler : ICell<Callback>)   
                                                   = cell (fun () -> _WeakEventSource.Value.Unsubscribe(handler.Value)
                                                                     _WeakEventSource.Value)

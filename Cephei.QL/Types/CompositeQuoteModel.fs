...
*)
    ...
    let _isValid                                   = cell (fun () -> _CompositeQuote.Value.isValid())
    let _update                                    = cell (fun () -> _CompositeQuote.Value.update()
                                                                     _CompositeQuote.Value)
    let _value                                     = cell (fun () -> _CompositeQuote.Value.value())
    let _value1                                    = cell (fun () -> _CompositeQuote.Value.value1())
    let _value2                                    = cell (fun () -> _CompositeQuote.Value.value2())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CompositeQuote.Value.registerWith(handler.Value)
                                                                     _CompositeQuote.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CompositeQuote.Value.unregisterWith(handler.Value)
                                                                     _CompositeQuote.Value)

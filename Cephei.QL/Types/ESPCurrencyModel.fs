...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _ESPCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _ESPCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _ESPCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _ESPCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _ESPCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _ESPCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _ESPCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _ESPCurrency.Value.symbol)

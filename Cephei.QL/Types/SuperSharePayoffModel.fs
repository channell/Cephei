...
*)
    ...
    let _cashPayoff                                = cell (fun () -> _SuperSharePayoff.Value.cashPayoff())
    let _description                               = cell (fun () -> _SuperSharePayoff.Value.description())
    let _name                                      = cell (fun () -> _SuperSharePayoff.Value.name())
    let _secondStrike                              = cell (fun () -> _SuperSharePayoff.Value.secondStrike())
    let _value                                     (price : ICell<double>)   
                                                   = cell (fun () -> _SuperSharePayoff.Value.value(price.Value))
    let _strike                                    = cell (fun () -> _SuperSharePayoff.Value.strike())
    let _optionType                                = cell (fun () -> _SuperSharePayoff.Value.optionType())
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _SuperSharePayoff.Value.accept(v.Value)

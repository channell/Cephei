...
*)
    ...
    let _primitive                                 (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.primitive(x.Value, allowExtrapolation.Value))
    let _secondDerivative                          (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.secondDerivative(x.Value, allowExtrapolation.Value))
    let _update                                    = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.update()
                                                                     _MixedLinearMonotonicCubicNaturalSpline.Value)
    let _value                                     (x : ICell<double>)   
                                                   = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.value(x.Value))
    let _value1                                    (x : ICell<double>) (allowExtrapolation : ICell<bool>)   
                                                   = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.value(x.Value, allowExtrapolation.Value))
    let _xMax                                      = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.xMax())
    let _xMin                                      = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.xMin())
    let _allowsExtrapolation                       = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.allowsExtrapolation())
    let _disableExtrapolation                      (b : ICell<bool>)   
                                                   = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.disableExtrapolation(b.Value)
                                                                     _MixedLinearMonotonicCubicNaturalSpline.Value)
    let _enableExtrapolation                       (b : ICell<bool>)   
                                                   = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.enableExtrapolation(b.Value)
                                                                     _MixedLinearMonotonicCubicNaturalSpline.Value)
    let _extrapolate                               = triv (fun () -> _MixedLinearMonotonicCubicNaturalSpline.Value.extrapolate)

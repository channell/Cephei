...
*)
    ...
    let _factory                                   (L : ICell<Object>) (bcs : ICell<Object>) (additionalFields : ICell<Object[]>)   
                                                   = cell (fun () -> _ImplicitEulerScheme.Value.factory(L.Value, bcs.Value, additionalFields.Value))
    let _numberOfIterations                        = cell (fun () -> _ImplicitEulerScheme.Value.numberOfIterations())
    let _setStep                                   (dt : ICell<double>)   
                                                   = cell (fun () -> _ImplicitEulerScheme.Value.setStep(dt.Value)
                                                                     _ImplicitEulerScheme.Value)
    let _step                                      (a : ICell<Object ref>) (t : ICell<double>) (theta : ICell<double>)   
                                                   = cell (fun () -> _ImplicitEulerScheme.Value.step(a.Value, t.Value, theta.Value)
                                                                     _ImplicitEulerScheme.Value)

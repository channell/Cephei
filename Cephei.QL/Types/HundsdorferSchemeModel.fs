...
*)
    ...
    let _factory                                   (L : ICell<Object>) (bcs : ICell<Object>) (additionalInputs : ICell<Object[]>)   
                                                   = cell (fun () -> _HundsdorferScheme.Value.factory(L.Value, bcs.Value, additionalInputs.Value))
    let _setStep                                   (dt : ICell<double>)   
                                                   = cell (fun () -> _HundsdorferScheme.Value.setStep(dt.Value)
                                                                     _HundsdorferScheme.Value)
    let _step                                      (a : ICell<Object ref>) (t : ICell<double>) (theta : ICell<double>)   
                                                   = cell (fun () -> _HundsdorferScheme.Value.step(a.Value, t.Value, theta.Value)
                                                                     _HundsdorferScheme.Value)

...
*)
    ...
    let _grid                                      (t : ICell<double>)   
                                                   = cell (fun () -> _TreeLattice1D.Value.grid(t.Value))
    let _underlying                                (i : ICell<int>) (index : ICell<int>)   
                                                   = cell (fun () -> _TreeLattice1D.Value.underlying(i.Value, index.Value))
    let _initialize                                (asset : ICell<DiscretizedAsset>) (t : ICell<double>)   
                                                   = cell (fun () -> _TreeLattice1D.Value.initialize(asset.Value, t.Value)
                                                                     _TreeLattice1D.Value)
    let _partialRollback                           (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = cell (fun () -> _TreeLattice1D.Value.partialRollback(asset.Value, To.Value)
                                                                     _TreeLattice1D.Value)
    let _presentValue                              (asset : ICell<DiscretizedAsset>)   
                                                   = cell (fun () -> _TreeLattice1D.Value.presentValue(asset.Value))
    let _rollback                                  (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = cell (fun () -> _TreeLattice1D.Value.rollback(asset.Value, To.Value)
                                                                     _TreeLattice1D.Value)
    let _statePrices                               (i : ICell<int>)   
                                                   = cell (fun () -> _TreeLattice1D.Value.statePrices(i.Value))
    let _stepback                                  (i : ICell<int>) (values : ICell<Vector>) (newValues : ICell<Vector>)   
                                                   = cell (fun () -> _TreeLattice1D.Value.stepback(i.Value, values.Value, newValues.Value)
                                                                     _TreeLattice1D.Value)
    let _timeGrid                                  = cell (fun () -> _TreeLattice1D.Value.timeGrid())

...
*)
    ...
    let _underlying                                (i : ICell<int>) (index : ICell<int>)   
                                                   = triv (fun () -> _TreeLattice1D.Value.underlying(i.Value, index.Value))
    let _initialize                                (asset : ICell<DiscretizedAsset>) (t : ICell<double>)   
                                                   = triv (fun () -> _TreeLattice1D.Value.initialize(asset.Value, t.Value)
                                                                     _TreeLattice1D.Value)
    let _partialRollback                           (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = triv (fun () -> _TreeLattice1D.Value.partialRollback(asset.Value, To.Value)
                                                                     _TreeLattice1D.Value)
    let _presentValue                              (asset : ICell<DiscretizedAsset>)   
                                                   = triv (fun () -> _TreeLattice1D.Value.presentValue(asset.Value))
    let _rollback                                  (asset : ICell<DiscretizedAsset>) (To : ICell<double>)   
                                                   = triv (fun () -> _TreeLattice1D.Value.rollback(asset.Value, To.Value)
                                                                     _TreeLattice1D.Value)
    let _statePrices                               (i : ICell<int>)   
                                                   = triv (fun () -> _TreeLattice1D.Value.statePrices(i.Value))
    let _stepback                                  (i : ICell<int>) (values : ICell<Vector>) (newValues : ICell<Vector>)   
                                                   = triv (fun () -> _TreeLattice1D.Value.stepback(i.Value, values.Value, newValues.Value)
                                                                     _TreeLattice1D.Value)
    let _timeGrid                                  = triv (fun () -> _TreeLattice1D.Value.timeGrid())

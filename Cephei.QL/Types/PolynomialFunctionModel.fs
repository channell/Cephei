...
*)
    ...
    let _definiteIntegral                          (t1 : ICell<double>) (t2 : ICell<double>)   
                                                   = triv (fun () -> _PolynomialFunction.Value.definiteIntegral(t1.Value, t2.Value))
    let _definiteIntegralCoefficients              (t : ICell<double>) (t2 : ICell<double>)   
                                                   = triv (fun () -> _PolynomialFunction.Value.definiteIntegralCoefficients(t.Value, t2.Value))
    let _derivative                                (t : ICell<double>)   
                                                   = triv (fun () -> _PolynomialFunction.Value.derivative(t.Value))
    let _derivativeCoefficients                    = triv (fun () -> _PolynomialFunction.Value.derivativeCoefficients())
    let _order                                     = triv (fun () -> _PolynomialFunction.Value.order())
    let _primitive                                 (t : ICell<double>)   
                                                   = triv (fun () -> _PolynomialFunction.Value.primitive(t.Value))
    let _primitiveCoefficients                     = triv (fun () -> _PolynomialFunction.Value.primitiveCoefficients())

...
*)
    let mutable
        _Handle                                    = make (fun () -> new Handle<'T> ())
    let _currentLink                               = triv _Handle (fun () -> _Handle.Value.currentLink())
    let _empty                                     = triv _Handle (fun () -> _Handle.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Handle (fun () -> _Handle.Value.Equals(o.Value))
    let _link                                      = triv _Handle (fun () -> _Handle.Value.link)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _Handle (fun () -> _Handle.Value.registerWith(handler.Value)
                                                                             _Handle.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _Handle (fun () -> _Handle.Value.unregisterWith(handler.Value)
                                                                             _Handle.Value)
...
*)
    let mutable
        _Handle                                    = make (fun () -> new Handle<'T> (h.Value, registerAsObserver.Value))
    let _currentLink                               = triv _Handle (fun () -> _Handle.Value.currentLink())
    let _empty                                     = triv _Handle (fun () -> _Handle.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Handle (fun () -> _Handle.Value.Equals(o.Value))
...
*)
    let mutable
        _Handle                                    = make (fun () -> new Handle<'T> (h.Value))
    let _currentLink                               = triv _Handle (fun () -> _Handle.Value.currentLink())
    let _empty                                     = triv _Handle (fun () -> _Handle.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Handle (fun () -> _Handle.Value.Equals(o.Value))

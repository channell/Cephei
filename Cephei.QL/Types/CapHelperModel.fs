...
(*
    Functions
*)
    let _CapHelper                                 = cell (fun () -> withEngine _pricingEngine.Value (new CapHelper (length.Value, volatility.Value, index.Value, fixedLegFrequency.Value, fixedLegDayCounter.Value, includeFirstSwaplet.Value, termStructure.Value, errorType.Value)))
    let _addTimesTo                                (times : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).addTimesTo(times.Value)
                                                                     _CapHelper.Value)
    let _blackPrice                                (sigma : ICell<double>)   
                                                   = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).blackPrice(sigma.Value))
    let _modelValue                                = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).modelValue())
    let _calibrationError                          = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).calibrationError())
    let _impliedVolatility                         (targetValue : ICell<double>) (accuracy : ICell<double>) (maxEvaluations : ICell<int>) (minVol : ICell<double>) (maxVol : ICell<double>)   
                                                   = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).impliedVolatility(targetValue.Value, accuracy.Value, maxEvaluations.Value, minVol.Value, maxVol.Value))
    let _marketValue                               = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).marketValue())
    let _setPricingEngine                          (engine : ICell<IPricingEngine>)   
                                                   = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).setPricingEngine(engine.Value)
                                                                     _CapHelper.Value)
    let _volatility                                = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).volatility())
    let _volatilityType                            = cell (fun () -> (withEvaluationDate _evaluationDate _CapHelper).volatilityType())

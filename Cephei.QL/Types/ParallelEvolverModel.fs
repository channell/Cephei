...
*)
    ...
    let _factory                                   (L : ICell<Object>) (bcs : ICell<Object>) (additionalFields : ICell<Object[]>)   
                                                   = cell (fun () -> _ParallelEvolver.Value.factory(L.Value, bcs.Value, additionalFields.Value))
    let _setStep                                   (dt : ICell<double>)   
                                                   = cell (fun () -> _ParallelEvolver.Value.setStep(dt.Value)
                                                                     _ParallelEvolver.Value)
    let _step                                      (o : ICell<Object ref>) (t : ICell<double>) (theta : ICell<double>)   
                                                   = cell (fun () -> _ParallelEvolver.Value.step(o.Value, t.Value, theta.Value)
                                                                     _ParallelEvolver.Value)

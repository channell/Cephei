...
(*
    Functions
*)
    let _HestonModelHelper                         = triv (fun () -> withEngine _pricingEngine.Value (new HestonModelHelper (maturity.Value, calendar.Value, s0.Value, strikePrice.Value, volatility.Value, riskFreeRate.Value, dividendYield.Value, errorType.Value)))

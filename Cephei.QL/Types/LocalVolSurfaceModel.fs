...
*)
    let mutable
        _LocalVolSurface                           = make (fun () -> new LocalVolSurface (blackTS.Value, riskFreeTS.Value, dividendTS.Value, underlying.Value))
    let _dayCounter                                = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.dayCounter())
    let _maxDate                                   = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.maxDate())
    let _maxStrike                                 = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.maxStrike())
    let _minStrike                                 = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.minStrike())
    let _referenceDate                             = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.referenceDate())
    let _localVol                                  (t : ICell<double>) (underlyingLevel : ICell<double>) (extrapolate : ICell<bool>)   
                                                   = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.localVol(t.Value, underlyingLevel.Value, extrapolate.Value))
    let _localVol1                                 (d : ICell<Date>) (underlyingLevel : ICell<double>) (extrapolate : ICell<bool>)   
                                                   = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.localVol(d.Value, underlyingLevel.Value, extrapolate.Value))
    let _businessDayConvention                     = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.businessDayConvention())
    let _optionDateFromTenor                       (p : ICell<Period>)   
                                                   = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.optionDateFromTenor(p.Value))
    let _calendar                                  = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.calendar())
    let _maxTime                                   = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.maxTime())
    let _settlementDays                            = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.settlementDays())
    let _timeFromReference                         (date : ICell<Date>)   
                                                   = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.timeFromReference(date.Value))
    let _update                                    = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.update()
                                                                                      _LocalVolSurface.Value)
    let _allowsExtrapolation                       = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.allowsExtrapolation())
    let _disableExtrapolation                      (b : ICell<bool>)   
                                                   = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.disableExtrapolation(b.Value)
                                                                                      _LocalVolSurface.Value)
    let _enableExtrapolation                       (b : ICell<bool>)   
                                                   = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.enableExtrapolation(b.Value)
                                                                                      _LocalVolSurface.Value)
    let _extrapolate                               = triv _LocalVolSurface (fun () -> _LocalVolSurface.Value.extrapolate)

...
*)
    ...
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = triv (fun () -> _CmsCoupon.Value.accruedAmount(d.Value))
    let _adjustedFixing                            = triv (fun () -> _CmsCoupon.Value.adjustedFixing)
    let _amount                                    = triv (fun () -> _CmsCoupon.Value.amount())
    let _convexityAdjustment                       = triv (fun () -> _CmsCoupon.Value.convexityAdjustment())
    let _dayCounter                                = triv (fun () -> _CmsCoupon.Value.dayCounter())
    let _fixingDate                                = triv (fun () -> _CmsCoupon.Value.fixingDate())
    let _fixingDays                                = triv (fun () -> _CmsCoupon.Value.fixingDays)
    let _gearing                                   = triv (fun () -> _CmsCoupon.Value.gearing())
    let _index                                     = triv (fun () -> _CmsCoupon.Value.index())
    let _indexFixing                               = triv (fun () -> _CmsCoupon.Value.indexFixing())
    let _isInArrears                               = triv (fun () -> _CmsCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = triv (fun () -> _CmsCoupon.Value.price(yts.Value))
    let _pricer                                    = triv (fun () -> _CmsCoupon.Value.pricer())
    let _rate                                      = triv (fun () -> _CmsCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = triv (fun () -> _CmsCoupon.Value.setPricer(pricer.Value)
                                                                     _CmsCoupon.Value)
    let _spread                                    = triv (fun () -> _CmsCoupon.Value.spread())
    let _update                                    = triv (fun () -> _CmsCoupon.Value.update()
                                                                     _CmsCoupon.Value)
    let _accrualDays                               = triv (fun () -> _CmsCoupon.Value.accrualDays())
    let _accrualEndDate                            = triv (fun () -> _CmsCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = triv (fun () -> _CmsCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = triv (fun () -> _CmsCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = triv (fun () -> _CmsCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = triv (fun () -> _CmsCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = triv (fun () -> _CmsCoupon.Value.date())
    let _exCouponDate                              = triv (fun () -> _CmsCoupon.Value.exCouponDate())
    let _nominal                                   = triv (fun () -> _CmsCoupon.Value.nominal())
    let _referencePeriodEnd                        = triv (fun () -> _CmsCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = triv (fun () -> _CmsCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = triv (fun () -> _CmsCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = triv (fun () -> _CmsCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = triv (fun () -> _CmsCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = triv (fun () -> _CmsCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = triv (fun () -> _CmsCoupon.Value.accept(v.Value)
                                                                     _CmsCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _CmsCoupon.Value.registerWith(handler.Value)
                                                                     _CmsCoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv (fun () -> _CmsCoupon.Value.unregisterWith(handler.Value)
                                                                     _CmsCoupon.Value)

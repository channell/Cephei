...
*)
    ...
    let _calibrate                                 (instruments : ICell<Generic.List<CalibrationHelper>>) (Method : ICell<OptimizationMethod>) (endCriteria : ICell<EndCriteria>) (additionalConstraint : ICell<Constraint>) (weights : ICell<Generic.List<double>>) (fixParameters : ICell<Generic.List<bool>>)   
                                                   = cell (fun () -> _CalibratedModel.Value.calibrate(instruments.Value, Method.Value, endCriteria.Value, additionalConstraint.Value, weights.Value, fixParameters.Value)
                                                                     _CalibratedModel.Value)
    let _constraint                                = cell (fun () -> _CalibratedModel.Value.CONSTRAINT())
    let _endCriteria                               = cell (fun () -> _CalibratedModel.Value.endCriteria())
    let _notifyObservers                           = cell (fun () -> _CalibratedModel.Value.notifyObservers()
                                                                     _CalibratedModel.Value)
    let _parameters                                = cell (fun () -> _CalibratedModel.Value.parameters())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CalibratedModel.Value.registerWith(handler.Value)
                                                                     _CalibratedModel.Value)
    let _setParams                                 (parameters : ICell<Vector>)   
                                                   = cell (fun () -> _CalibratedModel.Value.setParams(parameters.Value)
                                                                     _CalibratedModel.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CalibratedModel.Value.unregisterWith(handler.Value)
                                                                     _CalibratedModel.Value)
    let _update                                    = cell (fun () -> _CalibratedModel.Value.update()
                                                                     _CalibratedModel.Value)
    let _value                                     (parameters : ICell<Vector>) (instruments : ICell<Generic.List<CalibrationHelper>>)   
                                                   = cell (fun () -> _CalibratedModel.Value.value(parameters.Value, instruments.Value))

...
*)
    let mutable
        _CalibratedModel                           = make (fun () -> new CalibratedModel (nArguments.Value))
    let _calibrate                                 (instruments : ICell<Generic.List<CalibrationHelper>>) (Method : ICell<OptimizationMethod>) (endCriteria : ICell<EndCriteria>) (additionalConstraint : ICell<Constraint>) (weights : ICell<Generic.List<double>>) (fixParameters : ICell<Generic.List<bool>>)   
                                                   = triv _CalibratedModel (fun () -> _CalibratedModel.Value.calibrate(instruments.Value, Method.Value, endCriteria.Value, additionalConstraint.Value, weights.Value, fixParameters.Value)
                                                                                      _CalibratedModel.Value)
    let _constraint                                = triv _CalibratedModel (fun () -> _CalibratedModel.Value.CONSTRAINT())
    let _endCriteria                               = triv _CalibratedModel (fun () -> _CalibratedModel.Value.endCriteria())
    let _notifyObservers                           = triv _CalibratedModel (fun () -> _CalibratedModel.Value.notifyObservers()
                                                                                      _CalibratedModel.Value)
    let _parameters                                = triv _CalibratedModel (fun () -> _CalibratedModel.Value.parameters())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _CalibratedModel (fun () -> _CalibratedModel.Value.registerWith(handler.Value)
                                                                                      _CalibratedModel.Value)
    let _setParams                                 (parameters : ICell<Vector>)   
                                                   = triv _CalibratedModel (fun () -> _CalibratedModel.Value.setParams(parameters.Value)
                                                                                      _CalibratedModel.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _CalibratedModel (fun () -> _CalibratedModel.Value.unregisterWith(handler.Value)
                                                                                      _CalibratedModel.Value)
    let _update                                    = triv _CalibratedModel (fun () -> _CalibratedModel.Value.update()
                                                                                      _CalibratedModel.Value)
    let _value                                     (parameters : ICell<Vector>) (instruments : ICell<Generic.List<CalibrationHelper>>)   
                                                   = triv _CalibratedModel (fun () -> _CalibratedModel.Value.value(parameters.Value, instruments.Value))

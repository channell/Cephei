...
*)
    ...
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv (fun () -> _ATSCurrency.Value.Equals(o.Value))
    let _format                                    = triv (fun () -> _ATSCurrency.Value.format)
    let _fractionsPerUnit                          = triv (fun () -> _ATSCurrency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv (fun () -> _ATSCurrency.Value.fractionSymbol)
    let _name                                      = triv (fun () -> _ATSCurrency.Value.name)
    let _numericCode                               = triv (fun () -> _ATSCurrency.Value.numericCode)
    let _rounding                                  = triv (fun () -> _ATSCurrency.Value.rounding)
    let _symbol                                    = triv (fun () -> _ATSCurrency.Value.symbol)

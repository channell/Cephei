...
*)
    ...
    let _amount                                    (d : ICell<Date>) (notional : ICell<double>) (recoveryRate : ICell<double>)   
                                                   = cell (fun () -> _FaceValueAccrualClaim.Value.amount(d.Value, notional.Value, recoveryRate.Value))
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _FaceValueAccrualClaim.Value.registerWith(handler.Value)
                                                                     _FaceValueAccrualClaim.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _FaceValueAccrualClaim.Value.unregisterWith(handler.Value)
                                                                     _FaceValueAccrualClaim.Value)
    let _update                                    = cell (fun () -> _FaceValueAccrualClaim.Value.update()
                                                                     _FaceValueAccrualClaim.Value)

...
*)
    ...
    let _correlation                               (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LmConstWrapperCorrelationModel.Value.correlation(t.Value, x.Value))
    let _correlation1                              (i : ICell<int>) (j : ICell<int>) (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LmConstWrapperCorrelationModel.Value.correlation(i.Value, j.Value, t.Value, x.Value))
    let _factors                                   = cell (fun () -> _LmConstWrapperCorrelationModel.Value.factors())
    let _isTimeIndependent                         = cell (fun () -> _LmConstWrapperCorrelationModel.Value.isTimeIndependent())
    let _pseudoSqrt                                (t : ICell<double>) (x : ICell<Vector>)   
                                                   = cell (fun () -> _LmConstWrapperCorrelationModel.Value.pseudoSqrt(t.Value, x.Value))
    let _parameters                                = cell (fun () -> _LmConstWrapperCorrelationModel.Value.parameters())
    let _setParams                                 (arguments : ICell<Generic.List<Parameter>>)   
                                                   = cell (fun () -> _LmConstWrapperCorrelationModel.Value.setParams(arguments.Value)
                                                                     _LmConstWrapperCorrelationModel.Value)
    let _size                                      = cell (fun () -> _LmConstWrapperCorrelationModel.Value.size())

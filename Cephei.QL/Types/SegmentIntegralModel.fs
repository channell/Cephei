...
*)
    let mutable
        _SegmentIntegral                           = make (fun () -> new SegmentIntegral (intervals.Value))
    let _absoluteAccuracy                          = triv _SegmentIntegral (fun () -> _SegmentIntegral.Value.absoluteAccuracy())
    let _absoluteError                             = triv _SegmentIntegral (fun () -> _SegmentIntegral.Value.absoluteError())
    let _integrationSuccess                        = triv _SegmentIntegral (fun () -> _SegmentIntegral.Value.integrationSuccess())
    let _maxEvaluations                            = triv _SegmentIntegral (fun () -> _SegmentIntegral.Value.maxEvaluations())
    let _numberOfEvaluations                       = triv _SegmentIntegral (fun () -> _SegmentIntegral.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = triv _SegmentIntegral (fun () -> _SegmentIntegral.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                                      _SegmentIntegral.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = triv _SegmentIntegral (fun () -> _SegmentIntegral.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                                      _SegmentIntegral.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = triv _SegmentIntegral (fun () -> _SegmentIntegral.Value.value(f.Value, a.Value, b.Value))

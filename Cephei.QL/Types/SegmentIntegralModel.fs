...
*)
    ...
    let _absoluteAccuracy                          = cell (fun () -> _SegmentIntegral.Value.absoluteAccuracy())
    let _absoluteError                             = cell (fun () -> _SegmentIntegral.Value.absoluteError())
    let _integrationSuccess                        = cell (fun () -> _SegmentIntegral.Value.integrationSuccess())
    let _maxEvaluations                            = cell (fun () -> _SegmentIntegral.Value.maxEvaluations())
    let _numberOfEvaluations                       = cell (fun () -> _SegmentIntegral.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = cell (fun () -> _SegmentIntegral.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                     _SegmentIntegral.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = cell (fun () -> _SegmentIntegral.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                     _SegmentIntegral.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = cell (fun () -> _SegmentIntegral.Value.value(f.Value, a.Value, b.Value))

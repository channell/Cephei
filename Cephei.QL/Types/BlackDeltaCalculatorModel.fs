...
*)
    ...
    let _cumD1                                     (strike : ICell<double>)   
                                                   = triv (fun () -> _BlackDeltaCalculator.Value.cumD1(strike.Value))
    let _cumD2                                     (strike : ICell<double>)   
                                                   = triv (fun () -> _BlackDeltaCalculator.Value.cumD2(strike.Value))
    let _deltaFromStrike                           (strike : ICell<double>)   
                                                   = triv (fun () -> _BlackDeltaCalculator.Value.deltaFromStrike(strike.Value))
    let _nD1                                       (strike : ICell<double>)   
                                                   = triv (fun () -> _BlackDeltaCalculator.Value.nD1(strike.Value))
    let _nD2                                       (strike : ICell<double>)   
                                                   = triv (fun () -> _BlackDeltaCalculator.Value.nD2(strike.Value))
    let _setDeltaType                              (dt : ICell<DeltaVolQuote.DeltaType>)   
                                                   = triv (fun () -> _BlackDeltaCalculator.Value.setDeltaType(dt.Value)
                                                                     _BlackDeltaCalculator.Value)
    let _setOptionType                             (ot : ICell<Option.Type>)   
                                                   = triv (fun () -> _BlackDeltaCalculator.Value.setOptionType(ot.Value)
                                                                     _BlackDeltaCalculator.Value)

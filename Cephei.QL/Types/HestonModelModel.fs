...
*)
    ...
    let _kappa                                     = cell (fun () -> _HestonModel.Value.kappa())
    let _process                                   = cell (fun () -> _HestonModel.Value.PROCESS())
    let _rho                                       = cell (fun () -> _HestonModel.Value.rho())
    let _sigma                                     = cell (fun () -> _HestonModel.Value.sigma())
    let _theta                                     = cell (fun () -> _HestonModel.Value.theta())
    let _v0                                        = cell (fun () -> _HestonModel.Value.v0())
    let _calibrate                                 (instruments : ICell<Generic.List<CalibrationHelper>>) (Method : ICell<OptimizationMethod>) (endCriteria : ICell<EndCriteria>) (additionalConstraint : ICell<Constraint>) (weights : ICell<Generic.List<double>>) (fixParameters : ICell<Generic.List<bool>>)   
                                                   = cell (fun () -> _HestonModel.Value.calibrate(instruments.Value, Method.Value, endCriteria.Value, additionalConstraint.Value, weights.Value, fixParameters.Value)
                                                                     _HestonModel.Value)
    let _constraint                                = cell (fun () -> _HestonModel.Value.CONSTRAINT())
    let _endCriteria                               = cell (fun () -> _HestonModel.Value.endCriteria())
    let _notifyObservers                           = cell (fun () -> _HestonModel.Value.notifyObservers()
                                                                     _HestonModel.Value)
    let _parameters                                = cell (fun () -> _HestonModel.Value.parameters())
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _HestonModel.Value.registerWith(handler.Value)
                                                                     _HestonModel.Value)
    let _setParams                                 (parameters : ICell<Vector>)   
                                                   = cell (fun () -> _HestonModel.Value.setParams(parameters.Value)
                                                                     _HestonModel.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _HestonModel.Value.unregisterWith(handler.Value)
                                                                     _HestonModel.Value)
    let _update                                    = cell (fun () -> _HestonModel.Value.update()
                                                                     _HestonModel.Value)
    let _value                                     (parameters : ICell<Vector>) (instruments : ICell<Generic.List<CalibrationHelper>>)   
                                                   = cell (fun () -> _HestonModel.Value.value(parameters.Value, instruments.Value))

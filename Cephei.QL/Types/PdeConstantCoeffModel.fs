...
*)
    ...
    let _diffusion                                 (x : ICell<double>) (y : ICell<double>)   
                                                   = cell (fun () -> _PdeConstantCoeff.Value.diffusion(x.Value, y.Value))
    let _discount                                  (x : ICell<double>) (y : ICell<double>)   
                                                   = cell (fun () -> _PdeConstantCoeff.Value.discount(x.Value, y.Value))
    let _drift                                     (x : ICell<double>) (y : ICell<double>)   
                                                   = cell (fun () -> _PdeConstantCoeff.Value.drift(x.Value, y.Value))
    let _factory                                   (Process : ICell<GeneralizedBlackScholesProcess>)   
                                                   = cell (fun () -> _PdeConstantCoeff.Value.factory(Process.Value))
    let _generateOperator                          (t : ICell<double>) (tg : ICell<TransformedGrid>) (L : ICell<TridiagonalOperator>)   
                                                   = cell (fun () -> _PdeConstantCoeff.Value.generateOperator(t.Value, tg.Value, L.Value)

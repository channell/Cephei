...
*)
    ...
    let _factory                                   (nominal : ICell<double>) (paymentDate : ICell<Date>) (startDate : ICell<Date>) (endDate : ICell<Date>) (fixingDays : ICell<int>) (index : ICell<IborIndex>) (gearing : ICell<double>) (spread : ICell<double>) (cap : ICell<Nullable<double>>) (floor : ICell<Nullable<double>>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>) (dayCounter : ICell<DayCounter>) (isInArrears : ICell<bool>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.factory(nominal.Value, paymentDate.Value, startDate.Value, endDate.Value, fixingDays.Value, index.Value, gearing.Value, spread.Value, cap.Value, floor.Value, refPeriodStart.Value, refPeriodEnd.Value, dayCounter.Value, isInArrears.Value))
    let _cap                                       = cell (fun () -> _CappedFlooredIborCoupon.Value.cap())
    let _convexityAdjustment                       = cell (fun () -> _CappedFlooredIborCoupon.Value.convexityAdjustment())
    let _effectiveCap                              = cell (fun () -> _CappedFlooredIborCoupon.Value.effectiveCap())
    let _effectiveFloor                            = cell (fun () -> _CappedFlooredIborCoupon.Value.effectiveFloor())
    let _floor                                     = cell (fun () -> _CappedFlooredIborCoupon.Value.floor())
    let _isCapped                                  = cell (fun () -> _CappedFlooredIborCoupon.Value.isCapped())
    let _isFloored                                 = cell (fun () -> _CappedFlooredIborCoupon.Value.isFloored())
    let _rate                                      = cell (fun () -> _CappedFlooredIborCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.setPricer(pricer.Value)
                                                                     _CappedFlooredIborCoupon.Value)
    let _accruedAmount                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.accruedAmount(d.Value))
    let _adjustedFixing                            = cell (fun () -> _CappedFlooredIborCoupon.Value.adjustedFixing)
    let _amount                                    = cell (fun () -> _CappedFlooredIborCoupon.Value.amount())
    let _dayCounter                                = cell (fun () -> _CappedFlooredIborCoupon.Value.dayCounter())
    let _fixingDate                                = cell (fun () -> _CappedFlooredIborCoupon.Value.fixingDate())
    let _fixingDays                                = cell (fun () -> _CappedFlooredIborCoupon.Value.fixingDays)
    let _gearing                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.gearing())
    let _index                                     = cell (fun () -> _CappedFlooredIborCoupon.Value.index())
    let _indexFixing                               = cell (fun () -> _CappedFlooredIborCoupon.Value.indexFixing())
    let _isInArrears                               = cell (fun () -> _CappedFlooredIborCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.price(yts.Value))
    let _pricer                                    = cell (fun () -> _CappedFlooredIborCoupon.Value.pricer())
    let _spread                                    = cell (fun () -> _CappedFlooredIborCoupon.Value.spread())
    let _update                                    = cell (fun () -> _CappedFlooredIborCoupon.Value.update()
                                                                     _CappedFlooredIborCoupon.Value)
    let _accrualDays                               = cell (fun () -> _CappedFlooredIborCoupon.Value.accrualDays())
    let _accrualEndDate                            = cell (fun () -> _CappedFlooredIborCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = cell (fun () -> _CappedFlooredIborCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = cell (fun () -> _CappedFlooredIborCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = cell (fun () -> _CappedFlooredIborCoupon.Value.date())
    let _exCouponDate                              = cell (fun () -> _CappedFlooredIborCoupon.Value.exCouponDate())
    let _nominal                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.nominal())
    let _referencePeriodEnd                        = cell (fun () -> _CappedFlooredIborCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = cell (fun () -> _CappedFlooredIborCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.accept(v.Value)
                                                                     _CappedFlooredIborCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.registerWith(handler.Value)
                                                                     _CappedFlooredIborCoupon.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CappedFlooredIborCoupon.Value.unregisterWith(handler.Value)
                                                                     _CappedFlooredIborCoupon.Value)

...
*)
    ...
    let _value                                     = cell (fun () -> _AverageBMALeg.Value.value())
    let _withGearings                              (gearings : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _AverageBMALeg.Value.withGearings(gearings.Value))
    let _withGearings1                             (gearing : ICell<double>)   
                                                   = cell (fun () -> _AverageBMALeg.Value.withGearings(gearing.Value))
    let _withPaymentDayCounter                     (dayCounter : ICell<DayCounter>)   
                                                   = cell (fun () -> _AverageBMALeg.Value.withPaymentDayCounter(dayCounter.Value))
    let _withSpreads                               (spread : ICell<double>)   
                                                   = cell (fun () -> _AverageBMALeg.Value.withSpreads(spread.Value))
    let _withSpreads1                              (spreads : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _AverageBMALeg.Value.withSpreads(spreads.Value))
    let _withNotionals                             (notional : ICell<double>)   
                                                   = cell (fun () -> _AverageBMALeg.Value.withNotionals(notional.Value))
    let _withNotionals1                            (notionals : ICell<Generic.List<double>>)   
                                                   = cell (fun () -> _AverageBMALeg.Value.withNotionals(notionals.Value))
    let _withPaymentAdjustment                     (convention : ICell<BusinessDayConvention>)   
                                                   = cell (fun () -> _AverageBMALeg.Value.withPaymentAdjustment(convention.Value))

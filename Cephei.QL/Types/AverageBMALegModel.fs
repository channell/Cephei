...
*)
    ...
    let _withGearings1                             (gearing : ICell<double>)   
                                                   = triv (fun () -> _AverageBMALeg.Value.withGearings(gearing.Value))
    let _withPaymentDayCounter                     (dayCounter : ICell<DayCounter>)   
                                                   = triv (fun () -> _AverageBMALeg.Value.withPaymentDayCounter(dayCounter.Value))
    let _withSpreads                               (spread : ICell<double>)   
                                                   = triv (fun () -> _AverageBMALeg.Value.withSpreads(spread.Value))
    let _withSpreads1                              (spreads : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _AverageBMALeg.Value.withSpreads(spreads.Value))
    let _withNotionals                             (notional : ICell<double>)   
                                                   = triv (fun () -> _AverageBMALeg.Value.withNotionals(notional.Value))
    let _withNotionals1                            (notionals : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _AverageBMALeg.Value.withNotionals(notionals.Value))

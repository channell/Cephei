...
*)
    let mutable
        _GaussKronrodAdaptive                      = make (fun () -> new GaussKronrodAdaptive (absoluteAccuracy.Value, maxEvaluations.Value))
    let _absoluteAccuracy                          = triv _GaussKronrodAdaptive (fun () -> _GaussKronrodAdaptive.Value.absoluteAccuracy())
    let _absoluteError                             = triv _GaussKronrodAdaptive (fun () -> _GaussKronrodAdaptive.Value.absoluteError())
    let _integrationSuccess                        = triv _GaussKronrodAdaptive (fun () -> _GaussKronrodAdaptive.Value.integrationSuccess())
    let _maxEvaluations                            = triv _GaussKronrodAdaptive (fun () -> _GaussKronrodAdaptive.Value.maxEvaluations())
    let _numberOfEvaluations                       = triv _GaussKronrodAdaptive (fun () -> _GaussKronrodAdaptive.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = triv _GaussKronrodAdaptive (fun () -> _GaussKronrodAdaptive.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                                           _GaussKronrodAdaptive.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = triv _GaussKronrodAdaptive (fun () -> _GaussKronrodAdaptive.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                                           _GaussKronrodAdaptive.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = triv _GaussKronrodAdaptive (fun () -> _GaussKronrodAdaptive.Value.value(f.Value, a.Value, b.Value))

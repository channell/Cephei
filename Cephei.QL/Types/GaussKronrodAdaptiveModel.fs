...
*)
    ...
    let _absoluteAccuracy                          = cell (fun () -> _GaussKronrodAdaptive.Value.absoluteAccuracy())
    let _absoluteError                             = cell (fun () -> _GaussKronrodAdaptive.Value.absoluteError())
    let _integrationSuccess                        = cell (fun () -> _GaussKronrodAdaptive.Value.integrationSuccess())
    let _maxEvaluations                            = cell (fun () -> _GaussKronrodAdaptive.Value.maxEvaluations())
    let _numberOfEvaluations                       = cell (fun () -> _GaussKronrodAdaptive.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = cell (fun () -> _GaussKronrodAdaptive.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                     _GaussKronrodAdaptive.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = cell (fun () -> _GaussKronrodAdaptive.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                     _GaussKronrodAdaptive.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = cell (fun () -> _GaussKronrodAdaptive.Value.value(f.Value, a.Value, b.Value))

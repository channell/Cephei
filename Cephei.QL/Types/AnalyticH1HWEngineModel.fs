...
*)
    let mutable
        _AnalyticH1HWEngine                        = make (fun () -> (createEvaluationDate _evaluationDate (fun () ->new AnalyticH1HWEngine (model.Value, hullWhiteModel.Value, rhoSr.Value, integrationOrder.Value))))
    let _update                                    = triv _AnalyticH1HWEngine (fun () -> (curryEvaluationDate _evaluationDate _AnalyticH1HWEngine).Value.update()
                                                                                         _AnalyticH1HWEngine.Value)
    let _numberOfEvaluations                       = triv _AnalyticH1HWEngine (fun () -> (curryEvaluationDate _evaluationDate _AnalyticH1HWEngine).Value.numberOfEvaluations())
    let _setModel                                  (model : ICell<Handle<HestonModel>>)   
                                                   = triv _AnalyticH1HWEngine (fun () -> (curryEvaluationDate _evaluationDate _AnalyticH1HWEngine).Value.setModel(model.Value)
                                                                                         _AnalyticH1HWEngine.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv _AnalyticH1HWEngine (fun () -> (curryEvaluationDate _evaluationDate _AnalyticH1HWEngine).Value.registerWith(handler.Value)
                                                                                         _AnalyticH1HWEngine.Value)
    let _reset                                     = triv _AnalyticH1HWEngine (fun () -> (curryEvaluationDate _evaluationDate _AnalyticH1HWEngine).Value.reset()
                                                                                         _AnalyticH1HWEngine.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = triv _AnalyticH1HWEngine (fun () -> (curryEvaluationDate _evaluationDate _AnalyticH1HWEngine).Value.unregisterWith(handler.Value)
                                                                                         _AnalyticH1HWEngine.Value)
...
*)
    let mutable
        _AnalyticH1HWEngine                        = make (fun () -> (createEvaluationDate _evaluationDate (fun () ->new AnalyticH1HWEngine (model.Value, hullWhiteModel.Value, rhoSr.Value, relTolerance.Value, maxEvaluations.Value))))
    let _update                                    = triv _AnalyticH1HWEngine (fun () -> (curryEvaluationDate _evaluationDate _AnalyticH1HWEngine).Value.update()
                                                                                         _AnalyticH1HWEngine.Value)
    let _numberOfEvaluations                       = triv _AnalyticH1HWEngine (fun () -> (curryEvaluationDate _evaluationDate _AnalyticH1HWEngine).Value.numberOfEvaluations())
    let _setModel                                  (model : ICell<Handle<HestonModel>>)   

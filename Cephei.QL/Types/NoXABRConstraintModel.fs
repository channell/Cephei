...
*)
    ...
    let _config                                    (costFunction : ICell<ProjectedCostFunction>) (coeff : ICell<XABRCoeffHolder<'Model>>) (forward : ICell<double>)   
                                                   = cell (fun () -> _NoXABRConstraint.Value.config(costFunction.Value, coeff.Value, forward.Value)
                                                                     _NoXABRConstraint.Value)
    let _empty                                     = cell (fun () -> _NoXABRConstraint.Value.empty())
    let _lowerBound                                (parameters : ICell<Vector>)   
                                                   = cell (fun () -> _NoXABRConstraint.Value.lowerBound(parameters.Value))
    let _test                                      (p : ICell<Vector>)   
                                                   = cell (fun () -> _NoXABRConstraint.Value.test(p.Value))
    let _update                                    (p : ICell<Vector ref>) (direction : ICell<Vector>) (beta : ICell<double>)   
                                                   = cell (fun () -> _NoXABRConstraint.Value.update(p.Value, direction.Value, beta.Value))
    let _upperBound                                (parameters : ICell<Vector>)   
                                                   = cell (fun () -> _NoXABRConstraint.Value.upperBound(parameters.Value))

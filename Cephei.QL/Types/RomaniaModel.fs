...
*)
    let mutable
        _Romania                                   = make (fun () -> new Romania ())
    let _addedHolidays                             = triv _Romania (fun () -> _Romania.Value.addedHolidays)
    let _addHoliday                                (d : ICell<Date>)   
                                                   = triv _Romania (fun () -> _Romania.Value.addHoliday(d.Value)
                                                                              _Romania.Value)
    let _adjust                                    (d : ICell<Date>) (c : ICell<BusinessDayConvention>)   
                                                   = triv _Romania (fun () -> _Romania.Value.adjust(d.Value, c.Value))
    let _advance                                   (d : ICell<Date>) (p : ICell<Period>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Romania (fun () -> _Romania.Value.advance(d.Value, p.Value, c.Value, endOfMonth.Value))
    let _advance1                                  (d : ICell<Date>) (n : ICell<int>) (unit : ICell<TimeUnit>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Romania (fun () -> _Romania.Value.advance(d.Value, n.Value, unit.Value, c.Value, endOfMonth.Value))
    let _businessDaysBetween                       (from : ICell<Date>) (To : ICell<Date>) (includeFirst : ICell<bool>) (includeLast : ICell<bool>)   
                                                   = triv _Romania (fun () -> _Romania.Value.businessDaysBetween(from.Value, To.Value, includeFirst.Value, includeLast.Value))
    let _calendar                                  = triv _Romania (fun () -> _Romania.Value.calendar)
    let _empty                                     = triv _Romania (fun () -> _Romania.Value.empty())
    let _endOfMonth                                (d : ICell<Date>)   
                                                   = triv _Romania (fun () -> _Romania.Value.endOfMonth(d.Value))
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Romania (fun () -> _Romania.Value.Equals(o.Value))
    let _isBusinessDay                             (d : ICell<Date>)   
                                                   = triv _Romania (fun () -> _Romania.Value.isBusinessDay(d.Value))
    let _isEndOfMonth                              (d : ICell<Date>)   
                                                   = triv _Romania (fun () -> _Romania.Value.isEndOfMonth(d.Value))
    let _isHoliday                                 (d : ICell<Date>)   
                                                   = triv _Romania (fun () -> _Romania.Value.isHoliday(d.Value))
    let _isWeekend                                 (w : ICell<DayOfWeek>)   
                                                   = triv _Romania (fun () -> _Romania.Value.isWeekend(w.Value))
    let _name                                      = triv _Romania (fun () -> _Romania.Value.name())
    let _removedHolidays                           = triv _Romania (fun () -> _Romania.Value.removedHolidays)
    let _removeHoliday                             (d : ICell<Date>)   
                                                   = triv _Romania (fun () -> _Romania.Value.removeHoliday(d.Value)
                                                                              _Romania.Value)
...
*)
    ...
    member internal this.Inject v = _Romania <- v
    static member Cast (p : ICell<Romania>) = 
        if p :? RomaniaModel then 
            p :?> RomaniaModel
        else
            let o = new RomaniaModel ()
            o.Inject p
            o.Bind p
            o

...
*)
    ...
    let _dimension                                 = cell (fun () -> _SobolRsg.Value.dimension())
    let _factory                                   (dimensionality : ICell<int>) (seed : ICell<uint64>)   
                                                   = cell (fun () -> _SobolRsg.Value.factory(dimensionality.Value, seed.Value))
    let _lastSequence                              = cell (fun () -> _SobolRsg.Value.lastSequence())
    let _nextInt32Sequence                         = cell (fun () -> _SobolRsg.Value.nextInt32Sequence())
    let _nextSequence                              = cell (fun () -> _SobolRsg.Value.nextSequence())
    let _skipTo                                    (skip : ICell<uint64>)   
                                                   = cell (fun () -> _SobolRsg.Value.skipTo(skip.Value)
                                                                     _SobolRsg.Value)

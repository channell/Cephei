...
*)
    ...
    let _dates                                     = triv (fun () -> _InterpolatedZeroCurve.Value.dates())
    let _dates_                                    = triv (fun () -> _InterpolatedZeroCurve.Value.dates_)
    let _interpolation_                            = triv (fun () -> _InterpolatedZeroCurve.Value.interpolation_)
    let _interpolator_                             = triv (fun () -> _InterpolatedZeroCurve.Value.interpolator_)
    let _maxDate                                   = triv (fun () -> _InterpolatedZeroCurve.Value.maxDate())
    let _maxDate_                                  = triv (fun () -> _InterpolatedZeroCurve.Value.maxDate_)
    let _nodes                                     = triv (fun () -> _InterpolatedZeroCurve.Value.nodes())
    let _setupInterpolation                        = triv (fun () -> _InterpolatedZeroCurve.Value.setupInterpolation()
                                                                     _InterpolatedZeroCurve.Value)
    let _times                                     = triv (fun () -> _InterpolatedZeroCurve.Value.times())
    let _times_                                    = triv (fun () -> _InterpolatedZeroCurve.Value.times_)
    let _zeroRates                                 = triv (fun () -> _InterpolatedZeroCurve.Value.zeroRates())

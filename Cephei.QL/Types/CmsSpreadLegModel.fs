...
*)
    ...
    let _inArrears1                                (flag : ICell<bool>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.inArrears(flag.Value))
    let _withCaps                                  (caps : ICell<Generic.List<Nullable<double>>>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withCaps(caps.Value))
    let _withCaps1                                 (cap : ICell<Nullable<double>>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withCaps(cap.Value))
    let _withFixingDays                            (fixingDays : ICell<int>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withFixingDays(fixingDays.Value))
    let _withFixingDays1                           (fixingDays : ICell<Generic.List<int>>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withFixingDays(fixingDays.Value))
    let _withFloors                                (floor : ICell<Nullable<double>>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withFloors(floor.Value))
    let _withFloors1                               (floors : ICell<Generic.List<Nullable<double>>>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withFloors(floors.Value))
    let _withGearings                              (gearing : ICell<double>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withGearings(gearing.Value))
    let _withGearings1                             (gearings : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withGearings(gearings.Value))
    let _withPaymentDayCounter                     (dayCounter : ICell<DayCounter>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withPaymentDayCounter(dayCounter.Value))
    let _withSpreads                               (spread : ICell<double>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withSpreads(spread.Value))
    let _withSpreads1                              (spreads : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withSpreads(spreads.Value))
    let _withZeroPayments                          = triv (fun () -> _CmsSpreadLeg.Value.withZeroPayments())
    let _withZeroPayments1                         (flag : ICell<bool>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withZeroPayments(flag.Value))
    let _withNotionals                             (notional : ICell<double>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withNotionals(notional.Value))
    let _withNotionals1                            (notionals : ICell<Generic.List<double>>)   
                                                   = triv (fun () -> _CmsSpreadLeg.Value.withNotionals(notionals.Value))

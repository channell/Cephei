...
*)
    ...
    let _constraint                                = cell (fun () -> _PiecewiseConstantParameter.Value.CONSTRAINT())
    let _implementation                            = cell (fun () -> _PiecewiseConstantParameter.Value.implementation())
    let _parameters                                = cell (fun () -> _PiecewiseConstantParameter.Value.parameters())
    let _setParam                                  (i : ICell<int>) (x : ICell<double>)   
                                                   = cell (fun () -> _PiecewiseConstantParameter.Value.setParam(i.Value, x.Value)
                                                                     _PiecewiseConstantParameter.Value)
    let _size                                      = cell (fun () -> _PiecewiseConstantParameter.Value.size())
    let _testParams                                (p : ICell<Vector>)   
                                                   = cell (fun () -> _PiecewiseConstantParameter.Value.testParams(p.Value))
    let _value                                     (t : ICell<double>)   
                                                   = cell (fun () -> _PiecewiseConstantParameter.Value.value(t.Value))

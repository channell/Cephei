...
*)
    let mutable
        _Currency                                  = make (fun () -> new Currency ())
    let _code                                      = triv _Currency (fun () -> _Currency.Value.code)
    let _empty                                     = triv _Currency (fun () -> _Currency.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Currency (fun () -> _Currency.Value.Equals(o.Value))
    let _format                                    = triv _Currency (fun () -> _Currency.Value.format)
    let _fractionsPerUnit                          = triv _Currency (fun () -> _Currency.Value.fractionsPerUnit)
    let _fractionSymbol                            = triv _Currency (fun () -> _Currency.Value.fractionSymbol)
    let _name                                      = triv _Currency (fun () -> _Currency.Value.name)
    let _numericCode                               = triv _Currency (fun () -> _Currency.Value.numericCode)
    let _rounding                                  = triv _Currency (fun () -> _Currency.Value.rounding)
    let _symbol                                    = triv _Currency (fun () -> _Currency.Value.symbol)
    let _ToString                                  = triv _Currency (fun () -> _Currency.Value.ToString())
    let _triangulationCurrency                     = triv _Currency (fun () -> _Currency.Value.triangulationCurrency)
...
*)
    let mutable
        _Currency                                  = make (fun () -> new Currency (name.Value, code.Value, numericCode.Value, symbol.Value, fractionSymbol.Value, fractionsPerUnit.Value, rounding.Value, formatString.Value))
    let _code                                      = triv _Currency (fun () -> _Currency.Value.code)
    let _empty                                     = triv _Currency (fun () -> _Currency.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Currency (fun () -> _Currency.Value.Equals(o.Value))
...
*)
    let mutable
        _Currency                                  = make (fun () -> new Currency (name.Value, code.Value, numericCode.Value, symbol.Value, fractionSymbol.Value, fractionsPerUnit.Value, rounding.Value, formatString.Value, triangulationCurrency.Value))
    let _code                                      = triv _Currency (fun () -> _Currency.Value.code)
    let _empty                                     = triv _Currency (fun () -> _Currency.Value.empty())
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Currency (fun () -> _Currency.Value.Equals(o.Value))

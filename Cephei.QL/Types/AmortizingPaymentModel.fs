...
*)
    ...
    let _amount                                    = cell (fun () -> _AmortizingPayment.Value.amount())
    let _date                                      = cell (fun () -> _AmortizingPayment.Value.date())
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _AmortizingPayment.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _AmortizingPayment.Value.Equals(cf.Value))
    let _exCouponDate                              = cell (fun () -> _AmortizingPayment.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _AmortizingPayment.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _AmortizingPayment.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _AmortizingPayment.Value.accept(v.Value)
                                                                     _AmortizingPayment.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _AmortizingPayment.Value.registerWith(handler.Value)
                                                                     _AmortizingPayment.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _AmortizingPayment.Value.unregisterWith(handler.Value)
                                                                     _AmortizingPayment.Value)

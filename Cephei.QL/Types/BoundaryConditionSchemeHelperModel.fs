...
*)
    ...
    let _applyAfterSolving                         (a : ICell<Vector>)   
                                                   = triv (fun () -> _BoundaryConditionSchemeHelper.Value.applyAfterSolving(a.Value)
                                                                     _BoundaryConditionSchemeHelper.Value)
    let _applyBeforeApplying                       (op : ICell<IOperator>)   
                                                   = triv (fun () -> _BoundaryConditionSchemeHelper.Value.applyBeforeApplying(op.Value)
                                                                     _BoundaryConditionSchemeHelper.Value)
    let _applyBeforeSolving                        (op : ICell<IOperator>) (a : ICell<Vector>)   
                                                   = triv (fun () -> _BoundaryConditionSchemeHelper.Value.applyBeforeSolving(op.Value, a.Value)
                                                                     _BoundaryConditionSchemeHelper.Value)

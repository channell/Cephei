...
*)
    let mutable
        _BoundaryConditionSchemeHelper             = make (fun () -> new BoundaryConditionSchemeHelper (bcSet.Value))
    let _applyAfterApplying                        (a : ICell<Vector>)   
                                                   = triv _BoundaryConditionSchemeHelper (fun () -> _BoundaryConditionSchemeHelper.Value.applyAfterApplying(a.Value)
                                                                                                    _BoundaryConditionSchemeHelper.Value)
    let _applyAfterSolving                         (a : ICell<Vector>)   
                                                   = triv _BoundaryConditionSchemeHelper (fun () -> _BoundaryConditionSchemeHelper.Value.applyAfterSolving(a.Value)
                                                                                                    _BoundaryConditionSchemeHelper.Value)
    let _applyBeforeApplying                       (op : ICell<IOperator>)   
                                                   = triv _BoundaryConditionSchemeHelper (fun () -> _BoundaryConditionSchemeHelper.Value.applyBeforeApplying(op.Value)
                                                                                                    _BoundaryConditionSchemeHelper.Value)
    let _applyBeforeSolving                        (op : ICell<IOperator>) (a : ICell<Vector>)   
                                                   = triv _BoundaryConditionSchemeHelper (fun () -> _BoundaryConditionSchemeHelper.Value.applyBeforeSolving(op.Value, a.Value)
                                                                                                    _BoundaryConditionSchemeHelper.Value)    
    let _setTime                                   (t : ICell<double>)   
                                                   = triv _BoundaryConditionSchemeHelper (fun () -> _BoundaryConditionSchemeHelper.Value.setTime(t.Value)
                                                                                                    _BoundaryConditionSchemeHelper.Value)

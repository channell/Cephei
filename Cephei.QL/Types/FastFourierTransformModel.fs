...
*)
    ...
    let _inverse_transform                         (input : ICell<Generic.List<Complex>>) (inputBeg : ICell<int>) (inputEnd : ICell<int>) (output : ICell<Generic.List<Complex>>)   
                                                   = cell (fun () -> _FastFourierTransform.Value.inverse_transform(input.Value, inputBeg.Value, inputEnd.Value, output.Value)
                                                                     _FastFourierTransform.Value)
    let _output_size                               = cell (fun () -> _FastFourierTransform.Value.output_size())
    let _transform                                 (input : ICell<Generic.List<Complex>>) (inputBeg : ICell<int>) (inputEnd : ICell<int>) (output : ICell<Generic.List<Complex>>)   
                                                   = cell (fun () -> _FastFourierTransform.Value.transform(input.Value, inputBeg.Value, inputEnd.Value, output.Value)
                                                                     _FastFourierTransform.Value)

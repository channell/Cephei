...
*)
    let mutable
        _TrinomialTree                             = make (fun () -> new TrinomialTree (Process.Value, timeGrid.Value))
    let _descendant                                (i : ICell<int>) (index : ICell<int>) (branch : ICell<int>)   
                                                   = triv _TrinomialTree (fun () -> _TrinomialTree.Value.descendant(i.Value, index.Value, branch.Value))
    let _dx                                        (i : ICell<int>)   
                                                   = triv _TrinomialTree (fun () -> _TrinomialTree.Value.dx(i.Value))
    let _probability                               (i : ICell<int>) (index : ICell<int>) (branch : ICell<int>)   
                                                   = triv _TrinomialTree (fun () -> _TrinomialTree.Value.probability(i.Value, index.Value, branch.Value))
    let _size                                      (i : ICell<int>)   
                                                   = triv _TrinomialTree (fun () -> _TrinomialTree.Value.size(i.Value))
    let _timeGrid                                  = triv _TrinomialTree (fun () -> _TrinomialTree.Value.timeGrid())
    let _underlying                                (i : ICell<int>) (index : ICell<int>)   
                                                   = triv _TrinomialTree (fun () -> _TrinomialTree.Value.underlying(i.Value, index.Value))
    let _columns                                   = triv _TrinomialTree (fun () -> _TrinomialTree.Value.columns())
...
*)
    let mutable
        _TrinomialTree                             = make (fun () -> new TrinomialTree (Process.Value, timeGrid.Value, isPositive.Value))
    let _descendant                                (i : ICell<int>) (index : ICell<int>) (branch : ICell<int>)   
                                                   = triv _TrinomialTree (fun () -> _TrinomialTree.Value.descendant(i.Value, index.Value, branch.Value))
    let _dx                                        (i : ICell<int>)   
                                                   = triv _TrinomialTree (fun () -> _TrinomialTree.Value.dx(i.Value))

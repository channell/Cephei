...
*)
    ...
    let _reset                                     (size : ICell<int>)   
                                                   = cell (fun () -> _DiscretizedSwaption.Value.reset(size.Value)
                                                                     _DiscretizedSwaption.Value)
    let _withinNextWeek                            (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = cell (fun () -> _DiscretizedSwaption.Value.withinNextWeek(d1.Value, d2.Value))
    let _withinPreviousWeek                        (d1 : ICell<Date>) (d2 : ICell<Date>)   
                                                   = cell (fun () -> _DiscretizedSwaption.Value.withinPreviousWeek(d1.Value, d2.Value))
    let _mandatoryTimes                            = cell (fun () -> _DiscretizedSwaption.Value.mandatoryTimes())
    let _adjustValues                              = cell (fun () -> _DiscretizedSwaption.Value.adjustValues()
                                                                     _DiscretizedSwaption.Value)
    let _initialize                                (Method : ICell<Lattice>) (t : ICell<double>)   
                                                   = cell (fun () -> _DiscretizedSwaption.Value.initialize(Method.Value, t.Value)
                                                                     _DiscretizedSwaption.Value)
    let _method                                    = cell (fun () -> _DiscretizedSwaption.Value.METHOD())
    let _partialRollback                           (To : ICell<double>)   
                                                   = cell (fun () -> _DiscretizedSwaption.Value.partialRollback(To.Value)
                                                                     _DiscretizedSwaption.Value)
    let _postAdjustValues                          = cell (fun () -> _DiscretizedSwaption.Value.postAdjustValues()
                                                                     _DiscretizedSwaption.Value)
    let _preAdjustValues                           = cell (fun () -> _DiscretizedSwaption.Value.preAdjustValues()
                                                                     _DiscretizedSwaption.Value)
    let _presentValue                              = cell (fun () -> _DiscretizedSwaption.Value.presentValue())
    let _rollback                                  (To : ICell<double>)   
                                                   = cell (fun () -> _DiscretizedSwaption.Value.rollback(To.Value)
                                                                     _DiscretizedSwaption.Value)
    let _setTime                                   (t : ICell<double>)   
                                                   = cell (fun () -> _DiscretizedSwaption.Value.setTime(t.Value)
                                                                     _DiscretizedSwaption.Value)
    let _setValues                                 (v : ICell<Vector>)   
                                                   = cell (fun () -> _DiscretizedSwaption.Value.setValues(v.Value)
                                                                     _DiscretizedSwaption.Value)
    let _time                                      = cell (fun () -> _DiscretizedSwaption.Value.time())
    let _values                                    = cell (fun () -> _DiscretizedSwaption.Value.values())

...
*)
    ...
    let _values                                    (x : ICell<Vector>)   
                                                   = triv (fun () -> _PenaltyFunction.Value.values(x.Value))
    let _finiteDifferenceEpsilon                   = triv (fun () -> _PenaltyFunction.Value.finiteDifferenceEpsilon())
    let _gradient                                  (grad : ICell<Vector>) (x : ICell<Vector>)   
                                                   = triv (fun () -> _PenaltyFunction.Value.gradient(ref grad.Value, x.Value)
                                                                     _PenaltyFunction.Value)
    let _jacobian                                  (jac : ICell<Matrix>) (x : ICell<Vector>)   
                                                   = triv (fun () -> _PenaltyFunction.Value.jacobian(jac.Value, x.Value)
                                                                     _PenaltyFunction.Value)
    let _valueAndGradient                          (grad : ICell<Vector>) (x : ICell<Vector>)   
                                                   = triv (fun () -> _PenaltyFunction.Value.valueAndGradient(ref grad.Value, x.Value))
    let _valuesAndJacobian                         (jac : ICell<Matrix>) (x : ICell<Vector>)   
                                                   = triv (fun () -> _PenaltyFunction.Value.valuesAndJacobian(jac.Value, x.Value))

...
*)
    let mutable
        _PenaltyFunction                           = make (fun () -> new PenaltyFunction<'T,'U> (curve.Value, initialIndex.Value, rateHelpers.Value, start.Value, End.Value))
    let _value                                     (x : ICell<Vector>)   
                                                   = triv _PenaltyFunction (fun () -> _PenaltyFunction.Value.value(x.Value))
    let _values                                    (x : ICell<Vector>)   
                                                   = triv _PenaltyFunction (fun () -> _PenaltyFunction.Value.values(x.Value))
    let _finiteDifferenceEpsilon                   = triv _PenaltyFunction (fun () -> _PenaltyFunction.Value.finiteDifferenceEpsilon())
    let _gradient                                  (grad : ICell<Vector>) (x : ICell<Vector>)   
                                                   = triv _PenaltyFunction (fun () -> _PenaltyFunction.Value.gradient(ref grad.Value, x.Value)
                                                                                      _PenaltyFunction.Value)
    let _jacobian                                  (jac : ICell<Matrix>) (x : ICell<Vector>)   
                                                   = triv _PenaltyFunction (fun () -> _PenaltyFunction.Value.jacobian(jac.Value, x.Value)
                                                                                      _PenaltyFunction.Value)
    let _valueAndGradient                          (grad : ICell<Vector>) (x : ICell<Vector>)   
                                                   = triv _PenaltyFunction (fun () -> _PenaltyFunction.Value.valueAndGradient(ref grad.Value, x.Value))
    let _valuesAndJacobian                         (jac : ICell<Matrix>) (x : ICell<Vector>)   
                                                   = triv _PenaltyFunction (fun () -> _PenaltyFunction.Value.valuesAndJacobian(jac.Value, x.Value))

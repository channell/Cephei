...
*)
    ...
    let _amount                                    = cell (fun () -> _CPICashFlow.Value.amount())
    let _baseDate                                  = cell (fun () -> _CPICashFlow.Value.baseDate())
    let _baseFixing                                = cell (fun () -> _CPICashFlow.Value.baseFixing())
    let _frequency                                 = cell (fun () -> _CPICashFlow.Value.frequency())
    let _interpolation                             = cell (fun () -> _CPICashFlow.Value.interpolation())
    let _date                                      = cell (fun () -> _CPICashFlow.Value.date())
    let _fixingDate                                = cell (fun () -> _CPICashFlow.Value.fixingDate())
    let _growthOnly                                = cell (fun () -> _CPICashFlow.Value.growthOnly())
    let _index                                     = cell (fun () -> _CPICashFlow.Value.index())
    let _notional                                  = cell (fun () -> _CPICashFlow.Value.notional())
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = cell (fun () -> _CPICashFlow.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = cell (fun () -> _CPICashFlow.Value.Equals(cf.Value))
    let _exCouponDate                              = cell (fun () -> _CPICashFlow.Value.exCouponDate())
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = cell (fun () -> _CPICashFlow.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = cell (fun () -> _CPICashFlow.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = cell (fun () -> _CPICashFlow.Value.accept(v.Value)
                                                                     _CPICashFlow.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CPICashFlow.Value.registerWith(handler.Value)
                                                                     _CPICashFlow.Value)
    let _unregisterWith                            (handler : ICell<Callback>)   
                                                   = cell (fun () -> _CPICashFlow.Value.unregisterWith(handler.Value)
                                                                     _CPICashFlow.Value)

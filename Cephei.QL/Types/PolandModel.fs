...
*)
    let mutable
        _Poland                                    = make (fun () -> new Poland ())
    let _addedHolidays                             = triv _Poland (fun () -> _Poland.Value.addedHolidays)
    let _addHoliday                                (d : ICell<Date>)   
                                                   = triv _Poland (fun () -> _Poland.Value.addHoliday(d.Value)
                                                                             _Poland.Value)
    let _adjust                                    (d : ICell<Date>) (c : ICell<BusinessDayConvention>)   
                                                   = triv _Poland (fun () -> _Poland.Value.adjust(d.Value, c.Value))
    let _advance                                   (d : ICell<Date>) (p : ICell<Period>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Poland (fun () -> _Poland.Value.advance(d.Value, p.Value, c.Value, endOfMonth.Value))
    let _advance1                                  (d : ICell<Date>) (n : ICell<int>) (unit : ICell<TimeUnit>) (c : ICell<BusinessDayConvention>) (endOfMonth : ICell<bool>)   
                                                   = triv _Poland (fun () -> _Poland.Value.advance(d.Value, n.Value, unit.Value, c.Value, endOfMonth.Value))
    let _businessDaysBetween                       (from : ICell<Date>) (To : ICell<Date>) (includeFirst : ICell<bool>) (includeLast : ICell<bool>)   
                                                   = triv _Poland (fun () -> _Poland.Value.businessDaysBetween(from.Value, To.Value, includeFirst.Value, includeLast.Value))
    let _calendar                                  = triv _Poland (fun () -> _Poland.Value.calendar)
    let _empty                                     = triv _Poland (fun () -> _Poland.Value.empty())
    let _endOfMonth                                (d : ICell<Date>)   
                                                   = triv _Poland (fun () -> _Poland.Value.endOfMonth(d.Value))
    let _Equals                                    (o : ICell<Object>)   
                                                   = triv _Poland (fun () -> _Poland.Value.Equals(o.Value))
    let _isBusinessDay                             (d : ICell<Date>)   
                                                   = triv _Poland (fun () -> _Poland.Value.isBusinessDay(d.Value))
    let _isEndOfMonth                              (d : ICell<Date>)   
                                                   = triv _Poland (fun () -> _Poland.Value.isEndOfMonth(d.Value))
    let _isHoliday                                 (d : ICell<Date>)   
                                                   = triv _Poland (fun () -> _Poland.Value.isHoliday(d.Value))
    let _isWeekend                                 (w : ICell<DayOfWeek>)   
                                                   = triv _Poland (fun () -> _Poland.Value.isWeekend(w.Value))
    let _name                                      = triv _Poland (fun () -> _Poland.Value.name())
    let _removedHolidays                           = triv _Poland (fun () -> _Poland.Value.removedHolidays)
    let _removeHoliday                             (d : ICell<Date>)   
                                                   = triv _Poland (fun () -> _Poland.Value.removeHoliday(d.Value)
                                                                             _Poland.Value)
...
*)
    ...
    member internal this.Inject v = _Poland <- v

...
*)
    let mutable
        _FloatingLoan                              = cell (fun () -> withEngine pricingEngine evaluationDate (new FloatingLoan (Type.Value, nominal.Value, floatingSchedule.Value, floatingSpread.Value, floatingDayCount.Value, principalSchedule.Value, paymentConvention.Value, index.Value)))

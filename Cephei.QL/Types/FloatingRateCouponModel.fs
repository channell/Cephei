...
*)
    ...
    let _amount                                    = triv (fun () -> _FloatingRateCoupon.Value.amount())
    let _convexityAdjustment                       = triv (fun () -> _FloatingRateCoupon.Value.convexityAdjustment())
    let _dayCounter                                = triv (fun () -> _FloatingRateCoupon.Value.dayCounter())
    let _factory                                   (nominal : ICell<double>) (paymentDate : ICell<Date>) (startDate : ICell<Date>) (endDate : ICell<Date>) (fixingDays : ICell<int>) (index : ICell<InterestRateIndex>) (gearing : ICell<double>) (spread : ICell<double>) (refPeriodStart : ICell<Date>) (refPeriodEnd : ICell<Date>) (dayCounter : ICell<DayCounter>) (isInArrears : ICell<bool>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.factory(nominal.Value, paymentDate.Value, startDate.Value, endDate.Value, fixingDays.Value, index.Value, gearing.Value, spread.Value, refPeriodStart.Value, refPeriodEnd.Value, dayCounter.Value, isInArrears.Value))
    let _fixingDate                                = triv (fun () -> _FloatingRateCoupon.Value.fixingDate())
    let _fixingDays                                = triv (fun () -> _FloatingRateCoupon.Value.fixingDays)
    let _gearing                                   = triv (fun () -> _FloatingRateCoupon.Value.gearing())
    let _index                                     = triv (fun () -> _FloatingRateCoupon.Value.index())
    let _indexFixing                               = triv (fun () -> _FloatingRateCoupon.Value.indexFixing())
    let _isInArrears                               = triv (fun () -> _FloatingRateCoupon.Value.isInArrears())
    let _price                                     (yts : ICell<YieldTermStructure>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.price(yts.Value))
    let _pricer                                    = triv (fun () -> _FloatingRateCoupon.Value.pricer())
    let _rate                                      = triv (fun () -> _FloatingRateCoupon.Value.rate())
    let _setPricer                                 (pricer : ICell<FloatingRateCouponPricer>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.setPricer(pricer.Value)
                                                                     _FloatingRateCoupon.Value)
    let _spread                                    = triv (fun () -> _FloatingRateCoupon.Value.spread())
    let _update                                    = triv (fun () -> _FloatingRateCoupon.Value.update()
                                                                     _FloatingRateCoupon.Value)
    let _accrualDays                               = triv (fun () -> _FloatingRateCoupon.Value.accrualDays())
    let _accrualEndDate                            = triv (fun () -> _FloatingRateCoupon.Value.accrualEndDate())
    let _accrualPeriod                             = triv (fun () -> _FloatingRateCoupon.Value.accrualPeriod())
    let _accrualStartDate                          = triv (fun () -> _FloatingRateCoupon.Value.accrualStartDate())
    let _accruedDays                               (d : ICell<Date>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.accruedDays(d.Value))
    let _accruedPeriod                             (d : ICell<Date>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.accruedPeriod(d.Value))
    let _date                                      = triv (fun () -> _FloatingRateCoupon.Value.date())
    let _exCouponDate                              = triv (fun () -> _FloatingRateCoupon.Value.exCouponDate())
    let _nominal                                   = triv (fun () -> _FloatingRateCoupon.Value.nominal())
    let _referencePeriodEnd                        = triv (fun () -> _FloatingRateCoupon.Value.referencePeriodEnd)
    let _referencePeriodStart                      = triv (fun () -> _FloatingRateCoupon.Value.referencePeriodStart)
    let _CompareTo                                 (cf : ICell<CashFlow>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.CompareTo(cf.Value))
    let _Equals                                    (cf : ICell<Object>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.Equals(cf.Value))
    let _hasOccurred                               (refDate : ICell<Date>) (includeRefDate : ICell<Nullable<bool>>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.hasOccurred(refDate.Value, includeRefDate.Value))
    let _tradingExCoupon                           (refDate : ICell<Date>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.tradingExCoupon(refDate.Value))
    let _accept                                    (v : ICell<IAcyclicVisitor>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.accept(v.Value)
                                                                     _FloatingRateCoupon.Value)
    let _registerWith                              (handler : ICell<Callback>)   
                                                   = triv (fun () -> _FloatingRateCoupon.Value.registerWith(handler.Value)
                                                                     _FloatingRateCoupon.Value)

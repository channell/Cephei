...
*)
    let mutable
        _DiscreteTrapezoidIntegral                 = make (fun () -> new DiscreteTrapezoidIntegral ())
    let _value                                     (x : ICell<Vector>) (f : ICell<Vector>)   
                                                   = triv _DiscreteTrapezoidIntegral (fun () -> _DiscreteTrapezoidIntegral.Value.value(x.Value, f.Value))

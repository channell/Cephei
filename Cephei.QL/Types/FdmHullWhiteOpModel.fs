...
*)
    ...
    let _add                                       (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.apply(r.Value))
    let _apply_direction                           (direction : ICell<int>) (r : ICell<Vector>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.apply_direction(direction.Value, r.Value))
    let _apply_mixed                               (r : ICell<Vector>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.apply_mixed(r.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.applyTo(v.Value))
    let _Clone                                     = cell (fun () -> _FdmHullWhiteOp.Value.Clone())
    let _identity                                  (size : ICell<int>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.identity(size.Value))
    let _isTimeDependent                           = cell (fun () -> _FdmHullWhiteOp.Value.isTimeDependent())
    let _multiply                                  (a : ICell<double>) (D : ICell<IOperator>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.multiply(a.Value, D.Value))
    let _preconditioner                            (r : ICell<Vector>) (s : ICell<double>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.preconditioner(r.Value, s.Value))
    let _setTime                                   (t1 : ICell<double>) (t2 : ICell<double>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.setTime(t1.Value, t2.Value)
                                                                     _FdmHullWhiteOp.Value)
    let _setTime1                                  (t : ICell<double>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.setTime(t.Value)
                                                                     _FdmHullWhiteOp.Value)
    let _size                                      = cell (fun () -> _FdmHullWhiteOp.Value.size())
    let _solve_splitting                           (direction : ICell<int>) (r : ICell<Vector>) (s : ICell<double>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.solve_splitting(direction.Value, r.Value, s.Value))
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.solveFor(rhs.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = cell (fun () -> _FdmHullWhiteOp.Value.subtract(A.Value, B.Value))
    let _toMatrixDecomp                            = cell (fun () -> _FdmHullWhiteOp.Value.toMatrixDecomp())
    let _toMatrix                                  = cell (fun () -> _FdmHullWhiteOp.Value.toMatrix())

...
*)
    let mutable
        _FdmHullWhiteOp                            = make (fun () -> new FdmHullWhiteOp (mesher.Value, model.Value, direction.Value))
    let _add                                       (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.add(A.Value, B.Value))
    let _apply                                     (r : ICell<Vector>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.apply(r.Value))
    let _apply_direction                           (direction : ICell<int>) (r : ICell<Vector>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.apply_direction(direction.Value, r.Value))
    let _apply_mixed                               (r : ICell<Vector>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.apply_mixed(r.Value))
    let _applyTo                                   (v : ICell<Vector>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.applyTo(v.Value))
    let _Clone                                     = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.Clone())
    let _identity                                  (size : ICell<int>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.identity(size.Value))
    let _isTimeDependent                           = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.isTimeDependent())
    let _multiply                                  (a : ICell<double>) (D : ICell<IOperator>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.multiply(a.Value, D.Value))
    let _preconditioner                            (r : ICell<Vector>) (s : ICell<double>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.preconditioner(r.Value, s.Value))
    let _setTime                                   (t1 : ICell<double>) (t2 : ICell<double>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.setTime(t1.Value, t2.Value)
                                                                                     _FdmHullWhiteOp.Value)
    let _setTime1                                  (t : ICell<double>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.setTime(t.Value)
                                                                                     _FdmHullWhiteOp.Value)
    let _size                                      = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.size())
    let _solve_splitting                           (direction : ICell<int>) (r : ICell<Vector>) (s : ICell<double>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.solve_splitting(direction.Value, r.Value, s.Value))
    let _solveFor                                  (rhs : ICell<Vector>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.solveFor(rhs.Value))
    let _subtract                                  (A : ICell<IOperator>) (B : ICell<IOperator>)   
                                                   = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.subtract(A.Value, B.Value))
    let _toMatrixDecomp                            = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.toMatrixDecomp())
    let _toMatrix                                  = triv _FdmHullWhiteOp (fun () -> _FdmHullWhiteOp.Value.toMatrix())

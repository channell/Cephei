...
*)
    ...
    let _absoluteAccuracy                          = cell (fun () -> _DiscreteSimpsonIntegrator.Value.absoluteAccuracy())
    let _absoluteError                             = cell (fun () -> _DiscreteSimpsonIntegrator.Value.absoluteError())
    let _integrationSuccess                        = cell (fun () -> _DiscreteSimpsonIntegrator.Value.integrationSuccess())
    let _maxEvaluations                            = cell (fun () -> _DiscreteSimpsonIntegrator.Value.maxEvaluations())
    let _numberOfEvaluations                       = cell (fun () -> _DiscreteSimpsonIntegrator.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = cell (fun () -> _DiscreteSimpsonIntegrator.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                     _DiscreteSimpsonIntegrator.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = cell (fun () -> _DiscreteSimpsonIntegrator.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                     _DiscreteSimpsonIntegrator.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = cell (fun () -> _DiscreteSimpsonIntegrator.Value.value(f.Value, a.Value, b.Value))

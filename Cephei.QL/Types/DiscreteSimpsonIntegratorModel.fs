...
*)
    ...
    let _maxEvaluations                            = triv (fun () -> _DiscreteSimpsonIntegrator.Value.maxEvaluations())
    let _numberOfEvaluations                       = triv (fun () -> _DiscreteSimpsonIntegrator.Value.numberOfEvaluations())
    let _setAbsoluteAccuracy                       (accuracy : ICell<double>)   
                                                   = triv (fun () -> _DiscreteSimpsonIntegrator.Value.setAbsoluteAccuracy(accuracy.Value)
                                                                     _DiscreteSimpsonIntegrator.Value)
    let _setMaxEvaluations                         (maxEvaluations : ICell<int>)   
                                                   = triv (fun () -> _DiscreteSimpsonIntegrator.Value.setMaxEvaluations(maxEvaluations.Value)
                                                                     _DiscreteSimpsonIntegrator.Value)
    let _value                                     (f : ICell<Func<double,double>>) (a : ICell<double>) (b : ICell<double>)   
                                                   = triv (fun () -> _DiscreteSimpsonIntegrator.Value.value(f.Value, a.Value, b.Value))
